...
class ChannelManager:
            ...
            if first_start:
                ...
                if conf().get("use_linkai") and ( os.environ.get("CLOUD_DEPLOYMENT_ID") or conf().get("cloud_deployment_id") ):
                    try:
                        ...
                        threading.Thread( target=cloud_client.start, args=(self._primary_channel, self), daemon=True, ).start()
    ...
    def _run_channel(self, name, channel):
        try:
            channel.startup()
        except Exception as e:
    ...
    def restart(self, new_channel_name):
        """
...
def _clear_singleton_cache(channel_name):
    """
