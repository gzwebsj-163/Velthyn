...
detect_python_command() {
    ...
    # 除 PATH 外，额外探测常见安装路径（macOS Homebrew/官方Framework）：
    # 系统自带 /usr/bin/python3 可能很旧(如 3.8)，而 /usr/local/bin/python3
    # 可能是更新的官方安装，新版的私有IR VM 引擎需 python3.10+（list|tuple 语法）。
    _sys=/usr/bin/python3
    _local=/usr/local/bin/python3
    _brew_310=/usr/local/bin/python3.10
    _brew_311=/usr/local/bin/python3.11
    _brew_312=/usr/local/bin/python3.12
    _brew_313=/usr/local/bin/python3.13
    for cmd in python3.13 python3.12 python3.11 python3.10 python3 python \
               "$_brew_313" "$_brew_312" "$_brew_311" "$_brew_310" "$_local" "$_sys"; do
        # 跳过纯路径候选里不存在的
        if [[ "$cmd" == /* ]]; then
        ...
        if [[ "$major_version" != "3" ]]; then continue; fi
        # 支持范围 3.7+，但优先选择 3.10+(兼容新版VM引擎)；<3.10 仅作最后回退
        if (( minor_version < 7 )); then continue; fi
        if (( minor_version >= 10 )); then
            PYTHON_CMD=$cmd
            PYTHON_VERSION="${major_version}.${minor_version}"
            break
        fi
        # 3.7-3.9：记录为「可用但旧」的回退候选，继续往后找是否有 3.10+
        if [ -z "$FOUND_NEWER_VERSION" ]; then
            FOUND_NEWER_VERSION=$cmd
        fi
    done
    
    # 没有 3.10+ 时，回退到找到的第一个 3.7-3.9（旧引擎也能用，但新引擎可能跑不了）
    if [ -z "$PYTHON_CMD" ] && [ -n "$FOUND_NEWER_VERSION" ]; then
        PYTHON_CMD=$FOUND_NEWER_VERSION
        PYTHON_VERSION=$($FOUND_NEWER_VERSION -c 'import sys; print(f"{sys.version_info[0]}.{sys.version_info[1]}")' 2>/dev/null)
    fi
    
    if [ -z "$PYTHON_CMD" ]; then
        echo -e "${YELLOW}Tried: python3.13, python3.12, python3.11, python3.10, python3, python, /usr/local/bin/python3.*, /usr/bin/python3, ${NC}"
...
# Clone project
clone_project() {
    echo -e "${GREEN}🔍 Cloning Mocode_Chat project...${NC}"

    if [ -d "Mocode_Chat" ]; then
        # An existing directory is automatically backed up (no prompt) so the
        # installer stays one-shot / hands-off.
        local backup_dir="Mocode_Chat_backup_$(date +%s)"
        echo -e "${YELLOW}⚠️  $(t "目录 'Mocode_Chat' 已存在，自动备份到" "Directory 'Mocode_Chat' exists, backing up to") '$backup_dir'...${NC}"
        mv Mocode_Chat "$backup_dir"
    fi
    ...
    if ! command -v git &> /dev/null; then
        ...
        if command -v wget &> /dev/null; then
            ...
            curl -L "$zip_url" -o Mocode_Chat.zip
        else
        ...
        if command -v unzip &> /dev/null; then
            unzip Mocode_Chat.zip
        elif command -v python3 &> /dev/null; then
            python3 -m zipfile -e Mocode_Chat.zip .
        elif command -v python &> /dev/null; then
            python -m zipfile -e Mocode_Chat.zip .
        else
            echo -e "${RED}❌ Cannot extract archive. Please install 'unzip' or Python.${NC}"
            exit 1
        fi
        # Archive top-level dir name may vary (Mocode_Chat-master, etc.); detect it.
        local _extracted="Mocode_Chat-master"
        if [ ! -d "$_extracted" ]; then
            _extracted=$(ls -d Mocode_Chat-*/ 2>/dev/null | head -1 | sed 's:/*$::')
        fi
        [ -n "$_extracted" ] && [ -d "$_extracted" ] && mv "$_extracted" Mocode_Chat
        rm -f Mocode_Chat.zip
    else
    ...
    cd Mocode_Chat || { echo -e "${RED}❌ Failed to enter project directory.${NC}"; exit 1; }
    export BASE_DIR=$(pwd)
...
start_project() {
    echo ""
    echo -e "${GREEN}${EMOJI_ROCKET} Starting Mocode_Chat...${NC}"
    sleep 1
    ...
    if $USE_COW; then
        ...
        if [[ "$OS_TYPE" == "Linux" ]]; then
            ...
            echo -e "${GREEN}${EMOJI_COW} Mocode_Chat started on Linux (using $PYTHON_CMD)${NC}"
        elif [[ "$OS_TYPE" == "Darwin" ]]; then
            nohup $PYTHON_CMD "${BASE_DIR}/app.py" > "${BASE_DIR}/nohup.out" 2>&1 &
            echo -e "${GREEN}${EMOJI_COW} Mocode_Chat started on macOS (using $PYTHON_CMD)${NC}"
        else
    ...
    echo ""
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${GREEN}${EMOJI_CHECK} $(t "Mocode_Chat 已在后台运行" "Mocode_Chat is now running in background")!${NC}"
...
show_usage() {
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${CYAN}${BOLD}   ${EMOJI_COW} Mocode_Chat Management Script${NC}"
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo ""
...
cmd_start() {
    ...
    if has_cow; then
        ...
        if is_running; then
            echo -e "${YELLOW}${EMOJI_WARN} $(t "Mocode_Chat 已在运行中" "Mocode_Chat is already running") (PID: $(get_pid))${NC}"
...
cmd_stop() {
    ...
    if has_cow; then
        ...
        echo -e "${GREEN}${EMOJI_STOP} $(t "正在停止 Mocode_Chat" "Stopping Mocode_Chat")...${NC}"

        if ! is_running; then
            echo -e "${YELLOW}${EMOJI_WARN} $(t "Mocode_Chat 未在运行" "Mocode_Chat is not running")${NC}"
            return 0
        fi
        ...
        echo -e "${GREEN}${EMOJI_CHECK} $(t "Mocode_Chat 已停止" "Mocode_Chat stopped")${NC}"
    fi
}
...
cmd_status() {
    if has_cow; then
        ...
        echo -e "${CYAN}${BOLD}=========================================${NC}"
        echo -e "${CYAN}${BOLD}   ${EMOJI_COW} Mocode_Chat Status${NC}"
        echo -e "${CYAN}${BOLD}=========================================${NC}"
...
cmd_config() {
    ...
    select_language
    echo ""
    echo -e "${YELLOW}${EMOJI_WRENCH} $(t "正在重新配置 Mocode_Chat" "Reconfiguring Mocode_Chat")...${NC}"
...
# Update project
cmd_update() {
    echo -e "${GREEN}${EMOJI_WRENCH} $(t "正在更新 Mocode_Chat" "Updating Mocode_Chat")...${NC}"
    cd "${BASE_DIR}"
...
install_mode() {
    ...
    clear
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${CYAN}${BOLD}   ${EMOJI_COW} Mocode_Chat Installation${NC}"
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo ""
