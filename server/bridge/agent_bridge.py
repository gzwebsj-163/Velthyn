...
class AgentLLMModel(LLMModel):
    ...
    def __init__(self, bridge, bot_type = "chat"):
    ...
    def call(self, request):
        ...
        try:
            ...
            if hasattr(self.bot, 'call_with_tools'):
                ...
                kwargs = { 'messages': request.messages, 'tools': getattr(request, 'tools', None), 'stream': False, 'model': self.model   }
    ...
    def call_stream(self, request):
        ...
        try:
            if hasattr(self.bot, 'call_with_tools'):
                ...
                kwargs = { 'messages': request.messages, 'tools': getattr(request, 'tools', None), 'stream': True, 'model': self.model   }
...
class AgentBridge:
    ...
    def create_agent(self, system_prompt, tools = None, **kwargs):
        """
    ...
    def steer_session(self, session_id, instruction):
    ...
    def get_agent(self, session_id = None):
        ...
        # Check if agent exists for self session
    ...
    def sync_session_messages_from_store(self, session_id):
        ...
        try:
            ...
            # has for self session after deletion.
    ...
    def agent_reply(self, query, context = None, on_event=None, clear_history = False):
        ...
        try:
            ...
            # Get agent for self session (will auto-initialize if needed)
            ...
            # If self is a scheduled task execution, exclude scheduler tool to prevent recursion
            ...
            # completes; the final persist skips self already-stored user turn.
            pre_persisted = self._pre_persist_user_message( session_id, query, context, clear_history )

            # Mark self session as mid-run so the self-evolution idle scan does
            ...
            try:
                ...
                # Clear the mid-run flag so idle scans can review self session.
                try:
                    from agent.evolution.trigger import mark_run_active
            ...
            # Persist new messages generated during self run
            if session_id:
                ...
                if new_messages:
                    self._persist_messages( session_id, list(new_messages), channel_type, create_if_missing=not pre_persisted )

            # Record self user turn for the self-evolution idle trigger. Skip
    ...
    def _create_file_reply(self, file_info, text_response, context = None):
        """
    ...
    def _migrate_config_to_env(self, workspace_root):
        """
    ...
    def _pre_persist_user_message(self, session_id, query, context, clear_history):
    ...
    def _persist_messages(self, session_id, new_messages, channel_type = "", create_if_missing = True):
        """
        ...
        try:
            ...
            # The in-memory message list keeps them intact for self run's
    ...
    @staticmethod
    def _trim_in_memory_to_turns(agent, keep_turns):
    ...
    @classmethod
    def _prune_scheduled_in_memory(cls, agent, keep_last_n):
