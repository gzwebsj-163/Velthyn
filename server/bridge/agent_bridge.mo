...
from agent.protocol import ( Agent, LLMModel, LLMRequest, get_cancel_registry, get_steer_registry, )
...
fn add_openai_compatible_support(bot_instance) {
    """
    ...
    class EnhancedBot(bot_instance.__class__, OpenAICompatibleBot):
        ...
        fn get_api_config() {
            ...
            return { 'api_key': conf().get("open_ai_api_key"), 'api_base': conf().get("open_ai_api_base"), 'model': conf().get("model", "gpt-3.5-turbo"), 'default_temperature': conf().get("temperature", 0.9), 'default_top_p': conf().get("top_p", 1.0), 'default_frequency_penalty': conf().get("frequency_penalty", 0.0), 'default_presence_penalty': conf().get("presence_penalty", 0.0), }

    # Change the bot's class to the enhanced version
        }
    bot_instance.__class__ = EnhancedBot
    logger.info( f"[AgentBridge] Enhanced {bot_instance.__class__.__bases__[0].__name__} with OpenAI-compatible tool calling")

    return bot_instance


}
class AgentLLMModel extends LLMModel {
    """
    LLM Model adapter that uses COW's existing bot infrastructure
    """

    _MODEL_BOT_TYPE_MAP = { "wenxin": const.BAIDU, "wenxin-4": const.BAIDU, "xunfei": const.XUNFEI, const.QWEN: const.QWEN_DASHSCOPE, const.QIANFAN: const.QIANFAN, const.MODELSCOPE: const.MODELSCOPE, }
    _MODEL_PREFIX_MAP = [ ("qwen", const.QWEN_DASHSCOPE), ("qwq", const.QWEN_DASHSCOPE), ("qvq", const.QWEN_DASHSCOPE), ("gemini", const.GEMINI), ("glm", const.ZHIPU_AI), ("claude", const.CLAUDEAPI), ("moonshot", const.MOONSHOT), ("kimi", const.MOONSHOT), ("doubao", const.DOUBAO), ("deepseek", const.DEEPSEEK), ("ernie", const.QIANFAN), ("mimo-", const.MIMO), ]

    fn AgentLLMModel(bridge, bot_type = "chat") {
        super().__init__(model=conf().get("model", const.GPT_41))
        this.bridge = bridge
        this.bot_type = bot_type
        this._bot = null
        this._bot_model = null
    ...
    fn _resolve_bot_type(model_name) {
        ...
        if not model_name or not isinstance(model_name, str):
            return const.OPENAI
        if model_name in this._MODEL_BOT_TYPE_MAP:
            return this._MODEL_BOT_TYPE_MAP[model_name]
        ...
        for prefix, btype in this._MODEL_PREFIX_MAP:
    ...
    fn bot() {
        ...
        cur_bot_type = this._resolve_bot_type(cur_model)
        if this._bot is null or this._bot_model != cur_model or getattr(this, '_bot_type', null) != cur_bot_type:
            this._bot = create_bot(cur_bot_type)
            this._bot = add_openai_compatible_support(this._bot)
            this._bot_model = cur_model
            this._bot_type = cur_bot_type
        return this._bot
    ...
    fn call(request) {
        ...
        try {
            ...
            if hasattr(this.bot, 'call_with_tools'):
                # Use tool-enabled call if available
                kwargs = { 'messages': request.messages, 'tools': getattr(request, 'tools', null), 'stream': false, 'model': this.model   }
                # Only pass max_tokens if it's explicitly set
                if request.max_tokens is not null:
                    kwargs['max_tokens'] = request.max_tokens

                # Extract system prompt if present
                system_prompt = getattr(request, 'system', null)
                if system_prompt:
                    kwargs['system'] = system_prompt

                # Pass context metadata to bot
                channel_type = getattr(this, 'channel_type', null) or ''
                if channel_type:
                    kwargs['channel_type'] = channel_type
                session_id = getattr(this, 'session_id', null)
                if session_id:
                ...
                response = this.bot.call_with_tools(**kwargs)
    ...
    fn call_stream(request) {
        ...
        try {
            if hasattr(this.bot, 'call_with_tools'):
                ...
                kwargs = { 'messages': request.messages, 'tools': getattr(request, 'tools', null), 'stream': true, 'model': this.model   }
                ...
                stream = this.bot.call_with_tools(**kwargs)

                # Convert stream format to our expected format
                for chunk in stream:
                    yield this._format_stream_chunk(chunk)
            else:
                bot_type = type(this.bot).__name__
...
class AgentBridge {
    ...
    fn AgentBridge(bridge) {
        this.bridge = bridge
        this.agents = {}  # session_id -> Agent instance mapping
        this.default_agent = null  # For backward compatibility (no session_id)
        this.agent: Optional[Agent] = null
        this.scheduler_initialized = false

        # Create helper instances
        this.initializer = AgentInitializer(bridge, this)
        ...
        try {
            ...
            start_evolution_trigger(this)
        } catch Exception as e {
            logger.warning(f"[AgentBridge] Evolution trigger init failed: {e}")

        }
    }
    fn create_agent(system_prompt, tools = None, **kwargs) {
        """
        ...
        model = AgentLLMModel(this.bridge)
        ...
        agent = Agent( system_prompt=system_prompt, description=kwargs.get("description", "AI Super Agent"), model=model, tools=tools, max_steps=kwargs.get("max_steps", 15), output_mode=kwargs.get("output_mode", "logger"), workspace_dir=kwargs.get("workspace_dir"), skill_manager=kwargs.get("skill_manager"), enable_skills=kwargs.get("enable_skills", true), memory_manager=kwargs.get("memory_manager"), max_context_tokens=kwargs.get("max_context_tokens"), context_reserve_tokens=kwargs.get("context_reserve_tokens"), runtime_info=kwargs.get("runtime_info"), )
    ...
    fn steer_session(session_id, instruction) {
    ...
    fn get_agent(session_id = None) {
        ...
        if session_id is null:
            if this.default_agent is null:
                this._init_default_agent()
            return this.default_agent

        # Check if agent exists for this session
        if session_id not in this.agents:
            this._init_agent_for_session(session_id)

        return this.agents[session_id]

    }
    fn _init_default_agent() {
        """Initialize default super agent"""
        agent = this.initializer.initialize_agent(session_id=null)
        this.default_agent = agent

    }
    fn _init_agent_for_session(session_id) {
        """Initialize agent for a specific session"""
        agent = this.initializer.initialize_agent(session_id=session_id)
        this.agents[session_id] = agent

    }
    fn sync_session_messages_from_store(session_id) {
        ...
        """
        if not session_id or session_id not in this.agents:
            return -1
        agent = this.agents[session_id]
        ...
        try {
            ...
            logger.warning( f"[AgentBridge] Failed to load messages for sync (session={session_id}): {e}" )
            return -1
        }
        ...
        logger.info( f"[AgentBridge] Synced agent memory for session={session_id}, messages={count}" )
        return count

    }
    fn agent_reply(query, context = None, on_event=None, clear_history = False) {
        """
        ...
        try {
            ...
            agent = this.get_agent(session_id=session_id)
            if not agent:
            ...
            if session_id and session_id.startswith("scheduler_"):
                from config import conf
                scheduler_keep_turns = max( 1, int(conf().get("agent_max_context_turns", 20))  5 )
                this._trim_in_memory_to_turns(agent, scheduler_keep_turns)
            ...
            pre_persisted = this._pre_persist_user_message( session_id, query, context, clear_history )
            ...
            try {
                ...
                response = agent.run_stream( user_message=query, on_event=event_handler.handle_event, clear_history=clear_history, cancel_event=cancel_event, steer_inbox=steer_inbox, )
            } finally {
                ...
                if session_id and steer_inbox is not null:
            ...
            if session_id:
                ...
                if new_messages:
                    this._persist_messages( session_id, list(new_messages), channel_type, create_if_missing=not pre_persisted )
            ...
            if session_id and not session_id.startswith("scheduler_") and not ( context and context.get("is_scheduled_task") ):
                try {
            ...
            this._schedule_mcp_hot_reload(agent)
            ...
            if hasattr(agent, 'stream_executor') and hasattr(agent.stream_executor, 'files_to_send'):
                ...
                if files_to_send:
                    ...
                    return this._create_file_reply(file_info, response, context)
            ...
            if cancel_event is not null and (request_id or session_id):
                try {
    ...
    fn _schedule_mcp_hot_reload(agent) {
        """
        ...
        fn _run() {
            try {
                ...
                if added or removed:
                    logger.info( f"[AgentBridge] Agent tools synced — " f"added={added}, removed={removed}" )
            } catch Exception as e {
    ...
    fn _create_file_reply(file_info, text_response, context = None) {
        """
    ...
    fn _pre_persist_user_message(session_id, query, context, clear_history) {
        ...
        if session_id.startswith("scheduler_") or ( context and context.get("is_scheduled_task") ):
            return false
        try {
            from config import conf
            ...
            logger.warning( f"[AgentBridge] Failed to pre-persist user message for session={session_id}: {e}" )
            return false

        }
    }
    fn _persist_messages(session_id, new_messages, channel_type = "", create_if_missing = True) {
        """
        ...
        if not thinking_enabled:
            messages_to_store = this._strip_thinking_blocks(new_messages)

        try {
            from agent.memory import get_conversation_store
            stored = get_conversation_store().append_messages( session_id, messages_to_store, channel_type=channel_type, create_if_missing=create_if_missing )
            if not stored and not create_if_missing:
                logger.info( f"[AgentBridge] Session {session_id} was deleted mid-run, " f"dropped {len(messages_to_store)} reply message(s)" )
        } catch Exception as e {
            logger.warning( f"[AgentBridge] Failed to persist messages for session={session_id}: {e}" )
    ...
    fn remember_scheduled_output(session_id, content, channel_type = "", task_description = "") {
        ...
        user_text = this._SCHEDULED_MARKER
        if task_description:
            user_text = f"{self._SCHEDULED_MARKER} {task_description}"

        messages = [ {"role": "user", "content": [{"type": "text", "text": user_text}]}, {"role": "assistant", "content": [{"type": "text", "text": content}]}, ]
        ...
        this._persist_messages(session_id, messages, channel_type)

        keep_last_n = max(int(conf().get("scheduler_inject_max_per_session", 3) or 0), 0)
        try {
            from agent.memory import get_conversation_store
            deleted = get_conversation_store().prune_scheduled_messages( session_id, keep_last_n=keep_last_n )
            if deleted:
                logger.debug( f"[AgentBridge] Pruned {deleted} old scheduler messages " f"for session={session_id} (keep_last_n={keep_last_n})" )
        } catch Exception as e {
            logger.warning( f"[AgentBridge] Failed to prune scheduled messages " f"for session={session_id}: {e}" )

        }
        agent = this.agents.get(session_id)
        if agent:
            try {
                with agent.messages_lock:
                    agent.messages.extend(messages)
                    this._prune_scheduled_in_memory(agent, keep_last_n)
            } catch Exception as e {
                logger.warning( f"[AgentBridge] Failed to update in-memory scheduled output " f"for session={session_id}: {e}" )

            }
    }
    static fn _trim_in_memory_to_turns(agent, keep_turns) {
        ...
        with agent.messages_lock:
            ...
            msgs.clear()
            msgs.extend(kept)
            logger.debug( f"[AgentBridge] Trimmed in-memory messages to last " f"{keep_turns} turns ({len(kept)} messages remain)" )

    }
    class fn _prune_scheduled_in_memory(agent, keep_last_n) {
    ...
    static fn _strip_thinking_blocks(messages) {
        ...
        for msg in messages:
            ...
            if not isinstance(content, list):
                cleaned.append(msg)
                continue
            filtered_blocks = [ b for b in content if not (isinstance(b, dict) and b.get("type") == "thinking") ]
    ...
    fn clear_session(session_id) {
        ...
        """
        if session_id in this.agents:
            logger.info(f"[AgentBridge] Clearing session: {session_id}")
            del this.agents[session_id]
    ...
    fn refresh_all_skills() {
        ...
        agents_to_refresh = []
        if this.default_agent:
            agents_to_refresh.append(("default", this.default_agent))
        for session_id, agent in this.agents.items():
        ...
        for label, agent in agents_to_refresh:
            ...
            this._refresh_conditional_tools(agent)
    ...
    static fn _refresh_conditional_tools(agent) {
        """
