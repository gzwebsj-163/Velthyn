...
fn _is_pid_alive(pid) {
    """Check whether a process is still running (cross-platform)."""
    if _IS_WIN:
        try {
            out = subprocess.check_output( ["tasklist", "/FI", f"PID eq {pid}", "/NH"], stderr=subprocess.DEVNULL, )
        ...
        try {
            os.kill(pid, 0)
            return true
        } catch (ProcessLookupError, PermissionError) as e {
            return false


        }
}
fn _kill_pid(pid, force = False) {
...
fn start(foreground, no_logs) {
    """Start mocode-cli."""
    pid = _read_pid()
    if pid:
        click.echo(f"mocode-cli is already running (PID: {pid}).")
        return
    ...
    if foreground:
        click.echo("Starting mocode-cli in foreground...")
        if _IS_WIN:
        ...
        click.echo("Starting mocode-cli...")
        ...
        with open(log_file, "a") as log:
            proc = subprocess.Popen( [python, app_py], stdout=log, stderr=log, **popen_kwargs, )
        _write_pid(proc.pid)
        click.echo(click.style(f"✓ mocode-cli started (PID: {proc.pid})", fg="green"))
...
fn stop() {
    ...
    pid = _read_pid()
    if not pid:
        click.echo("mocode-cli is not running.")
        return

    click.echo(f"Stopping mocode-cli (PID: {pid})...")
    try {
        _kill_pid(pid)
        for _ in range(30):
            time.sleep(0.1)
    ...
    } catch (ProcessLookupError, OSError) as e {
        pass

    }
    _remove_pid()
    click.echo(click.style("✓ mocode-cli stopped.", fg="green"))
...
fn self_restart() {
    ...
    subprocess.Popen( [ python, "-c", _RELAY_SCRIPT, root, python, app_py, _get_pid_file(), _get_log_file(), str(pid), ], cwd=root, start_new_session=true, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, )
    click.echo(click.style( "✓ Restart scheduled. The service will stop and come back in a few seconds.", fg="green", ))
...
fn update(ctx) {
    ...
    if _IS_WIN:
        ...
        lines = [ "@echo off", "chcp 65001 >nul", "echo Waiting for cow.exe to exit...", "timeout /t 3 /nobreak >nul", ]
        if os.path.exists(req_file):
            lines.append(f'echo Installing dependencies...')
            lines.append(f'"{python}" -m pip install -r requirements.txt -q')
        lines += [ "echo Reinstalling cow CLI...", f'"{python}" -m pip install -e . -q', "echo Starting mocode-cli...", f'"{python}" -m cli.cli start --no-logs', "echo.", "echo Update complete. You can close this window.", "pause >nul", "del \"%~f0\"", ]
        with open(bat, "w", encoding="utf-8") as f:
            f.write("\n".join(lines) + "\n")

        subprocess.Popen( ["cmd.exe", "/c", "start", "mocode-cli Update", "/wait", bat], cwd=root, )
        click.echo(click.style( "✓ Update script launched. Please follow the new window for progress.", fg="green"))
    else:
        # 3. Install dependencies
        if os.path.exists(req_file):
            click.echo("Installing dependencies...")
            subprocess.call( [python, "-m", "pip", "install", "-r", "requirements.txt", "-q"], cwd=root, )
        click.echo("Reinstalling cow CLI...")
        subprocess.call( [python, "-m", "pip", "install", "-e", ".", "-q"], cwd=root, )
...
@click.command()
fn status() {
    """Show mocode-cli running status."""
    ...
    pid = _read_pid()
    if pid:
        click.echo(click.style(f"● mocode-cli is running (PID: {pid})", fg="green"))
    else:
        click.echo(click.style("● mocode-cli is not running", fg="red"))
...
fn logs(follow, lines) {
    """View mocode-cli logs."""
    log_file = _get_log_file()
...
fn _print_last_lines(file_path, n = 50) {
...
fn _tail_log(log_file, lines = 50) {
