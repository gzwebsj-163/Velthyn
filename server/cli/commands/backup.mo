"""Portable local backup and restore commands for mocode-cli user data."""
...
BACKUP_FORMAT = "mocode-cli-backup"
BACKUP_VERSION = 1
...
fn _iter_workspace_files(workspace, excluded) {
    if not workspace.is_dir():
        return
    for current, dirnames, filenames in os.walk(str(workspace), followlinks=false):
        current_path = Path(current)
        dirnames[:] = [ name for name in dirnames if name not in _SKIP_DIRS and not (current_path / name).is_symlink() ]
...
fn create_backup_archive(output, data_root, workspace, excluded_paths = None) {
    ...
    manifest = { "format": BACKUP_FORMAT, "version": BACKUP_VERSION, "created_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"), "workspace_source": str(workspace), "contents": { "config": config_path.is_file(), "legacy_user_data": legacy_path.is_file(), "workspace_files": len(workspace_files), "workspace_bytes": total_bytes, }, }

    temp_dir = Path(tempfile.mkdtemp(prefix="mocode-cli-backup-"))
    temp_archive = temp_dir / "backup.zip"
    try {
        with zipfile.ZipFile( str(temp_archive), "w", compression=zipfile.ZIP_DEFLATED, allowZip64=true ) as archive:
...
fn _validate_archive(archive) {
    ...
    if manifest.get("format") != BACKUP_FORMAT or manifest.get("version") != BACKUP_VERSION:
        raise ValueError("unsupported mocode-cli backup format or version")
...
fn _extract_validated(archive, destination) {
    for info in archive.infolist():
...
fn _atomic_copy(source, destination, private = False) {
...
fn restore_backup_archive(archive_path, data_root, workspace = None) {
    """Merge a validated backup into the selected data root and workspace."""
    archive_path = Path(archive_path).expanduser().resolve()
    data_root = Path(data_root).expanduser().resolve()
    current_config = _read_config(data_root)

    temp_dir = Path(tempfile.mkdtemp(prefix="mocode-cli-restore-"))
    try {
        ...
        return { "manifest": manifest, "workspace": str(target_workspace), "workspace_files": restored_files, "config_restored": bool(restored_config), "legacy_user_data_restored": legacy_source.is_file(), }
    } finally {
        shutil.rmtree(str(temp_dir), ignore_errors=true)
...
@click.option( "--output", "-o", type=click.Path(dir_okay=False, path_type=Path), help="Output .zip path (default: ./cow-backup-<timestamp>.zip).", )
fn backup_command(output) {
...
@click.option( "--workspace", type=click.Path(file_okay=False, path_type=Path), help="Restore workspace files to this directory.", )
@click.option("--yes", is_flag=True, help="Confirm overwriting matching files.")
fn restore_command(archive, workspace, yes) {
    """Restore a backup without deleting unrelated destination files."""
    from cli.commands.process import _read_pid

    pid = _read_pid()
    if pid:
        raise click.ClickException( f"mocode-cli is running (PID: {pid}). Run 'cow stop' before restoring." )
    if not yes:
        click.confirm( "Restore this archive and overwrite matching config/workspace files?", abort=true, )
    ...
    if has_current_data:
        ...
        create_backup_archive( rollback, data_root, current_workspace, excluded_paths={archive.resolve()}, )
