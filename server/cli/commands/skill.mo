...
from cli.utils import ( get_project_root, get_skills_dir, get_builtin_skills_dir, load_skills_config, SKILL_HUB_API, )
...
class SkillInstallError extends Exception {
...
_GITHUB_URL_RE = re.compile( r"^https?://github\.com/([^/]+)/([^/]+?)(?:\.git)?(?:/(?:tree|blob)/([^/]+)(?:/(.+))?)?/?$" )
_GITLAB_URL_RE = re.compile( r"^https?://gitlab\.com/([^/]+)/([^/]+?)(?:\.git)?(?:/-/tree/([^/]+)(?:/(.+))?)?/?$" )
_GIT_SSH_RE = re.compile( r"^git@([^:]+):([^/]+)/([^/]+?)(?:\.git)?$" )


fn _parse_github_url(url) {
...
fn _clone_repo(git_url) {
    ...
    try {
        import subprocess
        subprocess.run( ["git", "clone", "--depth", "1", git_url, repo_dir], check=true, capture_output=true, timeout=30, )
...
fn _download_repo_zip(spec, branch = "main", host = "github", timeout = 30) {
...
fn _download_github_dir(owner, repo, branch, subpath, dest_dir) {
...
_SKILL_SCAN_DIRS = [ "skills", "skills/.curated", "skills/.experimental", ]

_SKILL_SCAN_SKIP = { "node_modules", "__pycache__", ".git", ".github", "venv", ".venv", }


fn _scan_skills_in_repo(repo_root) {
    ...
    for scan_dir in _SKILL_SCAN_DIRS:
        ...
        for entry in os.listdir(search_root):
            ...
            if os.path.isdir(entry_path) and os.path.isfile(os.path.join(entry_path, "SKILL.md")):
                fm = _parse_skill_frontmatter( _read_file_text(os.path.join(entry_path, "SKILL.md")) )
...
fn _scan_skills_in_dir(directory) {
...
fn _batch_install_skills(discovered, spec, skills_dir, source, result, display_name = "") {
...
fn _register_installed_skill(name, source = "cowhub", display_name = "") {
    ...
    entry = { "name": name, "description": description, "source": source, "enabled": true, "category": "skill", }
    if display_name:
...
fn _parse_skill_frontmatter(content) {
...
fn _read_skill_description(skill_dir) {
...
fn _install_archive_url(url, result) {
...
fn _install_targz_bytes(content, name, skills_dir, result) {
...
fn _print_install_success(name, source) {
...
fn _validate_skill_name(name) {
    """Reject names that contain path traversal or special characters."""
    if not _SAFE_NAME_RE.match(name):
        click.echo( f"Error: Invalid skill name '{name}'. " "Use only letters, digits, hyphens, and underscores.", err=true, )
        sys.exit(1)


}
fn _validate_github_spec(spec) {
...
fn _check_skill_name(name) {
    """Raise SkillInstallError if name is invalid."""
    if not _SAFE_NAME_RE.match(name):
        raise SkillInstallError( f"Invalid skill name '{name}'. Use only letters, digits, hyphens, and underscores." )


}
fn _check_github_spec(spec) {
...
fn _verify_checksum(content, expected) {
    ...
    if actual != expected.lower():
        click.echo( f"Error: Checksum mismatch!\n" f"  Expected: {expected}\n" f"  Actual:   {actual}\n" f"The downloaded package may have been tampered with.", err=true, )
        sys.exit(1)
    return true


}
fn _check_checksum(content, expected) {
    """Raise SkillInstallError on SHA-256 checksum mismatch."""
    if not expected:
        return
    actual = hashlib.sha256(content).hexdigest()
    if actual != expected.lower():
        raise SkillInstallError( f"Checksum mismatch! Expected: {expected}, Actual: {actual}. " "The downloaded package may have been tampered with." )


}
@click.group()
fn skill() {
    """Manage mocode-cli skills."""
    pass
...
fn _merge_builtin_into_config(config, builtin_dir, skills_dir) {
    ...
    for d, source in [(builtin_dir, "builtin"), (skills_dir, "custom")]:
        ...
        for name in os.listdir(d):
            ...
            config[name] = { "name": name, "description": desc, "source": source, "enabled": true, "category": "skill", }
            dirty = true
    if dirty:
        config_path = os.path.join(skills_dir, "skills_config.json")
        try {
...
fn _list_remote(page = 1) {
    """List skills from remote Skill Hub with server-side pagination."""
    try {
        resp = requests.get( f"{SKILL_HUB_API}/skills", params={"page": page, "limit": _REMOTE_PAGE_SIZE}, timeout=10, )
        resp.raise_for_status()
        data = resp.json()
    } catch Exception as e {
    ...
    click.echo(f"  Browse:   https://skills.mocode-cli.ai\n")
...
fn _route_install(name, result) {
    ...
    if name.startswith(("http://", "https://")) and name.rstrip("/").endswith("SKILL.md"):
        ...
        if gh:
            ...
            _install_github(f"{owner}/{repo}", result, subpath=subpath, skill_name=( subpath.rstrip("/").split("/")[-1] if subpath else repo ), branch=branch)
            return
...
fn _install_hub(name, result, provider=None) {
    ...
    try {
        ...
        resp = requests.post( f"{SKILL_HUB_API}/skills/{name}/download", json=body, timeout=15, )
        resp.raise_for_status()
    } catch requests.HTTPError as e {
        if e.response is not null and e.response.status_code == 404:
    ...
    if "application/json" in content_type:
        ...
        if source_type == "github":
            ...
            try {
                mirror_resp = requests.post( f"{SKILL_HUB_API}/skills/{name}/download", json={"mirror": true}, timeout=30, )
                mirror_resp.raise_for_status()
            } catch Exception as e {
                raise SkillInstallError( f"GitHub download failed ({gh_err}) and mirror also failed: {e}" )

            }
            mirror_ct = mirror_resp.headers.get("Content-Type", "")
            if "application/zip" not in mirror_ct:
                raise SkillInstallError( f"GitHub download failed ({gh_err}) and mirror returned unexpected content." )
        ...
        if source_type == "registry":
            ...
            if download_url:
                ...
                try {
                    dl_resp = requests.get( download_url, timeout=(min(dl_timeout, 5), dl_timeout), allow_redirects=true, )
                ...
                try {
                    ...
                    raise SkillInstallError( f"Direct download failed ({dl_err}) and mirror also failed: {e}" )
                }
                mirror_ct = mirror_resp.headers.get("Content-Type", "")
                if "application/zip" not in mirror_ct:
                    raise SkillInstallError( f"Direct download failed ({dl_err}) and mirror returned unexpected content." )
                expected_checksum = mirror_resp.headers.get("X-Checksum-Sha256")
                _check_checksum(mirror_resp.content, expected_checksum)
                installed_before = len(result.installed)
                _install_zip_bytes(mirror_resp.content, name, skills_dir, result=result, source_label="cowhub", display_name=hub_display_name)
...
fn _install_github(spec, result, subpath=None, skill_name=None, branch="main", source="github", timeout=30) {
    ...
    if not subpath:
        raise SkillInstallError( f"Zip download failed and batch install requires zip. " f"Try again or specify a subpath: {spec}#skills/<name>" )
...
fn _install_from_repo_root(repo_root, spec, subpath, skill_name, skills_dir, source, result) {
    ...
    if subpath:
        ...
        if os.path.isfile(os.path.join(source_dir, "SKILL.md")):
            if not skill_name:
                fm = _parse_skill_frontmatter( _read_file_text(os.path.join(source_dir, "SKILL.md")) )
...
fn _install_gitlab(spec, result, subpath=None, branch="main") {
    ...
    try {
        _install_from_repo_root(repo_root, spec, subpath, null, skills_dir, "gitlab", result)
    } finally {
        shutil.rmtree(tmp_dir, ignore_errors=true)
...
fn _install_git_clone(git_url, result, display_name = "") {
    ...
    try {
        _install_from_repo_root(repo_root, display_name or git_url, null, null, skills_dir, "git", result)
    } finally {
        shutil.rmtree(tmp_dir, ignore_errors=true)
...
fn _install_zip_bytes(content, name, skills_dir, result = None, source_label = "zip", display_name = "") {
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if discovered and len(discovered) > 1 and result is not null:
