...
def _download_repo_zip(spec, branch = "main", host = "github", timeout = 30):
...
def _scan_skills_in_repo(repo_root):
...
def _scan_skills_in_dir(directory):
...
def _batch_install_skills(discovered, spec, skills_dir, source, result, display_name = ""):
...
def _install_local(path, result):
...
def _register_installed_skill(name, source = "cowhub", display_name = ""):
...
def _install_archive_url(url, result):
...
def _install_targz_bytes(content, name, skills_dir, result):
...
def _print_install_success(name, source):
...
def _validate_github_spec(spec):
...
def _verify_checksum(content, expected):
...
def _check_checksum(content, expected):
...
def _merge_builtin_into_config(config, builtin_dir, skills_dir):
...
def _route_install(name, result):
...
def _install_hub(name, result, provider=None):
...
def _install_github(spec, result, subpath=None, skill_name=None, branch="main", source="github", timeout=30):
...
def _install_from_repo_root(repo_root, spec, subpath, skill_name, skills_dir, source, result):
...
def _install_gitlab(spec, result, subpath=None, branch="main"):
...
def _install_git_clone(git_url, result, display_name = ""):
...
def _install_zip_bytes(content, name, skills_dir, result = None, source_label = "zip", display_name = ""):
