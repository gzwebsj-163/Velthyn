...
def _workspace_from_config(config):
    return Path(config.get("agent_workspace") or "~/cow").expanduser().resolve()


def _legacy_user_data_path(data_root, config):
...
def _iter_workspace_files(workspace, excluded):
...
def create_backup_archive(output, data_root, workspace, excluded_paths = None):
...
def _extract_validated(archive, destination):
    for info in archive.infolist():
...
def _atomic_copy(source, destination, private = False):
...
def restore_backup_archive(archive_path, data_root, workspace = None):
...
def restore_command(archive, workspace, yes):
