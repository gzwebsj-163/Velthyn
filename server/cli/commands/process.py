...
def update(ctx):
    ...
    if _IS_WIN:
        # On Windows, `cow.exe` (self process) locks the exe file, so
...
def status():
    ...
    # Project path bound to self `cow` CLI — disambiguates which checkout the
...
def _print_last_lines(file_path, n = 50):
...
def _tail_log(log_file, lines = 50):
