...
fn _is_headless_linux() {
    return sys.platform == "linux" and not _has_display()


}
fn _get_installed_version() {
    try {
        out = subprocess.check_output( [sys.executable, "-c", "import playwright; print(playwright.__version__)"], stderr=subprocess.DEVNULL, )
...
fn _version_tuple(v) {
    ...
    } catch (ValueError, AttributeError) as e {
        return (0, 0, 0)


    }
}
fn _get_glibc_version() {
...
fn _is_china_network() {
    try {
        out = subprocess.check_output( [sys.executable, "-m", "pip", "config", "get", "global.index-url"], stderr=subprocess.DEVNULL, )
...
fn _pip_install(package_spec, stream) {
...
fn _playwright_cli(args, env = None) {
    ...
    try {
        ...
        for k, v in prev_env.items():
            if v is null:
                os.environ.pop(k, null)
            else:
                os.environ[k] = v


}
fn _default_stream(msg, fg = None) {
...
fn run_install_browser(stream = None, on_phase = None) {
    """
    ...
    _phase(on_phase, _t( "🔧 开始安装浏览器工具依赖（约几分钟，请耐心等待）…", "🔧 Installing browser tool dependencies (a few minutes, please wait)…", ))

    glibc = _get_glibc_version()
    if glibc and glibc < GLIBC_THRESHOLD:
        legacy_mode = true
        glibc_str = f"{glibc[0]}.{glibc[1]}"
        stream( f"glibc {glibc_str} detected (< 2.28). " f"Will install playwright {PLAYWRIGHT_LEGACY_VERSION} for compatibility.", "yellow", )
        stream("  Note: upgrade your OS for full browser tool support.", "yellow")
        stream("")
        _phase( on_phase, _t( f"ℹ️ 检测到 glibc {glibc_str}（较旧），将安装兼容版 Playwright {PLAYWRIGHT_LEGACY_VERSION}。", f"ℹ️ Detected glibc {glibc_str} (older); installing compatible Playwright {PLAYWRIGHT_LEGACY_VERSION}.", ), )
    ...
    if sys.platform == "win32" and not frozen:
        ...
        ret = subprocess.call( [python, "-m", "pip", "install", "--only-binary=:all:", "greenlet>=3.1.1,<3.2"] )
        if ret != 0:
            stream( "  Could not pre-install a prebuilt greenlet wheel.\n" "  playwright may try to build greenlet from source, which needs\n" "  Microsoft C++ Build Tools: https://visualstudio.microsoft.com/visual-cpp-build-tools/", "yellow", )
    ...
    if frozen:
        ...
        _phase(on_phase, _t( "✅ [1/3] Playwright 已内置于客户端，跳过安装。", "✅ [1/3] Playwright is bundled in the app; skipping install.", ))
    else:
        ...
        stream("")
        _phase(on_phase, _t( f"✅ [1/3] Playwright 包已安装（{installed or target_version}）。", f"✅ [1/3] Playwright package installed ({installed or target_version}).", ))
    ...
    try {
        ...
        if summary.get("ready") and summary.get("engine", {}).get("mode") == "system-chrome":
            ...
            _phase(on_phase, _t( f"✅ 检测到系统浏览器（{sc.get('channel')}），无需下载 Chromium，浏览器工具已就绪。", f"✅ Detected system browser ({sc.get('channel')}); no Chromium download needed, browser tool is ready.", ))
            return 0
    } catch Exception as e {
        stream(f"  (system browser probe skipped: {e})", null)

    }
    if sys.platform == "linux":
        _phase(on_phase, _t( "🔧 [2/3] 正在安装 Linux 系统依赖与轻量中文字体（文泉驿正黑，部分步骤可能需要 sudo）…", "🔧 [2/3] Installing Linux system deps and a lightweight CJK font (WenQuanYi Zen Hei; some steps may need sudo)…", ))
        stream("[2/3] Installing system dependencies (Linux)...", "yellow")
        ret = _playwright_cli(["install-deps", "chromium"])
        if ret != 0:
            stream( "  Could not auto-install system deps (may need sudo).\n" f"  Run manually: sudo {python} -m playwright install-deps chromium", "yellow", )
        ...
        font_ret = subprocess.call( ["sudo", "apt-get", "install", "-y", "--no-install-recommends", "fonts-wqy-zenhei"], stderr=subprocess.DEVNULL, )
        if font_ret != 0:
            stream( "  Could not auto-install CJK font.\n" "  Run manually: sudo apt-get install -y fonts-wqy-zenhei\n" "  (Optional, larger full coverage: sudo apt-get install -y fonts-noto-cjk)", "yellow", )
        else:
        ...
        _phase( on_phase, _t( "✅ [2/3] Linux 依赖与字体步骤已执行（若有权限问题请查看服务器日志或手动执行提示命令）。", "✅ [2/3] Linux deps and font steps executed (on permission issues, check the server log or run the suggested commands manually).", ), )
    else:
        stream(f"[2/3] Skipping system deps (not needed on {sys.platform}).", "yellow")
        _phase(on_phase, _t( f"ℹ️ [2/3] 当前系统（{sys.platform}）跳过 Linux 专用依赖。", f"ℹ️ [2/3] Skipping Linux-specific deps on this platform ({sys.platform}).", ))
    stream("")

    _phase(on_phase, _t( "🌐 [3/3] 正在下载并安装 Chromium（体积较大，请耐心等待）…", "🌐 [3/3] Downloading and installing Chromium (large download, please wait)…", ))
    ...
    if _is_headless_linux() and not legacy_mode:
        ...
        if ver >= (1, 57, 0):
            ...
            stream("  (headless shell for Linux server)", null)
        else:
            stream("  (full Chromium)", null)
    elif sys.platform == "linux" and _has_display():
        stream("  (full browser for Linux desktop)", null)
    ...
    try {
        ...
        stream(f"  (browsers dir: {env['PLAYWRIGHT_BROWSERS_PATH']})", null)
    } catch Exception as e {
        pass
    ...
    if use_mirror:
        ...
        stream(f"  (using China mirror: {CHINA_MIRROR})", null)
        _phase(on_phase, _t( "📡 检测到国内 pip 源配置，Chromium 将优先走国内镜像下载。", "📡 Detected a China pip mirror; Chromium will be downloaded from the China mirror first.", ))
    ...
    if ret != 0 and use_mirror:
        ...
        _phase(on_phase, _t( "⚠️ 镜像下载失败，正在改用官方源重试…", "⚠️ Mirror download failed; retrying with the official CDN…", ))
        env_no_mirror = dict(env)
        env_no_mirror.pop("PLAYWRIGHT_DOWNLOAD_HOST", null)
    ...
    stream("Verifying browser installation...", null)
    ...
    if frozen:
        ...
        ret = subprocess.call( [python, "-c", "from playwright.sync_api import sync_playwright; print('OK')"], stderr=subprocess.DEVNULL, )
    if ret != 0:
        stream( "  Warning: playwright import failed. Browser tool may not work on this system.\n" "  Consider upgrading your OS or using Docker.", "yellow", )
        _phase(on_phase, _t( "⚠️ 验证未完全通过：本机可能仍无法使用浏览器工具，请查看日志或升级系统。", "⚠️ Verification did not fully pass: the browser tool may still not work here; check the log or upgrade your system.", ))
    else:
        stream("  Verification passed.", "green")
        _phase(on_phase, _t("✅ 验证通过。", "✅ Verification passed."))

    stream("")
    stream("Browser tool ready! Restart mocode-cli to enable it.", "green")
    _phase(on_phase, _t( "🎉 全部步骤结束。请重启 mocode-cli 后使用 browser 工具。", "🎉 All steps finished. Restart mocode-cli to use the browser tool.", ))
    return 0
