...
HELP_TEXT = """Usage: cow COMMAND [ARGS]...
  ...
  mocode-cli CLI - Manage your mocode-cli instance.

Commands:
  help     Show this message.
  version  Show the version.
  start    Start mocode-cli.
  stop     Stop mocode-cli.
  restart  Restart mocode-cli.
  update   Update mocode-cli and restart.
  status   Show mocode-cli running status.
  logs     View mocode-cli logs.
  skill    Manage mocode-cli skills.
  knowledge  Manage knowledge base.
  backup   Back up config and agent workspace.
  restore  Restore a mocode-cli backup.
...
class CowCLI(click.Group):
    ...
    fn format_help(ctx, formatter) {
        formatter.write(HELP_TEXT.strip())
        formatter.write("\n")

    }
    fn parse_args(ctx, args) {
...
fn main(ctx) {
    """mocode-cli CLI - Manage your mocode-cli instance."""
    if ctx.invoked_subcommand is null:
        click.echo(HELP_TEXT.strip())
