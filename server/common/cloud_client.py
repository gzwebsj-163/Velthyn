...
class CloudClient(LinkAIClient):
    def __init__(self, api_key, channel, host = "", port=None):
    ...
    def on_config(self, config):
        ...
        if self._set_channel_credentials(local_config, current_channel_type, config.get("app_id"), config.get("app_secret")):
            need_restart_channel = True
    ...
    # ------------------------------------------------------------------
    def _dispatch_channel_action(self, action, data):
    ...
    def _handle_channel_create(self, channel_type, data):
        local_config = conf()
        cred_changed = self._set_channel_credentials( local_config, channel_type, data.get("appId"), data.get("appSecret"))
        ...
        if skip_restart:
            ...
            threading.Thread( target=self._report_channel_startup, args=(channel_type,), daemon=True ).start()
            return

        threading.Thread( target=self._do_add_channel, args=(channel_type,), daemon=True ).start()

    def _handle_channel_update(self, channel_type, data):
        local_config = conf()
        ...
        if enabled == "N":
            threading.Thread( target=self._do_remove_channel, args=(channel_type,), daemon=True ).start()
        else:
            existing_ch = self.channel_mgr.get_channel(channel_type)
            ...
            if existing_ch and not needs_restart:
                ...
                threading.Thread( target=self._do_restart_channel, args=(self.channel_mgr, channel_type), daemon=True ).start()

    def _handle_channel_delete(self, channel_type, data):
        local_config = conf()
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _set_channel_credentials(local_config, channel_type, app_id, app_secret):
        """
    ...
    @staticmethod
    def _clear_channel_credentials(local_config, channel_type):
        cred = CREDENTIAL_MAP.get(channel_type)
        if not cred:
            return
        id_key, secret_key = cred
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _parse_channel_types(local_config):
    ...
    @staticmethod
    def _add_channel_type(local_config, channel_type):
    ...
    @staticmethod
    def _remove_channel_type(local_config, channel_type):
        types = CloudClient._parse_channel_types(local_config)
    ...
    # ------------------------------------------------------------------
    def _do_add_channel(self, channel_type):
        try:
    ...
    def _do_remove_channel(self, channel_type):
        try:
    ...
    def send_channel_qrcode(self, channel_type, qrcode_url):
    ...
    def _report_channel_startup(self, channel_type):
    ...
    # chat callback
    # ------------------------------------------------------------------
    def on_chat(self, data, send_chunk_fn):
        """
    ...
    def _dispatch_session(self, action, payload):
        """Delegate session actions to SessionService."""
        svc = self.session_service
        if svc is None:
            return {"action": action, "code": 500, "message": "SessionService not available", "payload": None}
        return svc.dispatch(action, payload)
    ...
    # ------------------------------------------------------------------
    def _restart_channel(self, new_channel_type):
        """
    ...
    def _do_restart_channel(self, mgr, new_channel_type):
        """
    ...
    # config persistence
    # ------------------------------------------------------------------
    def _save_config_to_file(self, local_config):
        """
...
def copy_send_file(src_path, workspace_root):
...
def build_website_prompt(workspace_dir):
...
def _report_existing_channels(client, mgr):
