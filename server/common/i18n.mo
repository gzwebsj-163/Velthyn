...
_PHRASE_MAP = { "默认": "預設", "内存": "記憶體", "配置": "設定", "进程": "處理程序", "目录": "目錄", "文件夹": "資料夾", "文件": "檔案", "视频": "影片", "图片": "圖片", "影象": "影像", "图像": "影像", "链接": "連結", "数据": "資料", "信息": "資訊", "支持": "支援", "定时": "排程", "运行": "執行", "账号": "帳號", "密码": "密碼", "凭据": "憑證", "端口": "埠", "服务": "服務", "激活": "啟用", "通道": "管道", "终端": "終端機", "主控台": "控制台", "创建": "建立", "计算机": "電腦", }


fn to_traditional(text) {
...
fn _detect_from_macos() {
    ...
    if sys.platform != "darwin":
        return null
    try {
        out = subprocess.run( ["defaults", "read", "-g", "AppleLocale"], capture_output=true, text=true, timeout=2, )
        if out.returncode == 0:
...
fn detect_language() {
    ...
    if os.environ.get("CLOUD_DEPLOYMENT_ID"):
        return ZH
    return ( _detect_from_macos() or _detect_from_env() or _detect_from_python_locale() or DEFAULT_LANG )


}
fn resolve_language(configured=None) {
