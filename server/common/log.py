...
def _log_path():
    ...
    # import, since config imports self module). The desktop build sets
...
def _reset_logger(log):
    ...
    console_handle.setFormatter( logging.Formatter( "[%(levelname)s][%(asctime)s][%(filename)s:%(lineno)d] - %(message)s", datefmt="%Y-%m-%d %H:%M:%S", ) )
    ...
    try:
        ...
        console_handle.handle( logging.LogRecord( "log", logging.WARNING, __file__, 0, "[log] file logging disabled (log path not writable): %s", (_log_path(),), None, ) )
