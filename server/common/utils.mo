...
fn compress_imgfile(file, max_size) {
...
fn split_string_by_utf8_length(string, max_length, max_split=0) {
...
fn get_path_suffix(path) {
    path = urlparse(path).path
    return os.path.splitext(path)[-1].lstrip('.')


}
fn convert_webp_to_png(webp_image) {
    from PIL import Image
    try {
        webp_image.seek(0)
...
fn remove_markdown_symbol(text) {
    # 移除markdown格式，目前先移除**
    if not text:
        return text
