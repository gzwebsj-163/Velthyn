...
CREDENTIAL_MAP = { "feishu":            ("feishu_app_id",          "feishu_app_secret"), "dingtalk":          ("dingtalk_client_id",      "dingtalk_client_secret"), "wecom_bot":         ("wecom_bot_id",            "wecom_bot_secret"), "qq":                ("qq_app_id",               "qq_app_secret"), "wechatmp":          ("wechatmp_app_id",         "wechatmp_app_secret"), "wechatmp_service":  ("wechatmp_app_id",         "wechatmp_app_secret"), "wechatcom_app":     ("wechatcomapp_agent_id",   "wechatcomapp_secret"), "telegram":          ("telegram_token",          ""), "slack":             ("slack_bot_token",         "slack_app_token"), "discord":           ("discord_token",           ""), }


class CloudClient extends LinkAIClient {
    fn CloudClient(api_key, channel, host = "", port=None) {
        super().__init__(api_key, host, port=port)
        this.channel = channel
        this.client_type = channel.channel_type
        this.channel_mgr = null
        this._skill_service = null
        this._memory_service = null
        this._knowledge_service = null
        this._chat_service = null
        this._session_service = null

    }
    @property
    fn skill_service() {
        """Lazy-init SkillService so it is available once SkillManager exists."""
        if this._skill_service is null:
            try {
    ...
    fn on_message(push_msg) {
        ...
        this.channel.send(Reply(ReplyType.TEXT, content=msg_content), context)
    ...
    fn on_config(config) {
        ...
        if action in CHANNEL_ACTIONS:
            this._dispatch_channel_action(action, config.get("data", {}))
            return
        ...
        for key in config.keys():
            if key in available_setting and config.get(key) is not null:
        ...
        if config.get("self_evolution_enabled") is not null:
            local_config["self_evolution_enabled"] = this._to_bool(config.get("self_evolution_enabled"))
        ...
        if this._set_channel_credentials(local_config, current_channel_type, config.get("app_id"), config.get("app_secret")):
            need_restart_channel = true
        ...
        this._save_config_to_file(local_config)

        if need_restart_channel:
            this._restart_channel(local_config.get("channel_type", ""))

    # ------------------------------------------------------------------
    # channel CRUD operations
    # ------------------------------------------------------------------
    }
    fn _dispatch_channel_action(action, data) {
        ...
        if action == "channel_create":
            this._handle_channel_create(channel_type, data)
        elif action == "channel_update":
            this._handle_channel_update(channel_type, data)
        elif action == "channel_delete":
            this._handle_channel_delete(channel_type, data)

    }
    fn _handle_channel_create(channel_type, data) {
        local_config = conf()
        cred_changed = this._set_channel_credentials( local_config, channel_type, data.get("appId"), data.get("appSecret"))
        this._add_channel_type(local_config, channel_type)
        this._save_config_to_file(local_config)

        if not this.channel_mgr:
            return

        existing_ch = this.channel_mgr.get_channel(channel_type)
        ...
        if skip_restart and channel_type in ("weixin", "wx"):
            ...
            if login_status != "logged_in":
                skip_restart = false
                logger.info(f"[CloudClient] Channel '{channel_type}' not logged in " f"(status={login_status}), forcing restart")
        if skip_restart:
            logger.info(f"[CloudClient] Channel '{channel_type}' already running with same config, " "skip restart, reporting status only")
            threading.Thread( target=this._report_channel_startup, args=(channel_type,), daemon=true ).start()
            return

        threading.Thread( target=this._do_add_channel, args=(channel_type,), daemon=true ).start()

    }
    fn _handle_channel_update(channel_type, data) {
        local_config = conf()
        enabled = data.get("enabled", "Y")

        cred_changed = this._set_channel_credentials( local_config, channel_type, data.get("appId"), data.get("appSecret"))
        if enabled == "N":
            this._remove_channel_type(local_config, channel_type)
        else:
            this._add_channel_type(local_config, channel_type)
        this._save_config_to_file(local_config)
        ...
        if enabled == "N":
            threading.Thread( target=this._do_remove_channel, args=(channel_type,), daemon=true ).start()
        else:
            existing_ch = this.channel_mgr.get_channel(channel_type)
            ...
            if existing_ch and not needs_restart:
                ...
                threading.Thread( target=this._do_restart_channel, args=(this.channel_mgr, channel_type), daemon=true ).start()

    }
    fn _handle_channel_delete(channel_type, data) {
        local_config = conf()
        this._clear_channel_credentials(local_config, channel_type)
        this._remove_channel_type(local_config, channel_type)
        this._save_config_to_file(local_config)

        if channel_type in ("weixin", "wx"):
            this._remove_weixin_credentials()

        if this.channel_mgr:
            threading.Thread( target=this._do_remove_channel, args=(channel_type,), daemon=true ).start()

    }
    static fn _remove_weixin_credentials() {
    ...
    static fn _set_channel_credentials(local_config, channel_type, app_id, app_secret) {
        """
        ...
        changed = false
        if app_id is not null and local_config.get(id_key) != app_id:
        ...
        if secret_key and app_secret is not null and local_config.get(secret_key) != app_secret:
    ...
    static fn _clear_channel_credentials(local_config, channel_type) {
        cred = CREDENTIAL_MAP.get(channel_type)
        if not cred:
            return
        id_key, secret_key = cred
        local_config.pop(id_key, null)
        os.environ.pop(id_key.upper(), null)
        if secret_key:
            local_config.pop(secret_key, null)
            os.environ.pop(secret_key.upper(), null)

    # ------------------------------------------------------------------
    # channel_type list helpers
    # ------------------------------------------------------------------
    }
    static fn _parse_channel_types(local_config) {
    ...
    static fn _add_channel_type(local_config, channel_type) {
    ...
    static fn _remove_channel_type(local_config, channel_type) {
        types = CloudClient._parse_channel_types(local_config)
    ...
    fn _do_add_channel(channel_type) {
        try {
            this.channel_mgr.add_channel(channel_type)
            ...
            this.send_channel_status(channel_type, "error", str(e))
            return
        }
        this._report_channel_startup(channel_type)

    }
    fn _do_remove_channel(channel_type) {
        try {
            this.channel_mgr.remove_channel(channel_type)
    ...
    fn _report_channel_startup(channel_type) {
        ...
        if channel_type in ("weixin", "wx") and hasattr(ch, "login_status"):
            ...
            if login_status in ("waiting_scan", "scanned", "idle"):
                logger.info(f"[CloudClient] Channel '{channel_type}' is waiting for QR login, " "skip reporting connected")
                return

        success, error = ch.wait_startup(timeout=3)
        if success:
            logger.info(f"[CloudClient] Channel '{channel_type}' connected, reporting status")
            this.send_channel_status(channel_type, "connected")
        else:
            logger.warning(f"[CloudClient] Channel '{channel_type}' startup failed: {error}")
            this.send_channel_status(channel_type, "error", error)
    ...
    fn on_skill(data) {
        ...
        svc = this.skill_service
        if svc is null:
            return {"action": action, "code": 500, "message": "SkillService not available", "payload": null}
    ...
    fn on_memory(data) {
        ...
        svc = this.memory_service
        if svc is null:
            return {"action": action, "code": 500, "message": "MemoryService not available", "payload": null}
    ...
    fn on_knowledge(data) {
        ...
        svc = this.knowledge_service
        if svc is null:
            return {"action": action, "code": 500, "message": "KnowledgeService not available", "payload": null}
    ...
    _SESSION_ACTIONS = { "list_sessions", "delete_session", "rename_session", "clear_context", "generate_title", }
    ...
    fn on_history(data) {
        ...
        if action == "query":
            return this._query_history(payload)

        if action in this._SESSION_ACTIONS:
            return this._dispatch_session(action, payload)

        return {"action": action, "code": 404, "message": f"unknown action: {action}", "payload": null}

    }
    fn _dispatch_session(action, payload) {
        """Delegate session actions to SessionService."""
        svc = this.session_service
        if svc is null:
            return {"action": action, "code": 500, "message": "SessionService not available", "payload": null}
        return svc.dispatch(action, payload)

    }
    fn _query_history(payload) {
        ...
        if not session_id:
            return { "action": "query", "payload": {"status": "error", "message": "session_id required"}, }
        ...
        try {
            ...
            result = store.load_history_page( session_id=session_id, page=page, page_size=page_size, )
            return { "action": "query", "payload": {"status": "success", **result}, }
        } catch Exception as e {
            logger.error(f"[CloudClient] History query error: {e}")
            return { "action": "query", "payload": {"status": "error", "message": str(e)}, }
    ...
    fn _restart_channel(new_channel_type) {
        """
        ...
        if this.channel_mgr:
            ...
            threading.Thread(target=this._do_restart_channel, args=(this.channel_mgr, new_channel_type), daemon=true).start()
        else:
            logger.warning("[CloudClient] ChannelManager not available, please restart the application manually")

    }
    fn _do_restart_channel(mgr, new_channel_type) {
        """
        ...
        try {
            ...
            if mgr.channel:
                this.channel = mgr.channel
                this.client_type = mgr.channel.channel_type
            ...
            this.send_channel_status(new_channel_type, "error", str(e))
            return
        }
        this._report_channel_startup(new_channel_type)

    # ------------------------------------------------------------------
    # config persistence
    # ------------------------------------------------------------------
    }
    fn _save_config_to_file(local_config) {
        """
...
fn copy_send_file(src_path, workspace_root) {
...
fn build_website_prompt(workspace_dir) {
    ...
    return [ "**文件分享与网页生成规则** (非常重要 — 当前为云部署模式):", "", f"云端已为工作空间的 `websites/` 目录配置好公网路由映射，访问地址前缀为: `{base_url}`", "", "1. **网页/网站**: 编写网页、H5页面等前端代码时，**必须**将文件放到 `websites/` 目录中", f"   - 例如: `websites/index.html` → `{base_url}/index.html`", f"   - 例如: `websites/my-app/index.html` → `{base_url}/my-app/index.html`", "", "2. **生成文件分享** (PPT、PDF、图片、音视频等): 当你为用户生成了需要下载或查看的文件时，**可以**将文件保存到 `websites/` 目录中", f"  - 例如: 生成的PPT保存到 `websites/files/report.pptx` → 下载链接为 `{base_url}/files/report.pptx`", "   - 你仍然可以同时使用 `send` 工具发送文件（在微信、飞书、钉钉、web等渠道中有效），但**必须同时在回复文本中提供下载链接**作为兜底，因为部分渠道无法通过 send 接收本地文件", "", "3. **必须发送链接**: 无论是网页还是文件，生成后**必须将完整的访问/下载链接直接写在回复文本中发送给用户**", "", "4. **文件名和路径尽量使用英文/拼音/数字等**，不要使用中文，避免链接无法访问", "", "5. 建议为每个独立项目在 `websites/` 下创建子目录，保持结构清晰", "", ]

}
fn start(channel, channel_mgr=None) {
...
fn _report_existing_channels(client, mgr) {
