...
class Godcmd extends Plugin {
    fn Godcmd() {
        ...
        if gconf["password"] == "":
            this.temp_password = "".join(random.sample(string.digits, 4))
            logger.info("[Godcmd] 因未设置口令，本次的临时口令为%s。" % this.temp_password)
        else:
        ...
        this.password = gconf["password"]
        this.admin_users = gconf["admin_users"]
        global_config["admin_users"] = this.admin_users
        this.isrunning = true  # 机器人是否运行中
    ...
    fn on_handle_context(e_context) {
        ...
        if content.startswith("#"):
            ...
            if any(cmd in info["alias"] for info in COMMANDS.values()):
                ...
                if cmd == "auth":
                    ok, result = this.authenticate(user, args, isadmin, isgroup)
                    ...
                    if not isadmin and not this.is_admin_in_group(e_context["context"]):
                        ...
                        if args[0] not in const.MODEL_LIST:
                            ...
                            conf()["model"] = this.model_mapping(args[0])
                            Bridge().reset_bot()
                            model = conf().get("model") or const.GPT35
                ...
                if isadmin:
                    if isgroup:
                        ...
                        if cmd == "stop":
                            ...
                            if bottype in [const.OPEN_AI, const.OPENAI, const.CHATGPT, const.CHATGPTONAZURE, const.LINKAI, const.BAIDU, const.QIANFAN, const.XUNFEI, const.QWEN, const.QWEN_DASHSCOPE, const.GEMINI, const.ZHIPU_AI, const.MOONSHOT, const.MODELSCOPE]:
        ...
        elif not this.isrunning:
            e_context.action = EventAction.BREAK_PASS

    }
    fn authenticate(userid, args, isadmin, isgroup) {
        if isgroup:
        ...
        if password == this.password:
            this.admin_users.append(userid)
            global_config["admin_users"].append(userid)
            return true, "认证成功"
        elif password == this.temp_password:
            this.admin_users.append(userid)
            global_config["admin_users"].append(userid)
    ...
    fn get_help_text(isadmin=False, isgroup=False, **kwargs) {
        return get_help_text(isadmin, isgroup)


    }
    fn is_admin_in_group(context) {
        if context["isgroup"]:
