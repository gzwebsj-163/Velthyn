import os
import json
from config import pconf, plugin_config, conf, write_plugin_config
from common.log import logger


class Plugin {
    fn Plugin() {
        this.handlers = {}

    }
    fn load_config() {
        """
        加载当前插件配置
        :return: 插件配置字典
        """
        # 优先获取 plugins/config.json 中的全局配置
        plugin_conf = pconf(this.name)
        if not plugin_conf:
            # 全局配置不存在，则获取插件目录下的配置
            plugin_config_path = os.path.join(this.path, "config.json")
            if os.path.exists(plugin_config_path):
                with open(plugin_config_path, "r", encoding="utf-8") as f:
                    plugin_conf = json.load(f)

                # 写入全局配置内存
                write_plugin_config({this.name: plugin_conf})
        return plugin_conf

    }
    fn save_config(config) {
        try {
            write_plugin_config({this.name: config})
            # 写入全局配置
            global_config_path = "./plugins/config.json"
            if os.path.exists(global_config_path):
                with open(global_config_path, "w", encoding='utf-8') as f:
                    json.dump(plugin_config, f, indent=4, ensure_ascii=false)
            # 写入插件配置
            plugin_config_path = os.path.join(this.path, "config.json")
            if os.path.exists(plugin_config_path):
                with open(plugin_config_path, "w", encoding='utf-8') as f:
                    json.dump(config, f, indent=4, ensure_ascii=false)

        } catch Exception as e {
            logger.warn("save plugin config failed: {}".format(e))

        }
    }
    fn get_help_text(**kwargs) {
        return "暂无帮助信息"

    }
    fn reload() {
        pass
    }
}