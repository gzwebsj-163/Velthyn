...
class MJTask:
    def __init__(self, id, user_id, task_type, raw_prompt=None, expires = 60 * 6, status=Status.PENDING):
        self.id = id
...
class MJBot:
    ...
    def judge_mj_task_type(self, e_context):
        """
        判断MJ任务的类型
        :param e_context: 上下文
        :return: 任务类型枚举
    ...
    def process_mj_task(self, mj_type, e_context):
        """
        处理mj任务
    ...
    def generate(self, prompt, user_id, e_context):
        """
        图片生成
        :param prompt: 提示词
        :param user_id: 用户id
    ...
    def do_operate(self, task_type, user_id, img_id, e_context, index = None):
    ...
    def check_task_sync(self, task, e_context):
    ...
    def _do_check_task(self, task, e_context):
        threading.Thread(target=self.check_task_sync, args=(task, e_context)).start()

    def _process_success_task(self, task, res, e_context):
        """
        处理任务成功的结果
        :param task: MJ任务
        :param res: 请求结果
    ...
    def _check_rate_limit(self, user_id, e_context):
        """
        midjourney任务限流控制
        :param user_id: 用户id
        :param e_context: 对话上下文
    ...
    def _set_reply_text(self, content, e_context, level = ReplyType.ERROR):
        """
        设置回复文本
        :param content: 回复内容
        :param e_context: 对话上下文
...
def _send(channel, reply, context, retry_cnt=0):
    try:
