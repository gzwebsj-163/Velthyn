...
class LinkAI(Plugin):
    ...
    def on_handle_context(self, e_context):
        ...
        if (context.type == ContextType.SHARING and self._is_summary_open(context)) or                  (context.type == ContextType.TEXT and self._is_summary_open(context) and LinkSummary().check_url(context.content)):
    ...
    # 插件管理功能
    def _process_admin_cmd(self, e_context):
        context = e_context['context']
    ...
    def _is_summary_open(self, context):
        # 获取远程应用插件状态
        ...
        # 基础条件：总开关开启且消息类型符合要求
        base_enabled = ( self.sum_config and self.sum_config.get("enabled") and (context.type.name in ( self.sum_config.get("type") or ["FILE", "SHARING"]) or context.type.name == "TEXT") )
    ...
    # LinkAI 对话任务处理
    def _is_chat_task(self, e_context):
        context = e_context['context']
        # 群聊应用管理
        return self.config.get("group_app_map") and context.kwargs.get("isgroup")

    def _process_chat_task(self, e_context):
        """
        处理LinkAI对话任务
    ...
    def _fetch_group_app_code(self, group_name):
        """
        根据群聊名称获取对应的应用code
    ...
    def _fetch_app_code(self, context):
        """
...
def _send_info(e_context, content):
...
def _set_reply_text(content, e_context, level = ReplyType.ERROR):
