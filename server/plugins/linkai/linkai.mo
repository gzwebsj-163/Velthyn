...
@plugins.register( name="linkai", desc="A plugin that supports knowledge base and midjourney drawing.", version="0.1.0", author="https://link-ai.tech", desire_priority=99 )
class LinkAI extends Plugin {
    fn LinkAI() {
        super().__init__()
        ...
        if this.config:
            this.mj_bot = MJBot(this.config.get("midjourney"), this._fetch_group_app_code)
        this.sum_config = {}
        if this.config:
            this.sum_config = this.config.get("summary")
    ...
    fn on_handle_context(e_context) {
        ...
        if context.type in [ContextType.FILE, ContextType.IMAGE] and this._is_summary_open(context):
            # 文件处理
            context.get("msg").prepare()
            file_path = context.content
            if not LinkSummary().check_file(file_path, this.sum_config):
                return
            if context.type != ContextType.IMAGE:
                _send_info(e_context, "正在为你加速生成摘要，请稍后")
            app_code = this._fetch_app_code(context)
        ...
        if (context.type == ContextType.SHARING and this._is_summary_open(context)) or                  (context.type == ContextType.TEXT and this._is_summary_open(context) and LinkSummary().check_url(context.content)):
        ...
        mj_type = this.mj_bot.judge_mj_task_type(e_context)
        if mj_type:
            # MJ作图任务处理
            this.mj_bot.process_mj_task(mj_type, e_context)
            return

        if context.content.startswith(f"{_get_trigger_prefix()}linkai"):
            # 应用管理功能
            this._process_admin_cmd(e_context)
            return
        ...
        if this._is_chat_task(e_context):
            # 文本对话任务处理
            this._process_chat_task(e_context)

    # 插件管理功能
    }
    fn _process_admin_cmd(e_context) {
        context = e_context['context']
        cmd = context.content.split()
        if len(cmd) == 1 or (len(cmd) == 2 and cmd[1] == "help"):
            _set_reply_text(this.get_help_text(verbose=true), e_context, level=ReplyType.INFO)
            return
        ...
        if len(cmd) == 3 and cmd[1] == "app":
            ...
            group_mapping = this.config.get("group_app_map")
            if group_mapping:
                group_mapping[group_name] = app_code
            else:
                this.config["group_app_map"] = {group_name: app_code}
            # 保存插件配置
            super().save_config(this.config)
    ...
    fn _is_summary_open(context) {
        # 获取远程应用插件状态
        remote_enabled = false
        if context.kwargs.get("isgroup"):
            # 群聊场景只查询群对应的app_code
            group_name = context.get("msg").from_user_nickname
            app_code = this._fetch_group_app_code(group_name)
            if app_code:
        ...
        # 基础条件：总开关开启且消息类型符合要求
        base_enabled = ( this.sum_config and this.sum_config.get("enabled") and (context.type.name in ( this.sum_config.get("type") or ["FILE", "SHARING"]) or context.type.name == "TEXT") )

        # 群聊：需要满足(总开关和群开关)或远程插件开启
        if context.kwargs.get("isgroup"):
            return (base_enabled and this.sum_config.get("group_enabled")) or remote_enabled
    ...
    fn _is_chat_task(e_context) {
        context = e_context['context']
        # 群聊应用管理
        return this.config.get("group_app_map") and context.kwargs.get("isgroup")

    }
    fn _process_chat_task(e_context) {
        """
        处理LinkAI对话任务
    ...
    fn _fetch_group_app_code(group_name) {
        """
        根据群聊名称获取对应的应用code
...
fn _send_info(e_context, content) {
