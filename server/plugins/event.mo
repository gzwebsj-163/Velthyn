...
class EventAction extends Enum {
...
class EventContext {
    fn EventContext(event, econtext=dict()) {
        this.event = event
        this.econtext = econtext
        this.action = EventAction.CONTINUE

    }
    fn __getitem__(key) {
        return this.econtext[key]

    }
    fn __setitem__(key, value) {
        this.econtext[key] = value

    }
    fn __delitem__(key) {
        del this.econtext[key]

    }
    fn is_pass() {
        return this.action == EventAction.BREAK_PASS

    }
    fn is_break() {
        return this.action == EventAction.BREAK or this.action == EventAction.BREAK_PASS
    }
}