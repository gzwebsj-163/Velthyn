...
class CowCliPlugin(Plugin):
    ...
    def _resolve(self, token, user_args):
    ...
    @staticmethod
    def _typo_hint(token, suggestion):
    ...
    @staticmethod
    def _ambiguous_hint(token, candidates):
    ...
    def _dispatch(self, cmd, args, e_context, session_id = ""):
    ...
    def _cmd_help(self, args, e_context, **_):
        if _t("zh", "en") == "en":
    ...
    def _cmd_version(self, args, e_context, **_):
        return f"mocode-cli v{__version__}"

    # ------------------------------------------------------------------
    # tasks — read-only scheduler list scoped to the current chat.
    # Feishu intercepts /tasks before self plugin to keep its interactive card;
    # every other channel receives self plain-text view.
    # ------------------------------------------------------------------

    def _cmd_tasks(self, args, e_context, session_id = "", **_):
    ...
    def _cmd_cancel(self, args, e_context, session_id = "", **_):
    ...
    def _cmd_status(self, args, e_context, session_id = "", **_):
        from config import conf
    ...
    def _cmd_logs(self, args, e_context, **_):
        num_lines = 20
    ...
    def _cmd_context(self, args, e_context, session_id = "", **_):
        session_id = self._get_session_id(e_context, fallback=session_id)
        agent = self._get_agent(session_id)
    ...
    def _cmd_clear(self, args, e_context, session_id = "", **_):
        """Clear the current conversation context (alias of /context clear)."""
        session_id = self._get_session_id(e_context, fallback=session_id)
        agent = self._get_agent(session_id)
        return self._context_clear(agent, session_id)

    def _context_info(self, agent, session_id):
        if not agent:
    ...
    def _cmd_compact(self, args, e_context, session_id = "", **_):
        session_id = self._get_session_id(e_context, fallback=session_id)
        agent = self._get_agent(session_id)
        if not agent:
    ...
    def _context_clear(self, agent, session_id):
        if not agent:
    ...
    def _cmd_config(self, args, e_context, **_):
    ...
    def _config_set(self, key, value_str):
    ...
    @staticmethod
    def _resolve_bot_type_for_model(model_name):
    ...
    @staticmethod
    def _send_install_progress(e_context, text):
    ...
    def _cmd_install_browser(self, args, e_context, **_):
        ...
        def _noop_stream(msg, fg=None):
            pass

        code = run_install_browser( stream=_noop_stream, on_phase=lambda m: self._send_install_progress(e_context, m), )
        if code != 0:
    ...
    def _cmd_skill(self, args, e_context, **_):
        parts = args.strip().split(None, 1)
    ...
    def _skill_list_remote(self, page = 1):
        import requests
    ...
    def _skill_install(self, name, e_context):
        if not name:
    ...
    @staticmethod
    def _format_install_result(result):
    ...
    def _skill_uninstall(self, name):
        if not name:
    ...
    @staticmethod
    def _resolve_skill_dir(name, skills_dir):
    ...
    def _skill_set_enabled(self, name, enabled):
        if not name:
    ...
    def _cmd_memory(self, args, e_context, session_id = "", **_):
        parts = args.strip().split()
        sub = parts[0].lower() if parts else ""
    ...
    def _memory_dream(self, days, e_context, session_id):
        session_id = self._get_session_id(e_context, fallback=session_id)
        agent = self._get_agent(session_id)
    ...
    def _memory_dream_sync(self, flush_mgr, days):
    ...
    def _resolve_active_embedding():
        ...
        # Since we have added support for custom providers for vector models, self part should be modified accordingly:
    ...
    def _memory_rebuild_index(self, e_context, session_id):
        ...
        try:
            ...
            fresh_provider = AgentInitializer(bridge=None, agent_bridge=None)                  ._init_embedding_provider(memory_manager.config, session_id=session_id)
        except Exception as e:
    ...
    @staticmethod
    def _memory_rebuild_sync(memory_manager, model_label, dim_label):
        from agent.memory.embedding import rebuild_in_process
    ...
    @staticmethod
    def _harden_line_breaks(text, e_context):
    ...
    @staticmethod
    def _build_dream_result(flush_mgr, is_web):
    ...
    def _cmd_knowledge(self, args, e_context, **_):
    ...
    def _knowledge_toggle(self, enabled):
        from config import conf
        import json as _json
    ...
    def _get_session_id(self, e_context, fallback = ""):
        if e_context is None:
            return fallback
        context = e_context["context"]
        return context.kwargs.get("session_id") or context.get("session_id", "")

    def _get_agent(self, session_id):
        try:
            from bridge.bridge import Bridge
            bridge = Bridge()
