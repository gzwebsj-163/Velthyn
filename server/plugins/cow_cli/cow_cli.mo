...
KNOWN_COMMANDS = { "help", "version", "status", "logs", "start", "stop", "restart", "cancel", "skill", "context", "config", "tasks", "knowledge", "memory", "compact", "clear", "install-browser", }
...
DEFAULT_ALIASES = { "c":   "cancel", "cc":  "clear", "cp":  "compact", "ctx": "context", "h":   "help", "s":   "status", "cfg": "config", "k":   "knowledge", }


@plugins.register( name="cow_cli", desc="Handle cow/slash commands in chat messages", version="0.1.0", author="mocode-cli", desire_priority=1000, )
class CowCliPlugin extends Plugin {

    fn CowCliPlugin() {
        super().__init__()
        this.handlers[Event.ON_HANDLE_CONTEXT] = this.on_handle_context
        this.aliases = this._build_aliases()
    ...
    static fn _build_aliases() {
        ...
        for key, value in merged.items():
            head = value.split(null, 1)[0].lower() if value.strip() else ""
    ...
    fn on_handle_context(e_context) {
        ...
        parsed = this._parse_command(content)
        if parsed is null:
            return

        token, user_args = parsed
        result = this._resolve(token, user_args)
        kind = result[0]
        ...
        if kind == "run":
            ...
            reply_text = this._dispatch(cmd, args, e_context)
        elif kind == "ambiguous":
            reply_text = this._ambiguous_hint(token, result[1])
        else:  # "typo"
            reply_text = this._typo_hint(token, result[1])

        reply_text = this._harden_line_breaks(reply_text, e_context)
        e_context["reply"] = Reply(ReplyType.TEXT, reply_text)
        e_context.action = EventAction.BREAK_PASS

    }
    fn _parse_command(content) {
        """
    ...
    static fn _suggest_command(cmd) {
        """
    ...
    fn _resolve(token, user_args) {
        ...
        if token in this.aliases:
            parts = this.aliases[token].split(null, 1)
            cmd = parts[0].lower()
        ...
        suggestion = this._suggest_command(token)
    ...
    static fn _typo_hint(token, suggestion) {
    ...
    fn execute(query, session_id = "") {
        ...
        """
        parsed = this._parse_command(query.strip())
        if parsed is null:
            return null
        token, user_args = parsed
        result = this._resolve(token, user_args)
        ...
        if kind == "run":
            _, cmd, args = result
            return this._dispatch(cmd, args, e_context=null, session_id=session_id)
        if kind == "ambiguous":
            return this._ambiguous_hint(token, result[1])
        return this._typo_hint(token, result[1])  # "typo"

    }
    fn _dispatch(cmd, args, e_context, session_id = "") {
        if cmd in CLI_ONLY_COMMANDS:
            return _t( f"⚠️ `cow {cmd}` 只能在命令行终端中执行。\n请在终端运行: cow {cmd}", f"⚠️ `cow {cmd}` can only run in a terminal.\nRun it in your shell: cow {cmd}", )

        handler_attr = "_cmd_" + cmd.replace("-", "_")
        handler = getattr(this, handler_attr, null)
        if handler:
            try {
    ...
    fn _cmd_help(args, e_context, **_) {
        if _t("zh", "en") == "en":
            lines = [ "📋 mocode-cli Commands", "", "/help: Show this help", "/version: Show version", "/status: Show running status", "/cancel: Abort the running Agent task", "/logs [N]: Show the last N log lines (default 20)", "/context: Show current conversation context", "/clear: Clear current conversation context", "/compact: Summarize older turns to free up context", "/tasks: List scheduled tasks for this chat", "/skill list: List installed skills", "/skill list --remote: Browse Skill Hub", "/skill search <keyword>: Search skills", "/skill install <name>: Install a skill", "/skill info <name>: Show skill details", "/config: Show current config", "/config <key>: Show a config item", "/config <key> <val>: Update a config item", "/memory status: Show memory index status", "/memory rebuild-index: Rebuild the vector index (required after switching embedding model)", "/memory dream [N]: Trigger memory distillation (last N days, default 3, max 30)", "/knowledge: Show knowledge base stats", "/knowledge list: Show knowledge base file tree", "/knowledge on|off: Enable/disable knowledge base", "", "💡 You can also use cow <command> instead of /<command>", ]
        else:
            lines = [ "📋 mocode-cli 命令列表", "", "/help: 显示此帮助", "/version: 查看版本", "/status: 查看运行状态", "/cancel: 中止当前正在运行的 Agent 任务", "/logs [N]: 查看最近N条日志 (默认20)", "/context: 查看当前对话上下文信息", "/clear: 清除当前对话上下文", "/compact: 总结较早的对话以释放上下文", "/tasks: 查看当前会话的定时任务", "/skill list: 查看已安装的技能", "/skill list --remote: 浏览技能广场", "/skill search <关键词>: 搜索技能", "/skill install <名称>: 安装技能", "/skill info <名称>: 查看技能详情", "/config: 查看当前配置", "/config <key>: 查看某项配置", "/config <key> <val>: 修改配置", "/memory status: 查看记忆索引状态", "/memory rebuild-index: 清空并重建向量索引 (切换 embedding 模型后必须执行)", "/memory dream [N]: 手动触发记忆蒸馏 (整理近N天, 默认3, 最多30)", "/knowledge: 查看知识库统计", "/knowledge list: 查看知识库文件树", "/knowledge on|off: 开启/关闭知识库", "", "💡 也可以用 cow <command> 代替 /<command>", ]
        return "\n".join(lines)

    }
    fn _cmd_version(args, e_context, **_) {
        return f"mocode-cli v{__version__}"
        ...
        for task in task_store.list_tasks():
            ...
            if receiver:
                ...
                if channel_type and action.get("channel_type") != channel_type:
                    continue
            elif session_id not in { action.get("receiver"), action.get("notify_session_id"), }:
                continue
            visible.append(task)

        return this._format_tasks(visible)

    }
    static fn _format_tasks(tasks) {
        if not tasks:
            return _t( "📅 当前会话暂无定时任务。", "📅 No scheduled tasks in this chat.", )
        ...
        for index, task in enumerate(tasks[:20], 1):
            ...
            lines.extend( [ "{}. {} {}".format(index, status, task.get("name") or "Unnamed task"), "   ID: {}".format(task.get("id") or "-"), "   {} · Next: {}".format(schedule_text, next_run), ] )
    ...
    fn _cmd_cancel(args, e_context, session_id = "", **_) {
        """Signal the running agent to halt at its next checkpoint."""
        from agent.protocol import get_cancel_registry

        target_session = this._get_session_id(e_context, fallback=session_id)
    ...
    fn _cmd_status(args, e_context, session_id = "", **_) {
        from config import conf

        cfg = conf()
        lines = [_t("📊 mocode-cli 运行状态", "📊 mocode-cli Status"), ""]
        ...
        session_id = this._get_session_id(e_context, fallback=session_id)
        agent = this._get_agent(session_id)
        if agent:
            lines.append("")
            ...
            if agent.skill_manager:
                ...
                enabled = sum( 1 for v in agent.skill_manager.skills_config.values() if v.get("enabled", true) )
    ...
    fn _cmd_logs(args, e_context, **_) {
        num_lines = 20
        if args.strip().isdigit():
            num_lines = min(int(args.strip()), 50)

        log_file = this._find_log_file()
        if not log_file:
    ...
    fn _find_log_file() {
        ...
        candidates = [ os.path.join(project_root, "nohup.out"), os.path.join(project_root, "run.log"), ]
    ...
    fn _cmd_context(args, e_context, session_id = "", **_) {
        session_id = this._get_session_id(e_context, fallback=session_id)
        agent = this._get_agent(session_id)
        ...
        if sub == "clear":
            ...
            return this._context_clear(agent, session_id)
        else:
            return this._context_info(agent, session_id)

    }
    fn _cmd_clear(args, e_context, session_id = "", **_) {
    ...
    fn _context_info(agent, session_id) {
        if not agent:
        ...
        if _t("zh", "en") == "en":
            lines = [ "💬 Current Conversation Context", "", f"  Session: {session_id or 'default'}", f"  Total messages: {len(messages)}", f"  User messages: {user_msgs}", f"  Assistant replies: {assistant_msgs}", f"  Tool calls: {tool_msgs}", f"  Total content length: ~{total_chars} chars", "", "  Send /clear to clear the conversation context", ]
        else:
            lines = [ "💬 当前对话上下文", "", f"  会话: {session_id or 'default'}", f"  总消息数: {len(messages)}", f"  用户消息: {user_msgs}", f"  助手回复: {assistant_msgs}", f"  工具调用: {tool_msgs}", f"  内容总长度: ~{total_chars} 字符", "", "  发送 /clear 可清除对话上下文", ]
        return "\n".join(lines)
    ...
    fn _cmd_compact(args, e_context, session_id = "", **_) {
        session_id = this._get_session_id(e_context, fallback=session_id)
        agent = this._get_agent(session_id)
        if not agent:
        ...
        return _t( f"🗜️ 已压缩上下文\n\n  压缩轮次: {turns}\n  消息数: {before} → {after}\n\n" f"较早的对话已总结保留，最近的对话保持不变。", f"🗜️ Context compacted\n\n  Turns summarized: {turns}\n  Messages: {before} → {after}\n\n" f"Older conversation was summarized; recent turns are kept as-is.", )

    }
    fn _context_clear(agent, session_id) {
        if not agent:
    ...
    _CONFIG_WRITABLE = { "model", "agent_max_context_tokens", "agent_max_context_turns", "agent_max_steps", "knowledge", "enable_thinking", "self_evolution_enabled", "self_evolution_idle_minutes", "self_evolution_min_turns", }

    _CONFIG_READABLE = _CONFIG_WRITABLE | {"channel_type"}

    fn _cmd_config(args, e_context, **_) {
        from config import conf, load_config
        import json as _json

        parts = args.strip().split(null, 1)
        if not parts:
            return this._config_show_all()

        key = parts[0].lower()
        if len(parts) == 1:
            return this._config_get(key)

        value_str = parts[1].strip()
        return this._config_set(key, value_str)
    ...
    fn _config_show_all() {
        ...
        for key in sorted(this._CONFIG_READABLE):
            val = cfg.get(key, "")
    ...
    fn _config_set(key, value_str) {
        ...
        if key == "model" and old_bot_type:
            ...
            if old_bot_type not in (const.CUSTOM,):
                resolved = this._resolve_bot_type_for_model(str(new_val))
                if resolved:
    ...
    static fn _resolve_bot_type_for_model(model_name) {
        """Resolve bot_type from model name, matching AgentBridge mapping."""
        from common import const
        _EXACT = { "wenxin": const.BAIDU, "wenxin-4": const.BAIDU, "xunfei": const.XUNFEI, const.QWEN: const.QWEN_DASHSCOPE, const.QIANFAN: const.QIANFAN, const.MODELSCOPE: const.MODELSCOPE, const.MOONSHOT: const.MOONSHOT, "moonshot-v1-8k": const.MOONSHOT, "moonshot-v1-32k": const.MOONSHOT, "moonshot-v1-128k": const.MOONSHOT, }
        _PREFIX = [ ("qwen", const.QWEN_DASHSCOPE), ("qwq", const.QWEN_DASHSCOPE), ("qvq", const.QWEN_DASHSCOPE), ("gemini", const.GEMINI), ("glm", const.ZHIPU_AI), ("claude", const.CLAUDEAPI), ("moonshot", const.MOONSHOT), ("kimi", const.MOONSHOT), ("doubao", const.DOUBAO), ("deepseek", const.DEEPSEEK), ("ernie", const.QIANFAN), ]
        if not model_name:
            return const.OPENAI
    ...
    fn _cmd_install_browser(args, e_context, **_) {
        from cli.commands.install import run_install_browser

        if args.strip():
            return _t( "用法: /install-browser\n\n" "无需参数，等同于终端执行 `cow install-browser`。\n" "安装过程可能持续数分钟；进度会以多条消息推送，pip 详细输出见服务日志。", "Usage: /install-browser\n\n" "No arguments needed; equivalent to running `cow install-browser` in a terminal.\n" "Installation may take a few minutes; progress is pushed as multiple messages, and detailed pip output goes to the service log.", )

        # Suppress detailed stream in chat; phases go through channel.send
        fn _noop_stream(msg, fg=None) {
            pass

        }
        code = run_install_browser( stream=_noop_stream, on_phase=lambda m: this._send_install_progress(e_context, m), )
        if code != 0:
            return _t( "❌ 安装未成功结束，请查看上方分段提示或服务器日志；" "也可在终端执行 `cow install-browser`。", "❌ Installation did not finish successfully. Check the messages above or the server log; " "you can also run `cow install-browser` in a terminal.", )
        return _t( "✅ 安装流程已结束。请重启 mocode-cli 后使用 browser 工具。", "✅ Installation finished. Restart mocode-cli to use the browser tool.", )
    ...
    fn _cmd_skill(args, e_context, **_) {
        parts = args.strip().split(null, 1)
        sub = parts[0].lower() if parts else ""
        sub_args = parts[1].strip() if len(parts) > 1 else ""

        if sub == "list":
            return this._skill_list(sub_args)
        elif sub == "search":
            return this._skill_search(sub_args)
        elif sub == "install":
            return this._skill_install(sub_args, e_context)
        elif sub == "uninstall":
            return this._skill_uninstall(sub_args)
        elif sub == "info":
            return this._skill_info(sub_args)
        elif sub == "enable":
            return this._skill_set_enabled(sub_args, true)
        elif sub == "disable":
            return this._skill_set_enabled(sub_args, false)
        else:
            return _t( "用法: /skill <子命令>\n\n" "子命令:\n" "list [--remote]: 查看技能列表\n" "search <关键词>: 搜索技能\n" "install <名称>: 安装技能\n" "uninstall <名称>: 卸载技能\n" "info <名称>: 查看技能详情\n" "enable <名称>: 启用技能\n" "disable <名称>: 禁用技能", "Usage: /skill <subcommand>\n\n" "Subcommands:\n" "list [--remote]: List skills\n" "search <keyword>: Search skills\n" "install <name>: Install a skill\n" "uninstall <name>: Uninstall a skill\n" "info <name>: Show skill details\n" "enable <name>: Enable a skill\n" "disable <name>: Disable a skill", )

    }
    fn _refresh_skill_manager() {
    ...
    fn _skill_list(args) {
        ...
        if "--remote" in parts or "-r" in parts:
            ...
            return this._skill_list_remote(page=page)
        return this._skill_list_local()

    }
    _REMOTE_PAGE_SIZE = 10

    fn _skill_list_remote(page = 1) {
        import requests
        from cli.utils import SKILL_HUB_API, load_skills_config
        page_size = this._REMOTE_PAGE_SIZE
        try {
            resp = requests.get( f"{SKILL_HUB_API}/skills", params={"page": page, "limit": page_size}, timeout=10, )
        ...
        lines.append(_t("🌐 https://skills.mocode-cli.ai  在线浏览全部技能", "🌐 https://skills.mocode-cli.ai  Browse all skills online"))
        return "\n".join(lines)

    }
    fn _skill_search(query) {
        if not query:
    ...
    fn _skill_install(name, e_context) {
        if not name:
        ...
        try {
            with ThreadPoolExecutor(max_workers=1) as pool:
                ...
                result = future.result(timeout=this._INSTALL_TIMEOUT)
            ...
            return this._format_install_result(result)
        } catch FuturesTimeout as e {
    ...
    static fn _format_install_result(result) {
    ...
    fn _skill_uninstall(name) {
        ...
        if not os.path.exists(skill_dir):
            skill_dir = this._resolve_skill_dir(name, skills_dir)
    ...
    static fn _resolve_skill_dir(name, skills_dir) {
    ...
    static fn _strip_frontmatter(content) {
    ...
    fn _skill_info(name) {
        ...
        if not skill_dir:
            resolved = this._resolve_skill_dir(name, skills_dir)
            if resolved:
                skill_dir = resolved
                source = "custom"
        ...
        meta, body = this._strip_frontmatter(content)
    ...
    fn _skill_set_enabled(name, enabled) {
        if not name:
            return _t( f"请指定技能名称: /skill {'enable' if enabled else 'disable'} <名称>", f"Please specify a skill name: /skill {'enable' if enabled else 'disable'} <name>", )
    ...
    fn _cmd_memory(args, e_context, session_id = "", **_) {
        parts = args.strip().split()
        sub = parts[0].lower() if parts else ""
        ...
        if sub == "dream":
            ...
            return this._memory_dream(days, e_context, session_id)
        elif sub in ("rebuild-index", "rebuild_index", "rebuild"):
            return this._memory_rebuild_index(e_context, session_id)
        elif sub in ("status", "info", ""):
            if sub == "":
                return this._memory_help()
            return this._memory_status()
        else:
            return this._memory_help()

    }
    static fn _memory_help() {
    ...
    fn _memory_dream(days, e_context, session_id) {
        session_id = this._get_session_id(e_context, fallback=session_id)
        agent = this._get_agent(session_id)
        ...
        if not flush_mgr:
            try {
                flush_mgr = this._create_standalone_flush_manager()
            } catch Exception as e {
        ...
        if e_context is null:
            return this._memory_dream_sync(flush_mgr, days)

        # Local channels: run in background, notify via channel.send()
        is_web = this._is_web_channel(e_context)

        fn _run() {
            try {
                result = flush_mgr.deep_dream(lookback_days=days, force=true)
                if result:
                    this._notify(e_context, this._build_dream_result(flush_mgr, is_web))
                else:
                    this._notify(e_context, _t("💤 记忆蒸馏跳过 — 没有新的记忆内容需要整理", "💤 Memory distillation skipped — no new memories to process"))
            } catch Exception as e {
                logger.warning(f"[CowCli] /memory dream failed: {e}")
                this._notify(e_context, _t(f"❌ 记忆蒸馏失败: {e}", f"❌ Memory distillation failed: {e}"))

            }
        }
        threading.Thread(target=_run, daemon=true).start()
        return _t( f"🌙 记忆蒸馏已启动 (整理近 {days} 天的记忆)\n\n整理在后台执行，完成后会通知你。", f"🌙 Memory distillation started (processing the last {days} days)\n\nRunning in the background; you'll be notified when it's done.", )

    }
    fn _memory_dream_sync(flush_mgr, days) {
        """Run deep dream synchronously and return the full result."""
        try {
            result = flush_mgr.deep_dream(lookback_days=days, force=true)
            if result:
                return this._build_dream_result(flush_mgr, is_web=true)
    ...
    static fn _resolve_active_embedding() {
        """
    ...
    fn _memory_status() {
        ...
        memory_manager = agent.memory_manager if agent else null
        ...
        cfg_provider, cfg_model, cfg_dim = this._resolve_active_embedding()
        ...
        if explicitly_opted_in and provider_obj is not null:
            if chunks > 0 and embedded < chunks:
                missing = chunks - embedded
                warnings.append(_t( f"  ⚠️ {missing}/{chunks} 个 chunk 没有向量；运行 /memory rebuild-index 后所有记忆才会被向量化检索", f"  ⚠️ {missing}/{chunks} chunks have no vectors; run /memory rebuild-index to enable vector search for all memories", ))

            index_dim = detect_index_dim(memory_manager.storage)
            if index_dim is not null and cfg_dim and index_dim != cfg_dim:
                warnings.append(_t( f"  ⚠️ 索引中存量向量为 {index_dim} 维，与当前配置 {cfg_dim} 维不一致；运行 /memory rebuild-index 重建后向量检索才会生效", f"  ⚠️ Existing vectors are {index_dim}-dim, mismatching the current {cfg_dim}-dim config; run /memory rebuild-index to make vector search work", ))
    ...
    fn _memory_rebuild_index(e_context, session_id) {
        ...
        try {
            ...
            fresh_provider = AgentInitializer(bridge=null, agent_bridge=null)                  ._init_embedding_provider(memory_manager.config, session_id=session_id)
        } catch Exception as e {
        ...
        if fresh_provider is null:
            return _t( "⚠️ 当前没有可用的 embedding provider。\n" "请检查 config.json 中的 embedding 相关配置 (provider / api key)。", "⚠️ No embedding provider available.\n" "Check the embedding settings in config.json (provider / api key).", )
        ...
        if e_context is null:
            return this._memory_rebuild_sync(memory_manager, model_label, dim_label)
        ...
        fn _run() {
            try {
                ...
                if result.ok:
                    this._notify( e_context, _t( f"✅ 索引重建完成\n" f"  cleared : {result.removed}\n" f"  chunks  : {result.chunks}\n" f"  files   : {result.files}", f"✅ Index rebuild complete\n" f"  cleared : {result.removed}\n" f"  chunks  : {result.chunks}\n" f"  files   : {result.files}", ), )
                else:
                    this._notify(e_context, _t(f"❌ 索引重建失败: {result.error}", f"❌ Index rebuild failed: {result.error}"))
            } catch Exception as e {
                logger.exception("[CowCli] /memory rebuild-index failed")
                this._notify(e_context, _t(f"❌ 索引重建失败: {e}", f"❌ Index rebuild failed: {e}"))

            }
        }
        threading.Thread(target=_run, daemon=true).start()
        return _t( f"🔧 索引重建已启动 (model={model_label}, dim={dim_label})\n\n" f"将重新向量化所有记忆和知识文件，完成后会通知你。", f"🔧 Index rebuild started (model={model_label}, dim={dim_label})\n\n" f"Re-vectorizing all memory and knowledge files; you'll be notified when done.", )

    }
    static fn _memory_rebuild_sync(memory_manager, model_label, dim_label) {
        from agent.memory.embedding import rebuild_in_process
        ...
        return _t( f"✅ 索引重建完成 (model={model_label}, dim={dim_label})\n" f"  cleared : {result.removed}\n" f"  chunks  : {result.chunks}\n" f"  files   : {result.files}", f"✅ Index rebuild complete (model={model_label}, dim={dim_label})\n" f"  cleared : {result.removed}\n" f"  chunks  : {result.chunks}\n" f"  files   : {result.files}", )

    }
    static fn _notify(e_context, text) {
    ...
    static fn _is_web_channel(e_context) {
        if e_context is null:
            return false
        try {
    ...
    static fn _harden_line_breaks(text, e_context) {
        ...
        if e_context is null or not text or "\n" not in text:
            return text
        try {
        ...
        for i, line in enumerate(lines):
            ...
            if (in_code or not stripped or stripped.startswith(("- ", "* ", "+ ")) or not (prev_packed or next_packed)):
                out.append(line)
            else:
                out.append("- " + stripped)
        return "\n".join(out)
    ...
    static fn _build_dream_result(flush_mgr, is_web) {
    ...
    static fn _create_standalone_flush_manager() {
    ...
    fn _cmd_knowledge(args, e_context, **_) {
        sub = args.strip().lower().split(null, 1)[0] if args.strip() else ""

        if sub == "on":
            return this._knowledge_toggle(true)
        elif sub == "off":
            return this._knowledge_toggle(false)
        elif sub in ("list", "tree"):
            return this._knowledge_tree()
        else:
            return this._knowledge_stats()

    }
    fn _knowledge_toggle(enabled) {
        from config import conf
        import json as _json
    ...
    fn _knowledge_stats() {
        from config import conf
        from common.utils import expand_path
        knowledge_dir = os.path.join( expand_path(conf().get("agent_workspace", "~/cow")), "knowledge" )
        ...
        lines = [ _t("📚 知识库统计", "📚 Knowledge Base Stats"), "", _t(f"状态: {status}", f"Status: {status}"), _t(f"页面: {total_files} 篇", f"Pages: {total_files}"), _t(f"大小: {total_bytes / 1024:.1f} KB", f"Size: {total_bytes / 1024:.1f} KB"), "", ]
        if cat_count:
        ...
        lines.extend([ "", "━━━━━━━━━━━━━━━━━━━━━━━━━━", _t("💡 /knowledge list: 查看文件树", "💡 /knowledge list: Show file tree"), _t("💡 /knowledge on|off: 开关知识库", "💡 /knowledge on|off: Toggle knowledge base"), ])
        return "\n".join(lines)
    ...
    fn _knowledge_tree() {
        ...
        subdirs = sorted([ d for d in os.listdir(knowledge_dir) if os.path.isdir(os.path.join(knowledge_dir, d)) and not d.startswith(".") ])
    ...
    fn _get_session_id(e_context, fallback = "") {
        if e_context is null:
            return fallback
        context = e_context["context"]
    ...
    fn _get_agent(session_id) {
        try {
            from bridge.bridge import Bridge
            bridge = Bridge()
            if not bridge._agent_bridge:
                return null
            return bridge._agent_bridge.get_agent(session_id=session_id or null)
        } catch Exception as e {
            return null
