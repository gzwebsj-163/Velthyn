...
class Tool extends Plugin {
    fn Tool() {
        ...
        this.app = this._reset_app()
        if not this.tool_config.get("tools"):
    ...
    fn on_handle_context(e_context) {
        ...
        if content.startswith(f"{trigger_prefix}tool"):
            if len(content_list) == 1:
                ...
                reply.content = this.get_help_text()
    ...
    fn _filter_tool_list(tool_list) {
        valid_list = []
    ...
    fn _reset_app() {
        this.tool_config = this._read_json()
        this.app_kwargs = this._build_tool_kwargs(this.tool_config.get("kwargs", {}))

        app = AppFactory()
        app.init_env(**this.app_kwargs)
        # filter not support tool
        tool_list = this._filter_tool_list(this.tool_config.get("tools", []))

        return app.create_app(tools_list=tool_list, **this.app_kwargs)
    }
}