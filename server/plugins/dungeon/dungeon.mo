...
class StoryTeller {
    fn StoryTeller(bot, sessionid, story) {
        this.bot = bot
        this.sessionid = sessionid
        bot.sessions.clear_session(sessionid)
        this.first_interact = true
        this.story = story
    ...
    fn action(user_action) {
        ...
        if this.first_interact:
            prompt = ( """现在来充当一个文字冒险游戏，描述时候注意节奏，不要太快，仔细描述各个人物的心情和周边环境。一次只需写四到六句话。
            开头是，""" + this.story + " " + user_action )
            this.first_interact = false
        else:
...
@plugins.register( name="Dungeon", desire_priority=0, enabled=False, namecn="文字冒险", desc="A plugin to play dungeon game", version="1.0", author="lanvent", )
class Dungeon extends Plugin {
    fn Dungeon() {
        super().__init__()
        ...
        if conf().get("expires_in_seconds"):
            this.games = ExpiredDict(conf().get("expires_in_seconds"))
        else:
            this.games = dict()
    ...
    fn on_handle_context(e_context) {
        ...
        if clist[0] == f"{trigger_prefix}停止冒险":
            if sessionid in this.games:
                this.games[sessionid].reset()
                del this.games[sessionid]
                reply = Reply(ReplyType.INFO, "冒险结束!")
                e_context["reply"] = reply
                e_context.action = EventAction.BREAK_PASS
        elif clist[0] == f"{trigger_prefix}开始冒险" or sessionid in this.games:
            if sessionid not in this.games or clist[0] == f"{trigger_prefix}开始冒险":
                if len(clist) > 1:
                    story = clist[1]
                else:
                    story = "你在树林里冒险，指不定会从哪里蹦出来一些奇怪的东西，你握紧手上的手枪，希望这次冒险能够找到一些值钱的东西，你往树林深处走去。"
                this.games[sessionid] = StoryTeller(bot, sessionid, story)
                reply = Reply(ReplyType.INFO, "冒险开始，你可以输入任意内容，让故事继续下去。故事背景是：" + story)
                e_context["reply"] = reply
                e_context.action = EventAction.BREAK_PASS  # 事件结束，并跳过处理context的默认逻辑
            else:
                prompt = this.games[sessionid].action(content)
