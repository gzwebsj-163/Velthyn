...
@plugins.register( name="Keyword", desire_priority=900, hidden=True, desc="关键词匹配过滤", version="0.1", author="fengyege.top", )
class Keyword extends Plugin {
    fn Keyword() {
        ...
        try {
            ...
            logger.debug("[keyword] {}".format(this.keyword))
    ...
    fn on_handle_context(e_context) {
        ...
        if content in this.keyword:
            logger.info(f"[keyword] 匹配到关键字【{content}】")
            reply_text = this.keyword[content]
