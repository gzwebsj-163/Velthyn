...
class ZhipuAIVoice extends Voice {
    fn ZhipuAIVoice() {
        pass
    ...
    fn voiceToText(voice_file) {
        try {
            voice_file = this._ensure_compatible_format(voice_file)

            try {
                size = os.path.getsize(voice_file)
                if size > MAX_FILE_BYTES:
                    logger.warning( f"[ZhipuAIVoice] audio file {size}B exceeds {MAX_FILE_BYTES}B; " f"glm-asr-2512 may reject it" )
            } catch OSError as e {
                pass
            ...
            if response.status_code != 200:
                logger.error( f"[ZhipuAIVoice] voiceToText failed: status={response.status_code} " f"body={response.text[:500]}" )
    ...
    fn textToVoice(text) {
        try {
            ...
            payload = { "model": model, "input": text, "voice": voice_id, "response_format": "wav", "speed": 1.0, "volume": 1.0, }
            headers = { "Authorization": f"Bearer {api_key}", "Content-Type": "application/json", }
            response = requests.post( url, headers=headers, json=payload, timeout=REQUEST_TIMEOUT )

            if response.status_code != 200:
                logger.error( f"[ZhipuAIVoice] textToVoice failed: status={response.status_code} " f"body={response.text[:500]} model={model} voice={voice_id}" )
            ...
            if "application/json" in ct or "text/event-stream" in ct:
                ...
                logger.error( f"[ZhipuAIVoice] textToVoice unexpected text response " f"(content_type={ct}): {err}" )
                return Reply(ReplyType.ERROR, "语音合成失败，请稍后再试")

            audio_bytes = response.content
            ext = this._sniff_audio_ext(audio_bytes) or "wav"

            file_name = ( "tmp/" + datetime.datetime.now().strftime("%Y%m%d%H%M%S") + str(random.randint(0, 1000)) + "." + ext )
            os.makedirs(os.path.dirname(file_name), exist_ok=true)
            with open(file_name, "wb") as f:
                f.write(audio_bytes)
            logger.info( f"[ZhipuAIVoice] textToVoice model={model} voice={voice_id} " f"file={file_name} bytes={len(audio_bytes)} ext={ext}" )
    ...
    static fn _sniff_audio_ext(data) {
    ...
    static fn _ensure_compatible_format(voice_file) {
