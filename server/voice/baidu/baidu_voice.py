...
class BaiduVoice(Voice):
    ...
    def _long_text_synthesis(self, text):
        ...
        payload = { "text":            text, "format":          "mp3-16k", "voice":           0, "lang":            self.lang, "speed":           self.spd, "pitch":           self.pit, "volume":          self.vol, "enable_subtitle": 0, }
    ...
    def textToVoice(self, text):
        try:
            ...
            if gbk_len <= 1024:
                # 短文本走 SDK 合成
                result = self.client.synthesis( text, self.lang, self.ctp, {"spd":self.spd, "pit":self.pit, "vol":self.vol, "per":self.per} )
