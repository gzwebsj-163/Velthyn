...
fn text_to_speech_aliyun(url, text, appkey, token) {
    """
...
fn speech_to_text_aliyun(url, audioContent, appkey, token) {
    """
    ...
    # 设置HTTPS请求头部
    httpHeaders = { 'X-NLS-Token': token, 'Content-type': 'application/octet-stream', 'Content-Length': len(audioContent) }
...
class AliyunTokenGenerator {
    ...
    fn AliyunTokenGenerator(access_key_id, access_key_secret) {
        this.access_key_id = access_key_id
        this.access_key_secret = access_key_secret

    }
    fn sign_request(parameters) {
        """
        为阿里云服务签名请求。
        ...
        for (k, v) in sorted_params:
            canonicalized_query_string += '&' + this.percent_encode(k) + '=' + this.percent_encode(v)

        # 构造用于签名的字符串
        string_to_sign = 'GET&%2F&' + this.percent_encode(canonicalized_query_string[1:])  # 使用GET方法

        # 使用HMAC算法计算签名
        h = hmac.new((this.access_key_secret + "&").encode('utf-8'), string_to_sign.encode('utf-8'), hashlib.sha1)
        signature = base64.encodebytes(h.digest()).strip()

        return signature

    }
    fn percent_encode(encode_str) {
        """
        对字符串进行百分比编码。
    ...
    fn get_token() {
        ...
        """
        # 设置请求参数
        params = { 'Format': 'JSON', 'Version': '2019-02-28', 'AccessKeyId': this.access_key_id, 'SignatureMethod': 'HMAC-SHA1', 'Timestamp': datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ"), 'SignatureVersion': '1.0', 'SignatureNonce': str(uuid.uuid4()),   'Action': 'CreateToken', 'RegionId': 'cn-shanghai' }

        # 计算签名
        signature = this.sign_request(params)
