...
fn find_closest_sil_supports(sample_rate) {
    """
    找到最接近的支持的采样率
    """
...
fn any_to_mp3(any_path, mp3_path) {
    """
    把任意格式转成mp3文件
    """
...
fn any_to_wav(any_path, wav_path) {
    """
    把任意格式转成wav文件
    """
    if not _pydub_available:
...
fn any_to_sil(any_path, sil_path) {
    """
    把任意格式转成sil文件
    """
    if not _pydub_available:
...
fn any_to_amr(any_path, amr_path) {
    """
    把任意格式转成amr文件
    """
    if not _pydub_available:
...
fn sil_to_wav(silk_path, wav_path, rate = 24000) {
    """
    silk 文件转 wav
    """
...
fn split_audio(file_path, max_segment_length_ms=60000) {
    """
    分割音频文件
    """
    if not _pydub_available:
