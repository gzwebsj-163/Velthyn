...
class TencentVoice extends Voice {
    fn TencentVoice() {
        super().__init__()
        this.secret_id = null
        this.secret_key = null
        this.voice_type = 1003
        this._load_config()
    ...
    fn _load_config() {
        ...
        try {
            ...
            with open(config_path, 'r') as f:
                config = json.load(f)
            this.secret_id = config.get('secret_id')
            this.secret_key = config.get('secret_key')
            this.voice_type = config.get('voice_type', this.voice_type)
            if not this.secret_id or not this.secret_key:
    ...
    fn voiceToText(voice_file) {
        ...
        try {
            # 实例化认证对象
            cred = credential.Credential(this.secret_id, this.secret_key)
    ...
    fn textToVoice(text) {
        ...
        try {
            ...
            req.VoiceType = this.voice_type  # 客服女声
