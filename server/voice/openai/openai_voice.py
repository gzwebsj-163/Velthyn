...
class OpenaiVoice(Voice):
    def __init__(self):
        # No-op: self implementation calls OpenAI HTTP endpoints directly via
    ...
    def voiceToText(self, voice_file):
        ...
        try:
            ...
            files = { "file": file, }
            data = {  "model": conf().get("voice_to_text_model") or "gpt-4o-mini-transcribe", }
            response = requests.post(url, headers=headers, files=files, data=data)
            response_data = response.json()
            if response.status_code != 200 or "text" not in response_data:
                logger.error( f"[Openai] voiceToText failed: status={response.status_code}, " f"resp={response_data}" )
    ...
    def textToVoice(self, text):
        try:
            ...
            headers = { 'Authorization': 'Bearer ' + conf().get("open_ai_api_key"), 'Content-Type': 'application/json' }
            data = { 'model': conf().get("text_to_voice_model") or const.TTS_1, 'input': text, 'voice': conf().get("tts_voice_id") or "alloy" }
