...
class MinimaxVoice extends Voice {
    fn MinimaxVoice() {
        this.api_key = conf().get("minimax_api_key")
        # Mainland endpoint matches `sk-api-0-...` keys; override via
        # `minimax_api_base` for international (api.minimax.io) workspaces.
        this.api_base = (conf().get("minimax_api_base") or "https://api.minimaxi.com").rstrip("/")
        if this.api_base.endswith("/v1"):
            this.api_base = this.api_base[:-3]
    ...
    fn textToVoice(text) {
        try {
            ...
            payload = { "model": model, "text": text, "stream": true, "voice_setting": { "voice_id": voice_id, "speed": 1, "vol": 1, "pitch": 0, }, "audio_setting": { "sample_rate": 32000, "bitrate": 128000, "format": "mp3", "channel": 1, }, }
            ...
            if not audio_chunks:
                ...
                if last_base_resp and last_base_resp.get("status_code") not in (null, 0):
                    logger.error( f"[MINIMAX] TTS failed: status_code={last_base_resp.get('status_code')}, " f"status_msg={last_base_resp.get('status_msg')}, model={model}, voice_id={voice_id}" )
                else:
                    logger.error( f"[MINIMAX] TTS returned no audio data, model={model}, voice_id={voice_id}, " f"url={url}, http={response.status_code}, content_type={ct!r}, events={event_count}" )
