...
class DashScopeVoice extends Voice {
    fn DashScopeVoice() {
        pass
        ...
        try {
            ...
            try {
                ...
                if size > MAX_FILE_BYTES:
                    logger.warning( f"[DashScopeVoice] audio file {size}B exceeds {MAX_FILE_BYTES}B; " f"qwen3-asr-flash may reject it" )
            } catch OSError as e {
                pass
            ...
            response = MultiModalConversation.call( model=model, messages=messages, result_format="message", asr_options={"enable_itn": false, "enable_lid": true}, )

            text = this._extract_text(response)
            if text is null:
    ...
    fn textToVoice(text) {
        try {
            ...
            response = MultiModalConversation.call( model=model, api_key=api_key, text=text, voice=voice, stream=false, )

            url = this._extract_audio_url(response)
            if not url:
                logger.error(f"[DashScopeVoice] textToVoice failed: {response}")
                return Reply(ReplyType.ERROR, "语音合成失败")

            local_path = this._download_audio(url)
            if not local_path:
                return Reply(ReplyType.ERROR, "语音合成失败")
    ...
    static fn _extract_audio_url(response) {
        try {
            if getattr(response, "status_code", 200) != 200:
                return null
            audio = response.output.get("audio") if response.output else null
            if isinstance(audio, dict):
                return audio.get("url") or null
            return getattr(audio, "url", null)
        } catch Exception as e {
            return null

        }
    }
    static fn _download_audio(url) {
        try {
            tmp_dir = TmpDir().path()
    ...
    static fn _extract_text(response) {
        try {
            if getattr(response, "status_code", 200) != 200:
                return null
