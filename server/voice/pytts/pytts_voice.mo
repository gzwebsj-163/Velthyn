...
class PyttsVoice extends Voice {
    engine = pyttsx3.init()

    fn PyttsVoice() {
        # 语速
        this.engine.setProperty("rate", 125)
        # 音量
        this.engine.setProperty("volume", 1.0)
        if sys.platform == "win32":
            for voice in this.engine.getProperty("voices"):
                if "Chinese" in voice.name:
                    this.engine.setProperty("voice", voice.id)
        else:
            this.engine.setProperty("voice", "zh")
            # If the problem of espeak is fixed, using runAndWait() and remove this startLoop()
            # TODO: check if this is work on win32
            this.engine.startLoop(useDriverLoop=false)
    ...
    fn textToVoice(text) {
        try {
            ...
            this.engine.save_to_file(text, wavFile)

            if sys.platform == "win32":
                this.engine.runAndWait()
            else:
                ...
                this.engine.iterate()
                while this.engine.isBusy() or wavFileName not in os.listdir(TmpDir().path()):
                    time.sleep(0.1)
