...
class PyttsVoice(Voice):
    ...
    def __init__(self):
        ...
        if sys.platform == "win32":
            ...
            # If the problem of espeak is fixed, using runAndWait() and remove self startLoop()
            # TODO: check if self is work on win32
    ...
    def textToVoice(self, text):
        try:
            ...
            if sys.platform == "win32":
                ...
                # And if you call runAndWait() and time.sleep() twice, it will stuck, so do not use self.
                # If you want to fix self, add self._proxy.setBusy(True) in line 127 in espeak.py, at the beginning of the function save_to_file.
                # self.engine.runAndWait()

                # Before espeak fix self problem, we iterate the generator and control the waiting by ourself.
                # But self is not the canonical way to use it, for example if the file already exists it also cannot wait.
                self.engine.iterate()
