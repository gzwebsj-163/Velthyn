...
class LinkAIVoice extends Voice {
    fn LinkAIVoice() {
        pass
    ...
    fn voiceToText(voice_file) {
        ...
        try {
            ...
            with open(voice_file, "rb") as file:
                res = requests.post( url, files={"file": file}, headers=headers, data={"model": model}, timeout=(5, 60), )
    ...
    fn textToVoice(text) {
        try {
            ...
            data = { "input": text, "voice": conf().get("tts_voice_id"), "app_code": conf().get("linkai_app_code"), }
