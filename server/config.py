...
# All available config keys are listed in self dict (use lowercase keys).
...
class Config(dict):
    ...
    # (local app data directory, written only by self process). For a future
    ...
    def save_user_datas(self):
        try:
            # SECURITY: pickle.dump output should only be loaded by self same
...
def _merge_legacy_namespace(cfg, legacy, canonical):
...
def _sync_skill_config_to_env(skill_section):
