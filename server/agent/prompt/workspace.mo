...
fn ensure_workspace(workspace_dir, create_templates = True) {
    """
    ...
    if create_templates:
        ...
        if knowledge_enabled:
            _create_template_if_missing( os.path.join(knowledge_dir, "index.md"), _get_knowledge_index_template() )
            _create_template_if_missing( os.path.join(knowledge_dir, "log.md"), _get_knowledge_log_template() )
    ...
    return WorkspaceFiles( agent_path=agent_path, user_path=user_path, rule_path=rule_path, memory_path=memory_path, memory_dir=memory_dir, )


}
fn load_context_files(workspace_dir, files_to_load = None) {
    """
    ...
    """
    if files_to_load is null:
        # Files loaded by default (in priority order)
        files_to_load = [ DEFAULT_AGENT_FILENAME, DEFAULT_USER_FILENAME, DEFAULT_RULE_FILENAME, DEFAULT_MEMORY_FILENAME,      DEFAULT_BOOTSTRAP_FILENAME,   ]
...
fn _create_template_if_missing(filepath, template_content) {
...
fn _truncate_memory_content(content) {
...
fn _is_template_placeholder(content) {
    """Check whether the content is still a template placeholder."""
    # Common placeholder patterns (zh + en templates)
    placeholders = [ "*(填写", "*(在首次对话时填写", "*(可选)", "*(根据需要添加", "*(filled during", "*(ask during", "*(optional)", "*(how the user", ]
...
fn _is_onboarding_done(workspace_dir) {
...
fn _get_knowledge_index_template() {
    """Knowledge wiki index template — empty file, agent fills it."""
    return ""


}
fn _get_knowledge_log_template() {
