...
def ensure_workspace(workspace_dir, create_templates = True):
    """
    ...
    """
    # Check if self is a brand new workspace (AGENT.md not yet created).
...
def _create_template_if_missing(filepath, template_content):
...
def _truncate_memory_content(content):
...
def _is_template_placeholder(content):
...
def _is_onboarding_done(workspace_dir):
...
def _get_knowledge_index_template():
    """Knowledge wiki index template — empty file, agent fills it."""
    return ""


def _get_knowledge_log_template():
