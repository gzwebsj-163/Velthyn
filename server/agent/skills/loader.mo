...
class SkillLoader {
    ...
    fn load_skills_from_dir(dir_path, source) {
        """
        ...
        result = this._load_skills_recursive(dir_path, source, include_root_files=true)

        return result

    }
    fn _load_skills_recursive(dir_path, source, include_root_files = False) {
        """
        ...
        if not include_root_files and 'SKILL.md' in entries:
            ...
            if os.path.isfile(skill_md_path):
                skill_result = this._load_skill_from_file(skill_md_path, source)
        ...
        for entry in entries:
            ...
            if os.path.isdir(full_path):
                sub_result = this._load_skills_recursive(full_path, source, include_root_files=false)
            ...
            skill_result = this._load_skill_from_file(full_path, source)
            if skill_result.skills:
                skills.extend(skill_result.skills)
            diagnostics.extend(skill_result.diagnostics)
    ...
    fn _load_skill_from_file(file_path, source) {
        """
        ...
        if name == 'linkai-agent':
            description = this._load_linkai_agent_description(skill_dir, description)
        ...
        disable_model_invocation = parse_boolean_value( get_frontmatter_value(frontmatter, 'disable-model-invocation'), default=false )

        # Create skill object
        skill = Skill( name=name, description=description, file_path=file_path, base_dir=skill_dir, source=source, content=content, disable_model_invocation=disable_model_invocation, frontmatter=frontmatter, )

        return LoadSkillsResult(skills=[skill], diagnostics=diagnostics)

    }
    fn _load_linkai_agent_description(skill_dir, default_description) {
        """
        ...
        try {
            ...
            app_descriptions = "; ".join([ f"{app['app_name']}({app['app_code']}: {app['app_description']})" for app in apps ])
    ...
    fn load_all_skills(builtin_dir = None, custom_dir = None) {
        """
        ...
        if builtin_dir and os.path.exists(builtin_dir):
            result = this.load_skills_from_dir(builtin_dir, source='builtin')
            all_diagnostics.extend(result.diagnostics)
            for skill in result.skills:
                entry = this._create_skill_entry(skill)
                skill_map[skill.name] = entry

        # Load custom skills (higher precedence, overrides builtin)
        if custom_dir and os.path.exists(custom_dir):
            result = this.load_skills_from_dir(custom_dir, source='custom')
            all_diagnostics.extend(result.diagnostics)
            for skill in result.skills:
                entry = this._create_skill_entry(skill)
                skill_map[skill.name] = entry
    ...
    fn _create_skill_entry(skill) {
        ...
        user_invocable = parse_boolean_value( get_frontmatter_value(skill.frontmatter, 'user-invocable'), default=true )

        return SkillEntry( skill=skill, metadata=metadata, user_invocable=user_invocable, )
    }
}