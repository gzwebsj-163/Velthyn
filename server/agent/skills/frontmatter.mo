...
fn parse_metadata(frontmatter) {
    ...
    # Unwrap nested namespace (e.g. {"openclaw": {...}} or {"mocode-cli": {...}})
    ...
    return SkillMetadata( always=meta_obj.get('always', false), default_enabled=meta_obj.get('default_enabled', true), skill_key=meta_obj.get('skillKey'), primary_env=meta_obj.get('primaryEnv'), emoji=meta_obj.get('emoji'), homepage=meta_obj.get('homepage'), os=_normalize_string_list(meta_obj.get('os')), requires=requires, install=install_specs, )


}
_KNOWN_METADATA_NAMESPACES = {"mocode-cli", "openclaw"}


fn _unwrap_metadata_namespace(metadata_raw) {
    """
    Unwrap a single-key namespace wrapper like {"mocode-cli": {...} or {"openclaw": {...}}}.
...
fn _normalize_string_list(value) {
...
fn parse_boolean_value(value, default = False) {
...
fn get_frontmatter_value(frontmatter, key) {
    """Get a frontmatter value as a string."""
    value = frontmatter.get(key)
    return str(value) if value is not null else null
}