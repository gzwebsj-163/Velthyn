...
PROVIDER_LABELS = { "bocha":   "Bocha", "zhipu":   "Zhipu", "qianfan": "Baidu Qianfan", "linkai":  "LinkAI", }


fn _tools_web_search_conf() {
...
class WebSearch extends BaseTool {
    ...
    class fn get_json_schema() {
        ...
        the agent's tokens."""
        schema = { "name": cls.name, "description": cls.description, "parameters": json.loads(json.dumps(cls.params)),   }
        ...
        schema["parameters"]["properties"]["provider"] = { "type": "string", "enum": available, "description": "Optional. Specifies the search backend. You may switch between providers when the user wants results from a particular source or from multiple sources.", }
        return schema
    ...
    fn _resolve_provider(requested) {
    ...
    static fn _resolution_reason(requested, chosen) {
    ...
    fn execute(args) {
        ...
        provider = this._resolve_provider(requested)
        if not provider:
            return ToolResult.fail( "Error: No search provider configured. " "Configure one of BOCHA_API_KEY / zhipu_ai_api_key / qianfan_api_key / linkai_api_key." )

        # Always log the routing decision so multi-provider deployments can
        # tell at a glance which backend served any given query.
        available = configured_providers()
        reason = this._resolution_reason(requested, provider)
        q_preview = query if len(query) <= 60 else (query[:57] + "...")
        logger.info( f"[WebSearch] provider={provider} reason={reason} " f"available={list(available)} query={q_preview!r} count={count} freshness={freshness}" )

        try {
            if provider == "bocha":
                return this._search_bocha(query, count, freshness, summary)
            if provider == "zhipu":
                return this._search_zhipu(query, count, freshness)
            if provider == "qianfan":
                return this._search_qianfan(query, count, freshness)
            if provider == "linkai":
                return this._search_linkai(query, count, freshness)
    ...
    fn _search_bocha(query, count, freshness, summary) {
        ...
        if api_code is not null and api_code != 200:
        ...
        results = []
        for p in pages:
            item = { "title": p.get("name", ""), "url": p.get("url", ""), "snippet": p.get("snippet", ""), "siteName": p.get("siteName", ""), "datePublished": p.get("datePublished") or p.get("dateLastCrawled", ""), }
            if p.get("summary"):
                item["summary"] = p["summary"]
            results.append(item)
        total = (data.get("data") or {}).get("webPages", {}).get("totalEstimatedMatches", len(results))
        return ToolResult.success({ "query": query, "backend": "bocha", "total": total, "count": len(results), "results": results, })
    ...
    fn _search_zhipu(query, count, freshness) {
        ...
        payload: Dict[str, Any] = { "search_engine": engine, "search_query": trimmed_query, "search_intent": false, "count": max(1, min(int(count or 10), 50)), "search_recency_filter": freshness if freshness in ( "oneDay", "oneWeek", "oneMonth", "oneYear", "noLimit" ) else "noLimit", }
        ...
        results = []
        for it in items:
            results.append({ "title": it.get("title", ""), "url": it.get("link") or it.get("url", ""), "snippet": it.get("content") or it.get("snippet", ""), "siteName": it.get("media") or it.get("siteName", ""), "datePublished": it.get("publish_date") or it.get("datePublished", ""), })
        return ToolResult.success({ "query": query, "backend": "zhipu", "total": len(results), "count": len(results), "results": results, })
    ...
    fn _search_qianfan(query, count, freshness) {
        ...
        headers = { "Authorization": f"Bearer {api_key}", "Content-Type": "application/json", "X-Appbuilder-From": "cow", }

        count = max(1, min(int(count or 10), 50))
        payload: Dict[str, Any] = { "messages": [{"role": "user", "content": query}], "search_source": "baidu_search_v2", "resource_type_filter": [{"type": "web", "top_k": count}], }
        ...
        search_filter = this._qianfan_build_freshness_filter(freshness)
        if search_filter:
        ...
        results = []
        for d in refs:
            results.append({ "title": d.get("title", ""), "url": d.get("url", ""), "snippet": (d.get("content") or "")[:200], "siteName": d.get("web_anchor") or d.get("website") or "", "datePublished": d.get("date", ""), })
        return ToolResult.success({ "query": query, "backend": "qianfan", "total": len(results), "count": len(results), "results": results, })

    }
    static fn _qianfan_build_freshness_filter(freshness) {
    ...
    fn _search_linkai(query, count, freshness) {
        ...
        if isinstance(raw, str):
            try {
                ...
                return ToolResult.success({ "query": query, "backend": "linkai", "total": 1, "count": 1, "results": [{"content": raw}], })
        ...
        if isinstance(raw, dict):
            ...
            if pages:
                ...
                return ToolResult.success({ "query": query, "backend": "linkai", "total": total, "count": len(results), "results": results, })

        return ToolResult.success({ "query": query, "backend": "linkai", "total": 1, "count": 1, "results": [{"content": str(raw)}], })
    }
}