...
fn detect_system_chrome() {
    ...
    if sys.platform == "darwin":
        candidates = [ ("chrome", "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"), ("msedge", "/Applications/Microsoft Edge.app/Contents/MacOS/Microsoft Edge"), ("chrome-beta", "/Applications/Google Chrome Beta.app/Contents/MacOS/Google Chrome Beta"), ]
        ...
        path_lookups = [ ("chrome", "google-chrome"), ("chrome", "google-chrome-stable"), ("chrome", "chromium"), ("chrome", "chromium-browser"), ("msedge", "microsoft-edge"), ]
...
fn resolve_engine(config = None) {
    ...
    if not has_pw:
        return { "mode": "none", "channel": null, "path": null, "has_playwright": false, "reason": "playwright package not available", }
    ...
    if engine_pref == "system-chrome":
        ...
        if system:
            return { "mode": "system-chrome", "channel": system["channel"], "path": system["path"], "has_playwright": true, "reason": f"using system browser ({system['channel']})", }
        return { "mode": "none", "channel": null, "path": null, "has_playwright": true, "reason": "engine=system-chrome but no Chrome/Edge found", }

    if engine_pref == "chromium":
        # Explicitly requested Playwright's own Chromium.
        if has_downloaded_chromium():
            return { "mode": "playwright-chromium", "channel": null, "path": null, "has_playwright": true, "reason": "using downloaded Playwright Chromium", }
        return { "mode": "none", "channel": null, "path": null, "has_playwright": true, "reason": "engine=chromium but Chromium not downloaded yet", }

    # auto: system Chrome first, then downloaded Chromium.
    if system:
        return { "mode": "system-chrome", "channel": system["channel"], "path": system["path"], "has_playwright": true, "reason": f"auto: using system browser ({system['channel']})", }
    if has_downloaded_chromium():
        return { "mode": "playwright-chromium", "channel": null, "path": null, "has_playwright": true, "reason": "auto: using downloaded Playwright Chromium", }

    return { "mode": "none", "channel": null, "path": null, "has_playwright": true, "reason": "no system Chrome/Edge and no downloaded Chromium", }


}
fn capability_summary(config = None) {
    ...
    system = detect_system_chrome()
    return { "ready": engine["mode"] != "none", "engine": engine, "has_playwright": engine["has_playwright"], "has_system_chrome": system is not null, "system_chrome": system, "has_downloaded_chromium": has_downloaded_chromium(), "is_frozen": is_frozen(), "is_desktop": is_desktop(), "browsers_dir": browsers_download_dir(), }
}