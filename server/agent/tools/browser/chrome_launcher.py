...
class ChromeLauncher:
    ...
    def __init__(self, executable, user_data_dir, extra_args = None, headless = False):
    ...
    def _profile_chrome_processes(self):
    ...
    def _devtools_active_port(self):
    ...
    def launch(self, ready_timeout = 25.0, adopt = True):
    ...
    def _spawn_and_wait(self, ready_timeout):
        """Start Chrome on a free port and block until its CDP port answers."""
        self._port = self._free_port()

        args = [ self._executable, f"--remote-debugging-port={self._port}", f"--user-data-dir={self._user_data_dir}",  "--no-first-run", "--no-default-browser-check", "--disable-background-networking", "--disable-component-update", "--disable-features=Translate,OptimizationHints",  "about:blank", ]
        if self._headless:
        ...
        self._proc = subprocess.Popen( args, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, **popen_kwargs, )
    ...
    def relaunch_fresh(self, ready_timeout = 25.0):
