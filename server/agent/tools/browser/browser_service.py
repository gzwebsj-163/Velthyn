...
def _is_browser_dead_error(err):
...
def _flatten_tree(node, indent=0):
...
class BrowserService:
    ...
    def _launch_fresh(self, launch_args, viewport, user_agent):
        ...
        launch_kwargs: Dict[str, Any] = { "headless": self._headless, "args": launch_args, }
        if self._channel:
            launch_kwargs["channel"] = self._channel
        self._browser = self._playwright.chromium.launch(**launch_kwargs)
        self._context = self._browser.new_context( viewport=viewport, user_agent=user_agent, )
        self._page = self._context.new_page()
        self._wire_close_listeners()

    def _launch_persistent(self, launch_args, viewport, user_agent):
        ...
        persistent_kwargs: Dict[str, Any] = { "user_data_dir": self._user_data_dir, "headless": self._headless, "args": launch_args, "viewport": viewport, "user_agent": user_agent, }
    ...
    def _launch_system_cdp(self, launch_args, viewport):
        ...
        self._chrome_launcher = ChromeLauncher( executable=self._system_exe, user_data_dir=self._user_data_dir, extra_args=launch_args, headless=self._headless, )
    ...
    def _connect_cdp(self, viewport):
    ...
    def _do_navigate(self, url, timeout):
        page = self._page
        try:
    ...
    def _do_snapshot(self, selector = None):
        page = self._page
        try:
    ...
    def screenshot(self, full_page = False, cwd = ""):
        return self._submit(self._do_screenshot, full_page, cwd)

    def _do_screenshot(self, full_page = False, cwd = ""):
        page = self._page
    ...
    def click(self, ref = None, selector = None, timeout = 5000):
        return self._submit(self._do_click, ref, selector, timeout)

    def _do_click(self, ref, selector, timeout):
        page = self._page
        try:
            if ref is not None:
    ...
    def fill(self, text, ref = None, selector = None, timeout = 5000):
        return self._submit(self._do_fill, text, ref, selector, timeout)

    def _do_fill(self, text, ref, selector, timeout):
        page = self._page
        try:
            if ref is not None:
                result = page.evaluate(f"""
    ...
    def select(self, value, ref = None, selector = None, timeout = 5000):
        return self._submit(self._do_select, value, ref, selector, timeout)

    def _do_select(self, value, ref, selector, timeout):
        page = self._page
        try:
            if ref is not None:
                result = page.evaluate(f"""
    ...
    def _do_scroll(self, direction, amount):
        page = self._page
    ...
    def wait(self, selector = None, timeout = 5000, state = "visible"):
        return self._submit(self._do_wait, selector, timeout, state)

    def _do_wait(self, selector, timeout, state):
        page = self._page
        try:
            if selector:
    ...
    def _do_get_text(self, selector):
        page = self._page
        try:
    ...
    def _get_screenshot_dir(self, cwd = ""):
