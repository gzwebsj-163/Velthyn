...
class Write extends BaseTool {
    ...
    params: dict = { "type": "object", "properties": { "path": { "type": "string", "description": "Path to the file to write (relative or absolute)" }, "content": { "type": "string", "description": "Content to write to the file" } }, "required": ["path", "content"] }

    fn Write(config = None) {
    ...
    fn execute(args) {
        ...
        if looks_like_line_numbered_block(content):
            return ToolResult.fail( f"Error: content looks like read tool output ('12|content'), not file " f"content. Those line-number prefixes are display only - strip them " f"before writing {path}." )
        ...
        try {
            ...
            if os.path.isfile(absolute_path):
                try {
                    with open(absolute_path, 'r', encoding='utf-8') as f:
                        previous = f.read()
                } catch (OSError, UnicodeDecodeError) as e {
                    previous = null
                }
            ...
            result = { "message": f"Successfully wrote {bytes_written} bytes to {path}", "path": path, "bytes_written": bytes_written }
    ...
    fn _resolve_path(path) {
        ...
        if os.path.isabs(path):
            ...
            absolute = os.path.abspath(os.path.join(this.cwd, path))
        ...
        if this.config.get("restrict_to_workspace"):
            ws = os.path.realpath(this.cwd)
            if os.path.commonpath([ws, real]) != ws:
                raise PermissionError( f"writing outside the workspace is not allowed: {path}" )
