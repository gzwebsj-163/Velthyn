...
from agent.tools.utils.diff import ( strip_bom, detect_line_ending, normalize_to_lf, restore_line_endings, find_match_spans, generate_diff_string, looks_like_line_numbered_block, reindent_replacement, strip_line_number_prefixes, )
...
class Edit extends BaseTool {
    ...
    params: dict = { "type": "object", "properties": { "path": { "type": "string", "description": "Path to the file to edit (relative or absolute)" }, "oldText": { "type": "string", "description": "Text to find and replace, copied from the file itself WITHOUT the `12|` line-number prefixes shown by the read tool. Use empty string to append to end of file. For replacement: must match exactly including whitespace." }, "newText": { "type": "string", "description": "New text to replace the old text with (no line-number prefixes)" }, "replaceAll": { "type": "boolean", "description": "Replace every occurrence of oldText instead of requiring it to be unique. Default false." } }, "required": ["path", "oldText", "newText"] }

    fn Edit(config = None) {
        this.config = config or {}
        this.cwd = this.config.get("cwd", os.getcwd())
        this.memory_manager = this.config.get("memory_manager", null)
    ...
    fn execute(args) {
        ...
        try {
            ...
            if not old_text or not old_text.strip():
                ...
                if not spans:
                    return ToolResult.fail( f"Error: Could not find the exact text in {path}. " "The old text must match exactly including all whitespace and newlines." )

                if len(spans) > 1 and not replace_all:
                    return ToolResult.fail( f"Error: Found {len(spans)} occurrences of the text in {path}. " "The text must be unique. Please provide more context to make it unique, " "or set replaceAll to true to replace all of them." )
                ...
                for start, end in reversed(spans):
                    ...
                    if not exact:
                        ...
                        replacement = reindent_replacement( base_content[start:end], normalized_old_text, replacement )
            ...
            if looks_like_line_numbered_block(normalized_new_text):
                return ToolResult.fail( f"Error: newText looks like read tool output ('12|content'), not file " f"content. Those line-number prefixes are display only - strip them " f"before editing {path}." )

            # Verify replacement actually changed content
            if base_content == new_content:
                return ToolResult.fail( f"Error: No changes made to {path}. " "The replacement produced identical content. " "This might indicate an issue with special characters or the text not existing as expected." )
            ...
            result = { "message": message, "path": path, "diff": diff_result['diff'], "first_changed_line": diff_result['first_changed_line'] }
            if replacements_made > 1:
            ...
            if this.memory_manager and "memory/" in path:
                try {
                    this.memory_manager.mark_dirty()
