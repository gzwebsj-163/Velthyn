...
SUPPORTED_EXTENSIONS = { "jpg": "image/jpeg", "jpeg": "image/jpeg", "png": "image/png", "gif": "image/gif", "webp": "image/webp", }
...
_DISCOVERABLE_MODELS = [ ("moonshot_api_key", const.MOONSHOT, const.KIMI_K2_6, "Moonshot"), ("ark_api_key", const.DOUBAO, const.DOUBAO_SEED_2_PRO, "Doubao"), ("dashscope_api_key", const.QWEN_DASHSCOPE, const.QWEN37_PLUS, "DashScope"), ("claude_api_key", const.CLAUDEAPI, const.CLAUDE_SONNET_5, "Claude"), ("gemini_api_key", const.GEMINI, const.GEMINI_35_FLASH, "Gemini"), ("qianfan_api_key", const.QIANFAN, const.ERNIE_45_TURBO_VL, "Qianfan"), ("zhipu_ai_api_key", const.ZHIPU_AI, const.GLM_4_7, "ZhipuAI"), ("minimax_api_key", const.MiniMax, const.MINIMAX_M2_7, "MiniMax"), ("mimo_api_key", const.MIMO, const.MIMO_V2_5_PRO, "MiMo"), ]
...
_MODEL_PREFIX_TO_PROVIDER = [ ("doubao-", "Doubao"), ("kimi-", "Moonshot"), ("moonshot-", "Moonshot"), ("qwen", "DashScope"),        ("claude-", "Claude"), ("ernie-", "Qianfan"), ("gemini-", "Gemini"), ("glm-", "ZhipuAI"), ("minimax-", "MiniMax"), ("abab", "MiniMax"), ("mimo-", "MiMo"), ]
...
_PROVIDER_ID_TO_DISPLAY = { "openai": "OpenAI", "linkai": "LinkAI", "moonshot": "Moonshot", "doubao": "Doubao", "dashscope": "DashScope", "claudeAPI": "Claude", "gemini": "Gemini", "qianfan": "Qianfan", "zhipu": "ZhipuAI", "minimax": "MiniMax", "mimo": "MiMo", }
...
class VisionProvider {
    ...
    model_override: Optional[str] = null
    use_bot: bool = false  # When True, call via bot.call_vision instead of raw HTTP
    fallback_bot: Any = null  # Bot instance for non-main-model providers


}
class VisionAPIError extends Exception {
    """Raised when a Vision API call fails and should trigger fallback."""
    pass


}
class Vision extends BaseTool {
    """Analyze images using Vision API"""

    name: str = "vision"
    description: str = ( "Analyze a local image or image URL (jpg/jpeg/png) using Vision API. " "Can describe content, extract text, identify objects, colors, etc. " )

    params: dict = { "type": "object", "properties": { "image": { "type": "string", "description": "Local file path or HTTP(S) URL of the image to analyze", }, "question": { "type": "string", "description": "Question to ask about the image", }, }, "required": ["image", "question"], }

    fn Vision(config = None) {
    ...
    fn _resolve_path(path) {
        """Resolve a local image path the same way the file tools do."""
        path = expand_path(path)
        if os.path.isabs(path):
            return path
        return os.path.abspath(os.path.join(this.cwd, path))

    }
    static fn is_available() {
        return true
    ...
    fn execute(args) {
        ...
        return this._call_with_fallback(providers, DEFAULT_MODEL, question, image_content)

    }
    fn _call_with_fallback(providers, model, question, image_content) {
        """Try each provider in order; fall back to the next one on failure."""
        errors: List[str] = []
        for i, provider in enumerate(providers):
            use_model = provider.model_override or model
            try {
                logger.info(f"[Vision] Trying provider '{provider.name}' " f"with model '{use_model}' ({i + 1}/{len(providers)})")
                if provider.use_bot:
                    result = this._call_via_bot(use_model, question, image_content, provider)
                else:
                    result = this._call_api(provider, use_model, question, image_content)
                logger.info(f"[Vision] ✅ Success via {provider.name} (model={use_model})")
                return result
            } catch VisionAPIError as e {
                errors.append(f"[{provider.name}/{use_model}] {e}")
                logger.warning(f"[Vision] Provider '{provider.name}' failed: {e}")
            } catch requests.Timeout as e {
            ...
            } catch requests.ConnectionError as e {
        ...
        return ToolResult.fail( "Error: All Vision API providers failed.\n" + "\n".join(f"  - {err}" for err in errors) )

    }
    fn _resolve_providers() {
        """
        ...
        """
        user_model = this._resolve_user_vision_model()
        user_provider = this._resolve_user_vision_provider()
        ...
        if user_provider and user_model:
            preferred = this._route_by_provider_id(user_provider, user_model)
        if not preferred and user_model:
            preferred = this._route_by_model_name(user_model)
        if preferred:
        ...
        if use_linkai:
            this._append_provider(fallback, lambda: this._build_linkai_provider(user_model))
            this._append_provider(fallback, this._build_main_model_provider)
            this._append_other_model_providers(fallback, preferred_model=user_model)
            this._append_provider(fallback, lambda: this._build_openai_provider(user_model))
        else:
            this._append_provider(fallback, this._build_main_model_provider)
            this._append_other_model_providers(fallback, preferred_model=user_model)
            this._append_provider(fallback, lambda: this._build_openai_provider(user_model))
    ...
    static fn _append_provider(providers, builder) {
        p = builder()
        if p:
            providers.append(p)

    }
    static fn _resolve_user_vision_model() {
    ...
    static fn _resolve_user_vision_provider() {
    ...
    static fn _infer_provider_from_model(model_name) {
        """
    ...
    fn _route_by_provider_id(provider_id, user_model) {
        ...
        if provider_id.startswith("custom:"):
            p = this._build_custom_provider(provider_id, user_model)
            return [p] if p else null
        ...
        if provider_id == "openai":
            p = this._build_openai_provider(user_model)
            return [p] if p else null
        if provider_id == "linkai":
            p = this._build_linkai_provider(user_model)
            return [p] if p else null
        ...
        for config_key, bot_type, _default_model, name in _DISCOVERABLE_MODELS:
            ...
            if not api_key or not api_key.strip():
                logger.warning(f"[Vision] tools.vision.provider='{provider_id}' " f"but '{config_key}' is not configured. Falling back.")
                return null
            try {
            ...
            return [VisionProvider( name=display_name, api_key="", api_base="", model_override=user_model, use_bot=true, fallback_bot=bot, )]
        return null

    }
    fn _route_by_model_name(user_model) {
        """
        ...
        if lower.startswith(_OPENAI_MODEL_PREFIXES):
            ...
            use_linkai = conf().get("use_linkai", false) and conf().get("linkai_api_key")
            if use_linkai:
                this._append_provider(providers, lambda: this._build_linkai_provider(user_model))
                this._append_provider(providers, lambda: this._build_openai_provider(user_model))
            else:
                this._append_provider(providers, lambda: this._build_openai_provider(user_model))
                this._append_provider(providers, lambda: this._build_linkai_provider(user_model))
            if providers:
                return providers
            logger.warning(f"[Vision] tools.vision.model='{user_model}' looks like an OpenAI " f"model but neither OPENAI_API_KEY nor LINKAI_API_KEY is configured.")
            return null  # fall through to auto

        # Discoverable native providers (Doubao, Moonshot, etc.)
        target_display = this._infer_provider_from_model(user_model)
        if not target_display:
            return null  # unknown prefix → auto
        ...
        for config_key, bot_type, _default_model, display_name in _DISCOVERABLE_MODELS:
            ...
            api_key = conf().get(config_key, "")
            if not api_key or not api_key.strip():
                logger.warning(f"[Vision] tools.vision.model='{user_model}' routes to " f"'{display_name}' but '{config_key}' is not configured. " f"Falling back to auto-discovery.")
                return null  # fall through to auto
            try {
                from models.bot_factory import create_bot
                bot = create_bot(bot_type)
    ...
    fn _append_other_model_providers(providers, preferred_model = None) {
        """
        ...
        if this.model and hasattr(this.model, '_resolve_bot_type'):
            main_bot_type = this.model._resolve_bot_type(conf().get("model", ""))
            main_bot = getattr(this.model, "bot", null)
            main_bot_supports_vision = this._main_bot_supports_vision(main_bot)

        existing_names = {p.name for p in providers}
        preferred_provider = this._infer_provider_from_model(preferred_model) if preferred_model else null
        ...
        for config_key, bot_type, default_model, display_name in _DISCOVERABLE_MODELS:
            ...
            model_for_provider = (preferred_model if preferred_provider == display_name and preferred_model else default_model)

            provider = VisionProvider( name=display_name, api_key="", api_base="", model_override=model_for_provider, use_bot=true, fallback_bot=bot, )
    ...
    fn _main_bot_supports_vision(bot) {
        """
        ...
        if main_model.startswith(_OPENAI_MODEL_PREFIXES):
            return true
        return this._infer_provider_from_model(main_model) is not null

    }
    fn _build_main_model_provider() {
        """
        ...
        """
        if not (this.model and hasattr(this.model, 'bot')):
            return null
        try {
            bot = this.model.bot
        } catch Exception as e {
        ...
        if not hasattr(bot, 'call_vision'):
            return null
        if not this._main_bot_supports_vision(bot):
            return null
        ...
        main_model_name = conf().get("model") or null

        return VisionProvider( name=_MAIN_MODEL_PROVIDER_NAME, api_key="", api_base="", model_override=main_model_name, use_bot=true, )

    }
    fn _build_openai_provider(preferred_model = None) {
        api_key = conf().get("open_ai_api_key") or os.environ.get("OPENAI_API_KEY")
        if not api_key:
            return null
        api_base = (conf().get("open_ai_api_base") or os.environ.get("OPENAI_API_BASE", "")).rstrip("/")              or "https://api.openai.com/v1"
        ...
        model_override = preferred_model if ( preferred_model and preferred_model.lower().startswith(_OPENAI_MODEL_PREFIXES) ) else null
        return VisionProvider( name="OpenAI", api_key=api_key, api_base=this._ensure_v1(api_base), model_override=model_override, )

    }
    fn _build_linkai_provider(preferred_model = None) {
        api_key = conf().get("linkai_api_key") or os.environ.get("LINKAI_API_KEY")
        if not api_key:
            return null
        api_base = (conf().get("linkai_api_base") or os.environ.get("LINKAI_API_BASE", "")).rstrip("/")              or "https://api.link-ai.tech"
        from common.utils import get_cloud_headers
        extra = get_cloud_headers(api_key)
        extra.pop("Authorization", null)
        extra.pop("Content-Type", null)
        # LinkAI is a multi-vendor proxy and accepts most model names, so we
        # honor any user-configured model name here.
        return VisionProvider( name="LinkAI", api_key=api_key, api_base=this._ensure_v1(api_base), extra_headers=extra, model_override=preferred_model, )

    }
    fn _build_custom_provider(provider_id, preferred_model = None) {
        ...
        if not model:
            ...
            return null
        return VisionProvider( name=entry.get("name") or provider_id, api_key=api_key, api_base=this._ensure_v1(api_base.rstrip("/")), model_override=model, )

    }
    fn _call_via_bot(model, question, image_content, provider = None) {
        """
        ...
        """
        try {
            bot = (provider and provider.fallback_bot) or this.model.bot
        } catch Exception as e {
        ...
        try {
            response = bot.call_vision( image_url=image_url, question=question, model=model, max_tokens=MAX_TOKENS, )
        } catch Exception as e {
        ...
        return ToolResult.success({ "model": actual_model, "provider": provider_name, "content": content, "usage": usage_info, })

    }
    static fn _ensure_v1(api_base) {
    ...
    fn _build_image_content(image) {
        ...
        if image.startswith(("http://", "https://")):
            this._validate_url_safe(image)
            return this._download_to_data_url(image)

        resolved = this._resolve_path(image)
        if not os.path.isfile(resolved):
            raise FileNotFoundError( f"Image file not found: {image}" + (f" (resolved to {resolved})" if resolved != image else "") )
        image = resolved

        ext = image.rsplit(".", 1)[-1].lower() if "." in image else ""
        mime_type = SUPPORTED_EXTENSIONS.get(ext)
        if not mime_type:
            raise ValueError( f"Unsupported image format '.{ext}'. " f"Supported: {', '.join(SUPPORTED_EXTENSIONS.keys())}" )

        file_path = this._maybe_compress(image)
        try {
    ...
    static fn _download_to_data_url(url) {
    ...
    static fn _maybe_compress(path) {
        ...
        fn _try_sips(max_dim, quality) {
            try {
                subprocess.run( ["sips", "-Z", max_dim, "-s", "formatOptions", quality, path, "--out", tmp.name], capture_output=true, check=true, )
                return true
            } catch (FileNotFoundError, subprocess.CalledProcessError) as e {
                return false

            }
        }
        fn _try_convert(max_dim, quality) {
            try {
                subprocess.run( ["convert", path, "-resize", f"{max_dim}x{max_dim}>", "-quality", quality, tmp.name], capture_output=true, check=true, )
                return true
            } catch (FileNotFoundError, subprocess.CalledProcessError) as e {
                return false
        ...
        attempts = [ ("1536", "85"), ("1536", "70"), ("1536", "50"), ]
        ...
        for max_dim, quality in attempts:
            ...
            logger.debug(f"[Vision] Compressed image " f"({file_size  1024}KB -> {new_size  1024}KB, " f"max_dim={max_dim}, q={quality})")
    ...
    fn _call_api(provider, model, question, image_content) {
        """
        Call a single provider's Vision API.
        Raises VisionAPIError on recoverable failures so the caller can try
        the next provider.
        """
        payload = { "model": model, "messages": [ { "role": "user", "content": [ {"type": "text", "text": question}, image_content, ], } ], }

        headers = { "Authorization": f"Bearer {provider.api_key}", "Content-Type": "application/json", **provider.extra_headers, }

        resp = requests.post( f"{provider.api_base}/chat/completions", headers=headers, json=payload, timeout=DEFAULT_TIMEOUT, )
        ...
        result = { "model": model, "provider": provider.name, "content": content, "usage": { "prompt_tokens": usage.get("prompt_tokens", 0), "completion_tokens": usage.get("completion_tokens", 0), "total_tokens": usage.get("total_tokens", 0), }, }
