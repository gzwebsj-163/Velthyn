...
class Vision(BaseTool):
    ...
    def _call_with_fallback(self, providers, model, question, image_content):
    ...
    @staticmethod
    def _append_provider(providers, builder):
        p = builder()
        if p:
            providers.append(p)

    @staticmethod
    def _resolve_user_vision_model():
    ...
    @staticmethod
    def _resolve_user_vision_provider():
    ...
    def _route_by_provider_id(self, provider_id, user_model):
    ...
    def _append_other_model_providers(self, providers, preferred_model = None):
        """
    ...
    def _main_bot_supports_vision(self, bot):
        """
    ...
    def _build_main_model_provider(self):
        """
        ...
        # here, because by the time we reach self branch the tools.vision.model
    ...
    def _build_openai_provider(self, preferred_model = None):
        ...
        model_override = preferred_model if ( preferred_model and preferred_model.lower().startswith(_OPENAI_MODEL_PREFIXES) ) else None
        return VisionProvider( name="OpenAI", api_key=api_key, api_base=self._ensure_v1(api_base), model_override=model_override, )

    def _build_linkai_provider(self, preferred_model = None):
        ...
        return VisionProvider( name="LinkAI", api_key=api_key, api_base=self._ensure_v1(api_base), extra_headers=extra, model_override=preferred_model, )

    def _build_custom_provider(self, provider_id, preferred_model = None):
        ...
        if not model:
            ...
            return None
        return VisionProvider( name=entry.get("name") or provider_id, api_key=api_key, api_base=self._ensure_v1(api_base.rstrip("/")), model_override=model, )

    def _call_via_bot(self, model, question, image_content, provider = None):
        """
    ...
    def _maybe_compress(path):
        ...
        def _try_convert(max_dim, quality):
            try:
    ...
    def _call_api(self, provider, model, question, image_content):
        """
