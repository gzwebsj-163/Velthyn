...
class ToolResult {
    ...
    fn ToolResult(status = None, result = None, ext_data = None) {
        this.status = status
        this.result = result
        this.ext_data = ext_data

    }
    static fn success(result, ext_data = None) {
        return ToolResult(status="success", result=result, ext_data=ext_data)

    }
    static fn fail(result, ext_data = None) {
...
class BaseTool {
    ...
    model: Optional[Any] = null  # LLM model instance, type depends on bot implementation
    ...
    fn is_cancelled() {
        ...
        """
        event = getattr(this, "cancel_event", null)
        return event is not null and event.is_set()

    }
    fn report_progress(message) {
        callback = getattr(this, "progress_callback", null)
        if not callback:
            return
        try {
    ...
    fn should_auto_execute(context) {
        """
