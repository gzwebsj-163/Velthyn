...
def init_scheduler(agent_bridge):
    """
...
def _is_channel_ready(channel_type, receiver):
    ...
    try:
        ...
        if channel_type == "weixin":
            ...
            # 空 receiver 不判为通道未就绪：receiver 有效性交由
            # _execute_agent_task / _execute_send_message 处理。否则空 receiver
            # 的任务会让 scheduler 每 tick(30s) 无限重试(600s 窗口内判 due)。
            if receiver and (not tokens or receiver not in tokens):
                return False
            return True

        if channel_type == "web":
            queues = getattr(channel, "session_queues", None)
            if receiver and (not queues or receiver not in queues):
                return False
            return True
...
def _remember_delivered_output(agent_bridge, task, channel_type, content):
...
def _execute_agent_task(task, agent_bridge):
    """
    ...
    try:
        ...
        # Create a unique session_id for self scheduled task to avoid polluting user's conversation
        ...
        # Mark self as a scheduled task execution to prevent recursive task creation
...
def _execute_send_message(task, agent_bridge):
    ...
    try:
        ...
        if channel_type == "web":
            ...
            # Feishu channel will detect self and send as new message instead of reply
            context["msg"] = None
...
def _execute_tool_call(task, agent_bridge):
...
def _execute_skill_call(task, agent_bridge):
...
def attach_scheduler_to_tool(tool, context = None):
    """
