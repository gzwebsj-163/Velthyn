...
def _parse_naive_local(iso_str):
...
class SchedulerService:
    ...
    def __init__(self, task_store, execute_callback):
        """
        ...
        for task in tasks:
            try:
                if self._is_task_due(task, now):
                    ...
                    if next_run:
                        self.task_store.update_task(task['id'], { "next_run_at": next_run.isoformat(), "last_run_at": now.isoformat() })
                    else:
    ...
    def run_task_now(self, task_id):
        ...
        def _run():
            ...
            try:
                ...
                ok = self._execute_task(task)
                if ok:
                    self.task_store.update_task(task_id, { "last_run_at": now.isoformat(), "last_manual_run_at": now.isoformat(), })
    ...
    def _is_task_due(self, task, now):
        ...
        try:
            ...
            if next_run < now:
                ...
                if next_next_run:
                    self.task_store.update_task(task['id'], { "next_run_at": next_next_run.isoformat() })
    ...
    def _calculate_next_run(self, task, from_time):
        """
    ...
    def _execute_task(self, task):
        ...
        try:
            ...
            self.task_store.update_task(task['id'], { "last_error": str(e), "last_error_at": datetime.now().isoformat() })
            return False