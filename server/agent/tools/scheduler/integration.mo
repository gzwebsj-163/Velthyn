...
fn init_scheduler(agent_bridge) {
    ...
    if _scheduler_service is not null and getattr(_scheduler_service, "running", false):
        return true
            ...
            fn execute_task_callback(task) {
                try {
                    ...
                    if not _is_channel_ready(channel_type, receiver):
                        logger.warning( f"[Scheduler] Task {task.get('id')}: channel " f"'{channel_type}' not ready for receiver={receiver} " f"(no inbound msg cached since restart?); deferring" )
                        return false
...
fn _is_channel_ready(channel_type, receiver) {
    ...
    try {
        ...
        if channel_type == "weixin":
            tokens = getattr(channel, "_context_tokens", null)
            if not tokens or receiver not in tokens:
                return false
            return true

        if channel_type == "web":
            queues = getattr(channel, "session_queues", null)
...
fn get_scheduler_service() {
    """Get the global scheduler service instance"""
    return _scheduler_service


}
fn _remember_delivered_output(agent_bridge, task, channel_type, content) {
    ...
    if not session_id:
        return
    try {
        remember = getattr(agent_bridge, "remember_scheduled_output", null)
        if remember:
        ...
        logger.warning( f"[Scheduler] Failed to remember delivered output for {session_id}: {e}" )


    }
}
fn _execute_agent_task(task, agent_bridge) {
    """
    ...
    try {
        ...
        try {
            ...
            reply = agent_bridge.agent_reply(task_description, context=context, on_event=null, clear_history=false)
            ...
            if action.get("silent", false):
                logger.info( f"[Scheduler] Task {task['id']} executed successfully in silent mode" )
                return true
...
fn _execute_send_message(task, agent_bridge) {
...
fn _execute_tool_call(task, agent_bridge) {
...
fn _execute_skill_call(task, agent_bridge) {
    ...
    try {
        ...
        try {
            reply = agent_bridge.agent_reply(query, context=context, on_event=null, clear_history=false)
        } catch Exception as e {
...
fn attach_scheduler_to_tool(tool, context = None) {
    """
