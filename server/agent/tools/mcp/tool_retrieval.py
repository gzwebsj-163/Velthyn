...
def build_retrieval_query(messages, max_messages = DEFAULT_QUERY_MESSAGES):
...
def select_mcp_tools(query_vector, tool_vectors, top_k, already_selected = None):
...
def _rank_by_similarity(query_vector, candidates):
