...
def load_server_record(server_name):
    with _STORE_LOCK:
        return dict(_load_store().get(server_name, {}))


def save_server_record(server_name, record):
    with _STORE_LOCK:
        store = _load_store()
...
def clear_server_record(server_name):
    with _STORE_LOCK:
        store = _load_store()
...
def _register_pending(state, handler):
    with _PENDING_LOCK:
...
def _http_post_form(url, fields, timeout = 20):
...
def discover_metadata(resource_url, www_authenticate = ""):
    """
...
def _parse_resource_metadata_url(www_authenticate):
...
class OAuthHandler:
    ...
    def __init__(self, server_name, resource_url, redirect_uri, scope = "", client_name = "mocode-cli"):
    ...
    def _persist(self):
        save_server_record(self.server_name, { "resource_url": self.resource_url, "metadata": self.metadata, "client_id": self.client_id, "client_secret": self.client_secret, "access_token": self.access_token, "refresh_token": self.refresh_token, "expires_at": self.expires_at, })

    # --- token access ------------------------------------------------

    def get_valid_access_token(self, leeway = 60):
    ...
    def refresh(self):
        ...
        if not self.refresh_token or not self.metadata.get("token_endpoint"):
            return False
        fields = { "grant_type": "refresh_token", "refresh_token": self.refresh_token, "client_id": self.client_id or "", }
    ...
    def ensure_registered(self, www_authenticate = ""):
    ...
    def _register_client(self):
        ...
        if not reg_endpoint:
            ...
            return False
        payload = { "client_name": self.client_name, "redirect_uris": [self.redirect_uri], "grant_types": ["authorization_code", "refresh_token"], "response_types": ["code"], "token_endpoint_auth_method": "none", }
        if self.scope:
    ...
    def build_authorization_url(self):
        ...
        params = { "response_type": "code", "client_id": self.client_id, "redirect_uri": self.redirect_uri, "code_challenge": challenge, "code_challenge_method": "S256", "state": state, }
        if self.scope:
    ...
    def finish_authorization(self, code):
        """Exchange an authorization code for tokens."""
        if not self.metadata.get("token_endpoint") or not self._verifier:
            return False
        fields = { "grant_type": "authorization_code", "code": code, "redirect_uri": self.redirect_uri, "client_id": self.client_id or "", "code_verifier": self._verifier, "resource": self.resource_url, }
        if self.client_secret:
            fields["client_secret"] = self.client_secret
        try:
            resp = _http_post_form(self.metadata["token_endpoint"], fields)
    ...
    def _absorb_token_response(self, resp):
