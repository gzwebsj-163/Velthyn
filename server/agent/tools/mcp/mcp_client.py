...
def notify_server_authorized(server_name):
...
class McpClient:
    ...
    def __init__(self, config):
        ...
        self.transport: str = ( "streamable-http" if raw_transport.lower() in _STREAMABLE_HTTP_ALIASES else raw_transport )
        ...
        # complete the browser authorization. Callers can surface self state.
        ...
        # do not acquire self lock (see _send_request).
    ...
    def shutdown(self):
        ...
        if self.transport == "streamable-http" and self._http_session_id and self._http_url:
            try:
                req = urllib.request.Request( self._http_url, method="DELETE", headers={"Mcp-Session-Id": self._http_session_id, **self._http_headers}, )
    ...
    def _init_stdio(self):
        ...
        self._proc = subprocess.Popen( [command] + list(args), stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding="utf-8", env=env, )
        logger.debug(f"[MCP:{self.name}] stdio process started (pid={self._proc.pid})")

        threading.Thread( target=self._drain_stderr, daemon=True, name=f"mcp-stderr-{self.name}" ).start()
        threading.Thread( target=self._drain_stdout, daemon=True, name=f"mcp-stdout-{self.name}" ).start()
    ...
    def _build_stdio_env(self, extra_env):
    ...
    def _sse_discover_endpoint(self):
    ...
    def _sse_send(self, message):
        """POST a JSON-RPC message to the server and return the response."""
        body = json.dumps(message).encode("utf-8")
        req = urllib.request.Request( self._post_url, data=body, method="POST", headers={"Content-Type": "application/json"}, )
    ...
    def _init_streamable_http(self):
        url = self.config.get("url")
        if not url:
        ...
        # Restore any previously stored OAuth credentials for self server so a
    ...
    def _maybe_load_oauth(self):
        ...
        if rec.get("access_token") or rec.get("client_id"):
            self._oauth = OAuthHandler( server_name=self.name, resource_url=self._http_url, redirect_uri=_oauth_redirect_uri(), scope=self.config.get("scope", ""), )

    def _current_bearer(self):
        """Return a valid access token, refreshing if needed."""
        if self._oauth is None:
            return None
        return self._oauth.get_valid_access_token()

    def _begin_oauth(self, www_authenticate = ""):
    ...
    def _streamable_http_send(self, message):
    ...
    def _handle_401(self, err, message, expect_response, retried):
    ...
    def _streamable_http_post(self, message, expect_response, _retried = False):
        """
        ...
        req = urllib.request.Request( self._http_url, data=body, method="POST", headers=headers, )
    ...
    def _read_sse_response(self, resp, expected_id):
    ...
    def _build_request(self, method, params):
        return { "jsonrpc": "2.0", "id": self._next_request_id(), "method": method, "params": params, }
