...
fn build_retrieval_query(messages, max_messages = DEFAULT_QUERY_MESSAGES) {
    ...
    for message in messages[-max_messages:]:
        content = message.get("content") if isinstance(message, dict) else null
...
fn select_mcp_tools(query_vector, tool_vectors, top_k, already_selected = None) {
    ...
    try {
        ...
        candidates = { name: vec for name, vec in tool_vectors.items() if vec and len(vec) == expected_dim }
        if not candidates:
            return null
...
fn _rank_by_similarity(query_vector, candidates) {
