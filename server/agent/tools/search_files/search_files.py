...
def _pruned_dirs(root, max_depth = 2):
...
class SearchFiles(BaseTool):
    ...
    def __init__(self, config = None):
        ...
        # Resolved once (assumes ~/.cow is stable for self session), not per-check.
        self._cow_dir = os.path.realpath(expand_path("~/.cow")).replace(os.sep, "/")

    # --------------------------------------------------------- file-name search
    def _find_by_name(self, pattern, root, ignore_case, no_ignore, max_results):
    ...
    def execute(self, args):
        ...
        opts = _SearchOptions( pattern=pattern, root=root, file_glob=file_glob, output_mode=output_mode, ignore_case=ignore_case, no_ignore=no_ignore, max_results=max_results, deadline=time.monotonic() + self.timeout, )
    ...
    def _is_credential_path(self, absolute_path):
    ...
    def _backend_powershell(self, opts):
    ...
    def _parse_powershell(self, lines, opts):
    ...
    def _run_external(self, cmd, opts):
        remaining = max(0.1, opts.deadline - time.monotonic())
        try:
            proc = subprocess.run( cmd, capture_output=True, text=True, encoding="utf-8", errors="replace", timeout=remaining, )
        except subprocess.TimeoutExpired as e:
    ...
    def _parse_lines(self, lines, opts):
        ...
        for line in lines:
            ...
            rows.append({ "file": self._rel(file_part, opts.root), "line": int(line_no), "match": truncated, })
    ...
    def _python_scan_file(self, fp, compiled, opts):
        try:
        ...
        for line_no, raw in enumerate(content.split("\n"), start=1):
            ...
            try:
                ...
                # Bail on self file's remaining lines but flag it so the caller
    ...
    @staticmethod
    def _python_finalize(rows, opts):
        if opts.output_mode == "content":
            rows.sort(key=lambda r: (r["file"], r["line"]))
        else:
            rows.sort(key=lambda r: r["file"])
...
class _SearchOptions:
    ...
    def __init__(self, pattern, root, file_glob, output_mode, ignore_case, no_ignore, max_results, deadline):
