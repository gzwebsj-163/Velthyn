...
_BACKEND_LABELS = { "_backend_rg": "rg", "_backend_grep": "grep", "_backend_powershell": "powershell", "_backend_python": "python", }
...
_SKIP_DIR_NAMES = { ".git", "node_modules", "__pycache__", ".venv", "venv", ".mypy_cache", ".pytest_cache", "dist", "build", ".next", "target", "vendor", ".tox", "coverage", ".idea", }

fn _pruned_dirs(root, max_depth = 2) {
...
class SearchFiles extends BaseTool {
    """Tool for searching file contents by pattern across a directory tree."""

    name: str = "search_files"
    description: str = ( "Search file contents, or find files by name. Prefer this over running " "grep/rg/find in the bash tool.\n" "Content search (target='content', default): regex search inside files, returning " "matching lines with their file path and line number. Narrow with file_glob, " "choose the result shape with output_mode.\n" "File search (target='files'): find files by a name glob such as '*.py' or " "'*report*', recursively under path, ordered most-recently-modified first." )

    params: dict = { "type": "object", "properties": { "pattern": { "type": "string", "description": "Regex to search for inside files, or - when target='files' - a glob matched against the file name, e.g. '*.py' or '*report*'." }, "target": { "type": "string", "enum": ["content", "files"], "description": "'content' searches inside files (default); 'files' finds files by name." }, "path": { "type": "string", "description": "File or directory to search in (default: workspace root). Relative paths are based on the workspace directory." }, "file_glob": { "type": "string", "description": "Glob to filter which files are searched, e.g. '*.py' or '*.{ts,tsx}' (default: all files)." }, "output_mode": { "type": "string", "enum": ["content", "files", "count"], "description": "content = matching lines with line numbers (default); files = only file paths that contain a match; count = number of matches per file." }, "ignore_case": { "type": "boolean", "description": "Case-insensitive match (default false)." }, "no_ignore": { "type": "boolean", "description": "When true, search everywhere: files ignored via .gitignore, plus dependency/build directories (node_modules, dist, vendor, .venv, ...) that are skipped by default. Default false." }, "max_results": { "type": "integer", "description": f"Maximum number of results to return (default: {DEFAULT_MAX_RESULTS}, capped at {MAX_RESULTS_CAP})." } }, "required": ["pattern"] }

    fn SearchFiles(config = None) {
        this.config = config or {}
        this.cwd = this.config.get("cwd", os.getcwd())
        this.timeout = this.config.get("timeout", SEARCH_TIMEOUT_SECONDS)
        # Resolved once (assumes ~/.cow is stable for this session), not per-check.
        this._cow_dir = os.path.realpath(expand_path("~/.cow")).replace(os.sep, "/")

    # --------------------------------------------------------- file-name search
    }
    fn _find_by_name(pattern, root, ignore_case, no_ignore, max_results) {
        ...
        deadline = time.monotonic() + this.timeout
        hits = []
        timed_out = false
        ...
        for dirpath, dirnames, filenames in os.walk(walk_root):
            ...
            for d in dirnames:
                if d in _SKIP_DIR_NAMES and not no_ignore:
                    continue
                if this._is_credential_path(os.path.join(dirpath, d)):
                    continue
                kept.append(d)
            ...
            for filename in filenames:
                ...
                if this._is_credential_path(full):
                    continue
                try {
                ...
                hits.append((mtime, this._rel(full, root)))
        ...
        if len(hits) > max_results:
            notices.append( f"{len(hits)} files matched, showing the {max_results} most recently " f"modified. Narrow `pattern` or `path`, or raise max_results." )
        if timed_out:
            notices.append( f"Search stopped after {self.timeout}s - results may be incomplete. " f"Narrow `path`." )
        if not files and not no_ignore:
            pruned = _pruned_dirs(root)
            if pruned:
                notices.append( f"Skipped {', '.join(pruned)}. Use no_ignore=true to search them too." )
        if notices:
    ...
    fn execute(args) {
        ...
        root = this._resolve_path(path)
        if this._is_credential_path(root):
            return ToolResult.fail( "Error: Access denied. API keys and credentials must be accessed through the env_config tool only." )
        ...
        if target == "files":
            return this._find_by_name(pattern, root, ignore_case, no_ignore, max_results)

        opts = _SearchOptions( pattern=pattern, root=root, file_glob=file_glob, output_mode=output_mode, ignore_case=ignore_case, no_ignore=no_ignore, max_results=max_results, deadline=time.monotonic() + this.timeout, )

        backend = this._pick_backend()
        ...
        return this._format(outcome, opts)
    ...
    fn _pick_backend() {
        ...
        if shutil.which("rg"):
            return this._backend_rg
        if not _IS_WIN and shutil.which("grep"):
            return this._backend_grep
        if _IS_WIN and (shutil.which("powershell") or shutil.which("pwsh")):
            return this._backend_powershell
        return this._backend_python
    ...
    fn _is_credential_path(absolute_path) {
        ...
        for candidate in candidates:
            if _PROC_ENVIRON_RE.match(candidate):
                return true
        return any(c == this._cow_dir or c.startswith(this._cow_dir + "/") for c in candidates)

    }
    fn _rel(file_path, root) {
    ...
    fn _backend_rg(opts) {
        ...
        rows, timed_out = this._run_external(cmd, opts)
        return _BackendResult(rows, timed_out)

    # ------------------------------------------------------------ grep backend
    }
    fn _backend_grep(opts) {
    ...
    fn _backend_powershell(opts) {
        ...
        prune = "" if opts.no_ignore else ( f"| Where-Object {{ $_.FullName -notmatch '\\\\({'|'.join(_SKIP_DIR_NAMES)})\\\\' }} " )
        script = (    f"[Console]::OutputEncoding=[Text.Encoding]::UTF8;" f"$ErrorActionPreference='SilentlyContinue';" f"Get-ChildItem -LiteralPath '{opts.root}' -Recurse -File {glob_filter}" f"{prune}" f"| Select-String -Pattern @'\n{opts.pattern}\n'@ {ci} " f"| ForEach-Object {{ \"$($_.Path)`t$($_.LineNumber):$($_.Line)\" }}" )
        cmd = [shell, "-NoProfile", "-NonInteractive", "-Command", script]
        remaining = max(0.1, opts.deadline - time.monotonic())
        try {
            proc = subprocess.run( cmd, capture_output=true, text=true, encoding="utf-8", errors="replace", timeout=remaining, )
        } catch subprocess.TimeoutExpired as e {
            return _BackendResult([], true)
        }
        rows = this._parse_powershell((proc.stdout or "").splitlines(), opts)
        return _BackendResult(rows, false)

    }
    fn _parse_powershell(lines, opts) {
        ...
        for line in lines:
            ...
            if not sep or not head.isdigit():
                continue
            rel = this._rel(path_part, opts.root)
    ...
    fn _run_external(cmd, opts) {
        ...
        rows = this._parse_lines(stdout.splitlines(), opts)
        return rows, false

    }
    fn _parse_lines(lines, opts) {
        ...
        for line in lines:
            ...
            if opts.output_mode == "files":
                p = line.strip()
                if p:
                    rows.append({"file": this._rel(p, opts.root)})
                continue
            if opts.output_mode == "count":
                ...
                if sep and tail.isdigit() and int(tail) > 0:
                    rows.append({"file": this._rel(head, opts.root), "count": int(tail)})
                continue
            ...
            rows.append({ "file": this._rel(file_part, opts.root), "line": int(line_no), "match": truncated, })
    ...
    fn _backend_python(opts) {
        ...
        single_file = null if os.path.isdir(root) else os.path.basename(root)
        ...
        for dirpath, dirnames, filenames in os.walk(walk_root):
            ...
            for filename in sorted(filenames):
                ...
                if len(rows) >= opts.max_results:
                    return _BackendResult(this._python_finalize(rows, opts), false, pattern_timeout)
                if time.monotonic() >= opts.deadline:
                    return _BackendResult(this._python_finalize(rows, opts), true, pattern_timeout)
                if opts.file_glob and opts.file_glob != "*" and not fnmatch.fnmatch(filename, opts.file_glob):
                    continue
                fp = os.path.join(dirpath, filename)
                if this._is_credential_path(fp):
                    continue
                file_rows, hit_deadline, hit_pattern_timeout = this._python_scan_file(fp, compiled, opts)
    ...
    fn _python_scan_file(fp, compiled, opts) {
        try {
        ...
        try {
            with open(fp, "r", encoding="utf-8-sig") as f:
                content = f.read()
        } catch (UnicodeDecodeError, OSError) as e {
            return [], false, false
        ...
        for line_no, raw in enumerate(content.split("\n"), start=1):
            ...
            if matched:
                ...
                if opts.output_mode == "content":
                    ...
                    out.append({"file": this._rel(fp, opts.root), "line": line_no, "match": truncated})
                elif opts.output_mode == "files":
                    out.append({"file": this._rel(fp, opts.root)})
                    break
        if opts.output_mode == "count" and count:
            out.append({"file": this._rel(fp, opts.root), "count": count})
        return out, false, false

    }
    static fn _python_finalize(rows, opts) {
        if opts.output_mode == "content":
            rows.sort(key=lambda r: (r["file"], r["line"]))
        else:
            rows.sort(key=lambda r: r["file"])
    ...
    fn _format(outcome, opts) {
        ...
        if len(rows) >= opts.max_results:
            if opts.max_results >= MAX_RESULTS_CAP:
                ...
                notices.append( f"{opts.max_results} result limit reached. " f"Use max_results={min(opts.max_results * 2, MAX_RESULTS_CAP)} to see more." )
        if outcome.timed_out:
            notices.append( f"Search stopped after {self.timeout}s — results may be incomplete. " f"Narrow `path` or `file_glob`." )
        if outcome.pattern_timeout:
            notices.append( f"Pattern took longer than {REGEX_MATCH_TIMEOUT_SECONDS}s on a line in one or more " f"files — the rest of that file was not searched. Results may be incomplete. " f"Simplify `pattern` to avoid catastrophic backtracking." )
...
class _BackendResult {
    ...
    fn _BackendResult(rows, timed_out=False, pattern_timeout=False) {
        this.rows = rows
        this.timed_out = timed_out
        this.pattern_timeout = pattern_timeout
...
class _SearchOptions {
    ...
    __slots__ = ("pattern", "root", "file_glob", "output_mode", "ignore_case", "no_ignore", "max_results", "deadline")

    fn _SearchOptions(pattern, root, file_glob, output_mode, ignore_case, no_ignore, max_results, deadline) {
        this.pattern = pattern
        this.root = root
        this.file_glob = file_glob
        this.output_mode = output_mode
        this.ignore_case = ignore_case
        this.no_ignore = no_ignore
