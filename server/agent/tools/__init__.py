...
def _import_optional_tools():
    ...
    try:
        ...
        logger.error( f"[Tools] Scheduler tool not loaded - missing dependency: {e}\n" f"  To enable scheduled tasks, run:\n" f"    pip install croniter>=2.0.0" )
    except Exception as e:
...
# BrowserTool: playwright is soft-imported inside browser_service, so self
...
__all__ = [ 'BaseTool', 'ToolManager', 'Read', 'Write', 'Edit', 'Bash', 'Ls', 'Send', 'SearchFiles', 'MemorySearchTool', 'MemoryGetTool', 'EvolutionUndoTool', 'EnvConfig', 'SchedulerTool', 'WebSearch', 'WebFetch', 'Vision', 'BrowserTool', 'McpTool', ]
