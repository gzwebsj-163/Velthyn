...
def _extract_charset_from_html_meta(raw_bytes):
...
class WebFetch(BaseTool):
    ...
    def _parse_document(self, file_path, suffix):
    ...
    def _handle_download_by_content_type(self, url, response, content_type):
        ...
        if detected_suffix and detected_suffix in ALL_DOC_SUFFIXES:
            ...
            return self._fetch_document(url if _get_url_suffix(url) in ALL_DOC_SUFFIXES else self._rewrite_url_with_suffix(url, detected_suffix))
        return ToolResult.fail(f"Error: URL returned binary content ({content_type}), not a supported document type")

    @staticmethod
    def _rewrite_url_with_suffix(url, suffix):
