...
def restore_line_endings(text, original_ending):
    """
...
class FuzzyMatchResult:
    ...
    def __init__(self, found, index = -1, match_length = 0, content_for_replacement = "", exact = False):
        self.found = found
        self.index = index
...
def _build_fuzzy_pattern(old_text):
    """
...
def find_match_spans(content, old_text):
    """
...
def looks_like_line_numbered_block(text):
    """
...
def strip_line_number_prefixes(text):
    """
...
def generate_diff_string(old_content, new_content):
    """
