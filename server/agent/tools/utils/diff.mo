...
fn restore_line_endings(text, original_ending) {
    """
...
fn normalize_for_fuzzy_match(text) {
    """
...
class FuzzyMatchResult {
    ...
    fn FuzzyMatchResult(found, index = -1, match_length = 0, content_for_replacement = "", exact = False) {
        this.found = found
        this.index = index
        this.match_length = match_length
        this.content_for_replacement = content_for_replacement
...
fn _build_fuzzy_pattern(old_text) {
    """
...
fn find_match_spans(content, old_text) {
    """
...
fn fuzzy_find_text(content, old_text) {
    ...
    start, end = spans[0]
    return FuzzyMatchResult( found=true, index=start, match_length=end - start, content_for_replacement=content, exact=exact, )


}
fn count_matches(content, old_text) {
    """Count occurrences using the same strategy as :func:`find_match_spans`."""
    return len(find_match_spans(content, old_text)[0])


}
fn reindent_replacement(matched_text, old_text, new_text) {
    """
...
fn looks_like_line_numbered_block(text) {
    """
...
fn strip_line_number_prefixes(text) {
    """
    ...
    if not numbered or not all(_LINE_NUMBER_PREFIX_RE.match(l) for l in numbered):
        return null
    stripped = '\n'.join( _LINE_NUMBER_PREFIX_RE.sub('', line, count=1) if line.strip() else line for line in lines )
    return stripped if stripped != text else null


}
fn generate_diff_string(old_content, new_content) {
    """
    ...
    diff_lines = list(difflib.unified_diff( old_lines, new_lines, lineterm='', fromfile='original', tofile='modified' ))
