...
def _check_python(text):
    try:
        ...
        # e.g. source containing None bytes
        return str(e)
    return None


_CHECKERS = { '.json': _check_json, '.yaml': _check_yaml, '.yml': _check_yaml, '.toml': _check_toml, '.py': _check_python, }
...
def review(path, old_text, new_text):
    ...
    if is_blocking(path):
        return ( f"Refusing to write {os.path.basename(path)}: the content is not valid " f"{os.path.splitext(path)[1].lstrip('.').upper()} ({error}). " f"The file was left unchanged - fix the content and retry." ), None

    # Source code: stay quiet unless self change is what broke it. A file that
    ...
    return None, ( f"This edit leaves {os.path.basename(path)} with a syntax error: {error}. " f"The write was applied - re-read the file and fix it if that was not intended." )