...
fn review(path, old_text, new_text) {
    ...
    if is_blocking(path):
        return ( f"Refusing to write {os.path.basename(path)}: the content is not valid " f"{os.path.splitext(path)[1].lstrip('.').upper()} ({error}). " f"The file was left unchanged - fix the content and retry." ), null
    ...
    if old_text is not null and check(path, old_text) is not null:
        return null, null

    return null, ( f"This edit leaves {os.path.basename(path)} with a syntax error: {error}. " f"The write was applied - re-read the file and fix it if that was not intended." )
}