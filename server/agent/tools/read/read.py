...
def _parse_page_range(pages, total_pages):
...
def number_lines(text, start_line):
...
class Read(BaseTool):
    ...
    def __init__(self, config = None):
        ...
        self.text_extensions = { '.txt', '.md', '.markdown', '.rst', '.log', '.csv', '.tsv', '.json', '.xml', '.yaml', '.yml', '.py', '.js', '.ts', '.java', '.c', '.cpp', '.h', '.hpp', '.go', '.rs', '.rb', '.php', '.html', '.css', '.scss', '.sass', '.less', '.vue', '.jsx', '.tsx', '.sh', '.bash', '.zsh', '.fish', '.ps1', '.bat', '.cmd', '.sql', '.r', '.m', '.swift', '.kt', '.scala', '.clj', '.erl', '.ex', '.dockerfile', '.makefile', '.cmake', '.gradle', '.properties', '.ini', '.conf', '.cfg', }
    ...
    def _paginate(all_lines, offset, limit, display_path):
        ...
        if total_lines == 0:
            return { "content": EMPTY_FILE_NOTICE, "total_lines": 0, "start_line": 0, "output_lines": 0, "is_empty": True, }, None
        ...
        if offset is not None:
            if offset < 0:
                ...
                if start_line >= total_lines:
                    return None, ( f"Error: Offset {offset} is beyond end of file " f"({total_lines} lines total)" )
        ...
        if truncation.first_line_exceeds_limit:
            ...
            return { "content": ( f"[Line {start_display} is {size}, exceeds " f"{format_size(DEFAULT_MAX_BYTES)} limit. Use bash tool to read: " f"head -c {DEFAULT_MAX_BYTES} {display_path} | tail -n +{start_display}]" ), "total_lines": total_lines, "start_line": start_display, "output_lines": 0, "details": {"truncation": truncation.to_dict()}, }, None
    ...
    def _return_file_metadata(self, absolute_path, file_type, file_size):
        """
    ...
    def _read_image(self, absolute_path, file_ext):
        """
    ...
    def _read_text(self, absolute_path, display_path, offset = None, limit = None):
        """
        Read text file
        ...
        try:
            ...
            result, error = self._paginate( split_lines(content), offset, limit, display_path )
            if error:
    ...
    def _read_office(self, absolute_path, display_path, file_ext, offset = None, limit = None):
        ...
        result, error = self._paginate( split_lines(text), offset, limit, display_path )
        if error:
            return ToolResult.fail(error)
    ...
    @staticmethod
    def _extract_office_text(absolute_path, file_ext):
    ...
    def _read_pdf(self, absolute_path, display_path, offset = None, limit = None, pages = None):
        """
