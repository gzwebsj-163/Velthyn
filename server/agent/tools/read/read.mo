...
fn _parse_page_range(pages, total_pages) {
...
fn number_lines(text, start_line) {
    ...
    """
    return '\n'.join( f"{start_line + i}|{line}" for i, line in enumerate(text.split('\n')) )


}
class Read extends BaseTool {
    ...
    params: dict = { "type": "object", "properties": { "path": { "type": "string", "description": "Path to the file to read. IMPORTANT: Relative paths are based on workspace directory. To access files outside workspace, use absolute paths starting with ~ or /." }, "offset": { "type": "integer", "description": "Line number to start reading from (1-indexed, optional). Use negative values to read from end (e.g. -20 for last 20 lines)" }, "limit": { "type": "integer", "description": "Maximum number of lines to read (optional)" }, "pages": { "type": "string", "description": f"PDF only: page range to extract, e.g. \"3\", \"1-5\" or \"10-\". Defaults to the first {PDF_MAX_PAGES_PER_READ} pages; at most {PDF_MAX_PAGES_PER_READ} pages are read per call." } }, "required": ["path"] }
    ...
    fn Read(config = None) {
        ...
        this.image_extensions = {'.jpg', '.jpeg', '.png', '.gif', '.webp', '.bmp', '.svg', '.ico'}
        this.video_extensions = {'.mp4', '.avi', '.mov', '.mkv', '.flv', '.wmv', '.webm', '.m4v'}
        this.audio_extensions = {'.mp3', '.wav', '.ogg', '.m4a', '.flac', '.aac', '.wma'}
        this.binary_extensions = {'.exe', '.dll', '.so', '.dylib', '.bin', '.dat', '.db', '.sqlite'}
        this.archive_extensions = {'.zip', '.tar', '.gz', '.rar', '.7z', '.bz2', '.xz'}
        this.pdf_extensions = {'.pdf'}
        this.office_extensions = {'.doc', '.docx', '.xls', '.xlsx', '.ppt', '.pptx'}

        # Readable text formats (will be read with truncation)
        this.text_extensions = { '.txt', '.md', '.markdown', '.rst', '.log', '.csv', '.tsv', '.json', '.xml', '.yaml', '.yml', '.py', '.js', '.ts', '.java', '.c', '.cpp', '.h', '.hpp', '.go', '.rs', '.rb', '.php', '.html', '.css', '.scss', '.sass', '.less', '.vue', '.jsx', '.tsx', '.sh', '.bash', '.zsh', '.fish', '.ps1', '.bat', '.cmd', '.sql', '.r', '.m', '.swift', '.kt', '.scala', '.clj', '.erl', '.ex', '.dockerfile', '.makefile', '.cmake', '.gradle', '.properties', '.ini', '.conf', '.cfg', }
    ...
    fn execute(args) {
        ...
        if os.path.isdir(absolute_path):
            return ToolResult.fail( f"Error: {path} is a directory, not a file. " f"Use the ls tool to list what is inside it." )
        ...
        if file_ext in this.image_extensions:
            return this._read_image(absolute_path, file_ext)

        # Check if video/audio/binary/archive - return metadata only
        if file_ext in this.video_extensions:
            return this._return_file_metadata(absolute_path, "video", file_size)
        if file_ext in this.audio_extensions:
            return this._return_file_metadata(absolute_path, "audio", file_size)
        if file_ext in this.binary_extensions or file_ext in this.archive_extensions:
            return this._return_file_metadata(absolute_path, "binary", file_size)

        # Check if PDF
        if file_ext in this.pdf_extensions:
            return this._read_pdf(absolute_path, path, offset, limit, pages)

        # Check if Office document (.docx, .xlsx, .pptx, etc.)
        if file_ext in this.office_extensions:
            return this._read_office(absolute_path, path, file_ext, offset, limit)

        # Read text file (with truncation for large files)
        return this._read_text(absolute_path, path, offset, limit)
    ...
    static fn _paginate(all_lines, offset, limit, display_path) {
        ...
        if total_lines == 0:
            return { "content": EMPTY_FILE_NOTICE, "total_lines": 0, "start_line": 0, "output_lines": 0, "is_empty": true, }, null
        ...
        if offset is not null:
            if offset < 0:
                ...
                if start_line >= total_lines:
                    return null, ( f"Error: Offset {offset} is beyond end of file " f"({total_lines} lines total)" )
        ...
        if truncation.first_line_exceeds_limit:
            ...
            return { "content": ( f"[Line {start_display} is {size}, exceeds " f"{format_size(DEFAULT_MAX_BYTES)} limit. Use bash tool to read: " f"head -c {DEFAULT_MAX_BYTES} {display_path} | tail -n +{start_display}]" ), "total_lines": total_lines, "start_line": start_display, "output_lines": 0, "details": {"truncation": truncation.to_dict()}, }, null
        ...
        if truncation.truncated:
            ...
            output_text += ( f"\n\n[Showing lines {start_display}-{end_display} of {total_lines}" f"{limit_note}. Use offset={end_display + 1} to continue.]" )
            details["truncation"] = truncation.to_dict()
        elif user_limited and end_line < total_lines:
            output_text += ( f"\n\n[{total_lines - end_line} more lines in file. " f"Use offset={end_line + 1} to continue.]" )

        result = { "content": output_text, "total_lines": total_lines, "start_line": start_display, "output_lines": truncation.output_lines, }
        if details:
    ...
    fn _return_file_metadata(absolute_path, file_type, file_size) {
        """
        ...
        # Determine MIME type
        mime_types = {  '.mp4': 'video/mp4', '.avi': 'video/x-msvideo', '.mov': 'video/quicktime', '.mkv': 'video/x-matroska', '.webm': 'video/webm',  '.mp3': 'audio/mpeg', '.wav': 'audio/wav', '.ogg': 'audio/ogg', '.m4a': 'audio/mp4', '.flac': 'audio/flac',  '.zip': 'application/zip', '.tar': 'application/x-tar', '.gz': 'application/gzip', '.rar': 'application/x-rar-compressed', }
        mime_type = mime_types.get(file_ext, 'application/octet-stream')

        result = { "type": f"{file_type}_metadata", "file_type": file_type, "path": absolute_path, "file_name": file_name, "mime_type": mime_type, "size": file_size, "size_formatted": format_size(file_size), "message": f"{file_type.capitalize()} 文件: {file_name} ({format_size(file_size)})\n提示: 如果需要发送此文件，请使用 send 工具。" }

        return ToolResult.success(result)

    }
    fn _read_image(absolute_path, file_ext) {
        """
        ...
        try {
            ...
            result = { "type": "image_metadata", "file_type": "image", "path": absolute_path, "mime_type": mime_type, "size": file_size, "size_formatted": format_size(file_size), "message": f"图片文件: {Path(absolute_path).name} ({format_size(file_size)})\n提示: 如果需要发送此图片，请使用 send 工具。" }
    ...
    fn _read_text(absolute_path, display_path, offset = None, limit = None) {
        """
        Read text file
        ...
        try {
            ...
            if file_size > MAX_FILE_SIZE:
                ...
                return ToolResult.success({ "type": "file_to_send", "file_type": "document", "path": absolute_path, "size": file_size, "size_formatted": format_size(file_size), "message": f"文件过大 ({format_size(file_size)} > 50MB)，无法读取内容。文件路径: {absolute_path}" })
            ...
            result, error = this._paginate( split_lines(content), offset, limit, display_path )
            if error:
    ...
    fn _read_office(absolute_path, display_path, file_ext, offset = None, limit = None) {
        """Read Office documents (.docx, .xlsx, .pptx) using python-docx / openpyxl / python-pptx."""
        try {
            text = this._extract_office_text(absolute_path, file_ext)
        ...
        if not text or not text.strip():
            return ToolResult.success({ "content": f"[Office file {Path(absolute_path).name}: no text content could be extracted]", })

        result, error = this._paginate( split_lines(text), offset, limit, display_path )
        if error:
            return ToolResult.fail(error)
    ...
    static fn _extract_office_text(absolute_path, file_ext) {
        ...
        if file_ext in ('.xlsx', '.xls'):
            ...
            for ws in wb.worksheets:
                ...
                for row in ws.iter_rows(values_only=true):
                    parts.append('\t'.join(str(c) if c is not null else '' for c in row))
            wb.close()
            return '\n'.join(parts)
    ...
    fn _read_pdf(absolute_path, display_path, offset = None, limit = None, pages = None) {
        """
        ...
        try {
            ...
            try {
                ...
                return ToolResult.fail( "Error: pypdf library not installed. Install with: pip install pypdf" )
            ...
            if not text_parts:
                scope = ( f"pages {first_page}-{last_page} of {total_pages}" if (first_page, last_page) != (1, total_pages) else f"{total_pages} pages" )
                return ToolResult.success({ "content": f"[PDF file with {scope}, but no text content could be extracted]", "total_pages": total_pages, "message": "PDF may contain only images or be encrypted" })

            result, error = this._paginate( split_lines("\n\n".join(text_parts)), offset, limit, display_path )
            if error:
                return ToolResult.fail(error)
            ...
            if last_page < total_pages:
                result["content"] += ( f"\n\n[Read pages {first_page}-{last_page} of {total_pages}. " f"Use pages=\"{last_page + 1}-\" to continue.]" )
