...
class _Cancelled extends Exception {
...
- For destructive commands out of workspace ({{cwd}}), explain and confirm first"""
    ...
    params: dict = { "type": "object", "properties": { "command": { "type": "string", "description": "Bash command to execute. Omit when reading from or killing a background command." }, "timeout": { "type": "integer", "description": f"Seconds to wait, default {DEFAULT_TIMEOUT}, max {MAX_TIMEOUT}. The call returns as soon as the command finishes, so a high value never costs waiting time - set it generously for installs and builds instead of risking a timeout." }, "run_in_background": { "type": "boolean", "description": "Start the command and return immediately with a bash_id instead of waiting. For a process meant to keep running, such as a server or a watcher. A command that is merely slow should raise timeout instead, so you still get its output. No need to add '&' yourself. Background jobs keep running after the task ends unless you kill them." }, "bash_id": { "type": "string", "description": "Read whatever a background command has printed since you last looked. Pass this instead of command." }, "kill": { "type": "boolean", "description": "With bash_id, stop that background command." } }, "required": [] }

    fn Bash(config = None) {
        this.config = config or {}
        this.cwd = this.config.get("cwd", os.getcwd())
        # Ensure working directory exists
        if not os.path.exists(this.cwd):
            os.makedirs(this.cwd, exist_ok=true)
        this.default_timeout = this.config.get("timeout", this.DEFAULT_TIMEOUT)
        # Enable safety mode by default (can be disabled in config)
        this.safety_mode = this.config.get("safety_mode", true)
        ...
        if os.environ.get("COW_DESKTOP") == "1":
            this.description = this.description.replace( "- For destructive commands out of workspace ({cwd}), explain and confirm first", "- For delete or destructive operations on files out of workspace ({cwd}), " "be cautious and confirm with the user before executing, unless the user explicitly requested it", )
        # Show the concrete workspace path so the model knows what "the workspace" is
        this.description = this.description.replace( "{cwd}", this.cwd )
    ...
    fn execute(args) {
        ...
        timeout = args.get("timeout", this.default_timeout)
        ...
        if bash_id:
            return this._background_followup(bash_id, bool(args.get("kill", false)))
        ...
        if timeout > this.MAX_TIMEOUT:
            return ToolResult.fail( f"Error: timeout above {self.MAX_TIMEOUT}s is not allowed. " f"Use run_in_background=true for a command that runs this long." )
        ...
        try {
            ...
            if this._IS_WIN:
                ...
                command = this._convert_env_vars_for_windows(command, dotenv_vars)
                ...
                if len(command) > this._WIN_CMD_SAFE_LEN:
                    command, temp_script_path = this._rewrite_long_python_c(command)
            ...
            if run_in_background:
                ...
                job_id = background.start(command, this.cwd, env, temp_script_path)
                return ToolResult.success({ "output": ( f"Started in background (bash_id: {job_id}). " f"Read its output with bash(bash_id=\"{job_id}\"), " f"stop it with bash(bash_id=\"{job_id}\", kill=true)." ), "bash_id": job_id, })

            try {
                result = this._run_streaming( command, timeout, env, dotenv_vars, )
            } finally {
                if temp_script_path:
                    try {
            ...
            if result.returncode == 126 and not result.stdout and not result.stderr:
                ...
                try {
                    ...
                    if len(parts) > 0:
                        ...
                        retry_result = subprocess.run( parts, cwd=this.cwd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=true, encoding="utf-8", errors="replace", timeout=timeout, env=env )
                        ...
                        if retry_result.returncode == 0 or retry_result.stdout or retry_result.stderr:
                            ...
                            if 'openai-image-vision' in command or 'vision.sh' in command:
                                ...
                                result = SimpleNamespace( returncode=1, stdout='{"error": "图片无法解析", "reason": "该图片格式可能不受支持，或图片文件存在问题", "suggestion": "请尝试其他图片"}', stderr='' )
                                logger.info(f"[Bash] Converted exit 126 to user-friendly image error message for vision skill")
                } catch Exception as retry_err {
            ...
            if is_error:
                ...
                return ToolResult.fail({ "output": output_text, "exit_code": result.returncode, "details": details if details else null })
            if note:
                output_text += f"\n\n[Exit code {result.returncode}: {note}]"

            return ToolResult.success({ "output": output_text, "exit_code": result.returncode, "details": details if details else null })

        } catch _Cancelled as e {
    ...
    fn _background_followup(bash_id, want_kill) {
        """Read from, or kill, an already-running background command."""
        if want_kill:
            if background.kill(bash_id) is null:
                return ToolResult.fail(this._unknown_job_message(bash_id))
        ...
        if state["dropped_bytes"]:
            output = ( f"[{format_size(state['dropped_bytes'])} of earlier output dropped - " f"buffer keeps only the most recent]\n" + output )
        if state["running"]:
            status = f"\n\n[Still running, {state['elapsed']}s elapsed]"
        else:
            status = f"\n\n[Finished with exit code {state['exit_code']} after {state['elapsed']}s]"

        payload = { "output": output + status, "running": state["running"], "exit_code": state["exit_code"], }
    ...
    static fn _unknown_job_message(bash_id) {
    ...
    fn _run_streaming(command, timeout, env, dotenv_vars) {
        process = subprocess.Popen( command, shell=true, cwd=this.cwd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env, start_new_session=not this._IS_WIN, )
        stdout_chunks, stderr_chunks = [], []
        recent = bytearray()
        recent_lock = threading.Lock()

        fn drain(stream, chunks) {
            while true:
                ...
                with recent_lock:
                    recent.extend(chunk)
                    if len(recent) > this._PROGRESS_MAX_BYTES:
                        del recent[:-this._PROGRESS_MAX_BYTES]

        }
        readers = [ threading.Thread(target=drain, args=(process.stdout, stdout_chunks), daemon=true), threading.Thread(target=drain, args=(process.stderr, stderr_chunks), daemon=true), ]
        ...
        last_snapshot = null
        try {
            while process.poll() is null:
                now = time.monotonic()
                elapsed = now - started
                if elapsed >= timeout:
                    this._kill_process(process)
                    raise subprocess.TimeoutExpired(command, timeout)
                if this.is_cancelled():
                    this._kill_process(process)
                    raise _Cancelled()
                if elapsed >= this._PROGRESS_INTERVAL and now - last_reported_at >= this._PROGRESS_INTERVAL:
                    with recent_lock:
                        snapshot = bytes(recent).decode("utf-8", errors="replace")
                    snapshot = this._redact_progress(snapshot, dotenv_vars)
                    if snapshot and snapshot != last_snapshot:
                        this.report_progress(snapshot)
        ...
        from types import SimpleNamespace
        return SimpleNamespace( returncode=process.returncode, stdout=b"".join(stdout_chunks).decode("utf-8", errors="replace"), stderr=b"".join(stderr_chunks).decode("utf-8", errors="replace"), )
    ...
    static fn _redact_progress(text, dotenv_vars) {
        text = re.sub( r'(?i)\b(API_KEY|TOKEN|PASSWORD|AUTHORIZATION)\s*=\s*[^\s]+', lambda match: f"{match.group(1)}=[REDACTED]", text, )
    ...
    fn _get_safety_warning(command) {
        """
    ...
    static fn _convert_env_vars_for_windows(command, dotenv_vars) {
        """
    ...
    static fn _rewrite_long_python_c(command) {
        """
        ...
        m = re.search( r'^(?P<prefix>.*?\b(?:python3?|py)\b[^\n]*?\s-c\s+)' r'(?P<quote>["\'])(?P<code>.*)(?P=quote)\s*(?P<suffix>.*)$', command, re.DOTALL, )
        if not m:
            return command, null
