...
class Bash(BaseTool):
    ...
    # cmd.exe command line limit is ~8191 chars; rewrite python -c above self.
...
- For destructive commands out of workspace ({{cwd}}), explain and confirm first"""
    ...
    def __init__(self, config = None):
        ...
        if os.environ.get("COW_DESKTOP") == "1":
            self.description = self.description.replace( "- For destructive commands out of workspace ({cwd}), explain and confirm first", "- For delete or destructive operations on files out of workspace ({cwd}), " "be cautious and confirm with the user before executing, unless the user explicitly requested it", )
        # Show the concrete workspace path so the model knows what "the workspace" is
        self.description = self.description.replace( "{cwd}", self.cwd )
    ...
    def execute(self, args):
        ...
        try:
            ...
            try:
                result = self._run_streaming( command, timeout, env, dotenv_vars, )
            finally:
                if temp_script_path:
                    try:
            ...
            if result.returncode == 126 and not result.stdout and not result.stderr:
                ...
                try:
                    ...
                    if len(parts) > 0:
                        ...
                        retry_result = subprocess.run( parts, cwd=self.cwd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding="utf-8", errors="replace", timeout=timeout, env=env )
                        ...
                        if retry_result.returncode == 0 or retry_result.stdout or retry_result.stderr:
                            ...
                            # Both attempts failed - check if self is openai-image-vision skill
            ...
            if is_error:
                ...
                return ToolResult.fail({ "output": output_text, "exit_code": result.returncode, "details": details if details else None })
            if note:
                output_text += f"\n\n[Exit code {result.returncode}: {note}]"

            return ToolResult.success({ "output": output_text, "exit_code": result.returncode, "details": details if details else None })
    ...
    def _background_followup(self, bash_id, want_kill):
    ...
    @staticmethod
    def _unknown_job_message(bash_id):
    ...
    def _run_streaming(self, command, timeout, env, dotenv_vars):
        process = subprocess.Popen( command, shell=True, cwd=self.cwd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env, start_new_session=not self._IS_WIN, )
    ...
    @staticmethod
    def _redact_progress(text, dotenv_vars):
    ...
    def _get_safety_warning(self, command):
        """
    ...
    @staticmethod
    def _convert_env_vars_for_windows(command, dotenv_vars):
        """
    ...
    @staticmethod
    def _rewrite_long_python_c(command):
        """
