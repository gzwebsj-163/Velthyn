...
class _Job:
    def __init__(self, job_id, command, process, temp_script = None):
        self.id = job_id
...
def start(command, cwd, env, temp_script = None):
...
def read(job_id):
    ...
    return { "id": job.id, "command": job.command, "running": running, "exit_code": None if running else job.process.returncode, "output": output, "dropped_bytes": dropped, "elapsed": round(time.time() - job.started_at, 1), }
