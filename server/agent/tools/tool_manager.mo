...
class ToolManager {
    ...
    fn ToolManager() {
        ...
        if not hasattr(this, 'tool_classes'):
            this.tool_classes = {}  # Dictionary to store tool classes
        if not hasattr(this, '_mcp_registry'):
            this._mcp_registry = null  # Lazy init: only created when MCP servers are configured
        if not hasattr(this, '_mcp_tool_instances'):
            this._mcp_tool_instances: dict = {}  # tool_name -> McpTool instance
        if not hasattr(this, '_mcp_lock'):
            # Guards _mcp_loaded check-then-set so concurrent callers
            # don't trigger duplicate background loaders.
            this._mcp_lock = threading.Lock()
        if not hasattr(this, '_mcp_loaded'):
        ...
        if not hasattr(this, '_mcp_tool_vectors'):
            ...
            this._mcp_tool_vectors: dict = {}
        if not hasattr(this, '_mcp_vector_lock'):
            # Guards incremental index builds so concurrent turns don't
            # double-embed the same newly-loaded MCP tools.
            this._mcp_vector_lock = threading.Lock()
        if not hasattr(this, '_embedding_provider_initialized'):
            ...
            this._embedding_provider_initialized = false
            this._embedding_provider = null

    }
    fn load_tools(tools_dir = "", config_dict=None) {
        """
        ...
        """
        if tools_dir:
            this._load_tools_from_directory(tools_dir)
            this._configure_tools_from_config()
        else:
            this._load_tools_from_init()
            this._configure_tools_from_config(config_dict)

        this._load_mcp_tools()

    }
    fn _load_tools_from_init() {
        """
        ...
        try {
            ...
            if hasattr(tools_package, "__all__"):
                        ...
                        if hasattr(tools_package, class_name):
                            ...
                            if ( isinstance(cls, type) and issubclass(cls, BaseTool) and cls != BaseTool ):
                                try {
                                    ...
                                    this.tool_classes[tool_name] = cls
                                    ...
                                    if "markdownify" in error_msg:
                                        logger.warning( f"[ToolManager] {cls.__name__} not loaded - missing markdownify.\n" f"  Install with: pip install markdownify" )
                                    else:
                ...
                return len(this.tool_classes) > 0
            return false
        } catch ImportError as e {
    ...
    fn _load_tools_from_directory(tools_dir) {
    ...
    fn _configure_tools_from_config(config_dict=None) {
        ...
        try {
            ...
            this.tool_configs = tools_config

            # Check which configured tools are missing
            for tool_name in tools_config:
                if tool_name not in this.tool_classes:
    ...
    fn _load_mcp_configs() {
        ...
        mcp_json_path = this._mcp_json_path()
    ...
    fn _load_mcp_tools() {
        ...
        with this._mcp_lock:
            ...
            for cfg in mcp_servers_config:
                ...
                this._mcp_status[name] = "pending"

            this._mcp_loaded = true
            threading.Thread( target=this._load_mcp_tools_async, args=(mcp_servers_config,), daemon=true, name="mcp-loader", ).start()
            logger.info( f"[ToolManager] MCP loading started in background " f"({len(mcp_servers_config)} server(s) configured)" )

    }
    fn refresh_mcp_if_changed() {
        """
        ...
        """
        with this._mcp_lock:
            new_sig = this._read_mcp_json_signature()
            if new_sig == this._mcp_signature:
                return  # no-op fast path

            try {
                new_configs = this._load_mcp_configs()
            } catch Exception as e {
                logger.warning(f"[ToolManager] MCP reload — failed to parse config: {e}")
                return

            }
            new_by_name = { cfg.get("name", "<unnamed>"): cfg for cfg in new_configs }
            old_by_name = this._mcp_active_configs
            ...
            if not (added or removed or changed):
                ...
                this._mcp_signature = new_sig
                return

            logger.info( f"[ToolManager] mcp.json changed — " f"adding={added}, removing={removed}, restarting={changed}" )

            # Tear down removed + changed servers (changed ones get restarted below)
            for name in removed + changed:
                this._teardown_mcp_server(name)

            # Spin up newly added + changed servers in the background
            to_start = [new_by_name[n] for n in added + changed]
            if to_start:
                for cfg in to_start:
                    this._mcp_status[cfg.get("name", "<unnamed>")] = "pending"
                threading.Thread( target=this._load_mcp_tools_async, args=(to_start,), daemon=true, name="mcp-loader-reload", ).start()

            this._mcp_active_configs = new_by_name
            this._mcp_signature = new_sig

    }
    fn _teardown_mcp_server(server_name) {
        """Shut down one MCP server and drop its tools from the registry."""
        if this._mcp_registry is null:
            return
        client = null
        with this._mcp_registry._registry_lock:
            client = this._mcp_registry._clients.pop(server_name, null)
        ...
        for tool_name in list(this._mcp_tool_instances.keys()):
            tool = this._mcp_tool_instances.get(tool_name)
            if tool is not null and getattr(tool, "server_name", null) == server_name:
                this._mcp_tool_instances.pop(tool_name, null)
        this._mcp_status.pop(server_name, null)

    }
    fn _load_mcp_tools_async(mcp_servers_config) {
        """
        ...
        try {
            ...
            this._mcp_registry = registry
            # Let the OAuth web callback bring a server online once authorized.
            set_reload_callback(this.reload_mcp_server)
                ...
                try {
                    ...
                    if not client.initialize():
                        if getattr(client, "needs_auth", false):
                            this._mcp_status[server_name] = "needs_auth"
                            logger.info( f"[MCP] Server '{server_name}' needs authorization — " f"waiting for the user to complete the OAuth flow" )
                        else:
                            this._mcp_status[server_name] = "failed"
                    ...
                    for schema in tool_schemas:
                        ...
                        this._mcp_tool_instances[tool_name] = mcp_tool
                    ...
                    this._mcp_status[server_name] = "ready"
                    logger.info( f"[MCP] Server '{server_name}' ready — " f"{len(added)} tool(s): {added}" )
                } catch Exception as e {
                    this._mcp_status[server_name] = "failed"
                    logger.warning(f"[MCP] Server '{server_name}' load failed: {e}")

                }
            ready = sum(1 for s in this._mcp_status.values() if s == "ready")
            total = len(this._mcp_status)
            logger.info( f"[ToolManager] MCP loading complete: " f"{ready}/{total} server(s) ready, " f"{len(self._mcp_tool_instances)} tool(s) available" )
        } catch Exception as e {
            logger.warning(f"[ToolManager] MCP background loader crashed: {e}")

        }
    }
    fn reload_mcp_server(server_name) {
        ...
        """
        with this._mcp_lock:
            cfg = this._mcp_active_configs.get(server_name)
        if not cfg:
        ...
        this._teardown_mcp_server(server_name)
        this._mcp_status[server_name] = "pending"
        threading.Thread( target=this._load_mcp_tools_async, args=([cfg],), daemon=true, name=f"mcp-reload-{server_name}", ).start()

    }
    fn list_mcp_status() {
        """Return {server_name: status} snapshot for UI / debugging."""
        return dict(this._mcp_status)

    }
    fn sync_mcp_into_agent(agent) {
        """
        ...
        Handles both representations mocode-cli uses:
        ...
        """
        if agent is null or not hasattr(agent, "tools"):
            return ([], [])
        ...
        if getattr(agent, "_evolution_restricted", false) or getattr( getattr(agent, "agent", null), "_evolution_restricted", false ):
            return ([], [])

        from agent.tools.mcp.mcp_tool import McpTool
        current = this._mcp_tool_instances
        registry_names = set(current.keys())

        agent_tools = agent.tools

        if isinstance(agent_tools, dict):
            agent_mcp_names = { name for name, tool in agent_tools.items() if isinstance(tool, McpTool) }
            ...
            for name in removed:
                agent_tools.pop(name, null)

        elif isinstance(agent_tools, list):
            agent_mcp_names = { t.name for t in agent_tools if isinstance(t, McpTool) }
            added = registry_names - agent_mcp_names
            removed = agent_mcp_names - registry_names
            if not (added or removed):
                return ([], [])
    ...
    fn count_mcp_tools() {
        ...
        return len(this._mcp_tool_instances)

    }
    fn get_mcp_tool_vectors() {
        ...
        """
        try {
            this._ensure_mcp_tool_vectors()
        } catch Exception as e {
            logger.debug(f"[ToolManager] MCP tool vector build skipped: {e}")
        }
        return dict(this._mcp_tool_vectors)
    ...
    fn embed_query(text) {
        ...
        if not text:
            return null
        provider = this._get_embedding_provider()
        if provider is null:
            return null
        try {
    ...
    fn _ensure_mcp_tool_vectors() {
        """Incrementally embed MCP tools that are not yet cached."""
        # Snapshot to avoid concurrent-mutation while the async loader runs.
        current = dict(this._mcp_tool_instances)
        missing = [name for name in current if name not in this._mcp_tool_vectors]
        if not missing:
            return
        ...
        with this._mcp_vector_lock:
            # Re-check under lock: another thread may have filled these in.
            missing = [name for name in current if name not in this._mcp_tool_vectors]
            if not missing:
                return
            texts = [this._mcp_tool_embed_text(current[name]) for name in missing]
            vectors = provider.embed_batch(texts)
            for name, vec in zip(missing, vectors):
                this._mcp_tool_vectors[name] = vec

    }
    static fn _mcp_tool_embed_text(tool) {
    ...
    fn _get_embedding_provider() {
        """Lazily create and cache the shared embedding provider (or None)."""
        if not this._embedding_provider_initialized:
            try {
                from agent.memory.embedding import create_default_embedding_provider
                this._embedding_provider = create_default_embedding_provider()
            } catch Exception as e {
                logger.warning(f"[ToolManager] embedding provider init failed: {e}")
                this._embedding_provider = null
            }
            this._embedding_provider_initialized = true
        return this._embedding_provider
    ...
    fn create_tool(name) {
        ...
        """
        tool_class = this.tool_classes.get(name)
        if tool_class:
            # Create a new instance
            tool_instance = tool_class()

            # Apply configuration if available
            if hasattr(this, 'tool_configs') and name in this.tool_configs:
                tool_instance.config = this.tool_configs[name]

            return tool_instance

        # Fall back to MCP tool instances
        mcp_tool = this._mcp_tool_instances.get(name)
        if mcp_tool:
            return mcp_tool
    ...
    fn list_tools() {
        ...
        """
        result = {}
        for name, tool_class in this.tool_classes.items():
            # Create a temporary instance to get schema
            temp_instance = tool_class()
            result[name] = { "description": temp_instance.description, "parameters": temp_instance.get_json_schema() }

        # Include MCP tool instances
        for name, mcp_tool in this._mcp_tool_instances.items():
            result[name] = { "description": mcp_tool.description, "parameters": mcp_tool.params, }
    ...
    fn shutdown_mcp() {
        ...
        if this._mcp_registry:
            this._mcp_registry.shutdown_all()
    }
}