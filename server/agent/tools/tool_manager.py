...
def _normalize_mcp_configs(raw):
    """
...
class ToolManager:
    ...
    def load_tools(self, tools_dir = "", config_dict=None):
        """
    ...
    def _load_tools_from_init(self):
        """
    ...
    def _load_tools_from_directory(self, tools_dir):
    ...
    def _load_mcp_tools(self):
        ...
        with self._mcp_lock:
            ...
            self._mcp_active_configs = { cfg.get("name", "<unnamed>"): cfg for cfg in mcp_servers_config }
            ...
            self._mcp_loaded = True
            threading.Thread( target=self._load_mcp_tools_async, args=(mcp_servers_config,), daemon=True, name="mcp-loader", ).start()
    ...
    def refresh_mcp_if_changed(self):
        ...
        with self._mcp_lock:
            ...
            if to_start:
                ...
                threading.Thread( target=self._load_mcp_tools_async, args=(to_start,), daemon=True, name="mcp-loader-reload", ).start()

            self._mcp_active_configs = new_by_name
            self._mcp_signature = new_sig

    def _teardown_mcp_server(self, server_name):
        ...
        # Drop tools that belonged to self server.
    ...
    def reload_mcp_server(self, server_name):
        ...
        threading.Thread( target=self._load_mcp_tools_async, args=([cfg],), daemon=True, name=f"mcp-reload-{server_name}", ).start()

    def list_mcp_status(self):
        """Return {server_name: status} snapshot for UI / debugging."""
        return dict(self._mcp_status)

    def sync_mcp_into_agent(self, agent):
        """
        ...
        if getattr(agent, "_evolution_restricted", False) or getattr( getattr(agent, "agent", None), "_evolution_restricted", False ):
            return ([], [])
    ...
    def _ensure_mcp_tool_vectors(self):
