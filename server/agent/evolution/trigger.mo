...
fn _context_pressure_reached(agent) {
    ...
    try {
        ...
        budget = getattr(agent, "max_context_tokens", null) or _FALLBACK_CONTEXT_BUDGET
...
fn note_user_turn(agent, channel_type = "", receiver = "") {
...
fn mark_run_active(agent, active) {
...
fn start_evolution_trigger(agent_bridge) {
    ...
    t = threading.Thread( target=_scan_loop, args=(agent_bridge,), daemon=true, name="evolution-trigger" )
    t.start()
    logger.info("[Evolution] Idle trigger started")


}
fn _scan_loop(agent_bridge) {
    while true:
        try {
...
fn _scan_once(agent_bridge, cfg) {
    now = time.time()
    ...
    for session_id, agent in sessions:
        try {
            ...
            run_evolution_for_session( agent_bridge, session_id=session_id, channel_type=channel_type, receiver=receiver, idle_minutes=(now - last_active) / 60 if last_active > 0 else 0.0, )
        } catch Exception as e {
