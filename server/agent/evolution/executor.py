...
def _build_transcript(messages, max_chars = 12000):
...
class _WorkspaceWriteGuard:
    ...
    def __init__(self, inner, workspace_dir):
        self._inner = inner
...
class _BashWorkspaceGuard:
    ...
    def _escapes_workspace(self, command):
...
def _guard_tools(tools, workspace_dir):
...
def _workspace_snapshot(workspace_dir):
...
def _workspace_changed(workspace_dir, pre):
    """True if any watched file was added, removed, or modified since ``pre``."""
    return _workspace_snapshot(workspace_dir) != pre


def run_evolution_for_session(agent_bridge, session_id, channel_type = "", receiver = "", user_id = None, idle_minutes = 0.0):
    ...
    try:
        ...
        review_agent = agent_bridge.create_agent( system_prompt="", tools=review_tools, description="Self-evolution review agent", max_steps=cfg.max_steps, workspace_dir=str(workspace_dir), skill_manager=getattr(agent, "skill_manager", None), memory_manager=getattr(agent, "memory_manager", None), enable_skills=True, runtime_info=getattr(agent, "runtime_info", None), )
        # Mark self as a restricted review agent so runtime MCP reconciliation
        # (ToolManager.sync_mcp_into_agent) will NOT silently re-inject MCP tools
        # that _select_tools()/_guard_tools() intentionally withheld. Without self
        ...
        # only looks at messages added after self point (silent or not).
        agent._evo_done_msg_count = total_msgs
...
def _inject_evolution_record(agent_bridge, session_id, channel_type, summary, backup_id):
...
def _notify_user(channel_type, receiver, summary):
