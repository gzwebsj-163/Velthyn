...
# Context-pressure trigger: evolve once the live context exceeds self fraction
...
def _context_pressure_reached(agent):
...
def note_user_turn(agent, channel_type = "", receiver = ""):
...
def mark_run_active(agent, active):
...
def start_evolution_trigger(agent_bridge):
