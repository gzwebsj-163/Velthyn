...
class KnowledgeService:
    ...
    def __init__(self, workspace_root, memory_manager=None):
        self.workspace_root = os.path.abspath(workspace_root)
        self.knowledge_dir = os.path.join(self.workspace_root, "knowledge")
        self._memory_manager = memory_manager

    def _resolve_path(self, rel_path, *, kind = None, allow_missing = True):
    ...
    def _ensure_not_protected(self, rel_path):
    ...
    def _manager(self):
        if self._memory_manager is None:
            ...
            self._memory_manager = MemoryManager( MemoryConfig(workspace_root=self.workspace_root), embedding_provider=embedding_provider, )
    ...
    def _sync_index(self, old_paths, force = False):
    ...
    @staticmethod
    def _extract_title(md_path, fallback):
    ...
    def _sanitize_document_name(self, filename):
    ...
    @staticmethod
    def _decode_document_content(content):
    ...
    def _resolve_import_destination(self, target_category, filename, conflict_strategy):
    ...
    def create_document(self, path, content = "", overwrite = False):
    ...
    def import_documents(self, target_category, files, conflict_strategy = "skip"):
        ...
        for item in files:
            ...
            try:
                ...
                if size > self.MAX_IMPORT_FILE_SIZE:
                    raise ValueError("file too large")
                rel_path, destination, mode = self._resolve_import_destination( target_category, filename, conflict_strategy )
                if mode == "skip":
                    skipped += 1
    ...
    def rename_category(self, path, new_path):
    ...
    def delete_category(self, path, confirm = False):
        rel_path, full_path = self._resolve_path(path, kind="category")
    ...
    def move_documents(self, paths, target_category):
        if not isinstance(paths, list):
            raise ValueError("paths must be a list")
        target_rel, target_full = self._resolve_path(target_category, kind="category")
        if not target_full.is_dir():
    ...
    def _scan_dir(self, dir_path, stats, is_root = False):
        """
    ...
    def dispatch(self, action, payload = None):
        ...
        try:
            if action == "list":
                ...
                result = self.create_document(payload.get("path"), payload.get("content", ""), payload.get("overwrite", False))
            elif action == "import_documents":
                result = self.import_documents( payload.get("target_category"), payload.get("files") or [], payload.get("conflict_strategy", "skip"), )
            else:
