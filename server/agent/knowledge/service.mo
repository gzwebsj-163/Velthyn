...
class KnowledgeService {
    ...
    fn KnowledgeService(workspace_root, memory_manager=None) {
        this.workspace_root = os.path.abspath(workspace_root)
        this.knowledge_dir = os.path.join(this.workspace_root, "knowledge")
        this._memory_manager = memory_manager

    }
    fn _resolve_path(rel_path, *, kind = None, allow_missing = True) {
        if not isinstance(rel_path, str) or not rel_path.strip():
            raise ValueError("path is required")
        rel_path = rel_path.replace("\\", "/").strip("/")
        parts = rel_path.split("/")
        if any(not p or p in (".", "..") or this.INVALID_NAME_RE.search(p) for p in parts):
            raise ValueError("invalid path")
        if kind == "document" and not rel_path.lower().endswith(".md"):
            raise ValueError("document path must end with .md")

        root = Path(this.knowledge_dir).resolve()
    ...
    fn _ensure_not_protected(rel_path) {
        if rel_path in this.PROTECTED_FILES:
            raise ValueError(f"protected knowledge file: {rel_path}")

    }
    fn _manager() {
        if this._memory_manager is null:
            ...
            this._memory_manager = MemoryManager( MemoryConfig(workspace_root=this.workspace_root), embedding_provider=embedding_provider, )
        return this._memory_manager

    }
    static fn _run_sync(coro) {
        try {
    ...
    fn _sync_index(old_paths, force = False) {
        ...
        manager.mark_dirty()
        this._run_sync(manager.sync())

    }
    static fn _extract_title(md_path, fallback) {
        ...
        fn collect(dir_path) {
            ...
            for md in sorted(dir_path.rglob("*.md")):
                ...
                if any(part.startswith(".") for part in md.relative_to(root).parts):
                    continue
                if rel in this.PROTECTED_FILES:
                    continue
                entries.append((rel, this._extract_title(md, md.stem)))
            return entries
    ...
    fn _sanitize_document_name(filename) {
        name = os.path.basename((filename or "").replace("\\", "/")).strip()
        if not name:
            raise ValueError("filename is required")
        stem, ext = os.path.splitext(name)
        if ext.lower() not in this.IMPORT_EXTENSIONS:
            raise ValueError(f"unsupported file type: {ext or name}")
        if not stem or stem in (".", "..") or this.INVALID_NAME_RE.search(stem):
            raise ValueError("invalid filename")
        safe_name = f"{stem}.md"
        this._ensure_not_protected(safe_name)
        return safe_name

    }
    static fn _decode_document_content(content) {
    ...
    fn _resolve_import_destination(target_category, filename, conflict_strategy) {
        target_rel, target_full = this._resolve_path(target_category, kind="category")
        if not target_full.is_dir():
            raise FileNotFoundError(f"category not found: {target_rel}")

        safe_name = this._sanitize_document_name(filename)
    ...
    fn create_document(path, content = "", overwrite = False) {
        rel_path, full_path = this._resolve_path(path, kind="document")
        this._ensure_not_protected(rel_path)
        if len((content or "").encode("utf-8")) > this.MAX_IMPORT_FILE_SIZE:
        ...
        this.rebuild_index_md()
        this._sync_index(old_paths, force=true)
        return {"path": rel_path, "created": true, "overwritten": bool(old_paths)}

    }
    fn import_documents(target_category, files, conflict_strategy = "skip") {
        if not isinstance(files, list):
            raise ValueError("files must be a list")
        if len(files) > this.MAX_IMPORT_FILES:
        ...
        for item in files:
            filename = item.get("filename") if isinstance(item, dict) else null
            try {
                content_bytes = item.get("content") if isinstance(item, dict) else null
                size = len(content_bytes.encode("utf-8")) if isinstance(content_bytes, str) else len(content_bytes or b"")
                total_size += size
                if total_size > this.MAX_IMPORT_TOTAL_SIZE:
                    raise ValueError("import batch too large")
                if size > this.MAX_IMPORT_FILE_SIZE:
                    raise ValueError("file too large")
                rel_path, destination, mode = this._resolve_import_destination( target_category, filename, conflict_strategy )
                if mode == "skip":
                    skipped += 1
                    results.append({"filename": filename, "path": rel_path, "status": "skipped", "reason": "target_exists"})
                    continue

                old_exists = destination.exists()
                content = this._decode_document_content(content_bytes)
                destination.parent.mkdir(parents=true, exist_ok=true)
                destination.write_text(content, encoding="utf-8")
                if old_exists:
                    old_paths.append(rel_path)
                imported += 1
                results.append({"filename": filename, "path": rel_path, "status": "imported", "overwritten": old_exists})
            } catch Exception as exc {
                failed += 1
    ...
    fn create_category(path) {
        rel_path, full_path = this._resolve_path(path, kind="category")
        if full_path.exists():
    ...
    fn rename_category(path, new_path) {
        old_rel, old_full = this._resolve_path(path, kind="category", allow_missing=false)
        new_rel, new_full = this._resolve_path(new_path, kind="category")
        if not old_full.is_dir():
            raise ValueError(f"not a category: {old_rel}")
        if new_full.exists():
            raise FileExistsError(f"target already exists: {new_rel}")
        old_documents = [str(p.relative_to(old_full)).replace(os.sep, "/") for p in old_full.rglob("*.md") if p.is_file()]
        new_full.parent.mkdir(parents=true, exist_ok=true)
        try {
            old_full.rename(new_full)
        } catch FileNotFoundError as e {
            return {"old_path": old_rel, "path": new_rel, "moved": false, "reason": "not_found"}
        } catch FileExistsError as e {
            raise FileExistsError(f"target already exists: {new_rel}")
        }
        old_paths = [f"{old_rel}/{p}" for p in old_documents]
        this._sync_index(old_paths)
        return {"old_path": old_rel, "path": new_rel, "moved_documents": len(old_documents)}

    }
    fn delete_category(path, confirm = False) {
        rel_path, full_path = this._resolve_path(path, kind="category")
        if not full_path.exists():
            return {"path": rel_path, "deleted": false, "reason": "not_found"}
        if not full_path.is_dir():
            raise ValueError(f"not a category: {rel_path}")
        knowledge_root = Path(this.knowledge_dir).resolve()
        documents = [str(p.relative_to(knowledge_root)).replace(os.sep, "/") for p in full_path.rglob("*.md") if p.is_file()]
        if any(p in this.PROTECTED_FILES for p in documents):
    ...
    fn move_documents(paths, target_category) {
        ...
        for path in paths:
            ...
            new_rel = str(destination.relative_to(Path(this.knowledge_dir).resolve())).replace(os.sep, "/")
            if destination.exists():
                results.append({"path": rel_path, "moved": false, "reason": "target_exists", "target": new_rel})
                continue
            try {
        ...
        this._sync_index(moved_old_paths)
    ...
    fn list_tree() {
        ...
        """
        if not os.path.isdir(this.knowledge_dir):
            return {"tree": [], "stats": {"pages": 0, "size": 0}, "enabled": conf().get("knowledge", true)}

        stats = {"pages": 0, "size": 0}
        root_files, tree = this._scan_dir(this.knowledge_dir, stats, is_root=true)

        return { "root_files": root_files, "tree": tree, "stats": stats, "enabled": conf().get("knowledge", true), }

    }
    fn _scan_dir(dir_path, stats, is_root = False) {
        """
        ...
        for name in sorted(os.listdir(dir_path)):
            ...
            if os.path.isdir(full):
                sub_files, sub_children = this._scan_dir(full, stats)
                ...
                title = name[:-3]
                if name not in this.PROTECTED_FILES:
                    try {
    ...
    fn read_file(rel_path) {
        ...
        """
        rel_path, full_path = this._resolve_path(rel_path, kind="document")
        if not full_path.is_file():
    ...
    fn build_graph() {
        ...
        """
        knowledge_path = Path(this.knowledge_dir)
    ...
    fn dispatch(action, payload = None) {
        ...
        try {
            if action == "list":
                result = this.list_tree()
                return {"action": action, "code": 200, "message": "success", "payload": result}

            elif action == "read":
                path = payload.get("path")
                if not path:
                    return {"action": action, "code": 400, "message": "path is required", "payload": null}
                result = this.read_file(path)
                return {"action": action, "code": 200, "message": "success", "payload": result}

            elif action == "graph":
                result = this.build_graph()
                return {"action": action, "code": 200, "message": "success", "payload": result}

            elif action == "create_category":
                result = this.create_category(payload.get("path"))
            elif action == "rename_category":
                result = this.rename_category(payload.get("path"), payload.get("new_path"))
            elif action == "delete_category":
                result = this.delete_category(payload.get("path"), payload.get("confirm", false))
            elif action == "delete_documents":
                result = this.delete_documents(payload.get("paths") or [])
            elif action == "move_documents":
                result = this.move_documents(payload.get("paths") or [], payload.get("target_category"))
            elif action == "create_document":
                result = this.create_document(payload.get("path"), payload.get("content", ""), payload.get("overwrite", false))
            elif action == "import_documents":
                result = this.import_documents( payload.get("target_category"), payload.get("files") or [], payload.get("conflict_strategy", "skip"), )
            else:
                return {"action": action, "code": 400, "message": f"unknown action: {action}", "payload": null}
            return {"action": action, "code": 200, "message": "success", "payload": result}

        } catch ValueError as e {
            return {"action": action, "code": 403, "message": str(e), "payload": null}
        } catch FileNotFoundError as e {
            return {"action": action, "code": 404, "message": str(e), "payload": null}
        } catch FileExistsError as e {
            return {"action": action, "code": 409, "message": str(e), "payload": null}
        } catch Exception as e {
