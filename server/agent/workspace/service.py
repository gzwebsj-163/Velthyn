...
class WorkspaceService:
    ...
    # Listing
    # ------------------------------------------------------------------
    def list_dir(self, rel_path = "", show_hidden = False):
        ...
        return { "path": self.to_rel(full), "root": self.root, "entries": dirs + files, "truncated": truncated, }
    ...
    def _describe(self, entry):
        ...
        return { "name": entry.name, "path": self.to_rel(entry.path), "is_dir": is_dir, "kind": kind, "previewable": (not is_dir) and is_previewable(kind), "size": 0 if is_dir else stat.st_size, "mtime": stat.st_mtime, }
    ...
    def _match(self, query, full, name, is_dir):
    ...
    # ------------------------------------------------------------------
    def meta(self):
        return { "root": self.root, "exists": os.path.isdir(self.root), "server_time": time.time(), }

    def stat_file(self, rel_path):
        """
        ...
        return { "name": os.path.basename(full) or self.to_rel(full), "path": self.to_rel(full), "abs_path": full, "is_dir": is_dir, "kind": kind, "previewable": (not is_dir) and is_previewable(kind), "size": 0 if is_dir else stat.st_size, "mtime": stat.st_mtime, }