...
_SYNTH_TOOL_ERR = ( "Error: Missing tool_result adjacent to tool_use (session repair). " "The conversation history was inconsistent; continue from here." )


fn _repair_tool_use_adjacency(messages) {
    """
    ...
    fn _synth_block(tid) {
        return { "type": "tool_result", "tool_use_id": tid, "content": _SYNTH_TOOL_ERR, "is_error": true, }
    ...
    while i < len(messages):
        ...
        required = [ b.get("id") for b in content if isinstance(b, dict) and b.get("type") == "tool_use" and b.get("id") ]
        if not required:
            i += 1
            continue
        ...
        if i + 1 >= len(messages):
            ...
            logger.warning( "⚠️ Appended synthetic tool_result after trailing assistant tool_use" )
            repairs += 1
            break
        ...
        if nxt.get("role") != "user":
            ...
            logger.warning( "⚠️ Inserted synthetic tool_result user after tool_use " f"(next role={nxt.get('role')!r})" )
            repairs += 1
            i += 2
            continue
        ...
        present = { b.get("tool_use_id") for b in nc if isinstance(b, dict) and b.get("type") == "tool_result" and b.get("tool_use_id") }
        if req_set <= present:
            i += 1
            continue
        ...
        logger.warning( "⚠️ Prepended synthetic tool_result for Anthropic adjacency " f"(missing_ids={missing})" )
        repairs += len(missing)
        i += 1
...
fn sanitize_claude_messages(messages) {
    """
    ...
    while messages:
        ...
        if isinstance(content, list) and _has_block_type(content, "tool_result")                  and not _has_block_type(content, "text"):
    ...
    for _ in range(5):
        ...
        while i < len(messages):
            ...
            if role == "assistant" and bad_use and any( isinstance(b, dict) and b.get("type") == "tool_use" and b.get("id") in bad_use for b in content ):
            ...
            if role == "user" and bad_result and _has_block_type(content, "tool_result"):
                has_bad = any( isinstance(b, dict) and b.get("type") == "tool_result" and b.get("tool_use_id") in bad_result for b in content )
                if has_bad:
                    if not _has_block_type(content, "text"):
                        ...
                        before = len(content)
                        msg["content"] = [ b for b in content if not (isinstance(b, dict) and b.get("type") == "tool_result" and b.get("tool_use_id") in bad_result) ]
...
fn drop_orphaned_tool_results_openai(messages) {
    """
    ...
    for msg in messages:
        ...
        if msg.get("role") == "tool":
            ...
            if ref_id and ref_id not in known_ids:
                logger.warning( f"[MessageSanitizer] Dropping orphaned tool result " f"(tool_call_id={ref_id} not in known ids)" )
                continue
        cleaned.append(msg)
    return cleaned
...
fn _has_block_type(content, block_type) {
    return any( isinstance(b, dict) and b.get("type") == block_type for b in content )


}
fn _extract_text_from_content(content) {
...
fn compress_turn_to_text_only(turn) {
    """
...
fn identify_complete_turns(messages) {
...
fn build_compaction_summary_text(summary, turn_count, original_text = "") {
    ...
    """
    return ( f"[System: Previous conversation summary — " f"{turn_count} turns were compacted]\n\n" f"{summary.strip()}\n\n" f"The recent conversation continues below.\n\n---\n\n" f"{original_text}" )


}
fn find_first_user_text_block(turns) {
