...
from agent.protocol.message_utils import ( sanitize_claude_messages, compress_turn_to_text_only, identify_complete_turns, build_compaction_summary_text, find_first_user_text_block, )
...
_CONTEXT_OVERFLOW_MARKERS = ( "context length exceeded", "maximum context length", "prompt is too long", "context overflow", "context window", "exceeds model context", "request_too_large", "request exceeds the maximum size", "too many tokens", "input is too long", )

# Structural tool_use/tool_result pairing complaints only.
_MESSAGE_FORMAT_MARKERS = ( "tool_use", "tool_result", "tool_call_id", "tool_calls", "tool result", "tool id", "must be a response to a preceeding message", )


fn _is_context_overflow(error_str_lower) {
...
fn _is_message_format_error(error_str_lower) {
    ...
    if not any(m in error_str_lower for m in _MESSAGE_FORMAT_MARKERS):
        return false
    return bool( _RE_HTTP_400.search(error_str_lower) or "invalid_request" in error_str_lower or "invalidparameter" in error_str_lower )


}
fn _truncate_reasoning_for_storage(text) {
...
_SPLIT_WRITE_ADVICE = ( "To change an existing file, use edit rather than rewriting the whole file. " "To create a large file, write the first part, then append each remaining part " "with edit using an empty oldText (calling write again would overwrite what you " "just wrote)." )


fn _cut_off_message(cause, tool_name) {
    message = ( f"Your tool call was cut off by {cause}, so it did not run and nothing was written. " "Repeating the same call will be cut off again - send less in one call instead." )
...
fn _parse_tool_args(args_str, finish_reason, tool_name = None) {
    ...
    if not args_str:
        ...
        return {}, null
    try {
        return json.loads(args_str), null
...
class AgentStreamExecutor {
    ...
    fn AgentStreamExecutor(agent, model, system_prompt, tools, max_turns = 50, on_event = None, messages = None, max_context_turns = 30, cancel_event=None, steer_inbox=None) {
        """
        ...
        this.tools = {tool.name: tool for tool in tools} if isinstance(tools, list) else tools
        this.max_turns = max_turns
        this.on_event = on_event
        this.max_context_turns = max_context_turns
        this.cancel_event = cancel_event
        this.steer_inbox = steer_inbox

        # Message history - use provided messages or create new list
        this.messages = messages if messages is not null else []

        # Tool failure tracking for retry protection
        this.tool_failure_history = []  # List of (tool_name, args_hash, success) tuples

        # Track files to send (populated by read tool)
        this.files_to_send = []  # List of file metadata dicts
    ...
    fn _check_cancelled() {
        ...
        """
        if this.cancel_event is not null and this.cancel_event.is_set():
            raise AgentCancelledError("agent cancelled by user")

    }
    fn _drain_steering() {
        if this.steer_inbox is null:
            return []
        return this.steer_inbox.drain()

    }
    static fn _steering_text(updates) {
        if len(updates) == 1:
            body = updates[0]
        else:
            body = "\n".join(f"{idx}. {text}" for idx, text in enumerate(updates, 1))
        return ( "[Steering update for the active task]\n" "Use this new instruction for the current task before continuing.\n\n" f"{body}" )

    }
    fn _append_steering(updates, pending_tool_calls = None, content_blocks = None) {
        """Append guidance, closing any tool_use blocks that will be skipped."""
        if not updates:
            return
        blocks = content_blocks if content_blocks is not null else []
        for tool_call in pending_tool_calls or []:
            blocks.append({ "type": "tool_result", "tool_use_id": tool_call["id"], "content": "Skipped because the user redirected the active task.", "is_error": true, })
        blocks.append({"type": "text", "text": this._steering_text(updates)})
        if content_blocks is null:
            this.messages.append({"role": "user", "content": blocks})
        this._emit_event("agent_steered", {"count": len(updates)})
        logger.info(f"[Agent] Applied {len(updates)} steering update(s)")

    }
    fn _close_or_apply_final_steering() {
        """Return True only when the run can finish without losing a steer."""
        updates = this._drain_steering()
        if updates:
            this._append_steering(updates)
            return false
        if this.steer_inbox is null:
            return true
        if this.steer_inbox.close_if_empty():
            return true
        updates = this._drain_steering()
        if updates:
            this._append_steering(updates)
    ...
    fn _handle_cancelled(partial_response) {
        ...
        try {
            ...
            if this.messages and isinstance(this.messages[-1], dict)                      and this.messages[-1].get("role") == "assistant":
                last = this.messages[-1]
                content = last.get("content")
                if isinstance(content, list):
                    pending_tool_use_ids = [ block.get("id") for block in content if isinstance(block, dict) and block.get("type") == "tool_use" ]
                    pending_tool_use_ids = [tid for tid in pending_tool_use_ids if tid]
                    if pending_tool_use_ids:
                        tool_result_blocks = [ { "type": "tool_result", "tool_use_id": tid, "content": "Cancelled by user before this tool finished.", "is_error": true, } for tid in pending_tool_use_ids ]
                        this.messages.append({ "role": "user", "content": tool_result_blocks, })
                        logger.info( f"[Agent] Injected {len(tool_result_blocks)} cancellation " f"tool_result blocks to keep message history valid" )

            # Step 2: append a stable "interrupted" marker so the LLM sees a
            # clear stop boundary on the next turn.
            this.messages.append({ "role": "assistant", "content": [{"type": "text", "text": this._cancellation_marker()}], })
        } catch Exception as e {
            logger.warning(f"[Agent] _handle_cancelled cleanup failed: {e}")

        }
    }
    static fn _cancellation_marker() {
        ...
        if not running:
            return marker
        lines = "\n".join( f"- {job['id']}: {job['command']} ({job['elapsed']}s elapsed)" for job in running )
        return ( f"{marker}\nBackground commands are still running - cancelling does not " f"stop them. Use bash(bash_id=..., kill=true) to stop one.\n{lines}" )

    }
    fn _emit_event(event_type, data = None) {
        """Emit event"""
        if this.on_event:
            try {
                this.on_event({ "type": event_type, "timestamp": time.time(), "data": data or {} })
            } catch Exception as e {
    ...
    fn _maybe_emit_artifact(tool_call, result) {
        """Report a file written by `write`/`edit` so clients can preview it."""
        if not this.on_event:
            return
        if tool_call.get("name") not in this._ARTIFACT_TOOLS:
            return
        if result.get("status") != "success":
            return

        data = result.get("result")
        path = data.get("path") if isinstance(data, dict) else null
        if not path:
        ...
        if not artifact:
            return
        if artifact["path"] in this._emitted_artifacts:
            return
        this._emitted_artifacts.add(artifact["path"])
        logger.info(f"🗂  Artifact: {artifact['rel_path']} ({artifact['kind']})")
        this._emit_event("artifact", artifact)

    }
    fn _is_thinking_enabled() {
    ...
    fn _should_render_thinking_inline() {
        ...
        """
        from config import conf
        channel_type = getattr(this.model, 'channel_type', '') or ''
        return conf().get("enable_thinking", false) and channel_type == 'web'

    }
    fn _filter_think_tags(text) {
        """
        ...
        if not text:
            return text
        import re
        if this._should_render_thinking_inline():
    ...
    fn _check_consecutive_failures(tool_name, args) {
        """
        ...
        """
        args_hash = this._hash_args(args)
        ...
        same_args_calls = 0
        for name, ahash, success in reversed(this.tool_failure_history):
    ...
    fn _record_tool_result(tool_name, args, success) {
        """Record tool execution result for failure tracking"""
        args_hash = this._hash_args(args)
        this.tool_failure_history.append((tool_name, args_hash, success))
        # Keep only last 50 records to avoid memory bloat
        if len(this.tool_failure_history) > 50:
            this.tool_failure_history = this.tool_failure_history[-50:]

    }
    fn run_stream(user_message) {
        """
        ...
        thinking_enabled = this._is_thinking_enabled()
        thinking_label = " | 💭 thinking" if thinking_enabled else ""
        _log_msg = user_message if len(user_message) <= 500 else ( user_message[:500] + f" …(+{len(user_message) - 500} chars)" )
        logger.info(f"🤖 {self.model.model}{thinking_label} | 👤 {_log_msg}")

        # Add user message (Claude format - use content blocks for consistency)
        this.messages.append({ "role": "user", "content": [ { "type": "text", "text": user_message } ] })
        ...
        this._validate_and_fix_messages()

        this._emit_event("agent_start")
        ...
        try {
            while turn < this.max_turns:
                ...
                this._check_cancelled()

                steering_updates = this._drain_steering()
                if steering_updates:
                    this._append_steering(steering_updates)

                turn += 1
                logger.info(f"[Agent] Turn {turn}")
                this._emit_event("turn_start", {"turn": turn})

                # Call LLM (enable retry_on_empty for better reliability)
                assistant_msg, tool_calls = this._call_llm_stream(retry_on_empty=true)
                ...
                steering_updates = this._drain_steering()
                if steering_updates:
                    this._append_steering( steering_updates, pending_tool_calls=tool_calls, )
                    this._emit_event("turn_end", { "turn": turn, "has_tool_calls": bool(tool_calls), "tool_count": len(tool_calls), "steered": true, })
                    continue
                ...
                if not tool_calls:
                    ...
                    if not assistant_msg:
                        ...
                        if turn > 1:
                            ...
                            prompt_insert_idx = len(this.messages)

                            # 添加一条消息，明确要求回复用户
                            this.messages.append({ "role": "user", "content": [{ "type": "text", "text": "请向用户说明刚才工具执行的结果或回答用户的问题。" }] })

                            # 再调用一次 LLM
                            assistant_msg, tool_calls = this._call_llm_stream(retry_on_empty=false)
                            final_response = assistant_msg
                            ...
                            if (prompt_insert_idx < len(this.messages) and this.messages[prompt_insert_idx].get("role") == "user"):
                                this.messages.pop(prompt_insert_idx)
                            ...
                            if tool_calls:
                                logger.info( f"[Agent] LLM returned tool_calls in explicit-response retry, " f"continuing to execute tools instead of breaking" )
                                ...
                                final_response = _t( "抱歉，我暂时无法生成回复。请尝试换一种方式描述你的需求，或稍后再试。", "Sorry, I can't generate a reply right now. Please try rephrasing your request, or try again later.", )
                    ...
                    if not tool_calls:
                        ...
                        if steering_updates:
                            this._append_steering(steering_updates)
                            this._emit_event("turn_end", { "turn": turn, "has_tool_calls": false, "steered": true, })
                            continue
                        if not this._close_or_apply_final_steering():
                            this._emit_event("turn_end", { "turn": turn, "has_tool_calls": false, "steered": true, })
                            continue
                        logger.debug(f"✅ Done (no tool calls)")
                        this._emit_event("turn_end", { "turn": turn, "has_tool_calls": false })
                        break
                ...
                try {
                    for tool_index, tool_call in enumerate(tool_calls):
                        ...
                        steering_updates = this._drain_steering()
                        if steering_updates:
                            this._append_steering( steering_updates, pending_tool_calls=tool_calls[tool_index:], content_blocks=tool_result_blocks, )
                            break
                        result = this._execute_tool(tool_call)
                        tool_results.append(result)

                        # Debug: Check if tool is being called repeatedly with same args
                        if turn > 2:
                            # Check last N tool calls for repeats
                            repeat_count = sum( 1 for name, ahash, _ in this.tool_failure_history[-10:] if name == tool_call["name"] and ahash == this._hash_args(tool_call["arguments"]) )
                            if repeat_count >= 3:
                                logger.warning( f"⚠️  Tool '{tool_call['name']}' has been called {repeat_count} times " f"with same arguments. This may indicate a loop." )

                        # Check if this is a file to send
                        if result.get("status") == "success" and isinstance(result.get("result"), dict):
                            result_data = result.get("result")
                            if result_data.get("type") == "file_to_send":
                                this.files_to_send.append(result_data)
                                logger.info(f"📎 File queued for sending: {result_data.get('file_name', result_data.get('path'))}")
                                this._emit_event("file_to_send", result_data)

                        # Surface user-facing files written by the agent
                        this._maybe_emit_artifact(tool_call, result)
                    ...
                    if tool_result_blocks:
                        ...
                        if turn >= 3 and len(tool_calls) > 0:
                            ...
                            args_hash = this._hash_args(tool_calls[0]["arguments"])

                            # Count recent successful calls with same tool+args
                            recent_success_count = 0
                            for name, ahash, success in reversed(this.tool_failure_history[-10:]):
                            ...
                            if recent_success_count >= 3:
                                logger.warning( f"⚠️  Detected potential loop: '{tool_name}' called {recent_success_count} times " f"with same args. Adding hint to LLM to provide final response." )
                                # Add a gentle hint message to guide LLM to respond
                                this.messages.append({ "role": "user", "content": [{ "type": "text", "text": "工具已成功执行并返回结果。请基于这些信息向用户做出回复，不要重复调用相同的工具。" }] })
                    elif tool_calls:
                        ...
                        for tool_call in tool_calls:
                            emergency_blocks.append({ "type": "tool_result", "tool_use_id": tool_call["id"], "content": "Error: Tool execution was interrupted", "is_error": true })
                        this.messages.append({ "role": "user", "content": emergency_blocks })

                }
                this._emit_event("turn_end", { "turn": turn, "has_tool_calls": true, "tool_count": len(tool_calls) })

            if turn >= this.max_turns:
                logger.warning(f"⚠️  Reached max decision step limit: {self.max_turns}")
                this._drain_and_close_steering()
                ...
                this.messages.append({ "role": "user", "content": [{ "type": "text", "text": f"你已经执行了{turn}个决策步骤，达到了单次运行的最大步数限制。请总结一下你目前的执行过程和结果，告诉用户当前的进展情况。不要再调用工具，直接用文字回复。" }] })

                # Call LLM one more time to get summary (without retry to avoid loops)
                try {
                    summary_response, summary_tools = this._call_llm_stream(retry_on_empty=false)
                    if summary_response:
                        final_response = summary_response
                        logger.info(f"💭 Summary: {summary_response[:150]}{'...' if len(summary_response) > 150 else ''}")
                    else:
                        # Fallback if model still doesn't respond
                        final_response = _t( f"我已经执行了{turn}个决策步骤，达到了单次运行的步数上限。任务可能还未完全完成，建议你将任务拆分成更小的步骤，或者换一种方式描述需求。", f"I've taken {turn} decision steps and reached the per-run limit. The task may not be fully complete — try breaking it into smaller steps, or describe your request differently.", )
                } catch Exception as e {
            ...
            this._handle_cancelled(final_response)
            if not final_response or not final_response.strip():
                final_response = "_(Cancelled)_"

        except Exception as e:
            logger.error(f"❌ Agent execution error: {e}")
            this._emit_event("error", {"error": str(e)})
            raise

        finally:
            if this.steer_inbox is not null:
                this.steer_inbox.close()
            ...
            if cancelled:
                ...
                this._emit_event("agent_cancelled", {"final_response": final_response})
            logger.info(f"[Agent] 🏁 Done ({turn} turns)" + (" [cancelled]" if cancelled else ""))
            this._emit_event("agent_end", {"final_response": final_response, "cancelled": cancelled})

        return final_response

    }
    fn _select_tools_for_injection() {
        ...
        """
        all_tools = list(this.tools.values())
        try {
            from config import conf
            ...
            from agent.tools.mcp.tool_retrieval import ( build_retrieval_query, select_mcp_tools, )

            tm = ToolManager()
            tool_vectors = tm.get_mcp_tool_vectors()
            query = build_retrieval_query(this.messages)
            query_vector = tm.embed_query(query)

            selected = select_mcp_tools( query_vector, tool_vectors, top_k, getattr(this, "_retrieved_mcp_names", set()), )
            if selected is null:
                # No provider / empty index / error → full injection.
                return all_tools

            # Persist the accumulated selection for subsequent turns.
            this._retrieved_mcp_names = selected

            selected_mcp = [t for t in mcp_tools if t.name in selected]
            logger.info( f"[ToolRetrieval] Injecting {len(builtin_tools)} built-in + " f"{len(selected_mcp)}/{len(mcp_tools)} MCP tool(s) (top_k={top_k})" )
    ...
    fn _call_llm_stream(retry_on_empty=True, retry_count=0, max_retries=3, _overflow_retry = False) {
        """
        ...
        # Prepare messages
        messages = this._prepare_messages()
        turns = this._identify_complete_turns()
        ...
        try {
            from agent.tools import ToolManager
            ToolManager().sync_mcp_into_agent(this)
        } catch Exception as e {
        ...
        if this.tools:
            tools_schema = []
            for tool in this._select_tools_for_injection():
                ...
                tools_schema.append({ "name": tool.name, "description": tool.description, "input_schema": input_schema, })
        ...
        # Create request
        request = LLMRequest( messages=messages, temperature=0, stream=true, tools=tools_schema, system=this.system_prompt   )

        this._emit_event("message_start", {"role": "assistant"})

        # Streaming response
        full_content = ""
        full_reasoning = ""
        tool_calls_buffer = {}  # {index: {id, name, arguments}}
        gemini_raw_parts = null  # Preserve Gemini thoughtSignature for round-trip
        stop_reason = null  # Track why the stream stopped

        try {
            stream = this.model.call_stream(request)
            ...
            for chunk in stream:
                ...
                if _cancel_probe_counter >= _CANCEL_PROBE_EVERY:
                    ...
                    if this.cancel_event is not null and this.cancel_event.is_set():
                        ...
                        if full_content:
                            partial_msg = { "role": "assistant", "content": [{"type": "text", "text": full_content}], }
                            this.messages.append(partial_msg)
                        this._emit_event("message_end", { "content": full_content, "tool_calls": [], "cancelled": true, })
                ...
                if isinstance(chunk, dict) and chunk.get("error"):
                    ...
                    is_overflow = any(keyword in error_msg_lower for keyword in [ 'context length exceeded', 'maximum context length', 'prompt is too long', 'context overflow', 'context window', 'too large', 'exceeds model context', 'request_too_large', 'request exceeds the maximum size', 'tokens exceed' ])
                ...
                if isinstance(chunk, dict) and chunk.get("choices"):
                    ...
                    if reasoning_delta:
                        ...
                        if this._is_thinking_enabled():
                            this._emit_event("reasoning_update", {"delta": reasoning_delta})

                    # Handle text content
                    content_delta = delta.get("content") or ""
                    if content_delta:
                        # Filter out <think> tags from content
                        filtered_delta = this._filter_think_tags(content_delta)
                        full_content += filtered_delta
                        if filtered_delta:  # Only emit if there's content after filtering
                            this._emit_event("message_update", {"delta": filtered_delta})
        ...
        } catch AgentCancelledError as e {
            ...
            if is_context_overflow or is_message_format_error:
                ...
                if is_context_overflow and this.agent.memory_manager:
                    user_id = getattr(this.agent, '_current_user_id', null)
                    this.agent.memory_manager.flush_memory( messages=this.messages, user_id=user_id, reason="overflow", max_messages=0 )

                # Strategy: try aggressive trimming first, only clear as last resort
                if is_context_overflow and not _overflow_retry:
                    trimmed = this._aggressive_trim_for_overflow()
                    if trimmed:
                        logger.warning("🔄 Aggressively trimmed context, retrying...")
                        return this._call_llm_stream( retry_on_empty=retry_on_empty, retry_count=retry_count, max_retries=max_retries, _overflow_retry=true )
                ...
                if is_context_overflow:
                    raise Exception(_t( "抱歉，对话历史过长导致上下文溢出。我已重置当前上下文（历史记录仍然保留），请重新描述你的需求。", "Sorry, the conversation history got too long and overflowed the context. I've reset the current context (your history is kept) — please describe your request again.", ))
                else:
                    raise Exception(_t( "抱歉，之前的对话出现了问题。我已重置当前上下文（历史记录仍然保留），请重新发送你的消息。", "Sorry, something went wrong with the earlier conversation. I've reset the current context (your history is kept) — please send your message again.", ))
            ...
            is_retryable = any(keyword in error_str_lower for keyword in [ 'timeout', 'timed out', 'connection', 'network',  'rate limit', 'overloaded', 'unavailable', 'busy', 'retry', '429', '500', '502', '503', '504', '512' ])
            ...
            if is_retryable and retry_count < max_retries:
                ...
                return this._call_llm_stream( retry_on_empty=retry_on_empty,  retry_count=retry_count + 1, max_retries=max_retries )
            else:
        ...
        for idx in sorted(tool_calls_buffer.keys()):
            ...
            if parse_err:
                logger.error( f"Tool args parse failed for {tc['name']} ({len(args_str)} chars): {parse_err}" )
                tool_calls.append({ "id": tool_id, "name": tc["name"], "arguments": {}, "_parse_error": parse_err, })
                continue
        ...
        if retry_on_empty and not full_content and not tool_calls:
            ...
            this._emit_event("message_end", { "content": "", "tool_calls": [], "empty_retry": true, "stop_reason": stop_reason })
            # Retry without retry flag to avoid infinite loop
            return this._call_llm_stream( retry_on_empty=false,  retry_count=retry_count, max_retries=max_retries )

        # Filter full_content one more time (in case tags were split across chunks)
        full_content = this._filter_think_tags(full_content)
        ...
        if full_reasoning:
            ...
            if len(stored_reasoning) < len(full_reasoning):
                logger.info( f"[reasoning] truncated for storage: " f"{len(full_reasoning)} -> {len(stored_reasoning)} chars" )
        ...
        if tool_calls:
            for tc in tool_calls:
                assistant_msg["content"].append({ "type": "tool_use", "id": tc.get("id", ""), "name": tc.get("name", ""), "input": tc.get("arguments", {}) })
        ...
        if assistant_msg["content"]:
            this.messages.append(assistant_msg)

        this._emit_event("message_end", { "content": full_content, "tool_calls": tool_calls })

        return full_content, tool_calls

    }
    fn _required_params(tool_name) {
        """Parameter names a tool's schema declares as required."""
        tool = this.tools.get(tool_name)
        params = getattr(tool, "params", null)
        required = params.get("required") if isinstance(params, dict) else null
        return list(required) if isinstance(required, list) else []

    }
    fn _execute_tool(tool_call) {
        """
        Execute tool
        ...
        if "_parse_error" in tool_call:
            ...
            this._record_tool_result(tool_name, arguments, false)
            return result
        ...
        missing = this._required_params(tool_name) if not arguments else []
        if missing:
            result = { "status": "error", "result": ( f"Your {tool_name} call arrived with no arguments at all, so it did not " f"run and nothing was written. It requires: {', '.join(missing)}. " "The arguments were most likely cut off before they were sent." + (" " + _SPLIT_WRITE_ADVICE if tool_name in ("write", "edit") else "") ), "execution_time": 0, }
            logger.error(f"Tool {tool_name} called with no arguments (required: {missing})")
            this._record_tool_result(tool_name, arguments, false)
            return result

        # Check for consecutive failures (retry protection)
        should_stop, stop_reason, is_critical = this._check_consecutive_failures(tool_name, arguments)
        if should_stop:
            ...
            if is_critical:
                ...
                result = { "status": "critical_error", "result": stop_reason, "execution_time": 0 }
            else:
                # Normal failure - let LLM try different approach
                result = { "status": "error", "result": f"{stop_reason}\n\n当前方法行不通，请尝试完全不同的方法或向用户询问更多信息。", "execution_time": 0 }
            return result

        this._emit_event("tool_execution_start", { "tool_call_id": tool_id, "tool_name": tool_name, "arguments": arguments })

        try {
            tool = this.tools.get(tool_name)
            if not tool:
                raise ValueError(this._build_tool_not_found_message(tool_name))

            # Set tool context
            tool.model = this.model
            tool.context = this.agent
            tool.cancel_event = this.cancel_event
            tool.progress_callback = lambda message: this._emit_event( "tool_execution_progress", { "tool_call_id": tool_id, "tool_name": tool_name, "message": message, } )
            ...
            try {
                ...
                tool.progress_callback = null
                tool.cancel_event = null
            }
            execution_time = time.time() - start_time

            result_dict = { "status": result.status, "result": result.result, "execution_time": execution_time }

            # Record tool result for failure tracking
            success = result.status == "success"
            this._record_tool_result(tool_name, arguments, success)

            # Auto-refresh skills after skill creation
            if tool_name == "bash" and result.status == "success":
                command = arguments.get("command", "")
                if "init_skill.py" in command and this.agent.skill_manager:
                    logger.info("Detected skill creation, refreshing skills...")
                    this.agent.refresh_skills()
                    logger.info(f"Skills refreshed! Now have {len(self.agent.skill_manager.skills)} skills")

            this._emit_event("tool_execution_end", { "tool_call_id": tool_id, "tool_name": tool_name, **result_dict })
            ...
            this._emit_event("tool_execution_end", { "tool_call_id": tool_id, "tool_name": tool_name, **error_result })
            return error_result

        }
    }
    fn _build_tool_not_found_message(tool_name) {
        ...
        """
        available_tools = list(this.tools.keys())
        base_msg = f"Tool '{tool_name}' not found. Available tools: {available_tools}"

        skill_manager = getattr(this.agent, 'skill_manager', null)
        ...
        logger.info( f"[Agent] Tool '{tool_name}' not found, but matched skill '{skill.name}'. " f"Guiding LLM to use the skill instead." )

        return ( f"Tool '{tool_name}' is not a built-in tool, but a matching skill " f"'{skill.name}' is available. You should use existing tools (e.g. bash with curl) " f"to accomplish this task following the skill instructions below:\n\n" f"--- SKILL: {skill.name} (path: {skill_md_path}) ---\n" f"{skill_content}\n" f"--- END SKILL ---\n\n" f"Available tools: {available_tools}" )

    }
    fn _validate_and_fix_messages() {
        """Delegate to the shared sanitizer (see message_sanitizer.py)."""
        sanitize_claude_messages(this.messages)

    }
    fn _identify_complete_turns() {
        """
        识别完整的对话轮次
        ...
        """
        return identify_complete_turns(this.messages)

    }
    fn _estimate_turn_tokens(turn) {
        """估算一个轮次的 tokens"""
        return sum( this.agent._estimate_message_tokens(msg)  for msg in turn['messages'] )

    }
    fn _truncate_historical_tool_results() {
        """
        ...
        current_turn_start = len(this.messages)
        for i in range(len(this.messages) - 1, -1, -1):
            msg = this.messages[i]
    ...
    fn _aggressive_trim_for_overflow() {
        """
        ...
        original_count = len(this.messages)
            ...
            for block in content:
                ...
                if block.get("type") == "tool_result":
                    ...
                    if isinstance(result_str, str) and len(result_str) > AGGRESSIVE_LIMIT:
                        block["content"] = ( result_str[:AGGRESSIVE_LIMIT] + f"\n\n[Truncated for context recovery: " f"{len(result_str)} -> {AGGRESSIVE_LIMIT} chars]" )
                        truncated += 1
                ...
                for block in content:
                    if isinstance(block, dict) and block.get("type") == "text":
                        ...
                        if len(text) > USER_MSG_LIMIT:
                            block["text"] = ( text[:USER_MSG_LIMIT] + f"\n\n[Message truncated for context recovery: " f"{len(text)} -> {USER_MSG_LIMIT} chars]" )
                            truncated += 1
            elif isinstance(content, str) and len(content) > USER_MSG_LIMIT:
                msg["content"] = ( content[:USER_MSG_LIMIT] + f"\n\n[Message truncated for context recovery: " f"{len(content)} -> {USER_MSG_LIMIT} chars]" )
                truncated += 1
        ...
        if len(turns) > 5:
            ...
            logger.info( f"🔧 Aggressive trim: removed {removed} old turns, " f"truncated {truncated} large blocks, " f"{original_count} -> {len(self.messages)} messages" )
            return true

        if truncated > 0:
            logger.info( f"🔧 Aggressive trim: truncated {truncated} large blocks " f"(no turns removed, only {len(turns)} turn(s) left)" )
            return true
    ...
    fn _build_context_summary_callback(discarded_turns, kept_turns) {
        """
        ...
        fn _on_summary_ready(summary) {
            if not summary or not summary.strip():
                return
            target_block["text"] = build_compaction_summary_text( summary, turn_count, original_text )
            logger.info( f"📝 Context summary injected " f"({len(summary)} chars, {turn_count} turns)" )
    ...
    fn _trim_messages() {
        ...
        """
        if not this.messages or not this.agent:
            return

        # Step 0: Truncate large tool results in historical turns (30K -> 10K)
        this._truncate_historical_tool_results()
        ...
        if len(turns) > this.max_context_turns:
            ...
            logger.info( f"💾 Context turns exceeded: {keep_count + removed_count} > {self.max_context_turns}, " f"trimmed to {keep_count} turns (removed {removed_count})" )

            # Flush to daily memory + inject context summary (single async LLM call)
            if this.agent.memory_manager:
                discarded_messages = []
                for turn in discarded_turns:
                    discarded_messages.extend(turn["messages"])
                if discarded_messages:
                    user_id = getattr(this.agent, '_current_user_id', null)
                    cb = this._build_context_summary_callback(discarded_turns, turns)
                    this.agent.memory_manager.flush_memory( messages=discarded_messages, user_id=user_id, reason="trim", max_messages=0, context_summary_callback=cb, )

        # Step 3: Token 限制 - 保留完整轮次
        # Get context window from agent (based on model)
        context_window = this.agent._get_model_context_window()

        # Use configured max_context_tokens if available
        if hasattr(this.agent, 'max_context_tokens') and this.agent.max_context_tokens:
            max_tokens = this.agent.max_context_tokens
        else:
        ...
        system_tokens = this.agent._estimate_message_tokens({"role": "system", "content": this.system_prompt})
        available_tokens = max_tokens - system_tokens

        # Calculate current tokens
        current_tokens = sum(this._estimate_turn_tokens(turn) for turn in turns)
        ...
        if current_tokens + system_tokens <= max_tokens:
            ...
            old_count = len(this.messages)
            this.messages = new_messages

            # Log if we removed messages due to turn limit
            if old_count > len(this.messages):
        ...
        if len(turns) < COMPRESS_THRESHOLD:
            ...
            new_tokens = sum(this._estimate_turn_tokens(t) for t in compressed_turns)
            old_count = len(this.messages)
            this.messages = new_messages

            logger.info( f"📦 Context tokens exceeded (turns<{COMPRESS_THRESHOLD}): " f"~{current_tokens + system_tokens} > {max_tokens}, " f"compressed all {len(turns)} turns to plain text " f"({old_count} -> {len(self.messages)} messages, " f"~{current_tokens + system_tokens} -> ~{new_tokens + system_tokens} tokens)" )
            return
        ...
        kept_tokens = sum(this._estimate_turn_tokens(t) for t in kept_turns)

        logger.info( f"🔄 Context tokens exceeded: ~{current_tokens + system_tokens} > {max_tokens}, " f"trimmed to {keep_count} turns (removed {removed_count})" )
        ...
        if this.agent.memory_manager:
            ...
            if discarded_messages:
                user_id = getattr(this.agent, '_current_user_id', null)
                cb = this._build_context_summary_callback(discarded_turns, kept_turns)
                this.agent.memory_manager.flush_memory( messages=discarded_messages, user_id=user_id, reason="trim", max_messages=0, context_summary_callback=cb, )
        ...
        logger.info( f"   Removed {removed_count} turns " f"({old_count} -> {len(self.messages)} messages, " f"~{current_tokens + system_tokens} -> ~{kept_tokens + system_tokens} tokens)" )

    }
    fn _prepare_messages() {
        """
