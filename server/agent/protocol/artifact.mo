...
INTERNAL_DIRS = { "memory", "knowledge", "skills", "tmp", "scheduler", "plans", }

# Workspace-root files that are part of the agent's own configuration.
INTERNAL_FILES = { "AGENT.md", "RULE.md", "MEMORY.md", "USER.md", "BOOTSTRAP.md", "mcp.json", }

_EXT_KINDS = { "html": {".html", ".htm"}, "markdown": {".md", ".markdown"}, "image": {".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp", ".svg", ".ico"}, "video": {".mp4", ".webm", ".mov", ".avi", ".mkv", ".m4v"}, "audio": {".mp3", ".wav", ".ogg", ".m4a", ".flac", ".aac"}, "pdf": {".pdf"}, "csv": {".csv", ".tsv"}, "code": { ".py", ".js", ".ts", ".tsx", ".jsx", ".java", ".c", ".cpp", ".h", ".go", ".rs", ".rb", ".php", ".sh", ".sql", ".css", ".scss", ".json", ".yaml", ".yml", ".xml", ".toml", ".ini", }, "text": {".txt", ".log"}, "office": {".doc", ".docx", ".xls", ".xlsx", ".ppt", ".pptx"}, }

# Kinds the frontend can render inline in the preview panel.
PREVIEWABLE_KINDS = { "html", "markdown", "image", "video", "audio", "pdf", "csv", "code", "text", }

_KIND_BY_EXT: Dict[str, str] = { ext: kind for kind, exts in _EXT_KINDS.items() for ext in exts }
...
fn is_previewable(kind) {
    return kind in PREVIEWABLE_KINDS


}
fn resolve_workspace_path(path, workspace_root) {
...
fn _is_internal(abs_path, workspace_root) {
...
fn build_artifact(path, workspace_root = None) {
    """
    ...
    return { "type": "artifact", "path": abs_path, "rel_path": rel_path, "dir": os.path.dirname(abs_path), "file_name": os.path.basename(abs_path), "kind": kind, "previewable": is_previewable(kind), "size": size, }


}
fn safe_build_artifact(path, workspace_root = None) {
