...
class Agent:
    def __init__(self, system_prompt, description = "AI Agent", model = None, tools=None, output_mode="print", max_steps=100, max_context_tokens=None, context_reserve_tokens=None, memory_manager=None, name = None, workspace_dir = None, skill_manager=None, enable_skills = True, runtime_info = None):
        """
    ...
    def get_skills_prompt(self, skill_filter=None):
        """
    ...
    def get_full_system_prompt(self, skill_filter=None):
        """
        ...
        try:
            ...
            full = builder.build( tools=self.tools, context_files=context_files, skill_manager=self.skill_manager, memory_manager=self.memory_manager, runtime_info=self.runtime_info, )
    ...
    def _execute_post_process_tools(self):
        ...
        for tool in post_process_tools:
            ...
            self.capture_tool_use( tool_name=tool.name, input_params={},   output=result.result, status=result.status, error_message=str(result.result) if result.status == "error" else None, execution_time=execution_time )
    ...
    def capture_tool_use(self, tool_name, input_params, output, status, thought=None, error_message=None, execution_time=0.0):
        """
        ...
        action = AgentAction( agent_id=self.id if hasattr(self, 'id') else str(id(self)), agent_name=self.name, action_type=AgentActionType.TOOL_USE, tool_result=tool_result, thought=thought )

        self.captured_actions.append(action)

        return action

    def run_stream(self, user_message, on_event=None, clear_history = False, skill_filter=None, cancel_event=None, steer_inbox=None):
        """
        ...
        # Create a copy of messages for self execution to avoid concurrent modification
        ...
        executor = AgentStreamExecutor( agent=self, model=self.model, system_prompt=full_system_prompt, tools=self.tools, max_turns=self.max_steps, on_event=on_event, messages=messages_copy,   max_context_turns=max_context_turns, cancel_event=cancel_event, steer_inbox=steer_inbox, )
        ...
        with self.messages_lock:
            ...
            # Track messages added in self run (user query + all assistant/tool messages)
