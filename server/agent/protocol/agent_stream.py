...
# time (subject to its own SSE / rendering limits); self bound only controls what
...
def _truncate_reasoning_for_storage(text):
...
def _cut_off_message(cause, tool_name):
...
def _parse_tool_args(args_str, finish_reason, tool_name = None):
...
class AgentStreamExecutor:
    ...
    def __init__(self, agent, model, system_prompt, tools, max_turns = 50, on_event = None, messages = None, max_context_turns = 30, cancel_event=None, steer_inbox=None):
        """
    ...
    def _append_steering(self, updates, pending_tool_calls = None, content_blocks = None):
    ...
    def _close_or_apply_final_steering(self):
    ...
    def _drain_and_close_steering(self):
    ...
    def _handle_cancelled(self, partial_response):
        ...
        try:
            ...
            if self.messages and isinstance(self.messages[-1], dict)                      and self.messages[-1].get("role") == "assistant":
            ...
            self.messages.append({ "role": "assistant", "content": [{"type": "text", "text": self._cancellation_marker()}], })
        except Exception as e:
    ...
    def _maybe_emit_artifact(self, tool_call, result):
    ...
    def _should_render_thinking_inline(self):
    ...
    def _check_consecutive_failures(self, tool_name, args):
        """
    ...
    def _record_tool_result(self, tool_name, args, success):
    ...
    def run_stream(self, user_message):
        ...
        # retrieval only grows self set within a run, so a tool that already
        ...
        try:
            while turn < self.max_turns:
                ...
                steering_updates = self._drain_steering()
                if steering_updates:
                    self._append_steering( steering_updates, pending_tool_calls=tool_calls, )
                    self._emit_event("turn_end", { "turn": turn, "has_tool_calls": bool(tool_calls), "tool_count": len(tool_calls), "steered": True, })
                    continue
                ...
                if not tool_calls:
                    ...
                    if not assistant_msg:
                        ...
                        if turn > 1:
                            ...
                            # 添加一条消息，明确要求回复用户
                            self.messages.append({ "role": "user", "content": [{ "type": "text", "text": "请向用户说明刚才工具执行的结果或回答用户的问题。" }] })
                            ...
                            if (prompt_insert_idx < len(self.messages) and self.messages[prompt_insert_idx].get("role") == "user"):
                    ...
                    # and continue down to the tool execution branch in self same iteration.
                    if not tool_calls:
                        steering_updates = self._drain_steering()
                        if steering_updates:
                            self._append_steering(steering_updates)
                            self._emit_event("turn_end", { "turn": turn, "has_tool_calls": False, "steered": True, })
                            continue
                ...
                try:
                    for tool_index, tool_call in enumerate(tool_calls):
                        ...
                        steering_updates = self._drain_steering()
                        if steering_updates:
                            self._append_steering( steering_updates, pending_tool_calls=tool_calls[tool_index:], content_blocks=tool_result_blocks, )
                            break
                        ...
                        if turn > 2:
                            ...
                            repeat_count = sum( 1 for name, ahash, _ in self.tool_failure_history[-10:] if name == tool_call["name"] and ahash == self._hash_args(tool_call["arguments"]) )
                            if repeat_count >= 3:
                                logger.warning( f"⚠️  Tool '{tool_call['name']}' has been called {repeat_count} times " f"with same arguments. This may indicate a loop." )

                        # Check if self is a file to send
                    ...
                    if tool_result_blocks:
                        ...
                        if turn >= 3 and len(tool_calls) > 0:
                            ...
                            if recent_success_count >= 3:
                                ...
                                self.messages.append({ "role": "user", "content": [{ "type": "text", "text": "工具已成功执行并返回结果。请基于这些信息向用户做出回复，不要重复调用相同的工具。" }] })
                    elif tool_calls:
                ...
                self._emit_event("turn_end", { "turn": turn, "has_tool_calls": True, "tool_count": len(tool_calls) })
            ...
            if turn >= self.max_turns:
                ...
                self.messages.append({ "role": "user", "content": [{ "type": "text", "text": f"你已经执行了{turn}个决策步骤，达到了单次运行的最大步数限制。请总结一下你目前的执行过程和结果，告诉用户当前的进展情况。不要再调用工具，直接用文字回复。" }] })
    ...
    def _select_tools_for_injection(self):
        ...
        try:
            ...
            selected = select_mcp_tools( query_vector, tool_vectors, top_k, getattr(self, "_retrieved_mcp_names", set()), )
            if selected is None:
    ...
    def _call_llm_stream(self, retry_on_empty=True, retry_count=0, max_retries=3, _overflow_retry = False):
        """
        ...
        # Pull in any MCP tools that finished loading since self turn started.
        ...
        # fall back to the static `tool.params` (MCP tools rely on self).
        tools_schema = None
        if self.tools:
            tools_schema = []
        ...
        # Create request
        request = LLMRequest( messages=messages, temperature=0, stream=True, tools=tools_schema, system=self.system_prompt   )
        ...
        try:
            ...
            for chunk in stream:
                ...
                if _cancel_probe_counter >= _CANCEL_PROBE_EVERY:
                    ...
                    if self.cancel_event is not None and self.cancel_event.is_set():
                        ...
                        self._emit_event("message_end", { "content": full_content, "tool_calls": [], "cancelled": True, })
                ...
                if isinstance(chunk, dict) and chunk.get("error"):
                    ...
                    # Check if self is a context overflow error (keyword-based, works for all models)
            ...
            if is_context_overflow or is_message_format_error:
                ...
                if is_context_overflow and self.agent.memory_manager:
                    ...
                    self.agent.memory_manager.flush_memory( messages=self.messages, user_id=user_id, reason="overflow", max_messages=0 )
                ...
                if is_context_overflow and not _overflow_retry:
                    ...
                    if trimmed:
                        ...
                        return self._call_llm_stream( retry_on_empty=retry_on_empty, retry_count=retry_count, max_retries=max_retries, _overflow_retry=True )
            ...
            if is_retryable and retry_count < max_retries:
                ...
                return self._call_llm_stream( retry_on_empty=retry_on_empty,  retry_count=retry_count + 1, max_retries=max_retries )
            else:
        ...
        if retry_on_empty and not full_content and not tool_calls:
            ...
            self._emit_event("message_end", { "content": "", "tool_calls": [], "empty_retry": True, "stop_reason": stop_reason })
    ...
    def _execute_tool(self, tool_call):
        ...
        self._emit_event("tool_execution_start", { "tool_call_id": tool_id, "tool_name": tool_name, "arguments": arguments })
        ...
        try:
            ...
            tool.progress_callback = lambda message: self._emit_event( "tool_execution_progress", { "tool_call_id": tool_id, "tool_name": tool_name, "message": message, } )
            ...
            self._emit_event("tool_execution_end", { "tool_call_id": tool_id, "tool_name": tool_name, **result_dict })
            ...
            self._emit_event("tool_execution_end", { "tool_call_id": tool_id, "tool_name": tool_name, **error_result })
            return error_result

    def _build_tool_not_found_message(self, tool_name):
    ...
    def _identify_complete_turns(self):
        """
        识别完整的对话轮次
    ...
    def _estimate_turn_tokens(self, turn):
        """估算一个轮次的 tokens"""
    ...
    def _aggressive_trim_for_overflow(self):
        """
    ...
    def _build_context_summary_callback(self, discarded_turns, kept_turns):
        """
        ...
        if len(turns) > self.max_context_turns:
            ...
            if self.agent.memory_manager:
                ...
                if discarded_messages:
                    ...
                    self.agent.memory_manager.flush_memory( messages=discarded_messages, user_id=user_id, reason="trim", max_messages=0, context_summary_callback=cb, )
