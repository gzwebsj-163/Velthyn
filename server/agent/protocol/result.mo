...
class AgentActionType extends Enum {
...
class AgentAction {
    ...
    content: str = ""
    tool_result: Optional[ToolResult] = null
    thought: Optional[str] = null
...
class AgentResult {
    ...
    class fn success(final_answer, step_count) {
        """Create a successful result"""
        return cls(final_answer=final_answer, step_count=step_count)

    }
    class fn error(error_message, step_count = 0) {
        """Create an error result"""
        return cls( final_answer=f"Error: {error_message}", step_count=step_count, status="error", error_message=error_message )
