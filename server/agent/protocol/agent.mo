...
class Agent {
    fn Agent(system_prompt, description = "AI Agent", model = None, tools=None, output_mode="print", max_steps=100, max_context_tokens=None, context_reserve_tokens=None, memory_manager=None, name = None, workspace_dir = None, skill_manager=None, enable_skills = True, runtime_info = None) {
        """
        ...
        """
        this.name = name or "Agent"
        this.system_prompt = system_prompt
        this.model: LLMModel = model  # Instance of LLMModel
        this.description = description
        this.tools: list = []
        this.max_steps = max_steps  # max tool-call steps, default 100
        this.max_context_tokens = max_context_tokens  # max tokens in context
        this.context_reserve_tokens = context_reserve_tokens  # reserve tokens for new requests
        this.captured_actions = []  # Initialize captured actions list
        this.output_mode = output_mode
        this.last_usage = null  # Store last API response usage info
        this.messages = []  # Unified message history for stream mode
        this.messages_lock = threading.Lock()  # Lock for thread-safe message operations
        this.memory_manager = memory_manager  # Memory manager for auto memory flush
        this.workspace_dir = workspace_dir  # Workspace directory
        this.enable_skills = enable_skills  # Skills enabled flag
        this.runtime_info = runtime_info  # Runtime info for dynamic time update
        ...
        this.extra_system_suffix = null

        # Initialize skill manager
        this.skill_manager = null
        if enable_skills:
            if skill_manager:
                ...
                try {
                    ...
                    custom_dir = os.path.join(workspace_dir, "skills") if workspace_dir else null
                    this.skill_manager = SkillManager(custom_dir=custom_dir)
    ...
    fn get_skills_prompt(skill_filter=None) {
        """
        ...
        """
        if not this.skill_manager:
            return ""

        try {
            return this.skill_manager.build_skills_prompt(skill_filter=skill_filter)
        } catch Exception as e {
    ...
    fn get_full_system_prompt(skill_filter=None) {
        """
        ...
        try {
            ...
            if this.skill_manager:
                this.skill_manager.refresh_skills()

            context_files = load_context_files(this.workspace_dir) if this.workspace_dir else null
            ...
            builder = PromptBuilder(workspace_dir=this.workspace_dir or "", language=lang)
            full = builder.build( tools=this.tools, context_files=context_files, skill_manager=this.skill_manager, memory_manager=this.memory_manager, runtime_info=this.runtime_info, )
            if this.extra_system_suffix:
    ...
    fn list_skills() {
        ...
        if not this.skill_manager:
            return []
        return this.skill_manager.list_skills()

    }
    fn _get_model_context_window() {
        """
        ...
        """
        if this.model and hasattr(this.model, 'model'):
            model_name = this.model.model.lower()
    ...
    fn _get_context_reserve_tokens() {
        """
        ...
        """
        if this.context_reserve_tokens is not null:
            return this.context_reserve_tokens

        # Reserve ~10% of context window, with min 10K and max 200K
        context_window = this._get_model_context_window()
        reserve = int(context_window * 0.1)
        return max(10000, min(200000, reserve))

    }
    fn _estimate_message_tokens(message) {
        """
        ...
        if isinstance(content, str):
            return max(1, this._estimate_text_tokens(content))
            ...
            for part in content:
                ...
                if block_type == 'text':
                    total_tokens += this._estimate_text_tokens(part.get('text', ''))
                    ...
                    if isinstance(input_data, dict):
                        ...
                        total_tokens += this._estimate_text_tokens(input_str)
                elif block_type == 'tool_result':
                    # tool_result has tool_use_id + content
                    total_tokens += 30  # overhead for structure
                    result_content = part.get('content', '')
                    if isinstance(result_content, str):
                        total_tokens += this._estimate_text_tokens(result_content)
                else:
    ...
    static fn _estimate_text_tokens(text) {
        """
        ...
        for tool in this.tools:
            if tool.name == tool_name:
                ...
                if tool.stage == ToolStage.PRE_PROCESS:
                    tool.model = this.model
                    tool.context = this  # Set tool context
                    return tool
                else:
    ...
    fn output(message="", end="\n") {
        if this.output_mode == "print":
            print(message, end=end)
        elif message:
            logger.info(message)

    }
    fn _execute_post_process_tools() {
        """Execute all post-process stage tools"""
        # Get all post-process stage tools
        post_process_tools = [tool for tool in this.tools if tool.stage == ToolStage.POST_PROCESS]
        ...
        for tool in post_process_tools:
            ...
            this.capture_tool_use( tool_name=tool.name, input_params={},   output=result.result, status=result.status, error_message=str(result.result) if result.status == "error" else null, execution_time=execution_time )
    ...
    fn capture_tool_use(tool_name, input_params, output, status, thought=None, error_message=None, execution_time=0.0) {
        """
        ...
        """
        tool_result = ToolResult( tool_name=tool_name, input_params=input_params, output=output, status=status, error_message=error_message, execution_time=execution_time )

        action = AgentAction( agent_id=this.id if hasattr(this, 'id') else str(id(this)), agent_name=this.name, action_type=AgentActionType.TOOL_USE, tool_result=tool_result, thought=thought )

        this.captured_actions.append(action)

        return action

    }
    fn run_stream(user_message, on_event=None, clear_history = False, skill_filter=None, cancel_event=None, steer_inbox=None) {
        """
        ...
        full_system_prompt = this.get_full_system_prompt(skill_filter=skill_filter)
        ...
        with this.messages_lock:
            messages_copy = this.messages.copy()
            original_length = len(this.messages)
        ...
        executor = AgentStreamExecutor( agent=this, model=this.model, system_prompt=full_system_prompt, tools=this.tools, max_turns=this.max_steps, on_event=on_event, messages=messages_copy,   max_context_turns=max_context_turns, cancel_event=cancel_event, steer_inbox=steer_inbox, )
        ...
        with this.messages_lock:
            this.messages = list(executor.messages)
            ...
            this._last_run_new_messages = list(executor.messages[trim_adjusted_start:])

        # Store executor reference for agent_bridge to access files_to_send
        this.stream_executor = executor

        # Execute all post-process tools
        this._execute_post_process_tools()
    ...
    fn compact_context(keep_recent_turns = 2) {
        ...
        """
        from agent.protocol.message_utils import ( identify_complete_turns, build_compaction_summary_text, find_first_user_text_block, _extract_text_from_content, )

        with this.messages_lock:
            before = len(this.messages)
            turns = identify_complete_turns(this.messages)

            if len(turns) <= keep_recent_turns:
                return { "ok": false, "reason": "nothing_to_compact", "compacted_turns": 0, "before": before, "after": before, }
        ...
        if this.memory_manager:
            flush_mgr = getattr(this.memory_manager, "flush_manager", null)
        if flush_mgr:
            try {
        ...
        if flush_mgr and llm_summary:
            try {
                user_id = getattr(this, "_current_user_id", null)
        ...
        with this.messages_lock:
            ...
            if target_block is not null:
                target_block["text"] = build_compaction_summary_text( summary, turn_count, target_block.get("text", "") )
            else:
                # Fallback: no injectable target, prepend a standalone note.
                new_messages.insert(0, { "role": "user", "content": [{ "type": "text", "text": build_compaction_summary_text(summary, turn_count, ""), }], })

            this.messages = new_messages
            after = len(this.messages)

        logger.info( f"[Agent] Manual compact: {turn_count} turns summarized, " f"{before} -> {after} messages" )
        return { "ok": true, "reason": "compacted", "compacted_turns": turn_count, "before": before, "after": after, }
    }
}