...
class MemoryFlushManager:
    ...
    def __init__(self, workspace_dir, llm_model = None):
    ...
    def get_today_memory_file(self, user_id = None, ensure_exists = False):
    ...
    def get_main_memory_file(self, user_id = None):
    ...
    def get_status(self):
        return { 'last_flush_time': self.last_flush_timestamp.isoformat() if self.last_flush_timestamp else None, 'today_file': str(self.get_today_memory_file()), 'main_file': str(self.get_main_memory_file()) }

    # ---- Flush execution (called by agent_stream or scheduler) ----

    def flush_from_messages(self, messages, user_id = None, reason = "trim", max_messages = 0, context_summary_callback = None):
        """
        ...
        try:
            ...
            thread = threading.Thread( target=self._flush_worker, args=(snapshot, user_id, reason, max_messages, context_summary_callback), daemon=True, )
            thread.start()
    ...
    def _flush_worker(self, messages, user_id, reason, max_messages, context_summary_callback = None):
    ...
    def write_daily_summary(self, summary, user_id = None, reason = "trim"):
    ...
    def create_daily_summary(self, messages, user_id = None):
        ...
        """
        import hashlib
        content = "".join( self._extract_text_from_content(m.get("content", "")) for m in messages )
        ...
        return self.flush_from_messages( messages=messages, user_id=user_id, reason="daily_summary", max_messages=0, )
    ...
    def _read_main_memory(self, user_id = None):
    ...
    def _read_recent_dailies(self, user_id = None, lookback_days = 1):
        """
    ...
    def _write_dream_diary(self, content, user_id = None):
    ...
    def _format_conversation_for_summary(self, messages, max_messages = 0):
    ...
    def _call_llm_for_summary(self, conversation_text):
    ...
    @staticmethod
    def _extract_first_meaningful_line(text, max_len = 120):
    ...
    @staticmethod
    def _extract_summary_fallback(messages, max_messages = 0):
        """
...
def create_memory_files_if_needed(workspace_dir, user_id = None):
    """
...
def ensure_daily_memory_file(workspace_dir, user_id = None):
    """
