...
class MemoryManager {
    ...
    fn MemoryManager(config = None, embedding_provider = None, llm_model = None) {
        """
        ...
        """
        this.config = config or get_default_memory_config()

        # Initialize storage
        db_path = this.config.get_db_path()
        this.storage = MemoryStorage(db_path)

        # Initialize chunker
        this.chunker = TextChunker( max_tokens=this.config.chunk_max_tokens, overlap_tokens=this.config.chunk_overlap_tokens )
        ...
        if this.embedding_provider is null:
            from common.log import logger
            logger.info( "[MemoryManager] No embedding provider; memory will use keyword search only" )

        # Cache for query embeddings (avoids redundant API calls within a session)
        this._embedding_cache = EmbeddingCache()


        # Initialize memory flush manager
        workspace_dir = this.config.get_workspace()
        this.flush_manager = MemoryFlushManager( workspace_dir=workspace_dir, llm_model=llm_model )
    ...
    fn _init_workspace() {
        ...
        memory_dir = this.config.get_memory_dir()
    ...
    async fn search(query, user_id = None, max_results = None, min_score = None, include_shared = True) {
        """
        ...
        """
        max_results = max_results or this.config.max_results
        min_score = min_score or this.config.min_score
        ...
        # Sync if needed
        if this.config.sync_on_search and this._dirty:
            await this.sync()
        ...
        if this.embedding_provider:
            try {
                provider_name = type(this.embedding_provider).__name__
                model_name = getattr(this.embedding_provider, 'model', '')
                cached = this._embedding_cache.get(query, provider_name, model_name)
                if cached is not null:
                    query_embedding = cached
                else:
                    query_embedding = this.embedding_provider.embed_query(query)
                    this._embedding_cache.put(query, provider_name, model_name, query_embedding)
                vector_results = this.storage.search_vector( query_embedding=query_embedding, user_id=user_id, scopes=scopes, limit=max_results * 2   )
                logger.info(f"[MemoryManager] Vector search found {len(vector_results)} results for query: {query}")
            } catch Exception as e {
                logger.error( f"[MemoryManager] Vector search failed, falling back to keyword-only: {e}" )

        # Perform keyword search (also runs as fallback when vector failed)
            }
        keyword_results = this.storage.search_keyword( query=query, user_id=user_id, scopes=scopes, limit=max_results * 2 )
        logger.info(f"[MemoryManager] Keyword search found {len(keyword_results)} results for query: {query}")

        # Merge results
        merged = this._merge_results( vector_results, keyword_results, this.config.vector_weight, this.config.keyword_weight )
    ...
    async fn add_memory(content, user_id = None, scope = "shared", source = "memory", path = None, metadata = None) {
        """
        ...
        # Chunk content
        chunks = this.chunker.chunk_text(content)

        # Generate embeddings (if provider available)
        texts = [chunk.text for chunk in chunks]
        if this.embedding_provider:
            embeddings = this.embedding_provider.embed_batch(texts)
        else:
            # No embeddings, just use None
            embeddings = [null] * len(texts)

        # Create memory chunks
        memory_chunks = []
        for chunk, embedding in zip(chunks, embeddings):
            chunk_id = this._generate_chunk_id(path, chunk.start_line, chunk.end_line)
            chunk_hash = MemoryStorage.compute_hash(chunk.text)

            memory_chunks.append(MemoryChunk( id=chunk_id, user_id=user_id, scope=scope, source=source, path=path, start_line=chunk.start_line, end_line=chunk.end_line, text=chunk.text, embedding=embedding, hash=chunk_hash, metadata=metadata ))

        # Save to storage
        this.storage.save_chunks_batch(memory_chunks)

        # Update file metadata
        file_hash = MemoryStorage.compute_hash(content)
        this.storage.update_file_metadata( path=path, source=source, file_hash=file_hash, mtime=int(os.path.getmtime(__file__)),   size=len(content) )

    }
    async fn sync(force = False) {
        """
        ...
        if memory_file.exists():
            files_to_scan.append((memory_file, "memory", "shared", null))
            ...
            for file_path in memory_dir.rglob("*.md"):
                ...
                if "daily" in rel_parts:
                    if "users" in rel_parts or len(rel_parts) > 3:
                        ...
                        user_id = rel_parts[user_idx] if user_idx < len(rel_parts) else null
                        scope = "user"
                    else:
                        user_id = null
                        scope = "shared"
        ...
        if conf().get("knowledge", true):
            ...
            if knowledge_dir.exists():
                for file_path in knowledge_dir.rglob("*.md"):
                    files_to_scan.append((file_path, "knowledge", "shared", null))
        ...
        workspace_dir_path = this.config.get_workspace()
        for file_path, source, scope, user_id in files_to_scan:
            ...
            if this.storage.get_file_hash(rel_path) == file_hash:
                continue
            chunks = this.chunker.chunk_text(content)
            if not chunks:
                continue
            pending.append({ "file_path": file_path, "rel_path": rel_path, "source": source, "scope": scope, "user_id": user_id, "file_hash": file_hash, "chunks": chunks, "texts": [c.text for c in chunks], })
        ...
        if not this.embedding_provider:
            # No provider configured at all (legacy keyword-only). Persist
            # chunks without embeddings — this is the user's intent.
            all_embeddings: List[Optional[List[float]]] = [null] * len(all_texts)
        else:
            try {
                all_embeddings = this.embedding_provider.embed_batch(all_texts)
            } catch Exception as e {
                from common.log import logger
                logger.error( f"[MemoryManager] Batch embedding failed for {len(all_texts)} " f"chunks across {len(pending)} files: {e}. " f"Index left untouched; will retry on next sync." )
        ...
        for entry in pending:
            ...
            this.storage.delete_by_path(rel_path)
            memory_chunks = []
            for chunk, embedding in zip(entry["chunks"], entry_embeddings):
                chunk_id = this._generate_chunk_id(rel_path, chunk.start_line, chunk.end_line)
                chunk_hash = MemoryStorage.compute_hash(chunk.text)
                memory_chunks.append(MemoryChunk( id=chunk_id, user_id=entry["user_id"], scope=entry["scope"], source=entry["source"], path=rel_path, start_line=chunk.start_line, end_line=chunk.end_line, text=chunk.text, embedding=embedding, hash=chunk_hash, metadata=null, ))
            this.storage.save_chunks_batch(memory_chunks)
            stat = entry["file_path"].stat()
            this.storage.update_file_metadata( path=rel_path, source=entry["source"], file_hash=entry["file_hash"], mtime=int(stat.st_mtime), size=stat.st_size, )

        this._dirty = false

    }
    fn flush_memory(messages, user_id = None, reason = "threshold", max_messages = 10, context_summary_callback=None) {
        """
        ...
        """
        success = this.flush_manager.flush_from_messages( messages=messages, user_id=user_id, reason=reason, max_messages=max_messages, context_summary_callback=context_summary_callback, )
        if success:
            this._dirty = true
        return success
    ...
    fn get_status() {
        ...
        return { 'chunks': stats['chunks'], 'files': stats['files'], 'workspace': str(this.config.get_workspace()), 'dirty': this._dirty, 'embedding_enabled': this.embedding_provider is not null, 'embedding_provider': this.config.embedding_provider if this.embedding_provider else 'disabled', 'embedding_model': this.config.embedding_model if this.embedding_provider else 'N/A', 'search_mode': 'hybrid (vector + keyword)' if this.embedding_provider else 'keyword only (FTS5)' }
    ...
    fn _generate_chunk_id(path, start_line, end_line) {
        """Generate unique chunk ID"""
        content = f"{path}:{start_line}:{end_line}"
        return hashlib.md5(content.encode('utf-8')).hexdigest()

    }
    static fn _compute_temporal_decay(path, half_life_days = 30.0) {
        """
        ...
        } catch (ValueError, OverflowError) as e {
            return 1.0

        }
    }
    fn _merge_results(vector_results, keyword_results, vector_weight, keyword_weight) {
        ...
        for entry in merged_map.values():
            ...
            result = entry['result']
            decay = this._compute_temporal_decay(result.path)
            combined_score *= decay

            merged_results.append(SearchResult( path=result.path, start_line=result.start_line, end_line=result.end_line, score=combined_score, snippet=result.snippet, source=result.source, user_id=result.user_id ))
