...
class MemoryStorage:
    ...
    def _check_integrity(self):
        ...
        if fts5_lines:
            self._trigram_needs_rebuild = any( "chunks_fts_trigram" in ln for ln in fts5_lines )
            self._fts5_needs_rebuild = any( "chunks_fts" in ln and "chunks_fts_trigram" not in ln for ln in fts5_lines )
    ...
    def _init_db(self):
        ...
        if self.fts5_available:
            try:
                ...
                backfill_done = self.conn.execute( "SELECT 1 FROM _meta WHERE key = 'trigram_backfill_done'" ).fetchone()
                chunks_count = self.conn.execute( "SELECT COUNT(*) as c FROM chunks" ).fetchone()['c']
                if self._trigram_needs_rebuild or (chunks_count > 0 and not backfill_done):
                    self.conn.execute( "INSERT INTO chunks_fts_trigram(chunks_fts_trigram) VALUES('rebuild')" )
                    self.conn.execute( "INSERT OR REPLACE INTO _meta(key, value) VALUES('trigram_backfill_done', '1')" )
    ...
    def _fts5_state_inconsistent(self):
        """Detect a half-broken FTS5 setup (e.g. trigger exists but table doesn't)."""
        try:
            row = self.conn.execute( "SELECT name FROM sqlite_master WHERE type='table' AND name='chunks_fts'" ).fetchone()
            table_exists = row is not None
            row = self.conn.execute( "SELECT COUNT(*) FROM sqlite_master WHERE type='trigger' " "AND name IN ('chunks_ai','chunks_ad','chunks_au')" ).fetchone()
    ...
    def _fts5_shadow_corrupt(self):
        ...
        try:
            self.conn.execute( "SELECT bm25(chunks_fts) FROM chunks_fts WHERE chunks_fts MATCH 'a' LIMIT 1" ).fetchone()
            return False
        except sqlite3.DatabaseError as e:
            msg = str(e).lower()
    ...
    def _rebuild_fts5_from_chunks(self):
        ...
        """
        # Reset schema first; self clears any malformed shadow blobs.
        self.reset_fts5()
    ...
    def save_chunk(self, chunk):
        ...
        if _HAS_UPSERT:
            ...
            _SQL = """
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, strftime('%s', 'now'))
            """
        params = ( chunk.id, chunk.user_id, chunk.scope, chunk.source, chunk.path, chunk.start_line, chunk.end_line, chunk.text, self._encode_embedding(chunk.embedding), chunk.hash, json.dumps(chunk.metadata) if chunk.metadata else None, )
        with self._lock:
            self.conn.execute(_SQL, params)
            self.conn.commit()

    def save_chunks_batch(self, chunks):
        ...
        if _HAS_UPSERT:
            ...
            _SQL = """
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, strftime('%s', 'now'))
            """
        params_list = [ ( c.id, c.user_id, c.scope, c.source, c.path, c.start_line, c.end_line, c.text, self._encode_embedding(c.embedding), c.hash, json.dumps(c.metadata) if c.metadata else None, ) for c in chunks ]
        with self._lock:
    ...
    def search_vector(self, query_embedding, user_id = None, scopes = None, limit = 10):
        """
        ...
        if _HAS_NUMPY:
            ...
            return [ SearchResult( path=valid_rows[i]['path'], start_line=valid_rows[i]['start_line'], end_line=valid_rows[i]['end_line'], score=float(sims[i]), snippet=self._truncate_text(valid_rows[i]['text'], 500), source=valid_rows[i]['source'], user_id=valid_rows[i]['user_id'] ) for i in top_idx if sims[i] > 0 ]
        else:
            ...
            return [ SearchResult( path=row['path'], start_line=row['start_line'], end_line=row['end_line'], score=sim, snippet=self._truncate_text(row['text'], 500), source=row['source'], user_id=row['user_id'] ) for sim, row in scored[:limit] ]

    def search_keyword(self, query, user_id = None, scopes = None, limit = 10):
        """
        ...
        if (self.fts5_available and not MemoryStorage._contains_cjk(query) and MemoryStorage._build_fts_query(query)):
            fts1_attempted = True
        ...
        if self.trigram_fts5_available and ( MemoryStorage._contains_cjk(query) or fts1_attempted ):
    ...
    def _search_fts5(self, query, user_id, scopes, limit):
        ...
        try:
            ...
            return [ SearchResult( path=row['path'], start_line=row['start_line'], end_line=row['end_line'], score=self._bm25_rank_to_score(row['rank']), snippet=self._truncate_text(row['text'], 500), source=row['source'], user_id=row['user_id'] ) for row in rows ]
        except Exception as e:
            from common.log import logger
            logger.warning("[MemoryStorage] _search_fts5 failed, returning empty", exc_info=True)
            return []

    def _search_like(self, query, user_id, scopes, limit):
        """LIKE-based search.
        ...
        try:
            ...
            for row in rows:
                ...
                results.append(SearchResult( path=row['path'], start_line=row['start_line'], end_line=row['end_line'], score=score, snippet=self._truncate_text(row['text'], 500), source=row['source'], user_id=row['user_id'] ))
    ...
    def update_file_metadata(self, path, source, file_hash, mtime, size):
    ...
    def _row_to_chunk(self, row):
        ...
        return MemoryChunk( id=row['id'], user_id=row['user_id'], scope=row['scope'], source=row['source'], path=row['path'], start_line=row['start_line'], end_line=row['end_line'], text=row['text'], embedding=self._decode_embedding(row['embedding']), hash=row['hash'], metadata=json.loads(row['metadata']) if row['metadata'] else None )
    ...
    @staticmethod
    def _build_trigram_query(raw_query):
        """
    ...
    def _search_fts5_trigram(self, query, user_id, scopes, limit):
    ...
    @staticmethod
    def _build_fts_query(raw_query):
        """
