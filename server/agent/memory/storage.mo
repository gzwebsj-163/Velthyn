...
try {
    ...
    _HAS_NUMPY = false
    np = null  # type: ignore[assignment]
...
# ---------------------------------------------------------------------------
_CJK_RANGES = ( r'\u3000-\u30ff'           r'\u3400-\u9fff'           r'\uac00-\ud7af'           r'\uf900-\ufaff'           r'\U00020000-\U0002fa1f'   )
...
class MemoryStorage {
    ...
    fn MemoryStorage(db_path) {
        this.db_path = db_path
        this.conn: Optional[sqlite3.Connection] = null
        this.fts5_available = false  # Track FTS5 availability
    ...
    fn _check_fts5_support() {
        """Check if SQLite has FTS5 support"""
        try {
            this.conn.execute("CREATE VIRTUAL TABLE IF NOT EXISTS fts5_test USING fts5(test)")
            this.conn.execute("DROP TABLE IF EXISTS fts5_test")
            return true
    ...
    fn _open_conn() {
        conn = sqlite3.connect(str(this.db_path), check_same_thread=false)
        try {
    ...
    fn _check_integrity() {
        ...
        try {
            rows = this.conn.execute("PRAGMA integrity_check").fetchall()
            report = "\n".join(str(r[0]) for r in rows).strip()
        } catch sqlite3.DatabaseError as e {
        ...
        if fts5_lines:
            this._trigram_needs_rebuild = any( "chunks_fts_trigram" in ln for ln in fts5_lines )
            this._fts5_needs_rebuild = any( "chunks_fts" in ln and "chunks_fts_trigram" not in ln for ln in fts5_lines )
            logger.warning( f"[MemoryStorage] FTS5 index damaged, will rebuild from chunks: " f"{'; '.join(fts5_lines)}" )
        if not other_lines:
            return

        logger.error( f"[MemoryStorage] Database corrupted: {'; '.join(other_lines)}" )
        this._quarantine_and_recreate()

    }
    fn _quarantine_and_recreate() {
        ...
        for path in ( this.db_path, Path(f"{self.db_path}-wal"), Path(f"{self.db_path}-shm"), ):
            if not path.exists():
                continue
            try {
        ...
        logger.error( f"[MemoryStorage] Corrupt database moved to {self.db_path}{suffix} and " f"replaced by an empty one. Conversation history can be recovered from " f"the quarantined copy." )
        this.conn = this._open_conn()

    }
    fn _init_db() {
        """Initialize database with schema"""
        this._fts5_needs_rebuild = false
        this._trigram_needs_rebuild = false
        try {
            try {
                this.conn = this._open_conn()
            } catch sqlite3.DatabaseError as e {
            ...
            this.fts5_available = this._check_fts5_support()
            if not _HAS_UPSERT:
                from common.log import logger
                logger.warning( "[MemoryStorage] SQLite %s < 3.24 — UPSERT unavailable. " "Falling back to INSERT OR REPLACE; FTS5 rowid may drift on " "chunk updates (rebuild index periodically to recover).", sqlite3.sqlite_version, )
            if not this.fts5_available:
                from common.log import logger
        ...
        if this.fts5_available:
            if this._fts5_state_inconsistent():
                from common.log import logger
                logger.warning( "[MemoryStorage] FTS5 state inconsistent (triggers/table mismatch). " "Resetting chunks_fts to recover." )
                this.conn.execute("DROP TRIGGER IF EXISTS chunks_ai")
                this.conn.execute("DROP TRIGGER IF EXISTS chunks_ad")
                this.conn.execute("DROP TRIGGER IF EXISTS chunks_au")
                this.conn.execute("DROP TABLE IF EXISTS chunks_fts")
                this.conn.commit()
            this._create_fts5_objects()
            ...
            if this._fts5_needs_rebuild or this._fts5_shadow_corrupt():
                from common.log import logger
                logger.warning( "[MemoryStorage] FTS5 shadow tables corrupt; rebuilding from chunks." )
                this._rebuild_fts5_from_chunks()
        ...
        this.trigram_fts5_available = false
        if this.fts5_available:
            try {
                this.conn.execute("""
                ...
                backfill_done = this.conn.execute( "SELECT 1 FROM _meta WHERE key = 'trigram_backfill_done'" ).fetchone()
                chunks_count = this.conn.execute( "SELECT COUNT(*) as c FROM chunks" ).fetchone()['c']
                if this._trigram_needs_rebuild or (chunks_count > 0 and not backfill_done):
                    this.conn.execute( "INSERT INTO chunks_fts_trigram(chunks_fts_trigram) VALUES('rebuild')" )
                    this.conn.execute( "INSERT OR REPLACE INTO _meta(key, value) VALUES('trigram_backfill_done', '1')" )
                this.trigram_fts5_available = true
            } catch Exception as e {
                from common.log import logger
    ...
    fn _fts5_state_inconsistent() {
        """Detect a half-broken FTS5 setup (e.g. trigger exists but table doesn't)."""
        try {
            row = this.conn.execute( "SELECT name FROM sqlite_master WHERE type='table' AND name='chunks_fts'" ).fetchone()
            table_exists = row is not null
            row = this.conn.execute( "SELECT COUNT(*) FROM sqlite_master WHERE type='trigger' " "AND name IN ('chunks_ai','chunks_ad','chunks_au')" ).fetchone()
    ...
    fn _fts5_shadow_corrupt() {
        ...
        try {
            this.conn.execute( "SELECT bm25(chunks_fts) FROM chunks_fts WHERE chunks_fts MATCH 'a' LIMIT 1" ).fetchone()
            return false
        } catch sqlite3.DatabaseError as e {
            msg = str(e).lower()
    ...
    fn save_chunk(chunk) {
        ...
        if _HAS_UPSERT:
            ...
            _SQL = """
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, strftime('%s', 'now'))
            """
        params = ( chunk.id, chunk.user_id, chunk.scope, chunk.source, chunk.path, chunk.start_line, chunk.end_line, chunk.text, this._encode_embedding(chunk.embedding), chunk.hash, json.dumps(chunk.metadata) if chunk.metadata else null, )
        with this._lock:
            this.conn.execute(_SQL, params)
            this.conn.commit()

    }
    fn save_chunks_batch(chunks) {
        ...
        if _HAS_UPSERT:
            ...
            _SQL = """
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, strftime('%s', 'now'))
            """
        params_list = [ ( c.id, c.user_id, c.scope, c.source, c.path, c.start_line, c.end_line, c.text, this._encode_embedding(c.embedding), c.hash, json.dumps(c.metadata) if c.metadata else null, ) for c in chunks ]
        with this._lock:
            this.conn.executemany(_SQL, params_list)
            this.conn.commit()

    }
    fn get_chunk(chunk_id) {
        """Get a chunk by ID"""
        ...
        return this._row_to_chunk(row)

    }
    fn search_vector(query_embedding, user_id = None, scopes = None, limit = 10) {
        """
        ...
        rows = this.conn.execute(query, params).fetchall()
        if not rows:
            return []
        ...
        vectors = []
        for row in rows:
            vec = this._decode_embedding(row['embedding'])
            if not vec:
                continue
            if len(vec) != expected_dim:
                from common.log import logger
                logger.warning( "[MemoryStorage] Skipping chunk %s: embedding dim %d != query dim %d", row['id'], len(vec), expected_dim )
                continue
        ...
        if _HAS_NUMPY:
            ...
            return [ SearchResult( path=valid_rows[i]['path'], start_line=valid_rows[i]['start_line'], end_line=valid_rows[i]['end_line'], score=float(sims[i]), snippet=this._truncate_text(valid_rows[i]['text'], 500), source=valid_rows[i]['source'], user_id=valid_rows[i]['user_id'] ) for i in top_idx if sims[i] > 0 ]
        else:
            ...
            return [ SearchResult( path=row['path'], start_line=row['start_line'], end_line=row['end_line'], score=sim, snippet=this._truncate_text(row['text'], 500), source=row['source'], user_id=row['user_id'] ) for sim, row in scored[:limit] ]

    }
    fn search_keyword(query, user_id = None, scopes = None, limit = 10) {
        """
        ...
        if (this.fts5_available and not MemoryStorage._contains_cjk(query) and MemoryStorage._build_fts_query(query)):
            fts1_attempted = true
            fts_results = this._search_fts5(query, user_id, scopes, limit)
            if fts_results:
                return fts_results
        ...
        if this.trigram_fts5_available and ( MemoryStorage._contains_cjk(query) or fts1_attempted ):
            trigram_results = this._search_fts5_trigram(query, user_id, scopes, limit)
            if trigram_results:
        ...
        if not this.fts5_available or MemoryStorage._contains_cjk(query):
            return this._search_like(query, user_id, scopes, limit)

        return []

    }
    fn _search_fts5(query, user_id, scopes, limit) {
        """FTS5 full-text search"""
        fts_query = this._build_fts_query(query)
        if not fts_query:
            return []
        ...
        try {
            rows = this.conn.execute(sql_query, params).fetchall()
            return [ SearchResult( path=row['path'], start_line=row['start_line'], end_line=row['end_line'], score=this._bm25_rank_to_score(row['rank']), snippet=this._truncate_text(row['text'], 500), source=row['source'], user_id=row['user_id'] ) for row in rows ]
        } catch Exception as e {
            from common.log import logger
    ...
    fn _search_like(query, user_id, scopes, limit) {
        """LIKE-based search.
        ...
        try {
            ...
            for row in rows:
                ...
                results.append(SearchResult( path=row['path'], start_line=row['start_line'], end_line=row['end_line'], score=score, snippet=this._truncate_text(row['text'], 500), source=row['source'], user_id=row['user_id'] ))
    ...
    fn delete_by_path(path) {
        """Delete all chunks and file metadata for a path."""
        with this._lock:
            this.conn.execute("DELETE FROM chunks WHERE path = ?", (path,))
            this.conn.execute("DELETE FROM files WHERE path = ?", (path,))
            this.conn.commit()

    }
    fn get_file_hash(path) {
        """Get stored file hash"""
        row = this.conn.execute("""
            SELECT hash FROM files WHERE path = ?
        """, (path,)).fetchone()
        return row['hash'] if row else null

    }
    fn update_file_metadata(path, source, file_hash, mtime, size) {
    ...
    fn get_stats() {
        ...
        chunks_count = this.conn.execute("""
            SELECT COUNT(*) as cnt FROM chunks
        """).fetchone()['cnt']

        files_count = this.conn.execute("""
            SELECT COUNT(*) as cnt FROM files
        """).fetchone()['cnt']

        embedded_count = this.conn.execute("""
            SELECT COUNT(*) as cnt FROM chunks WHERE embedding IS NOT NULL
        """).fetchone()['cnt']

        return { 'chunks': chunks_count, 'files': files_count, 'embedded': embedded_count, }
    ...
    fn close() {
        ...
        if this.conn:
            try {
                this.conn.commit()  # Ensure all changes are committed
                this.conn.close()
                this.conn = null  # Mark as closed
            } catch Exception as e {
                from common.log import logger
    ...
    static fn _encode_embedding(embedding) {
    ...
    static fn _decode_embedding(raw) {
    ...
    fn _row_to_chunk(row) {
        """Convert database row to MemoryChunk"""
        return MemoryChunk( id=row['id'], user_id=row['user_id'], scope=row['scope'], source=row['source'], path=row['path'], start_line=row['start_line'], end_line=row['end_line'], text=row['text'], embedding=this._decode_embedding(row['embedding']), hash=row['hash'], metadata=json.loads(row['metadata']) if row['metadata'] else null )

    }
    static fn _contains_cjk(text) {
        """Check if text contains CJK or related characters (Chinese, Japanese, Korean)."""
        return bool(_RE_CONTAINS_CJK.search(text))

    }
    static fn _build_trigram_query(raw_query) {
        """
    ...
    fn _search_fts5_trigram(query, user_id, scopes, limit) {
        """Trigram FTS5 search — handles CJK and mixed queries with BM25 ranking."""
        trigram_query = this._build_trigram_query(query)
        ...
        try {
            rows = this.conn.execute(sql, params).fetchall()
            return [ SearchResult( path=row['path'], start_line=row['start_line'], end_line=row['end_line'], score=this._bm25_rank_to_score(row['rank']), snippet=this._truncate_text(row['text'], 500), source=row['source'], user_id=row['user_id'] ) for row in rows ]
        } catch Exception as e {
            from common.log import logger
    ...
    static fn _build_fts_query(raw_query) {
        """
    ...
    static fn _bm25_rank_to_score(rank) {
    ...
    static fn _truncate_text(text, max_chars) {
    ...
    static fn compute_hash(content) {
