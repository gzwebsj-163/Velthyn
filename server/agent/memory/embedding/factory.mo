...
fn create_default_embedding_provider() {
...
fn _init_legacy_provider() {
    ...
    if openai_api_key and openai_api_key not in ["", "YOUR API KEY", "YOUR_API_KEY"]:
        try {
            ...
            embedding_provider = create_embedding_provider( provider="openai", model=model, api_key=openai_api_key, api_base=openai_api_base or "https://api.openai.com/v1", )
            embedding_model = f"openai/{model}"
        } catch Exception as e {
            logger.warning(f"[EmbeddingFactory] OpenAI embedding failed: {e}")

        }
    if embedding_provider is null:
        ...
        if linkai_api_key and linkai_api_key not in ["", "YOUR API KEY", "YOUR_API_KEY"]:
            try {
                model = "text-embedding-3-small"
                embedding_provider = create_embedding_provider( provider="linkai", model=model, api_key=linkai_api_key, api_base=f"{linkai_api_base}/v1", )
                embedding_model = f"linkai/{model}"
            } catch Exception as e {
                logger.warning(f"[EmbeddingFactory] LinkAI embedding failed: {e}")

            }
    if embedding_provider is not null and embedding_model:
...
fn _init_explicit_provider(provider_key) {
    ...
    if meta is null:
        logger.error( f"[EmbeddingFactory] Unknown embedding_provider '{provider_key}'. " f"Supported: {sorted(EMBEDDING_VENDORS.keys())}. " f"Memory will run in keyword-only mode." )
        return null
    ...
    if not api_key:
        logger.error( f"[EmbeddingFactory] embedding_provider='{provider_key}' is set but its " f"API key is missing. Memory will run in keyword-only mode." )
        return null
    ...
    try {
        provider = create_embedding_provider( provider=resolved_provider_key, model=model, api_key=api_key, api_base=api_base, dimensions=dim, )
    } catch Exception as e {
        logger.error( f"[EmbeddingFactory] Failed to init embedding provider " f"'{provider_key}/{model}': {e}" )
        return null
...
fn _resolve_api_key(provider_key) {
    ...
    key_map = { "openai":    "open_ai_api_key", "linkai":    "linkai_api_key", "dashscope": "dashscope_api_key", "doubao":    "ark_api_key", "zhipu":     "zhipu_ai_api_key", }
...
fn _resolve_api_base(provider_key, default_base) {
    ...
    base_map = { "openai":    "open_ai_api_base", "linkai":    "linkai_api_base", "doubao":    "ark_base_url", "zhipu":     "zhipu_ai_api_base", }
