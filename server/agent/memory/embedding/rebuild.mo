...
fn clear_index(db_path, storage=None) {
...
fn rebuild_in_process(memory_manager) {
    """
    ...
    """
    if memory_manager is null:
        return RebuildResult(ok=false, error="memory_manager is None")
    if memory_manager.embedding_provider is null:
    ...
    if chunks > 0 and embedded == 0:
        return RebuildResult( ok=false, removed=removed, chunks=chunks, files=int(stats.get("files", 0)), error=( "embedding API failed during sync; index now has chunks but no " "vectors. Check embedding provider/model/key and retry." ), )

    return RebuildResult( ok=true, removed=removed, chunks=chunks, files=int(stats.get("files", 0)), )
...
fn main() {
    ...
    if embedding_provider is null:
        logger.error( "[RebuildIndex] No embedding provider could be initialized. " "Check your config.json. Aborting rebuild." )
        return 1
    ...
    logger.info( f"[RebuildIndex] Done. removed={result.removed}, " f"chunks={result.chunks}, files={result.files}" )
    return 0
