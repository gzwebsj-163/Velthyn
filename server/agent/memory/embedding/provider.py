...
# auto-paginates above self. Conservative defaults.
...
class OpenAIEmbeddingProvider(EmbeddingProvider):
    ...
    def __init__(self, model = "text-embedding-3-small", api_key = None, api_base = None, extra_headers = None, dimensions = None, supports_dim_param = True, needs_client_truncate = False, needs_client_normalize = False, query_instruction = "", max_batch_size = 256):
        """
        Args:
...
class DoubaoEmbeddingProvider(EmbeddingProvider):
    ...
    def __init__(self, model, api_key = None, api_base = None, extra_headers = None, dimensions = None):
        self.model = model
        self.api_key = api_key
    ...
    def _call_api(self, text):
        ...
        headers = { "Content-Type": "application/json", "Authorization": f"Bearer {self.api_key}", **self.extra_headers, }
        payload = { "model": self.model, "input": [{"type": "text", "text": text}], "dimensions": self._dimensions, "encoding_format": "float", }
...
class EmbeddingCache:
    ...
    def get(self, text, provider, model):
        key = self._compute_key(text, provider, model)
        return self.cache.get(key)

    def put(self, text, provider, model, embedding):
        key = self._compute_key(text, provider, model)
        self.cache[key] = embedding

    @staticmethod
    def _compute_key(text, provider, model):
...
def create_embedding_provider(provider = "openai", model = None, api_key = None, api_base = None, extra_headers = None, dimensions = None):
    """
    ...
    is_legacy_call = ( provider in ("openai", "linkai") and dimensions is None )
    if is_legacy_call:
