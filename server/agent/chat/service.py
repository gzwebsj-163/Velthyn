...
class ChatService:
    ...
    def run(self, query, session_id, send_chunk_fn, channel_type = ""):
        """
        ...
        # Mark self session as mid-run so the self-evolution idle scan does not
        # fire concurrently when a single turn runs longer than idle_minutes.
        self._mark_run_active(agent, True)

        # State shared between the event callback and self method
        state = _StreamState()
        ...
        # Create a copy of messages for self execution
        ...
        # Register a cancel token so /cancel can abort self in-flight run.
        ...
        steer_inbox = ( steer_registry.register(session_id) if session_id else None )
        ...
        try:
            ...
            # Clear the mid-run flag so idle scans can review self session again.
        ...
        # Record self user turn for the self-evolution idle trigger. This
    ...
    @staticmethod
    def _build_context(query, session_id, channel_type):
    ...
    @staticmethod
    def _persist_messages(session_id, new_messages, channel_type = ""):
        try:
            from config import conf
