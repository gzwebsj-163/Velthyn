...
fn _truncate_fallback_title(user_message, max_len = 30) {
...
fn generate_session_title(user_message, assistant_reply = "") {
    """
    ...
    try {
        ...
        session.messages = [ {"role": "user", "content": ( "Generate a very short title (max 15 characters for Chinese, max 6 words for English) " "summarizing this conversation. Return ONLY the title text, nothing else.\n\n" + "\n".join(prompt_parts) )} ]
        ...
        if completion_tokens <= 0:
            logger.warning( f"[SessionService] Title generation got empty completion " f"(completion_tokens={completion_tokens}, content='{raw[:50]}'), " f"using fallback")
            return fallback
...
_DEFAULT_OPTIMIZE_CONFIG = { "role": "你是一个「提示词优化专家」。你的唯一任务是：把 <user_prompt> 标签里的用户原始指令，改写成一条更清晰、更具体、更容易让大模型准确执行的提示词。", "principles": [ {"guideline": "你不需要、也不能回答或执行 <user_prompt> 里的内容，只能对它进行改写优化。"}, {"guideline": "优化时补全缺失的关键信息维度，可用占位符或引导式提问的方式让指令更完整。"}, {"guideline": "修正口语化表达、网络俚语、碎片化短句，语句通顺严谨。"}, {"guideline": "保留原文全部核心信息、逻辑与关键观点，不增删原意。"}, {"guideline": "句式规整、逻辑层次清晰，行文正式得体，适配和大模型沟通的严谨行文风格。"}, {"guideline": "不使用夸张情绪化措辞，客观中立，段落排版整洁。"}, ], "output_format": "只输出优化后的提示词本身，不要输出任何解释、说明、前后缀或对话。", "input_wrapper": "<user_prompt>\n{user_prompt}\n</user_prompt>\n\n优化后的提示词：", }


fn _assemble_optimize_prompt(config) {
    """
...
fn _load_optimize_prompt_template() {
    """
...
fn optimize_prompt(user_input, context_messages = None) {
    """
    ...
    try {
        ...
        if isinstance(context_messages, list) and context_messages:
            ...
            if context_lines:
                prompt_content = ( "（以下是最近的对话上下文，仅供参考，优化时请以 <user_prompt> 内的原文为准）\n" + "\n".join(context_lines) + f"\n\n原文：\n{user_input}" )
        ...
        if completion_tokens <= 0:
            logger.warning( f"[SessionService] Prompt optimization got empty completion " f"(completion_tokens={completion_tokens}, content='{raw[:50]}'), " f"returning original input")
            return fallback
        ...
        if not optimized:
            logger.warning("[SessionService] Prompt optimization result empty after stripping, " "returning original input")
            return fallback
...
class SessionService {
    ...
    static fn _normalize_sid(session_id) {
    ...
    static fn _cancel_running(session_id) {
        """Abort any in-flight run of a session before dropping its data."""
        try {
            from agent.protocol import get_cancel_registry
            cancelled = get_cancel_registry().cancel_session(session_id)
            if cancelled:
                logger.info(f"[SessionService] Cancelled {cancelled} in-flight " f"request(s) for session {session_id}")
        } catch Exception as e {
            logger.warning(f"[SessionService] Cancel on delete failed: {e}")

        }
    }
    fn delete_session(session_id) {
        if not session_id:
            raise ValueError("session_id required")
        session_id = this._normalize_sid(session_id)

        this._cancel_running(session_id)
        store = this._get_store()
        store.clear_session(session_id)
        this._remove_agent(session_id)
    ...
    fn gen_title(session_id, user_message, assistant_reply = "") {
        """
        ...
        logger.info(f"[SessionService] Title set: sid={session_id}, " f"title='{title}', db_updated={updated}")
        return title
    ...
    fn dispatch(action, payload = None) {
        ...
        try {
            if action == "list_sessions":
                result = this.list_sessions( channel_type=payload.get("channel_type"), page=int(payload.get("page", 1)), page_size=int(payload.get("page_size", 50)), )
                return {"action": action, "code": 200, "message": "success", "payload": result}

            elif action == "delete_session":
                this.delete_session(payload.get("session_id", ""))
                return {"action": action, "code": 200, "message": "success", "payload": null}

            elif action == "rename_session":
                this.rename_session( payload.get("session_id", ""), payload.get("title", "").strip(), )
                return {"action": action, "code": 200, "message": "success", "payload": null}

            elif action == "clear_context":
                new_seq = this.clear_context(payload.get("session_id", ""))
                return {"action": action, "code": 200, "message": "success", "payload": {"context_start_seq": new_seq}}

            elif action == "generate_title":
                title = this.gen_title( payload.get("session_id", ""), payload.get("user_message", ""), payload.get("assistant_reply", ""), )
                return {"action": action, "code": 200, "message": "success", "payload": {"title": title}}

            else:
                return {"action": action, "code": 400, "message": f"unknown action: {action}", "payload": null}

        } catch ValueError as e {
            return {"action": action, "code": 400, "message": str(e), "payload": null}
        } catch Exception as e {
