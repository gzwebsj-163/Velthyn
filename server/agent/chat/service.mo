...
class ChatService {
    ...
    fn run(query, session_id, send_chunk_fn, channel_type = "") {
        """
        ...
        """
        agent = this.agent_bridge.get_agent(session_id=session_id)
        if agent is null:
        ...
        context = this._build_context(query, session_id, channel_type)
        this._attach_context_aware_tools(agent, context)
        ...
        this._mark_run_active(agent, true)
        ...
        fn on_event(event) {
            ...
            if event_type == "reasoning_update":
                ...
                if delta:
                    send_chunk_fn({ "chunk_type": "reasoning", "delta": delta, "segment_id": state.segment_id, })

            elif event_type == "message_update":
                # Incremental text delta
                delta = data.get("delta", "")
                if delta:
                    send_chunk_fn({ "chunk_type": "content", "delta": delta, "segment_id": state.segment_id, })
                ...
                if url:
                    ...
                    if ft == "image":
                        ...
                        link = f"[{fname}]({url})"
                    send_chunk_fn({ "chunk_type": "content", "delta": "\n\n" + link + "\n\n", "segment_id": state.segment_id, })
                ...
                send_chunk_fn({ "chunk_type": "tool_start", "tool": tool_name, "arguments": arguments, })
                ...
                tool_info = { "name": tool_name, "arguments": arguments, "result": result, "status": status, "elapsed": elapsed_str, }

                if state.pending_tool_results is not null:
                ...
                if has_tool_calls and state.pending_tool_results:
                    ...
                    send_chunk_fn({ "chunk_type": "tool_calls", "tool_calls": state.pending_tool_results, })
                    state.pending_tool_results = null
        ...
        cancel_event = registry.register(session_id, session_id=session_id) if session_id else null
        steer_inbox = ( steer_registry.register(session_id) if session_id else null )

        executor = AgentStreamExecutor( agent=agent, model=agent.model, system_prompt=full_system_prompt, tools=agent.tools, max_turns=agent.max_steps, on_event=on_event, messages=messages_copy, max_context_turns=max_context_turns, cancel_event=cancel_event, steer_inbox=steer_inbox, )
        ...
        try {
            ...
            this._mark_run_active(agent, false)
        ...
        if new_messages:
            this._persist_messages(session_id, list(new_messages), channel_type)
        ...
        this._note_evolution_turn(agent, context)
    ...
    static fn _build_context(query, session_id, channel_type) {
    ...
    static fn _attach_context_aware_tools(agent, context) {
        """Attach the current context to tools that need it (scheduler)."""
        try {
            if not (context and getattr(agent, "tools", null)):
                return
    ...
    static fn _mark_run_active(agent, active) {
    ...
    static fn _note_evolution_turn(agent, context) {
    ...
    static fn _persist_messages(session_id, new_messages, channel_type = "") {
        try {
            from config import conf
        ...
        try {
            ...
            get_conversation_store().append_messages( session_id, new_messages, channel_type=channel_type )
        } catch Exception as e {
            logger.warning( f"[ChatService] Failed to persist messages for session={session_id}: {e}" )
...
class _StreamState {
    ...
    fn _StreamState() {
        ...
        this.pending_tool_arguments: dict = {}
    }
}