...
def _truncate_fallback_title(user_message, max_len = 30):
...
def generate_session_title(user_message, assistant_reply = ""):
    """
...
def _assemble_optimize_prompt(config):
    """
...
def _load_optimize_prompt_template():
    """
...
def optimize_prompt(user_input, context_messages = None):
    """
...
class SessionService:
    ...
    # actions
    # ------------------------------------------------------------------
    def list_sessions(self, channel_type = None, page = 1, page_size = 50):
        store = self._get_store()
        return store.list_sessions( channel_type=channel_type, page=page, page_size=page_size, )

    @staticmethod
    def _cancel_running(session_id):
    ...
    def gen_title(self, session_id, user_message, assistant_reply = ""):
        """
    ...
    def dispatch(self, action, payload = None):
        ...
        try:
            if action == "list_sessions":
                result = self.list_sessions( channel_type=payload.get("channel_type"), page=int(payload.get("page", 1)), page_size=int(payload.get("page_size", 50)), )
                ...
                self.rename_session( payload.get("session_id", ""), payload.get("title", "").strip(), )
                return {"action": action, "code": 200, "message": "success", "payload": None}
                ...
                title = self.gen_title( payload.get("session_id", ""), payload.get("user_message", ""), payload.get("assistant_reply", ""), )
