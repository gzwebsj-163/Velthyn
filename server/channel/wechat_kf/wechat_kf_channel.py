...
class WechatKfChannel(ChatChannel):
    ...
    def __init__(self):
        super().__init__()
        self.corp_id = conf().get("wechat_kf_corp_id") or ""
        self.secret = conf().get("wechat_kf_secret") or ""
        self.token = conf().get("wechat_kf_token") or ""
        self.aes_key = conf().get("wechat_kf_aes_key") or ""
        self._http_server = None
        logger.info( "[wechat_kf] Initializing WeCom customer-service channel, corp_id: {}".format( self.corp_id ) )
        ...
        self._callback_executor = ThreadPoolExecutor( max_workers=4, thread_name_prefix="wxkf-cb" )
    ...
    def send(self, reply, context):
        ...
        if reply.type in [ReplyType.TEXT, ReplyType.ERROR, ReplyType.INFO]:
            ...
            try:
                response = self.client.media.upload( "video", requests.get(video_url, stream=True).content )
            except WeChatClientException as e:
            ...
            try:
                with open(file_path, "rb") as f:
                    response = self.client.media.upload( "file", (os.path.basename(file_path), f.read()) )
            except WeChatClientException as e:
    ...
    # ------------------------------------------------------------------
    def _get_kf_lock(self, open_kfid):
        with self._kf_locks_guard:
            return self._kf_locks[open_kfid]

    def submit_callback(self, token, open_kfid):
        """
    ...
    def _run_callback(self, token, open_kfid):
    ...
    def consume_callback(self, token, open_kfid):
        """
        ...
        for raw in msgs:
            ...
            # without self the attachment is effectively lost.
            ...
            context = self._compose_context( kf_msg.ctype, kf_msg.content, isgroup=False, msg=kf_msg, )
            if context:
                self.produce(context)
            time.sleep(0.05)  # tiny gap between messages of the same batch

    def _initialize_cursor(self, token, open_kfid):
        """
    ...
    def _pull_messages(self, token, open_kfid, next_cursor):
    ...
    def _call_sync_msg(self, token, open_kfid, cursor):
    ...
    # ------------------------------------------------------------------
    def _post_send_msg(self, payload):
    ...
    def _send_text(self, external_userid, open_kfid, content):
        return self._post_send_msg({ "touser": external_userid, "open_kfid": open_kfid, "msgtype": "text", "text": {"content": content}, })

    def _send_image(self, external_userid, open_kfid, media_id):
        return self._post_send_msg({ "touser": external_userid, "open_kfid": open_kfid, "msgtype": "image", "image": {"media_id": media_id}, })

    def _send_voice(self, external_userid, open_kfid, media_id):
        return self._post_send_msg({ "touser": external_userid, "open_kfid": open_kfid, "msgtype": "voice", "voice": {"media_id": media_id}, })

    def _send_video(self, external_userid, open_kfid, media_id):
        return self._post_send_msg({ "touser": external_userid, "open_kfid": open_kfid, "msgtype": "video", "video": {"media_id": media_id}, })

    def _send_file(self, external_userid, open_kfid, media_id):
        return self._post_send_msg({ "touser": external_userid, "open_kfid": open_kfid, "msgtype": "file", "file": {"media_id": media_id}, })

    def _send_link(self, external_userid, open_kfid, link_data):
        return self._post_send_msg({ "touser": external_userid, "open_kfid": open_kfid, "msgtype": "link", "link": link_data, })


def _dedup_image_text_pair(messages):
    """
