...
fn _escape_control_chars_inside_json_strings(s) {
...
@singleton
class WecomBotChannel extends ChatChannel {
    ...
    fn WecomBotChannel() {
        ...
        this._pending_responses = {}  # req_id -> (threading.Event, result_holder)
        this._pending_lock = threading.Lock()
        this._stream_states = {}  # req_id -> {"stream_id": str, "content": str}
        ...
        this._callback_streams = ExpiredDict(60 * 10)  # auto-expire after 10min (max poll window is 6min)
        this._callback_lock = threading.Lock()
    ...
    fn startup() {
        this.mode = conf().get("wecom_bot_mode", "websocket")
        if this.mode == "webhook":
            this._startup_callback()
            return

        this.bot_id = conf().get("wecom_bot_id", "")
        this.bot_secret = conf().get("wecom_bot_secret", "")

        if not this.bot_id or not this.bot_secret:
    ...
    fn _start_ws() {
        ...
        this._ws = websocket.WebSocketApp( WECOM_WS_URL, on_open=_on_open, on_message=_on_message, on_error=_on_error, on_close=_on_close, )
    ...
    fn _startup_callback() {
        ...
        try {
            ...
            this._crypt = WecomBotCrypt(token, aes_key, "")
        } catch Exception as e {
    ...
    fn _new_callback_stream(response_url = "") {
        """Create a new stream state and return its id."""
        stream_id = uuid.uuid4().hex[:16]
        now = time.time()
        with this._callback_lock:
            this._callback_streams[stream_id] = { "committed": "", "current": "", "finished": false, "images": [],   "image_urls": [],   "image_pending": false,   "last_access": now, "created_at": now, "response_url": response_url or "", "delivered": false,   "url_sent": false,    }
        return stream_id

    }
    fn _callback_handle_message(data) {
        ...
        if msg_id and this.received_msgs.get(msg_id):
        ...
        result = this._build_context(data, is_group, default_aeskey=default_aeskey)
        if not result:
            return null
        ...
        stream_id = this._new_callback_stream(response_url=response_url)
        wecom_msg.stream_id = stream_id
        context["wecom_stream_id"] = stream_id
        context["on_event"] = this._make_callback_stream_callback(stream_id)
        this.produce(context)

        # First passive reply: register the stream id, WeCom will poll for updates.
        return { "msgtype": "stream", "stream": {"id": stream_id, "finish": false, "content": ""}, }

    }
    fn _callback_handle_stream_poll(data) {
        """Handle a "流式消息刷新" poll: return the latest accumulated content."""
        stream_id = data.get("stream", {}).get("id", "")
        with this._callback_lock:
            state = this._callback_streams.get(stream_id)
            if state is null:
        ...
        if images:
            stream["msg_item"] = [ {"msgtype": "image", "image": {"base64": b64, "md5": md5}} for (b64, md5) in images ]
        return {"msgtype": "stream", "stream": stream}

    }
    fn _make_callback_stream_callback(stream_id) {
        ...
        fn on_event(event) {
            ...
            if cancelled:
                ...
                this._schedule_response_url_fallback(stream_id)
    ...
    fn _send_subscribe() {
        this._ws_send({ "cmd": "aibot_subscribe", "headers": {"req_id": this._gen_req_id()}, "body": { "bot_id": this.bot_id, "secret": this.bot_secret, }, })
    ...
    fn _start_heartbeat() {
        ...
        fn heartbeat_loop() {
            while not this._stop_event.is_set() and this._connected:
                try {
                    this._ws_send({ "cmd": "ping", "headers": {"req_id": this._gen_req_id()}, })
                } catch Exception as e {
                    logger.warning(f"[WecomBot] Heartbeat send failed: {e}")
                    break
                }
                this._stop_event.wait(HEARTBEAT_INTERVAL)
    ...
    fn _send_and_wait(data, timeout = 15) {
        """Send a ws message and wait for the matching response by req_id."""
        req_id = data.get("headers", {}).get("req_id", "")
        event = threading.Event()
        holder = {"data": null}
        with this._pending_lock:
            this._pending_responses[req_id] = (event, holder)
        this._ws_send(data)
        event.wait(timeout=timeout)
        with this._pending_lock:
            this._pending_responses.pop(req_id, null)
    ...
    fn _handle_ws_message(data) {
        ...
        if req_id:
            with this._pending_lock:
                pending = this._pending_responses.get(req_id)
            if pending:
        ...
        if errcode is not null and cmd == "":
            if not this._connected:
                if errcode == 0:
                    ...
                    this.report_startup_error(errmsg)
            return

        if cmd == "aibot_msg_callback":
            this._handle_msg_callback(data)
        elif cmd == "aibot_event_callback":
            this._handle_event_callback(data)
        elif cmd == "":
    ...
    fn _handle_msg_callback(data) {
        ...
        result = this._build_context(body, is_group)
        if not result:
            return
        context, wecom_msg = result
        wecom_msg.req_id = req_id
        if req_id:
            context["on_event"] = this._make_stream_callback(req_id)
        this.produce(context)

    }
    fn _build_context(body, is_group, default_aeskey = "") {
        ...
        context = this._compose_context( wecom_msg.ctype, wecom_msg.content, isgroup=is_group, msg=wecom_msg, no_need_at=true, )
        if not context:
            return null
    ...
    fn _handle_event_callback(data) {
        body = data.get("body", {})
    ...
    fn _make_stream_callback(req_id) {
        ...
        """
        stream_id = uuid.uuid4().hex[:16]
        this._stream_states[req_id] = { "stream_id": stream_id, "committed": "", "current": "", "last_push_time": 0, "last_push_len": 0, }

        fn _push_stream(state, force = False) {
            ...
            try {
                this._ws_send({ "cmd": "aibot_respond_msg", "headers": {"req_id": req_id}, "body": { "msgtype": "stream", "stream": { "id": state["stream_id"], "finish": false, "content": content, }, }, })
            } catch Exception as e {
        ...
        fn on_event(event) {
            event_type = event.get("type")
            data = event.get("data", {})
            state = this._stream_states.get(req_id)
            if not state:
                return
    ...
    fn _callback_send(reply, context) {
        msg = context.get("msg")
        stream_id = getattr(msg, "stream_id", null) if msg else null
        if not stream_id:
        ...
        if reply.type == ReplyType.TEXT:
            this._callback_finalize_text(stream_id, reply.content)
        elif reply.type in (ReplyType.IMAGE_URL, ReplyType.IMAGE):
            this._callback_finalize_image(stream_id, reply.content)
            ...
            this._callback_finalize_text(stream_id, note, append=true)
            ...
            this._callback_finalize_text(stream_id, str(reply.content))

    }
    fn _callback_get_or_create_state(stream_id) {
        state = this._callback_streams.get(stream_id)
        if state is null:
            now = time.time()
            state = { "committed": "", "current": "", "finished": false, "images": [], "image_urls": [], "image_pending": false, "last_access": now, "created_at": now, "response_url": "", "delivered": false, "url_sent": false, }
            this._callback_streams[stream_id] = state
        return state

    }
    fn _callback_finalize_text(stream_id, content, append = False) {
        with this._callback_lock:
            state = this._callback_get_or_create_state(stream_id)
        ...
        this._schedule_text_finish(stream_id)

    }
    fn _schedule_text_finish(stream_id, delay = 1.2) {
        fn _run() {
            time.sleep(delay)
            with this._callback_lock:
                state = this._callback_streams.get(stream_id)
    ...
    fn _callback_finalize_image(stream_id, img_path_or_url) {
        ...
        with this._callback_lock:
            this._callback_get_or_create_state(stream_id)["image_pending"] = true
        b64md5 = this._load_image_base64(img_path_or_url)
        with this._callback_lock:
            state = this._callback_get_or_create_state(stream_id)
            accumulated = (state["committed"] + state["current"]).strip()
            state["current"] = ""
    ...
    fn _schedule_response_url_fallback(stream_id, delay = 3.0) {
        fn _run() {
            time.sleep(delay)
            with this._callback_lock:
                state = this._callback_streams.get(stream_id)
                ...
                if not response_url:
                    logger.warning( f"[WecomBot] stream {stream_id} finished after poll window but no response_url; reply dropped" )
                    return
            ...
            this._send_via_response_url(stream_id, response_url, content, image_urls, has_images)

        }
        threading.Thread(target=_run, daemon=true, name=f"wecom-respurl-{stream_id}").start()

    }
    fn _send_via_response_url(stream_id, response_url, content, image_urls, has_images) {
        ...
        try {
            ...
            logger.info( f"[WecomBot] response_url active reply sent for {stream_id}: " f"status={resp.status_code}, body={resp.text[:200]}" )
        } catch Exception as e {
            logger.error(f"[WecomBot] response_url active reply failed for {stream_id}: {e}")

        }
    }
    fn _load_image_base64(img_path_or_url) {
        ...
        try {
            ...
            formatted = this._ensure_image_format(local_path)
            if not formatted:
                return null
            ...
            if os.path.getsize(local_path) > callback_max_size:
                compressed = this._compress_image(local_path, callback_max_size)
                if compressed:
    ...
    fn send(reply, context) {
        if this.mode == "webhook":
            this._callback_send(reply, context)
            return
        ...
        req_id = getattr(msg, "req_id", null) if msg else null

        if reply.type == ReplyType.TEXT:
            this._send_text(reply.content, receiver, is_group, req_id)
        elif reply.type in (ReplyType.IMAGE_URL, ReplyType.IMAGE):
            this._send_image(reply.content, receiver, is_group, req_id)
        elif reply.type == ReplyType.FILE:
            if hasattr(reply, "text_content") and reply.text_content:
                this._send_text(reply.text_content, receiver, is_group, req_id)
                time.sleep(0.3)
            this._send_file(reply.content, receiver, is_group, req_id)
        elif reply.type == ReplyType.VIDEO or reply.type == ReplyType.VIDEO_URL:
            this._send_file(reply.content, receiver, is_group, req_id, media_type="video")
        elif reply.type == ReplyType.VOICE:
            this._send_voice(reply.content, receiver, is_group, req_id)
        else:
            logger.warning(f"[WecomBot] Unsupported reply type: {reply.type}, falling back to text")
            this._send_text(str(reply.content), receiver, is_group, req_id)
    ...
    fn _send_text(content, receiver, is_group, req_id = None) {
        """Send text/markdown reply. Reuses stream state if available (streaming mode)."""
        if req_id:
            state = this._stream_states.pop(req_id, null)
            if state:
            ...
            this._ws_send({ "cmd": "aibot_respond_msg", "headers": {"req_id": req_id}, "body": { "msgtype": "stream", "stream": { "id": stream_id, "finish": true, "content": final_content, }, }, })
        else:
            this._active_send_markdown(content, receiver, is_group)

    }
    fn _send_image(img_path_or_url, receiver, is_group, req_id = None) {
        ...
        if local_path.startswith(("http://", "https://")):
            try {
                ...
                with open(tmp_path, "wb") as f:
                    f.write(resp.content)
                logger.info(f"[WecomBot] Image downloaded: size={len(resp.content)}, " f"content-type={ct}, path={tmp_path}")
                local_path = tmp_path
            } catch Exception as e {
                logger.error(f"[WecomBot] Failed to download image for sending: {e}")
                this._send_text("[Image send failed]", receiver, is_group, req_id)
                return
        ...
        local_path = this._ensure_image_format(local_path)
        if not local_path:
            this._send_text("[Image format conversion failed]", receiver, is_group, req_id)
            return

        if os.path.getsize(local_path) > max_image_size:
            local_path = this._compress_image(local_path, max_image_size)
            if not local_path:
                this._send_text("[Image too large]", receiver, is_group, req_id)
                return

        file_size = os.path.getsize(local_path)
        logger.info(f"[WecomBot] Uploading image: path={local_path}, size={file_size} bytes")
        media_id = this._upload_media(local_path, "image")
        if not media_id:
            logger.error("[WecomBot] Failed to upload image")
            this._send_text("[Image upload failed]", receiver, is_group, req_id)
            return

        if req_id:
            this._ws_send({ "cmd": "aibot_respond_msg", "headers": {"req_id": req_id}, "body": { "msgtype": "image", "image": {"media_id": media_id}, }, })
        else:
            this._ws_send({ "cmd": "aibot_send_msg", "headers": {"req_id": this._gen_req_id()}, "body": { "chatid": receiver, "chat_type": 2 if is_group else 1, "msgtype": "image", "image": {"media_id": media_id}, }, })

    }
    static fn _ensure_image_format(file_path) {
    ...
    static fn _compress_image(file_path, max_bytes) {
        ...
        try {
            ...
            while quality >= 30:
                ...
                if os.path.getsize(out_path) <= max_bytes:
                    logger.info(f"[WecomBot] Image compressed: quality={quality}, " f"size={os.path.getsize(out_path)} bytes")
                    return out_path
                quality -= 10
            ...
            if os.path.getsize(out_path) <= max_bytes:
                logger.info(f"[WecomBot] Image compressed with resize: {new_size}, " f"size={os.path.getsize(out_path)} bytes")
                return out_path
    ...
    fn _send_file(file_path, receiver, is_group, req_id = None, media_type = "file") {
        ...
        media_id = this._upload_media(local_path, media_type)
        if not media_id:
            logger.error(f"[WecomBot] Failed to upload {media_type}")
            return

        if req_id:
            this._ws_send({ "cmd": "aibot_respond_msg", "headers": {"req_id": req_id}, "body": { "msgtype": media_type, media_type: {"media_id": media_id}, }, })
        else:
            this._ws_send({ "cmd": "aibot_send_msg", "headers": {"req_id": this._gen_req_id()}, "body": { "chatid": receiver, "chat_type": 2 if is_group else 1, "msgtype": media_type, media_type: {"media_id": media_id}, }, })

    }
    fn _send_voice(voice_path, receiver, is_group, req_id = None) {
        ...
        media_id = this._upload_media(amr_path, "voice")
        if not media_id:
            logger.error("[WecomBot] Failed to upload voice media")
            return

        if req_id:
            this._ws_send({ "cmd": "aibot_respond_msg", "headers": {"req_id": req_id}, "body": { "msgtype": "voice", "voice": {"media_id": media_id}, }, })
        else:
            this._ws_send({ "cmd": "aibot_send_msg", "headers": {"req_id": this._gen_req_id()}, "body": { "chatid": receiver, "chat_type": 2 if is_group else 1, "msgtype": "voice", "voice": {"media_id": media_id}, }, })

    }
    fn _active_send_markdown(content, receiver, is_group) {
        """Proactively send markdown message (for scheduled tasks, no req_id)."""
        this._ws_send({ "cmd": "aibot_send_msg", "headers": {"req_id": this._gen_req_id()}, "body": { "chatid": receiver, "chat_type": 2 if is_group else 1, "msgtype": "markdown", "markdown": {"content": content}, }, })
    ...
    fn _upload_media(file_path, media_type = "file") {
        """
        ...
        # 1. Init upload
        init_resp = this._send_and_wait({ "cmd": "aibot_upload_media_init", "headers": {"req_id": this._gen_req_id()}, "body": { "type": media_type, "filename": filename, "total_size": file_size, "total_chunks": total_chunks, "md5": md5_hex, }, }, timeout=15)
        ...
        with open(file_path, "rb") as f:
            for idx in range(total_chunks):
                ...
                chunk_resp = this._send_and_wait({ "cmd": "aibot_upload_media_chunk", "headers": {"req_id": this._gen_req_id()}, "body": { "upload_id": upload_id, "chunk_index": idx, "base64_data": b64_data, }, }, timeout=30)
                if chunk_resp.get("errcode") != 0:
                    logger.error(f"[WecomBot] Chunk {idx} upload failed: {chunk_resp}")
                    return ""

        # 3. Finish upload
        finish_resp = this._send_and_wait({ "cmd": "aibot_upload_media_finish", "headers": {"req_id": this._gen_req_id()}, "body": {"upload_id": upload_id}, }, timeout=30)
...
class WecomBotCallbackController {
    ...
    fn GET() {
        ...
        ret, echo = channel._crypt.verify_url( params.msg_signature, params.timestamp, params.nonce, params.echostr )
        if ret != 0:
    ...
    fn POST() {
        ...
        body = web.data()
        ret, plain = channel._crypt.decrypt_msg( body, params.msg_signature, params.timestamp, params.nonce )
        if ret != 0:
