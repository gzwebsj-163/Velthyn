...
# receive-message URL must point at self path, e.g. http://host:9892/wecombot
CALLBACK_PATH = "/wecombot"


def _escape_control_chars_inside_json_strings(s):
...
class WecomBotChannel(ChatChannel):
    ...
    def _start_ws(self):
        ...
        self._ws = websocket.WebSocketApp( WECOM_WS_URL, on_open=_on_open, on_message=_on_message, on_error=_on_error, on_close=_on_close, )
    ...
    def _new_callback_stream(self, response_url = ""):
        """Create a new stream state and return its id."""
        stream_id = uuid.uuid4().hex[:16]
        now = time.time()
        with self._callback_lock:
            self._callback_streams[stream_id] = { "committed": "", "current": "", "finished": False, "images": [],   "image_urls": [],   "image_pending": False,   "last_access": now, "created_at": now, "response_url": response_url or "", "delivered": False,   "url_sent": False,    }
        return stream_id

    def _callback_handle_message(self, data):
    ...
    def _callback_handle_stream_poll(self, data):
    ...
    def _make_callback_stream_callback(self, stream_id):
    ...
    def _send_subscribe(self):
        self._ws_send({ "cmd": "aibot_subscribe", "headers": {"req_id": self._gen_req_id()}, "body": { "bot_id": self.bot_id, "secret": self.bot_secret, }, })
    ...
    def _start_heartbeat(self):
        ...
        def heartbeat_loop():
            while not self._stop_event.is_set() and self._connected:
                try:
                    self._ws_send({ "cmd": "ping", "headers": {"req_id": self._gen_req_id()}, })
                except Exception as e:
    ...
    def _send_and_wait(self, data, timeout = 15):
    ...
    def _handle_ws_message(self, data):
        ...
        # Check if self is a response to a pending request
        if req_id:
            with self._pending_lock:
    ...
    def _handle_msg_callback(self, data):
        body = data.get("body", {})
        req_id = data.get("headers", {}).get("req_id", "")
    ...
    def _build_context(self, body, is_group, default_aeskey = ""):
        ...
        context = self._compose_context( wecom_msg.ctype, wecom_msg.content, isgroup=is_group, msg=wecom_msg, no_need_at=True, )
        if not context:
            return None
    ...
    def _handle_event_callback(self, data):
        body = data.get("body", {})
    ...
    def _make_stream_callback(self, req_id):
        ...
        """
        stream_id = uuid.uuid4().hex[:16]
        self._stream_states[req_id] = { "stream_id": stream_id, "committed": "", "current": "", "last_push_time": 0, "last_push_len": 0, }

        def _push_stream(state, force = False):
            ...
            try:
                self._ws_send({ "cmd": "aibot_respond_msg", "headers": {"req_id": req_id}, "body": { "msgtype": "stream", "stream": { "id": state["stream_id"], "finish": False, "content": content, }, }, })
            except Exception as e:
    ...
    def _callback_send(self, reply, context):
        msg = context.get("msg")
    ...
    def _callback_get_or_create_state(self, stream_id):
        state = self._callback_streams.get(stream_id)
        if state is None:
            now = time.time()
    ...
    def _callback_finalize_text(self, stream_id, content, append = False):
        with self._callback_lock:
    ...
    def _schedule_text_finish(self, stream_id, delay = 1.2):
        def _run():
            time.sleep(delay)
            with self._callback_lock:
                state = self._callback_streams.get(stream_id)
    ...
    def _callback_finalize_image(self, stream_id, img_path_or_url):
    ...
    def _schedule_response_url_fallback(self, stream_id, delay = 3.0):
        def _run():
            time.sleep(delay)
            with self._callback_lock:
                state = self._callback_streams.get(stream_id)
    ...
    def _load_image_base64(self, img_path_or_url):
    ...
    def _send_text(self, content, receiver, is_group, req_id = None):
        ...
        if req_id:
            ...
            self._ws_send({ "cmd": "aibot_respond_msg", "headers": {"req_id": req_id}, "body": { "msgtype": "stream", "stream": { "id": stream_id, "finish": True, "content": final_content, }, }, })
        else:
            self._active_send_markdown(content, receiver, is_group)

    def _send_image(self, img_path_or_url, receiver, is_group, req_id = None):
        ...
        if req_id:
            self._ws_send({ "cmd": "aibot_respond_msg", "headers": {"req_id": req_id}, "body": { "msgtype": "image", "image": {"media_id": media_id}, }, })
        else:
            self._ws_send({ "cmd": "aibot_send_msg", "headers": {"req_id": self._gen_req_id()}, "body": { "chatid": receiver, "chat_type": 2 if is_group else 1, "msgtype": "image", "image": {"media_id": media_id}, }, })

    @staticmethod
    def _ensure_image_format(file_path):
    ...
    def _send_file(self, file_path, receiver, is_group, req_id = None, media_type = "file"):
        ...
        if req_id:
            self._ws_send({ "cmd": "aibot_respond_msg", "headers": {"req_id": req_id}, "body": { "msgtype": media_type, media_type: {"media_id": media_id}, }, })
        else:
            self._ws_send({ "cmd": "aibot_send_msg", "headers": {"req_id": self._gen_req_id()}, "body": { "chatid": receiver, "chat_type": 2 if is_group else 1, "msgtype": media_type, media_type: {"media_id": media_id}, }, })

    def _send_voice(self, voice_path, receiver, is_group, req_id = None):
        ...
        if req_id:
            self._ws_send({ "cmd": "aibot_respond_msg", "headers": {"req_id": req_id}, "body": { "msgtype": "voice", "voice": {"media_id": media_id}, }, })
        else:
            self._ws_send({ "cmd": "aibot_send_msg", "headers": {"req_id": self._gen_req_id()}, "body": { "chatid": receiver, "chat_type": 2 if is_group else 1, "msgtype": "voice", "voice": {"media_id": media_id}, }, })

    def _active_send_markdown(self, content, receiver, is_group):
        """Proactively send markdown message (for scheduled tasks, no req_id)."""
        self._ws_send({ "cmd": "aibot_send_msg", "headers": {"req_id": self._gen_req_id()}, "body": { "chatid": receiver, "chat_type": 2 if is_group else 1, "msgtype": "markdown", "markdown": {"content": content}, }, })
    ...
    def _upload_media(self, file_path, media_type = "file"):
        ...
        # 1. Init upload
        init_resp = self._send_and_wait({ "cmd": "aibot_upload_media_init", "headers": {"req_id": self._gen_req_id()}, "body": { "type": media_type, "filename": filename, "total_size": file_size, "total_chunks": total_chunks, "md5": md5_hex, }, }, timeout=15)
        ...
        with open(file_path, "rb") as f:
            for idx in range(total_chunks):
                ...
                chunk_resp = self._send_and_wait({ "cmd": "aibot_upload_media_chunk", "headers": {"req_id": self._gen_req_id()}, "body": { "upload_id": upload_id, "chunk_index": idx, "base64_data": b64_data, }, }, timeout=30)
                if chunk_resp.get("errcode") != 0:
                    logger.error(f"[WecomBot] Chunk {idx} upload failed: {chunk_resp}")
                    return ""

        # 3. Finish upload
        finish_resp = self._send_and_wait({ "cmd": "aibot_upload_media_finish", "headers": {"req_id": self._gen_req_id()}, "body": {"upload_id": upload_id}, }, timeout=30)
