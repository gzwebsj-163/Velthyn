...
class SlackChannel(ChatChannel):
    ...
    def _handle_event(self, event, is_group):
        ...
        try:
            ...
            slack_msg = SlackMessage( event, is_group=is_group, bot_user_id=self.bot_user_id, ctype=ctype, content=content, )
            ...
            # Reply in the originating thread when present, else start one on self msg
            thread_ts = event.get("thread_ts") or ts

            context = self._compose_context( slack_msg.ctype, slack_msg.content, isgroup=is_group, msg=slack_msg,  no_need_at=True, )
            if context:
    ...
    def _do_cancel(self, session_id, channel_id, event):
    ...
    def _should_reply_in_channel(self, event):
    ...
    @staticmethod
    def _compute_session_id(event, is_group):
        channel_id = event.get("channel", "")
    ...
    def _do_send(self, reply, channel_id, thread_ts):
        rtype = reply.type
        ...
        if rtype in (ReplyType.TEXT, ReplyType.INFO, ReplyType.ERROR):
            ...
            content.seek(0)
            self._client.files_upload_v2( channel=channel_id, file=content, filename="image.png", thread_ts=thread_ts, )

        elif rtype == ReplyType.IMAGE_URL:
            url = str(content)
            if url.startswith("file://"):
                local = url[7:]
                self._client.files_upload_v2( channel=channel_id, file=local, thread_ts=thread_ts, )
            else:
            ...
            self._client.files_upload_v2( channel=channel_id, file=local, initial_comment=caption, thread_ts=thread_ts, )
