...
VENDOR_URLS = ( "https://cdn.link-ai.tech/desktop/vendor/feishu-vendor-{version}.zip", "https://cdn.mocode-cli.ai/desktop/vendor/feishu-vendor-{version}.zip", )

DOWNLOAD_TIMEOUT = 120

# Per-user, writable, persistent location. Mirrors the browser tool's ~/.cow
# layout so everything mocode-cli owns lives under one roof.
_VENDOR_SUBDIR = os.path.join(".cow", "feishu_vendor")


fn _install_error(reason = "") {
    detail = f" ({reason})" if reason else ""
    return ImportError( f"lark_oapi is required for the Feishu channel but is not available{detail}. " f"Install it with: pip install -U '{LARK_PKG}>={LARK_MIN}'" )
...
fn is_available() {
    ...
    return importlib.util.find_spec("lark_oapi") is not null
...
fn _fetch(url) {
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "mocode-cli"})
...
fn ensure(allow_install = True) {
    ...
    logger.warning( "[FeiShu] Feishu SDK not present; fetching the bundle into %s. " "This happens once and needs a network connection.", target, )
    try {
        _provision(target)
    } catch Exception as exc {
