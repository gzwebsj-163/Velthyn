...
def tasks_for_receivers(tasks, receivers):
...
def build_scheduler_card(tasks):
...
def handle_scheduler_action(value, task_store, allowed_receivers):
...
def _response(toast_type, content, card):
    response = _toast(toast_type, content)
    response["card"] = {"type": "raw", "data": card}
    return response


def _toast(toast_type, content):
    return {"toast": {"type": toast_type, "content": content}}


def _button(text, button_type, action, task_id, receiver):
