...
def _print_qr_to_terminal(qr_url):
...
def _persist_feishu_credentials(app_id, app_secret):
...
def _register_via_qr_in_terminal():
...
class FeiShuChanel(ChatChannel):
    ...
    def __init__(self):
        ...
        logger.debug("[FeiShu] app_id={}, app_secret={}, verification_token={}, event_mode={}".format( self.feishu_app_id, self.feishu_app_secret, self.feishu_token, self.feishu_event_mode))
        # 无需群校验和前缀
    ...
    def _startup_websocket(self):
        ...
        def handle_message_recalled_event(data):
        ...
        def start_client_with_retry():
            ...
            # call (in self new thread) to raise "This event loop is already running".
            ...
            for attempt in range(2):
                try:
                    ...
                    ws_client = lark.ws.Client( self.feishu_app_id, self.feishu_app_secret, event_handler=event_handler, log_level=lark.LogLevel.WARNING )
    ...
    def _is_mention_bot(self, mentions):
    ...
    def _send_scheduler_card(self, feishu_msg, is_group, receive_id_type):
    ...
    def _handle_card_action_event(self, event):
        ...
        response = handle_scheduler_action( value, self._get_scheduler_task_store(), allowed_receivers, )
        logger.info( "[FeiShu] scheduler card action handled, " f"action={value.get('action')}, task_id={value.get('task_id')}" )
        return response

    def _handle_message_recalled_event(self, event):
    ...
    def _handle_message_event(self, event):
        ...
        if feishu_msg.ctype == ContextType.FILE:
            try:
                ...
                try:
                    ...
                    self._send(err_reply, self._compose_context( ContextType.TEXT, "", isgroup=is_group, msg=feishu_msg, receive_id_type=receive_id_type, no_need_at=True, ))
                except Exception as e:
                    pass
                return
        ...
        context = self._compose_context( feishu_msg.ctype, feishu_msg.content_with_quote(), isgroup=is_group, msg=feishu_msg, receive_id_type=receive_id_type, no_need_at=True )
        if context:
    ...
    def send(self, reply, context):
        ...
        # Build content JSON
        content_json = prepared_content_json or ( json.dumps(reply_content, ensure_ascii=False) if content_key is None else json.dumps({content_key: reply_content}, ensure_ascii=False) )
    ...
    def _make_feishu_stream_callback_progress(self, context, access_token):
        ...
        def _update_full_card(streaming):
    ...
    def _make_feishu_stream_callback_plain(self, context, access_token):
        ...
        def _close_streaming_mode(final_text = ""):
