...
class FeishuMessage(ChatMessage):
    def __init__(self, event, is_group=False, access_token=None):
        ...
        if msg_type == "text":
            ...
            tmp_dir = os.path.join(workspace_root, "tmp")
            os.makedirs(tmp_dir, exist_ok=True)
            self.content = os.path.join( tmp_dir, f"{file_key}.{utils.get_path_suffix(file_name)}" )
    ...
    def content_with_quote(self):
        ...
        if not self.quoted_content:
            return self.content
        return ( "[Quoted message]\n{}\n[/Quoted message]\n\n{}".format( self.quoted_content, self.content, ) )

    def _fetch_quoted_content(self, parent_id):
