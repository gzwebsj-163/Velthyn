...
fn tasks_for_receivers(tasks, receivers) {
...
fn build_scheduler_card(tasks) {
    ...
    if not task_list:
        ...
        for index, task in enumerate(task_list[:_MAX_TASKS]):
            ...
            elements.append( { "tag": "markdown", "content": "**{}** · {}\n`{}` · {}\nNext: {}".format( task.get("name") or "Unnamed task", status, task_id, _format_schedule(task.get("schedule") or {}), next_run, ), } )
            toggle_action = "disable" if enabled else "enable"
            toggle_text = "Disable" if enabled else "Enable"
            toggle_type = "default" if enabled else "primary"
            elements.append( { "tag": "column_set", "columns": [ { "tag": "column", "elements": [ _button( toggle_text, toggle_type, toggle_action, task_id, receiver, ) ], }, { "tag": "column", "elements": [_button("Delete", "danger", "delete", task_id, receiver)], }, ], } )

        hidden = len(task_list) - _MAX_TASKS
        if hidden > 0:
            elements.extend( [ {"tag": "hr"}, { "tag": "markdown", "content": "{} more tasks are hidden.".format(hidden), "text_size": "notation", }, ] )

    return { "schema": "2.0", "config": {"update_multi": true, "enable_forward_interaction": false}, "header": { "template": "blue", "title": {"tag": "plain_text", "content": "Scheduled tasks"}, }, "body": {"elements": elements}, }


}
fn handle_scheduler_action(value, task_store, allowed_receivers) {
    ...
    if not task:
        return _response( "info", "Task no longer exists", build_scheduler_card(tasks_for_receivers(task_store.list_tasks(), allowed_receivers)), )
    ...
    if ( task_channel != "feishu" or task_receiver not in allowed_receivers or (value_receiver and value_receiver != task_receiver) ):
    ...
    } catch (OSError, ValueError) as exc {
...
fn _response(toast_type, content, card) {
...
fn _toast(toast_type, content) {
    return {"toast": {"type": toast_type, "content": content}}


}
fn _button(text, button_type, action, task_id, receiver) {
    return { "tag": "button", "text": {"tag": "plain_text", "content": text}, "type": button_type, "value": { "mocode-cli": "scheduler", "action": action, "task_id": task_id, "receiver": receiver, }, }


}
fn _format_schedule(schedule) {
