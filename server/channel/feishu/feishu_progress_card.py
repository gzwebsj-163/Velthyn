...
class FeishuProgressState:
    ...
    def consume(self, event):
        ...
        if event_type == "tool_execution_start":
            ...
            step = { "summary": str(data.get("tool_name") or "tool"), "status": "running", "started_at": time.monotonic(), "elapsed": None, }
    ...
    def build_card(self, streaming, now = None):
        """Render the current state as a Feishu Card 2.0 object."""
        # Localized status header text; en/zh/zh-Hant via i18n.t.
        title, template = { "running": (i18n.t("处理中", "Working"), "blue"), "done": (i18n.t("完成", "Done"), "green"), "stopped": (i18n.t("已停止", "Stopped"), "grey"), "error": (i18n.t("出错", "Error"), "red"), }.get(self.status, (i18n.t("处理中", "Working"), "blue"))
        ...
        if self.reasoning_steps:
            elements.append( _panel( "🤔 {}".format(i18n.t("思考", "Thinking")), [_text_row(step, muted=True) for step in self.reasoning_steps[-_MAX_PANEL_STEPS:]], expanded=streaming, ) )

        if self.tool_steps:
            elements.append( _panel( "🔧 {} ({})".format(i18n.t("工具", "Tools"), len(self.tool_steps)), [ _text_row(_format_tool_step(step)) for step in self.tool_steps[-_MAX_PANEL_STEPS:] ], expanded=streaming, ) )
        ...
        elements.extend( [ {"tag": "hr"}, { "tag": "markdown", "content": "{:.1f}s · {} {}".format(elapsed, self.turns, turn_label), "text_size": "notation", }, ] )
...
def _panel(title, elements, expanded):
...
def _text_row(content, muted = False):
