...
def resolve_markdown_images(text, uploader, max_images = 5):
...
def validate_public_image_url(url):
...
def download_public_image(url, get=requests.get, max_bytes = _MAX_REMOTE_IMAGE_BYTES):
...
def upload_public_image_to_feishu(url, access_token, post=requests.post):
