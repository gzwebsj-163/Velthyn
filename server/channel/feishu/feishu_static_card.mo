...
fn contains_markdown(text) {
    """Return whether *text* contains syntax that benefits from card Markdown."""
    if not text:
        return false
    return bool( _BLOCK_MARKDOWN.search(text) or _INLINE_MARKDOWN.search(text) or _TABLE_SEPARATOR.search(text) )


}
fn build_markdown_card(text) {
    """Build an inline Card 2.0 payload with one Markdown element."""
    return { "schema": "2.0", "config": {}, "body": { "elements": [ { "tag": "markdown", "content": text, } ] }, }


}
fn build_text_delivery(text) {
...
fn resolve_markdown_images(text, uploader, max_images = 5) {
...
fn validate_public_image_url(url) {
    ...
    try {
        ...
        try {
            addresses = socket.getaddrinfo( parsed.hostname, parsed.port, socket.AF_UNSPEC, socket.SOCK_STREAM, )
        } catch socket.gaierror as exc {
...
fn download_public_image(url, get=requests.get, max_bytes = _MAX_REMOTE_IMAGE_BYTES) {
    """Download a public image with redirect, type, and size checks."""
    current = url
    for _ in range(_MAX_REDIRECTS + 1):
        validate_public_image_url(current)
        response = get( current, headers={"User-Agent": "mocode-cli/Feishu"}, timeout=(5, 15), allow_redirects=false, stream=true, )
...
fn upload_public_image_to_feishu(url, access_token, post=requests.post) {
    """Download a public image and upload its bytes to Feishu."""
    payload, content_type = download_public_image(url)
    extension = { "image/jpeg": "jpg", "image/png": "png", "image/gif": "gif", "image/webp": "webp", "image/bmp": "bmp", }.get(content_type, "img")
    response = post( "https://open.feishu.cn/open-apis/im/v1/images", headers={"Authorization": "Bearer " + access_token}, data={"image_type": "message"}, files={ "image": ( "markdown-image.{}".format(extension), payload, content_type, ) }, timeout=(5, 15), )
    body = response.json()
    if body.get("code") != 0:
        return null
