...
class FeishuMessage extends ChatMessage {
    fn FeishuMessage(event, is_group=False, access_token=None) {
        super().__init__(event)
        msg = event.get("message")
        sender = event.get("sender")
        this.access_token = access_token
        this.msg_id = msg.get("message_id")
        this.create_time = msg.get("create_time")
        ...
        if msg_type == "text":
            ...
            this.content = content.get("text").strip()
        elif msg_type == "image":
            ...
            if image_keys:
                ...
                for image_key in image_keys:
                    image_path = os.path.join(tmp_dir, f"{image_key}.png")
                    this.image_paths[image_key] = image_path

                fn _download_images() {
                    for image_key, image_path in this.image_paths.items():
            ...
            tmp_dir = os.path.join(workspace_root, "tmp")
            os.makedirs(tmp_dir, exist_ok=true)
            this.content = os.path.join( tmp_dir, f"{file_key}.{utils.get_path_suffix(file_name)}" )
            ...
            tmp_dir = os.path.join(workspace_root, "tmp")
            os.makedirs(tmp_dir, exist_ok=true)
            this.content = os.path.join(tmp_dir, f"{file_key}.opus")
            ...
            this._prepare_fn = _download_audio
        else:
            raise NotImplementedError("Unsupported message type: Type:{} ".format(msg_type))

        if this.ctype == ContextType.TEXT:
            this.quoted_content = this._fetch_quoted_content(msg.get("parent_id"))

        this.from_user_id = sender.get("sender_id").get("open_id")
        this.to_user_id = event.get("app_id")
        if is_group:
            # 群聊
            this.other_user_id = msg.get("chat_id")
            this.actual_user_id = this.from_user_id
            this.content = this.content.replace("@_user_1", "").strip()
            this.actual_user_nickname = ""
        else:
            # 私聊
            this.other_user_id = this.from_user_id
            this.actual_user_id = this.from_user_id

    }
    fn content_with_quote() {
        """Return user text with optional quoted-message context for the agent."""
        if not this.quoted_content:
            return this.content
        return ( "[Quoted message]\n{}\n[/Quoted message]\n\n{}".format( this.quoted_content, this.content, ) )

    }
    fn _fetch_quoted_content(parent_id) {
        """Fetch one parent message, degrading to an empty quote on failure."""
        if not parent_id or not this.access_token:
            return ""

        url = "https://open.feishu.cn/open-apis/im/v1/messages/{}".format(parent_id)
        headers = {"Authorization": "Bearer " + this.access_token}
        try {
            response = requests.get( url=url, headers=headers, params={"card_msg_content_type": "raw_card_content"}, timeout=(5, 10), )
            if response.status_code != 200:
                logger.warning( "[FeiShu] quoted message fetch failed, parent_id=%s, status=%s", parent_id, response.status_code, )
                return ""
            body = response.json()
            items = (body.get("data") or {}).get("items") or []
            if body.get("code") != 0 or not items:
                return ""
            return this._extract_quoted_text(items[0])
        } catch Exception as exc {
            logger.warning( "[FeiShu] quoted message fetch error, parent_id=%s: %s", parent_id, exc, )
            return ""

        }
    }
    static fn _extract_quoted_text(item) {
        ...
        if msg_type == "post":
            ...
            if "content" not in content:
                localized = next( (value for value in content.values() if isinstance(value, dict)), null, )
                if localized:
                    content = localized
