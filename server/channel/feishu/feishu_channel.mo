...
from channel.feishu.feishu_static_card import ( build_text_delivery, resolve_markdown_images, upload_public_image_to_feishu, )
from channel.feishu.feishu_progress_card import FeishuProgressState
from channel.feishu.feishu_scheduler_card import ( build_scheduler_card, handle_scheduler_action, tasks_for_receivers, )
...
lark = null  # will be populated on first use via _ensure_lark_imported()


fn _ensure_lark_imported() {
...
fn _print_qr_to_terminal(qr_url) {
    ...
    header = "=" * 60
    banner = [ "", header, "  飞书一键创建应用：请使用 飞书 App 扫描下方二维码", "  （二维码 10 分钟内有效，仅供一次扫描）", header, ]
    footer = [ f"  或点击链接创建: {qr_url}", "  等待扫码...", "", ]
    full = banner + qr_lines + footer
    logger.info("[FeiShu] One-click 飞书应用创建二维码（请用飞书 App 扫码）：\n" + "\n".join(full))


}
fn _persist_feishu_credentials(app_id, app_secret) {
...
fn _register_via_qr_in_terminal() {
    ...
    if not hasattr(lark_mod, "register_app"):
        ...
        logger.error( f"[FeiShu] 当前 lark-oapi 版本 ({installed}) 不支持一键创建应用，需要 >= 1.5.5。" "请执行 pip install -U 'lark-oapi>=1.5.5' 后重试，或手动在 config.json 中填入凭据。" )
        return false

    logger.info("[FeiShu] 检测到尚未配置 feishu_app_id / feishu_app_secret，" "正在向飞书申请一键创建应用...")
    ...
    try {
        result = lark_mod.register_app( on_qr_code=_on_qr, on_status_change=_on_status, source="mocode-cli", )
    } catch Exception as e {
    ...
    if not _persist_feishu_credentials(app_id, app_secret):
        logger.error( "[FeiShu] 应用创建成功但写入 config.json 失败，请手动复制以下值到配置文件：\n" f"        feishu_app_id     = {app_id}\n" f"        feishu_app_secret = {app_secret}" )
        return false
...
@singleton
class FeiShuChanel extends ChatChannel {
    ...
    fn FeiShuChanel() {
        super().__init__()
        # 历史消息id暂存，用于幂等控制
        this.receivedMsgs = ExpiredDict(60 * 60 * 7.1)
        # Route recall events back to the session that accepted the message.
        this._message_sessions = ExpiredDict(60 * 60 * 7.1)
        this._http_server = null
        this._ws_client = null
        this._ws_thread = null
        this._bot_open_id = null  # cached bot open_id for @-mention matching
        logger.debug("[FeiShu] app_id={}, app_secret={}, verification_token={}, event_mode={}".format( this.feishu_app_id, this.feishu_app_secret, this.feishu_token, this.feishu_event_mode))
        # 无需群校验和前缀
        conf()["group_name_white_list"] = ["ALL_GROUP"]
        conf()["single_chat_prefix"] = [""]

        # 验证配置
        if this.feishu_event_mode == 'websocket':
            try {
    ...
    fn startup() {
        this.feishu_app_id = conf().get('feishu_app_id')
        this.feishu_app_secret = conf().get('feishu_app_secret')
        this.feishu_token = conf().get('feishu_token')
        this.feishu_event_mode = conf().get('feishu_event_mode', 'websocket')
        ...
        if not this.feishu_app_id or not this.feishu_app_secret:
        ...
        this._fetch_bot_open_id()
        if this.feishu_event_mode == 'websocket':
            this._startup_websocket()
        else:
            this._startup_webhook()

    }
    fn _fetch_bot_open_id() {
        ...
        try {
            ...
            if resp.status_code == 200:
                data = resp.json()
                if data.get("code") == 0:
                    this._bot_open_id = data.get("bot", {}).get("open_id")
    ...
    fn stop() {
        ...
        ws_client = this._ws_client
        this._ws_client = null
        ws_thread = this._ws_thread
        this._ws_thread = null
    ...
    fn _startup_websocket() {
        ...
        fn handle_message_recalled_event(data) {
            """Cancel only the task created by the recalled Feishu message."""
            try {
                logger.info("[FeiShu] websocket received message recall event")
                event_dict = json.loads(lark.JSON.marshal(data))
                this._handle_message_recalled_event(event_dict.get("event", {}))
            } catch Exception as e {
                logger.error( f"[FeiShu] websocket handle message recall error: {e}", exc_info=true, )

            }
        }
        fn handle_card_action(data) {
            """Handle Card 2.0 button callbacks and update the card in place."""
            try {
                event_dict = json.loads(lark.JSON.marshal(data))
                response = this._handle_card_action_event(event_dict.get("event", {}))
                return P2CardActionTriggerResponse(response)
            } catch Exception as e {
                logger.error(f"[FeiShu] websocket handle card action error: {e}", exc_info=true)
                return P2CardActionTriggerResponse( {"toast": {"type": "error", "content": "Task update failed"}} )

        # 构建事件分发器
            }
        }
        event_handler = ( lark.EventDispatcherHandler.builder("", "") .register_p2_im_message_receive_v1(handle_message_event) .register_p2_im_message_recalled_v1(handle_message_recalled_event) .register_p2_card_action_trigger(handle_card_action) .build() )

        fn start_client_with_retry() {
            ...
            fn create_unverified_context(*args, **kwargs) {
            ...
            for attempt in range(2):
                try {
                    ...
                    ws_client = lark.ws.Client( this.feishu_app_id, this.feishu_app_secret, event_handler=event_handler, log_level=lark.LogLevel.WARNING )
            ...
            if startup_error:
                this.report_startup_error(startup_error)
            try {
                loop.close()
            } catch Exception as e {
                pass
    ...
    fn _is_mention_bot(mentions) {
        ...
        """
        if this._bot_open_id:
            return any( m.get("id", {}).get("open_id") == this._bot_open_id for m in mentions )
    ...
    fn _send_scheduler_card(feishu_msg, is_group, receive_id_type) {
        """Reply to ``/tasks`` with tasks scoped to the current chat."""
        task_store = this._get_scheduler_task_store()
        ...
        headers = { "Authorization": "Bearer " + feishu_msg.access_token, "Content-Type": "application/json", }
        content = json.dumps(card, ensure_ascii=false)

        if is_group and feishu_msg.msg_id:
            url = ( "https://open.feishu.cn/open-apis/im/v1/messages/" f"{feishu_msg.msg_id}/reply" )
            response = requests.post( url, headers=headers, json={"msg_type": "interactive", "content": content}, timeout=(5, 10), )
        else:
            url = "https://open.feishu.cn/open-apis/im/v1/messages"
            response = requests.post( url, headers=headers, params={"receive_id_type": receive_id_type}, json={ "receive_id": feishu_msg.other_user_id, "msg_type": "interactive", "content": content, }, timeout=(5, 10), )

        result = response.json()
        if result.get("code") == 0:
            logger.info("[FeiShu] scheduler card sent")
            return true
        logger.error( "[FeiShu] scheduler card failed, " f"code={result.get('code')}, msg={result.get('msg')}" )
        return false

    }
    fn _handle_card_action_event(event) {
        """Apply a scheduler card action within its chat/operator ownership scope."""
        action = event.get("action") or {}
        value = action.get("value") or {}
        if value.get("mocode-cli") != "scheduler":
            return {}

        context = event.get("context") or {}
        operator = event.get("operator") or {}
        callback_receivers = { receiver for receiver in (context.get("open_chat_id"), operator.get("open_id")) if receiver }
        target_receiver = value.get("receiver")
        allowed_receivers = ( {target_receiver} if target_receiver and target_receiver in callback_receivers else set() )
        response = handle_scheduler_action( value, this._get_scheduler_task_store(), allowed_receivers, )
        logger.info( "[FeiShu] scheduler card action handled, " f"action={value.get('action')}, task_id={value.get('task_id')}" )
        return response

    }
    fn _handle_message_recalled_event(event) {
        ...
        session_id = this._message_sessions.get(message_id)
        if not session_id:
            logger.info( f"[FeiShu] ignored recall for unknown message, message_id={message_id}" )
            return 0, false

        result = this.cancel_message(session_id, message_id)
        this._message_sessions.pop(message_id, null)
        logger.info( "[FeiShu] recalled message cancelled, " f"message_id={message_id}, session_id={session_id}, " f"queued={result[0]}, active={result[1]}" )
        return result
    ...
    fn _handle_message_event(event) {
        ...
        if this.receivedMsgs.get(msg_id):
            logger.warning(f"[FeiShu] repeat msg filtered, msg_id={msg_id}")
            return
        this.receivedMsgs[msg_id] = true
        ...
        if chat_type == "group":
            ...
            if msg.get("mentions") and msg.get("message_type") == "text":
                if not this._is_mention_bot(msg.get("mentions")):
                    return
            # 群聊
            is_group = true
        ...
        # 构造飞书消息对象
        feishu_msg = FeishuMessage(event, is_group=is_group, access_token=this.fetch_access_token())
        if not feishu_msg:
            return

        if feishu_msg.ctype == ContextType.TEXT and feishu_msg.content.strip().lower() == "/tasks":
            if not this._send_scheduler_card(feishu_msg, is_group, receive_id_type):
        ...
        if feishu_msg.ctype == ContextType.FILE:
            try {
                ...
                try {
                    ...
                    this._send(err_reply, this._compose_context( ContextType.TEXT, "", isgroup=is_group, msg=feishu_msg, receive_id_type=receive_id_type, no_need_at=true, ))
                } catch Exception as e {
                    pass
                }
                return
        ...
        context = this._compose_context( feishu_msg.ctype, feishu_msg.content_with_quote(), isgroup=is_group, msg=feishu_msg, receive_id_type=receive_id_type, no_need_at=true )
        if context:
            ...
            this._message_sessions[msg_id] = context["session_id"]
            ...
            if conf().get("feishu_stream_reply", true):
                context["on_event"] = this._make_feishu_stream_callback(context, feishu_msg.access_token)
    ...
    fn send(reply, context) {
        ...
        content_key = "text"
        prepared_content_json = null
        if reply.type == ReplyType.TEXT:
            # Render Markdown text replies as Feishu cards; falls back to plain text automatically.
            delivery_text = resolve_markdown_images( reply.content, lambda url: upload_public_image_to_feishu( url, access_token, ), )
            msg_type, prepared_content_json = build_text_delivery(delivery_text)
        elif reply.type == ReplyType.IMAGE_URL:
            # 图片上传
            reply_content = this._upload_image_url(reply.content, access_token)
            ...
            if is_video:
                # 视频上传（包含duration信息）
                upload_data = this._upload_video_url(reply.content, access_token)
                ...
                content_key = null  # 直接序列化整个对象
            else:
                # 其他文件使用 file 类型
                file_key = this._upload_file_url(reply.content, access_token)
                if not file_key:
            ...
            file_key = this._upload_audio(reply.content, access_token)
            if not file_key:
        ...
        # Build content JSON
        content_json = prepared_content_json or ( json.dumps(reply_content, ensure_ascii=false) if content_key is null else json.dumps({content_key: reply_content}, ensure_ascii=false) )
        ...
        if can_reply:
            ...
            data = { "receive_id": context.get("receiver"), "msg_type": msg_type, "content": content_json }
            res = requests.post(url=url, headers=headers, params=params, json=data, timeout=(5, 10))
        res = res.json()
        if res.get("code") == 0:
            logger.info(f"[FeiShu] send message success")
        elif msg_type == "interactive" and reply.type == ReplyType.TEXT:
            logger.warning( "[FeiShu] Markdown card failed, falling back to text, " f"code={res.get('code')}, msg={res.get('msg')}" )
            fallback_data = { "msg_type": "text", "content": json.dumps({"text": reply.content}, ensure_ascii=false), }
            if can_reply:
                fallback_res = requests.post( url=url, headers=headers, json=fallback_data, timeout=(5, 10), )
            else:
                fallback_data["receive_id"] = context.get("receiver")
                fallback_res = requests.post( url=url, headers=headers, params=params, json=fallback_data, timeout=(5, 10), )
            fallback_body = fallback_res.json()
            if fallback_body.get("code") == 0:
                logger.info("[FeiShu] text fallback sent successfully")
            else:
                logger.error( "[FeiShu] text fallback failed, " f"code={fallback_body.get('code')}, msg={fallback_body.get('msg')}" )
        else:
            logger.error(f"[FeiShu] send message failed, code={res.get('code')}, msg={res.get('msg')}")

    }
    fn _make_feishu_stream_callback(context, access_token) {
        ...
        if conf().get("feishu_detailed_card", true):
            return this._make_feishu_stream_callback_progress(context, access_token)
        return this._make_feishu_stream_callback_plain(context, access_token)

    }
    fn _make_feishu_stream_callback_progress(context, access_token) {
        """
        ...
        fn _create_and_send_card() {
            ...
            try {
                ...
                res = requests.post( create_url, headers=headers, json=create_body, timeout=(5, 10) )
                res_json = res.json()
                if res_json.get("code") != 0:
                    logger.warning( f"[FeiShu] Stream: create card failed " f"(code={res_json.get('code')}, msg={res_json.get('msg')}). " f"本次回复已自动降级为普通文本回复（一次性返回完整内容）。" f"如需开启流式打字机效果与完整 Markdown 渲染，请到飞书开放平台 " f"https://open.feishu.cn/app 给机器人开通 cardkit:card:write 权限" f"（创建与更新卡片）并重新发布版本，同时确保飞书客户端 >= 7.20。" )
                    with lock:
                        disabled[0] = true
                    return
                ...
                content_payload = json.dumps( {"type": "card", "data": {"card_id": cid}}, ensure_ascii=false )
                can_reply = is_group and msg and hasattr(msg, "msg_id") and msg.msg_id
                if can_reply:
                    send_url = ( f"https://open.feishu.cn/open-apis/im/v1/messages/" f"{msg.msg_id}/reply" )
                    send_body = {"msg_type": "interactive", "content": content_payload}
                    send_res = requests.post( send_url, headers=headers, json=send_body, timeout=(5, 10) )
                else:
                    send_url = "https://open.feishu.cn/open-apis/im/v1/messages"
                    params = {"receive_id_type": receive_id_type}
                    send_body = { "receive_id": receiver, "msg_type": "interactive", "content": content_payload, }
                    send_res = requests.post( send_url, headers=headers, params=params, json=send_body, timeout=(5, 10), )
                send_json = send_res.json()
                if send_json.get("code") != 0:
                    logger.warning( f"[FeiShu] Stream: send card failed: {send_json}. 降级为普通文本。" )
                    with lock:
                        disabled[0] = true
                    return
                ...
                with lock:
                    message_id[0] = mid
                logger.info( f"[FeiShu] Stream: card created and sent, " f"card_id={cid}, message_id={mid}" )
            } catch Exception as e {
                logger.warning( f"[FeiShu] Stream: create/send card exception: {e}. 降级为普通文本。" )
                with lock:
                    disabled[0] = true
            } finally {
                with lock:
        ...
        fn _stream_update_text(full_text) {
            """PUT 流式更新文本组件。content 必须是当前组件的全量文本。"""
            with lock:
                cid = card_id[0]
            if not cid:
                return
            url = ( f"https://open.feishu.cn/open-apis/cardkit/v1/cards/" f"{cid}/elements/{ELEMENT_ID}/content" )
            body = { "content": full_text, "sequence": _next_sequence(), }
            try {
                res = requests.put(url, headers=headers, json=body, timeout=(5, 10))
                res_json = res.json()
                if res_json.get("code") != 0:
                    logger.warning( f"[FeiShu] Stream: update text failed: {res_json}" )
            } catch Exception as e {
                logger.warning(f"[FeiShu] Stream: update text exception: {e}")

            }
        }
        fn _update_full_card(streaming) {
            ...
            put_body = { "card": {"type": "card_json", "data": json.dumps(full_card, ensure_ascii=false)}, "sequence": _next_sequence(), }
            try {
                res = requests.put(put_url, headers=headers, json=put_body, timeout=(5, 10))
                res_json = res.json()
                if res_json.get("code") != 0:
                    logger.warning( f"[FeiShu] Stream: full card update failed: {res_json}" )
            } catch Exception as e {
                logger.warning( f"[FeiShu] Stream: full card update exception: {e}" )
        ...
        fn on_event(event) {
            ...
            if event_type == "agent_start":
                with lock:
                    ...
                    if card_id[0] is null and not init_in_flight[0]:
            ...
            if event_type == "message_update":
                ...
                with lock:
                    progress_state.consume(event)
                    has_card = card_id[0] is not null
                if has_card:
                    _drain_push_queue()
                ...
                final_text = resolve_markdown_images( final_text, lambda url: upload_public_image_to_feishu(url, access_token), )
                with lock:
    ...
    fn _make_feishu_stream_callback_plain(context, access_token) {
        """
        基于飞书官方"流式更新卡片"API 实现打字机回复。
        ...
        card_id = [null]  # 当前流式卡片的实体 ID（每段独立）
        message_id = [null]  # 当前卡片发送后的消息 ID（仅日志用）
        # 占位发送是同步进行的，但用一个 in-flight 标记防止并发的多条 message_update
        # 事件各自触发一次创建+发送，导致发出多张卡片。
        init_in_flight = [false]
        # 一旦初始化失败就长期标记为 disabled，本次回复不再尝试任何流式调用
        ...
        fn _build_card_json() {
            ...
            return json.dumps({ "schema": "2.0", "config": { "streaming_mode": true, "summary": {"content": "[正在生成回复...]"}, "streaming_config": { "print_frequency_ms": {"default": print_freq_ms}, "print_step": {"default": print_step}, "print_strategy": print_strategy, }, }, "body": { "elements": [ { "tag": "markdown", "content": "...", "element_id": ELEMENT_ID, } ], },   }, ensure_ascii=false)
        ...
        fn _close_streaming_mode(final_text = "") {
            ...
            full_card = { "schema": "2.0", "config": { "streaming_mode": false, "summary": {"content": preview or " "}, }, "body": { "elements": [ { "tag": "markdown", "content": final_text or " ", "element_id": ELEMENT_ID, } ], }, }
            put_url = f"https://open.feishu.cn/open-apis/cardkit/v1/cards/{cid}"
            put_body = { "card": {"type": "card_json", "data": json.dumps(full_card, ensure_ascii=false)}, "sequence": _next_sequence(), }
            try {
                res = requests.put(put_url, headers=headers, json=put_body, timeout=(5, 10))
                res_json = res.json()
                if res_json.get("code") != 0:
                    logger.warning( f"[FeiShu] Stream: finalize card (close+summary) failed: {res_json}" )
            } catch Exception as e {
                logger.warning( f"[FeiShu] Stream: finalize card exception: {e}" )
    ...
    fn fetch_access_token() {
        ...
        req_body = { "app_id": this.feishu_app_id, "app_secret": this.feishu_app_secret }
    ...
    fn _upload_image_url(img_url, access_token) {
    ...
    fn _get_video_duration(file_path) {
        """
        获取视频时长（毫秒）
    ...
    fn _upload_video_url(video_url, access_token) {
        """
        ...
        try {
            ...
            # Get video duration
            duration = this._get_video_duration(local_path)
    ...
    fn _upload_audio(audio_path, access_token) {
        """
        ...
        try {
            with open(upload_path, "rb") as f:
                upload_response = requests.post( upload_url, files={"file": f}, data=data, headers=headers, timeout=(5, 30) )
                logger.info( f"[FeiShu] upload audio response, status={upload_response.status_code}, res={upload_response.content}")
                response_data = upload_response.json()
                if response_data.get("code") == 0:
    ...
    fn _upload_file_url(file_url, access_token) {
        """
        ...
        if file_url.startswith("file://"):
            ...
            file_type_map = { '.opus': 'opus', '.mp4': 'mp4', '.pdf': 'pdf', '.doc': 'doc', '.docx': 'doc', '.xls': 'xls', '.xlsx': 'xls', '.ppt': 'ppt', '.pptx': 'ppt', }
            ...
            try {
                with open(local_path, "rb") as file:
                    upload_response = requests.post( upload_url, files={"file": file}, data=data, headers=headers, timeout=(5, 30)   )
...
class FeishuController {
    ...
    fn POST() {
        try {
            ...
            callback_token = ( header.get("token") or event.get("token") or request.get("token") )
            if callback_token != channel.feishu_token:
                return this.FAILED_MSG

            if event_type == this.CARD_ACTION_TYPE and event:
                return json.dumps( channel._handle_card_action_event(event), ensure_ascii=false, )

            # 3. Handle message events.
            if event_type == this.MESSAGE_RECEIVE_TYPE and event:
                channel._handle_message_event(event)
            elif event_type == this.MESSAGE_RECALLED_TYPE and event:
