...
class ChatChannel(Channel):
    ...
    def _compose_context(self, ctype, content, **kwargs):
        ...
        if ctype == ContextType.TEXT:
            ...
            if ( "desire_rtype" not in context and (conf().get("voice_reply_voice") or conf().get("always_reply_voice")) and ReplyType.VOICE not in self.NOT_SUPPORT_REPLYTYPE ):
                context["desire_rtype"] = ReplyType.VOICE
        return context
    ...
    def _extract_and_send_images(self, reply, context):
        """
    ...
    def _send(self, reply, context, retry_cnt=0):
        try:
