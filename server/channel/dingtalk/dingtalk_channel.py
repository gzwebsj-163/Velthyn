...
class CustomAICardReplier(CardReplier):
    ...
    def start(self, card_template_id, card_data, recipients = None, support_forward = True):
        """
        AI卡片的创建接口
        ...
        return self.create_and_send_card( card_template_id, card_data_with_status, at_sender=True, at_all=False, recipients=recipients, support_forward=support_forward, )
...
class DingTalkChanel(ChatChannel, dingtalk_stream.ChatbotHandler):
    ...
    def __init__(self):
        ...
        logger.debug("[DingTalk] client_id={}, client_secret={} ".format( self.dingtalk_client_id, self.dingtalk_client_secret))
        # 无需群校验和前缀
    ...
    def get_access_token(self):
        ...
        data = { "appKey": self.dingtalk_client_id, "appSecret": self.dingtalk_client_secret }
    ...
    def send_single_message(self, user_id, content, robot_code):
        """
    ...
    def send_group_message(self, conversation_id, content, robot_code = None):
        """
        主动发送群消息
    ...
    def send_image_with_media_id(self, access_token, media_id, incoming_message, is_group):
        """
        发送图片消息（使用 media_id）
    ...
    def send_image_message(self, receiver, media_id, is_group, robot_code):
        """
        发送图片消息
    ...
    def send(self, reply, context):
        ...
        if reply.type == ReplyType.IMAGE_URL:
            ...
            if media_id:
                ...
                access_token = self.get_access_token()
                if access_token:
                    success = self.send_image_with_media_id( access_token, media_id, incoming_message, isgroup )
                    if not success:
            ...
            if is_video:
                ...
                if media_id:
                    ...
                    success = self._send_file_message( access_token, incoming_message, "sampleVideo", msg_param, isgroup )
                    if not success:
                ...
                if media_id:
                    ...
                    success = self._send_file_message( access_token, incoming_message, "sampleFile", msg_param, isgroup )
                    if not success:
        ...
        # Native sampleAudio. Upload only accepts ogg/amr, so convert TTS mp3/wav to amr.
            ...
            success = self._send_file_message( access_token, incoming_message, "sampleAudio", msg_param, isgroup )
            if not success:
    ...
    def _send_file_message(self, access_token, incoming_message, msg_key, msg_param, is_group):
        """
        发送文件/视频消息的通用方法
