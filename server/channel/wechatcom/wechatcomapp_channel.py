...
class WechatComAppChannel(ChatChannel):
    ...
    def __init__(self):
        super().__init__()
        self.corp_id = conf().get("wechatcom_corp_id") or ""
        self.secret = conf().get("wechatcomapp_secret") or ""
        self.agent_id = conf().get("wechatcomapp_agent_id") or ""
        self.token = conf().get("wechatcomapp_token") or ""
        self.aes_key = conf().get("wechatcomapp_aes_key") or ""
        self._http_server = None
        logger.info( "[wechatcom] Initializing WeCom app channel, corp_id: {}, agent_id: {}".format(self.corp_id, self.agent_id) )
