...
class QQChannel(ChatChannel):
    ...
    def _refresh_access_token(self):
        try:
            resp = requests.post( "https://bots.qq.com/app/getAppAccessToken", json={"appId": self.app_id, "clientSecret": self.app_secret}, timeout=10, )
    ...
    def _get_ws_url(self):
        try:
            resp = requests.get( f"{QQ_API_BASE}/gateway", headers=self._get_auth_headers(), timeout=10, )
            resp.raise_for_status()
    ...
    def _start_ws(self):
        ...
        self._ws = websocket.WebSocketApp( ws_url, on_open=_on_open, on_message=_on_message, on_error=_on_error, on_close=_on_close, )
    ...
    def _send_identify(self):
        self._ws_send({ "op": OP_IDENTIFY, "d": { "token": f"QQBot {self._get_access_token()}", "intents": DEFAULT_INTENTS, "shard": [0, 1], "properties": { "$os": "linux", "$browser": "chatgpt-on-wechat", "$device": "chatgpt-on-wechat", }, }, })
        logger.debug(f"[QQ] Identify sent with intents={DEFAULT_INTENTS}")

    def _send_resume(self):
        self._ws_send({ "op": OP_RESUME, "d": { "token": f"QQBot {self._get_access_token()}", "session_id": self._session_id, "seq": self._last_seq, }, })
        logger.debug(f"[QQ] Resume sent: session_id={self._session_id}, seq={self._last_seq}")

    def _start_heartbeat(self, interval_ms):
    ...
    def _handle_msg_event(self, event_data, event_type):
        ...
        context = self._compose_context( qq_msg.ctype, qq_msg.content, isgroup=is_group, msg=qq_msg, no_need_at=True, )
        if context:
    ...
    def send(self, reply, context):
        ...
        if not msg:
            ...
            self._active_send_text(reply.content if reply.type == ReplyType.TEXT else str(reply.content), receiver, is_group)
            return
    ...
    def _get_next_msg_seq(self, msg_id):
        seq = self._msg_seq_counter.get(msg_id, 1)
        self._msg_seq_counter[msg_id] = seq + 1
        return seq

    def _build_msg_url_and_base_body(self, msg, event_type, msg_id):
        ...
        if event_type == "GROUP_AT_MESSAGE_CREATE":
            ...
            body = { "msg_id": msg_id, "msg_seq": self._get_next_msg_seq(msg_id), }
    ...
    def _post_message(self, url, body, event_type):
        try:
    ...
    def _active_send_text(self, content, receiver, is_group):
    ...
    def _send_text(self, content, msg, event_type, msg_id):
    ...
    def _upload_rich_media(self, file_url, file_type, msg, event_type):
        """
        ...
        try:
            resp = requests.post( upload_url, json=upload_body, headers=self._get_auth_headers(), timeout=30, )
    ...
    def _upload_rich_media_base64(self, file_path, file_type, msg, event_type):
    ...
    def _send_media_msg(self, file_info, msg, event_type, msg_id):
    ...
    def _send_image(self, img_path_or_url, msg, event_type, msg_id):
        ...
        if img_path_or_url.startswith(("http://", "https://")):
            file_info = self._upload_rich_media( img_path_or_url, QQ_FILE_TYPE_IMAGE, msg, event_type)
        elif os.path.exists(img_path_or_url):
            file_info = self._upload_rich_media_base64( img_path_or_url, QQ_FILE_TYPE_IMAGE, msg, event_type)
        else:
    ...
    def _send_file(self, file_path_or_url, msg, event_type, msg_id):
        """Send file reply."""
        if event_type not in ("GROUP_AT_MESSAGE_CREATE", "C2C_MESSAGE_CREATE"):
        ...
        if file_path_or_url.startswith(("http://", "https://")):
            file_info = self._upload_rich_media( file_path_or_url, QQ_FILE_TYPE_FILE, msg, event_type)
        elif os.path.exists(file_path_or_url):
            file_info = self._upload_rich_media_base64( file_path_or_url, QQ_FILE_TYPE_FILE, msg, event_type)
        else:
    ...
    def _send_media(self, path_or_url, msg, event_type, msg_id, file_type):
