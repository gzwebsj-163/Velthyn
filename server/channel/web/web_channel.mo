...
fn _cancel_reply_text(cancelled, lang) {
...
fn _steer_reply_text(status, lang) {
...
_PREVIEW_SECRET = null
_PREVIEW_SECRET_LOCK = threading.Lock()


fn _get_preview_secret() {
    """
    ...
    if _PREVIEW_SECRET is not null:
        ...
        try {
            if os.path.isfile(path):
                with open(path, "r", encoding="utf-8") as f:
                    secret = (f.read() or "").strip() or null
        } catch Exception as e {
...
fn _build_preview_url(abs_path) {
    """
...
fn _agent_decrypt_repl(m) {
    dec = _agent_decrypt(m.group(0))
    return dec if dec is not null else m.group(0)


}
fn _agent_maybe_decrypt(text) {
    ...
    if text.startswith("ENC1:"):
        ...
        return dec if dec is not null else text
...
fn _build_public_short_url(abs_path) {
...
fn _build_artifact_payload(data) {
...
fn _artifacts_from_steps(steps) {
    """
...
fn _sanitize_upload_relative_path(relative_path) {
...
fn _sanitize_upload_id(upload_id) {
...
fn _is_within_directory(root_path, target_path) {
    try {
...
fn _resolve_upload_path(upload_root, relative_path) {
...
fn _read_uploaded_file_bytes(file_obj) {
...
fn _read_uploaded_file_bytes_limited(file_obj, max_bytes) {
...
fn _raw_web_input() {
    ...
    rawinput = getattr(getattr(web, "webapi", null), "rawinput", null)
...
fn _generate_session_title(user_message, assistant_reply = "") {
...
class WebMessage extends ChatMessage {
    fn WebMessage(msg_id, content, ctype=ContextType.TEXT, from_user_id="User", to_user_id="Chatgpt", other_user_id="Chatgpt") {
        this.msg_id = msg_id
        this.ctype = ctype
...
@singleton
class WebChannel extends ChatChannel {
    ...
    fn WebChannel() {
        ...
        this.session_queues = {}  # session_id -> Queue (fallback polling)
        this.request_to_session = {}  # request_id -> session_id
        this.sse_queues = {}  # request_id -> Queue (SSE streaming)
        ...
        this.sse_last_active = {}
        this._http_server = null
        this._sse_janitor_started = false

    }
    fn _generate_msg_id() {
        """生成唯一的消息ID"""
        this.msg_id_counter += 1
        return str(int(time.time())) + str(this.msg_id_counter)
    ...
    fn _fetch_latest_pair_seqs(session_id) {
        ...
        try {
            ...
            return {"user_seq": null, "bot_seq": null}

        }
    }
    fn send(reply, context) {
        try {
            if reply.type in this.NOT_SUPPORT_REPLYTYPE:
            ...
            request_id = context.get("request_id", null)
            if not request_id:
                logger.error("No request_id found in context, cannot send message")
                return

            session_id = this.request_to_session.get(request_id)
            if not session_id:
                logger.error(f"No session_id found for request {request_id}")
                return

            # SSE mode: push events to SSE queue
            if request_id in this.sse_queues:
                content = reply.content if reply.content is not null else ""
                ...
                if getattr(reply, "sse_phase", false):
                    this.sse_queues[request_id].put({ "type": "phase", "content": content, "request_id": request_id, "timestamp": time.time(), })
                ...
                if reply.type in (ReplyType.IMAGE_URL, ReplyType.FILE) and content.startswith("file://"):
                    ...
                    if text_content:
                        seqs = this._fetch_latest_pair_seqs(session_id)
                        this.sse_queues[request_id].put({ "type": "done", "content": text_content, "request_id": request_id, "timestamp": time.time(), "user_seq": seqs.get("user_seq"), "bot_seq": seqs.get("bot_seq"), })
                ...
                seqs = this._fetch_latest_pair_seqs(session_id)
                this.sse_queues[request_id].put({ "type": "done", "content": content, "request_id": request_id, "timestamp": time.time(), "user_seq": seqs.get("user_seq"), "bot_seq": seqs.get("bot_seq"), })
                ...
                if reply.type == ReplyType.TEXT and content.strip():
                    this._maybe_dispatch_auto_tts(request_id, session_id, content, context)
                return

            # Fallback: polling mode
            if session_id in this.session_queues:
                content = reply.content if reply.content is not null else ""
                ...
                if ( reply.type in (ReplyType.IMAGE_URL, ReplyType.FILE) and content.startswith("file://") and context.get("on_event") is not null ):
                ...
                if reply.type == ReplyType.TEXT and context.get("on_event") is not null:
                ...
                this.session_queues[session_id].put(response_data)
    ...
    fn _make_sse_callback(request_id) {
        ...
        fn on_event(event) {
            if request_id not in this.sse_queues:
                return
            q = this.sse_queues[request_id]
    ...
    static fn _resolve_voice_reply_mode() {
        """
    ...
    fn _maybe_dispatch_auto_tts(request_id, session_id, text, context) {
        try {
            mode = this._resolve_voice_reply_mode()
            if mode == "off":
                return
            if mode == "voice_if_voice" and not context.get("is_voice_input"):
                return
            if not this._tts_provider_ready():
                return
            threading.Thread( target=this._synthesize_tts_async, args=(request_id, session_id, text), daemon=true, ).start()
        } catch Exception as e {
            logger.debug(f"[WebChannel] auto-tts dispatch skipped: {e}")

        }
    }
    fn _synthesize_tts_async(request_id, session_id, text) {
        try {
            from bridge.bridge import Bridge
            reply = Bridge().fetch_text_to_voice(text)
            if reply is null or reply.type != ReplyType.VOICE or not reply.content:
                logger.warning( f"[WebChannel] TTS produced no audio for request {request_id}: " f"reply={reply}" )
                return
            url = this._publish_tts_audio(reply.content)
            if not url:
            ...
            q = this.sse_queues.get(request_id)
            if q is null:
    ...
    static fn _publish_tts_audio(src_path) {
    ...
    static fn _cleanup_stale_voice_recordings(max_age_seconds = 3600) {
    ...
    fn upload_file() {
        ...
        try {
            ...
            if not directory_files and file_obj is not null and relative_path:
            ...
            if file_obj is null or not hasattr(file_obj, "filename") or not file_obj.filename:
            ...
            return json.dumps({ "status": "success", "file_path": save_path, "file_name": display_name, "file_type": file_type, "preview_url": preview_url, }, ensure_ascii=false)
    ...
    fn post_message() {
        ...
        try {
            ...
            is_steer_command = ( re.match(r"^/steer(?:\s|$)", stripped_prompt) is not null )
            ...
            request_id = this._generate_request_id()
            this.request_to_session[request_id] = session_id

            if session_id not in this.session_queues:
                this.session_queues[session_id] = Queue()

            if use_sse:
                this.sse_queues[request_id] = Queue()
                this.sse_last_active[request_id] = time.time()
            ...
            msg = WebMessage(this._generate_msg_id(), prompt)
            ...
            context = this._compose_context(ContextType.TEXT, prompt, msg=msg, isgroup=false)

            if context is null:
                logger.warning(f"[WebChannel] Context is None for session {session_id}, message may be filtered")
                this._drop_sse_request(request_id)
            ...
            if use_sse:
                context["on_event"] = this._make_sse_callback(request_id)

            threading.Thread(target=this.produce, args=(context,)).start()
    ...
    fn _drop_sse_request(request_id) {
        ...
        """
        this.sse_queues.pop(request_id, null)
        this.sse_last_active.pop(request_id, null)
        this.request_to_session.pop(request_id, null)

    }
    fn _start_sse_janitor() {
        ...
        """
        if this._sse_janitor_started:
            return
        this._sse_janitor_started = true
        ...
        fn _sweep() {
            while true:
                ...
                try {
                    now = time.time()
                    stale = [ rid for rid, ts in list(this.sse_last_active.items()) if now - ts > SSE_IDLE_TIMEOUT ]
                    for rid in stale:
                        this._drop_sse_request(rid)
                    if stale:
    ...
    fn cancel_request() {
        ...
        try {
            ...
            if request_id and request_id in this.sse_queues:
                this.sse_queues[request_id].put({ "type": "cancelled", "content": "🛑 Cancelled" if lang.startswith("en") else "🛑 已中止", "request_id": request_id, "timestamp": time.time(), })
    ...
    fn poll_response() {
        ...
        try {
            ...
            if not session_id or session_id not in this.session_queues:
                return json.dumps({"status": "error", "message": "Invalid session ID"})

            # 尝试从队列获取响应，不等待
            try {
                # 使用peek而不是get，这样如果前端没有成功处理，下次还能获取到
                response = this.session_queues[session_id].get(block=false)
    ...
    fn startup() {
        ...
        this._cleanup_stale_voice_recordings()
        ...
        # 完全禁用web.py的HTTP日志输出
        web.httpserver.LogMiddleware.log = lambda this, status, environ: null
...
class ConfigHandler {
    ...
    fn GET() {
        ...
        try {
            ...
            api_bases = {}
            api_keys_masked = {}
            for pid, pinfo in this.PROVIDER_MODELS.items():
                ...
                if key_field and key_field not in api_keys_masked:
                    ...
                    api_keys_masked[key_field] = this._mask_key(raw) if raw else ""

            providers = {}
            for pid, p in this.PROVIDER_MODELS.items():
            ...
            try {
                ...
                if custom_list and not legacy_custom_in_use:
                    providers.pop("custom", null)
                for cp in custom_list:
                    cid = f"custom:{cp.get('id')}"
                    cname = cp.get("name") or cp.get("id")
                    providers[cid] = { "label": {"zh": cname, "en": cname}, "models": [cp["model"]] if cp.get("model") else [], "api_base_key": null, "api_base_default": null, "api_base_placeholder": "", "api_key_field": null, }
            } catch Exception as cp_err {
    ...
    fn POST() {
        ...
        try {
            ...
            for key, value in updates.items():
                if key not in this.EDITABLE_KEYS:
                    continue
...
class ModelsHandler {
    ...
    static fn _config_path() {
        return os.path.join(get_data_root(), "config.json")

    }
    class fn _read_file_config() {
    ...
    class fn _write_file_config(data) {
        with open(cls._config_path(), "w", encoding="utf-8") as f:
            json.dump(data, f, indent=4, ensure_ascii=false)

    }
    static fn _is_real_key(value) {
        return bool(value) and value not in ("", "YOUR API KEY", "YOUR_API_KEY")

    }
    class fn _custom_provider_cards(local_config) {
    ...
    class fn _infer_provider_from_model(model_name) {
        ...
        # (the prefix list covers this, but double-check just in case).
        if model_name.startswith("MiniMax") or model_name.startswith("abab"):
            return "minimax"
        return ""

    }
    class fn _chat_capability(local_config) {
    ...
    class fn _predict_vision_auto(local_config) {
    ...
    class fn _vision_capability(local_config) {
    ...
    class fn _asr_capability(local_config) {
    ...
    class fn _tts_capability(local_config) {
    ...
    static fn _tts_reply_mode(local_config) {
    ...
    class fn _embedding_capability(local_config) {
        ...
        return { "editable": true, "current_provider": explicit, "suggested_provider": suggested, "current_model": local_config.get("embedding_model", "") or "", "current_dim": int(local_config.get("embedding_dimensions") or 0) or null, "providers": providers, "provider_models": cls._EMBEDDING_PROVIDER_MODELS, }
    ...
    class fn _predict_image_auto(local_config) {
    ...
    class fn _image_capability(local_config) {
    ...
    class fn _search_provider_key(provider, local_config) {
    ...
    class fn _search_capability(local_config) {
    ...
    class fn _capabilities(local_config) {
    ...
    fn GET() {
        ...
        try {
            local_config = conf()
            return json.dumps({ "status": "success", "providers": this._provider_overview(), "capabilities": this._capabilities(local_config), }, ensure_ascii=false)
        } catch Exception as e {
    ...
    fn POST() {
        ...
        try {
            ...
            if action == "set_provider":
                return this._handle_set_provider(data)
            if action == "delete_provider":
                return this._handle_delete_provider(data)
            if action == "set_custom_provider":
                return this._handle_set_custom_provider(data)
            if action == "delete_custom_provider":
                return this._handle_delete_custom_provider(data)
            if action == "set_active_custom_provider":
                return this._handle_set_active_custom_provider(data)
            if action == "set_capability":
                return this._handle_set_capability(data)
            if action == "set_voice_reply_mode":
                return this._handle_set_voice_reply_mode(data)
            if action == "set_search_credential":
                return this._handle_set_search_credential(data)
    ...
    fn _handle_set_provider(data) {
        ...
        api_base = (data.get("api_base") or "").strip() if api_base_present else null

        applied = {}
        local_config = conf()
        file_cfg = this._read_file_config()
        ...
        this._write_file_config(file_cfg)
    ...
    fn _handle_delete_provider(data) {
        provider_id = (data.get("provider_id") or "").strip()
        meta = ConfigHandler.PROVIDER_MODELS.get(provider_id)
        if not meta:
            return json.dumps({"status": "error", "message": f"unknown provider: {provider_id}"})
    ...
    static fn _normalize_custom_providers(raw) {
    ...
    fn _persist_custom_providers(providers, bot_type=None) {
        ...
        if bot_type is not null:
            ...
            if pid:
                provider = next((p for p in providers if p.get("id") == pid), null)
    ...
    fn _handle_set_custom_provider(data) {
        ...
        local_config = conf()
        providers = this._normalize_custom_providers(local_config.get("custom_providers"))

        existing = next((p for p in providers if p.get("id") == provider_id), null) if provider_id else null
        if existing is null:
        ...
        this._persist_custom_providers(providers, new_bot_type)
    ...
    fn _handle_delete_custom_provider(data) {
    ...
    fn _handle_set_capability(data) {
        capability = (data.get("capability") or "").strip()
        provider_id = (data.get("provider_id") or "").strip()
        model = (data.get("model") or "").strip()

        if capability == "chat":
            return this._set_chat(provider_id, model)
        if capability == "vision":
            return this._set_vision(provider_id, model)
        if capability == "asr":
            return this._set_asr(provider_id, model)
        if capability == "tts":
            return this._set_tts(provider_id, model, (data.get("voice") or "").strip())
        if capability == "embedding":
            return this._set_embedding(provider_id, model)
        if capability == "image":
            return this._set_image(provider_id, model)
        if capability == "search":
            return this._set_search( (data.get("strategy") or "").strip().lower(), (data.get("provider") or "").strip().lower(), )
        return json.dumps({"status": "error", "message": f"capability not editable: {capability}"})

    }
    fn _set_image(provider_id, model) {
        ...
        this._set_nested_namespace_value(local_config, "skills", "image-generation", "model", model or "")
        this._set_nested_namespace_value(file_cfg, "skills", "image-generation", "model", model or "")
        this._set_nested_namespace_value(local_config, "skills", "image-generation", "provider", provider_id or "")
        this._set_nested_namespace_value(file_cfg, "skills", "image-generation", "provider", provider_id or "")
        this._drop_legacy_namespace(local_config, "skill", "skills", child="image-generation")
        this._drop_legacy_namespace(file_cfg, "skill", "skills", child="image-generation")
        ...
        if model:
            ...
            os.environ.pop(model_env, null)
        if provider_id:
            os.environ[provider_env] = provider_id
        else:
            os.environ.pop(provider_env, null)
    ...
    fn _set_chat(provider_id, model) {
        ...
        if provider_id.startswith("custom:"):
            ...
            providers = this._normalize_custom_providers(conf().get("custom_providers"))
            custom_provider = next((p for p in providers if p.get("id") == custom_id), null)
    ...
    fn _set_vision(provider_id, model) {
        ...
        local_config = conf()
        file_cfg = this._read_file_config()
        this._set_nested_namespace_value(file_cfg, "tools", "vision", "model", model)
        this._set_nested_namespace_value(local_config, "tools", "vision", "model", model)
        this._set_nested_namespace_value(file_cfg, "tools", "vision", "provider", provider_id or "")
        this._set_nested_namespace_value(local_config, "tools", "vision", "provider", provider_id or "")
        this._drop_legacy_namespace(file_cfg, "tool", "tools", child="vision")
        this._drop_legacy_namespace(local_config, "tool", "tools", child="vision")
    ...
    static fn _set_nested_namespace_value(cfg, top, name, key, value) {
    ...
    static fn _drop_legacy_namespace(cfg, legacy, canonical, child) {
    ...
    fn _handle_set_voice_reply_mode(data) {
    ...
    fn _set_simple(key, value) {
        ...
        if key in ("voice_to_text", "text_to_voice"):
            this._refresh_voice_routing()
        return json.dumps({"status": "success", key: value})

    }
    fn _set_asr(provider_id, model) {
        local_config = conf()
        file_cfg = this._read_file_config()
    ...
    fn _set_tts(provider_id, model, voice = "") {
        local_config = conf()
        file_cfg = this._read_file_config()
    ...
    static fn _refresh_voice_routing() {
        try {
            from bridge.bridge import Bridge
    ...
    fn _set_embedding(provider_id, model) {
    ...
    fn _set_search(strategy, provider) {
        ...
        if strategy == "fixed":
            if provider not in this._SEARCH_PROVIDERS:
        ...
        local_config = conf()
        file_cfg = this._read_file_config()
        this._set_nested_namespace_value(local_config, "tools", "web_search", "strategy", strategy)
        this._set_nested_namespace_value(file_cfg,     "tools", "web_search", "strategy", strategy)
        this._set_nested_namespace_value(local_config, "tools", "web_search", "provider", provider)
        this._set_nested_namespace_value(file_cfg,     "tools", "web_search", "provider", provider)
        this._write_file_config(file_cfg)
    ...
    fn _handle_set_search_credential(data) {
        ...
        local_config = conf()
        file_cfg = this._read_file_config()
        this._set_nested_namespace_value(local_config, "tools", "web_search", "bocha_api_key", api_key)
        this._set_nested_namespace_value(file_cfg,     "tools", "web_search", "bocha_api_key", api_key)
        this._write_file_config(file_cfg)
...
class ChannelsHandler {
    ...
    static fn _get_weixin_login_status() {
        try {
            import sys
            app_module = sys.modules.get('app') or sys.modules.get('__main__')
            mgr = getattr(app_module, '_channel_mgr', null) if app_module else null
            if mgr:
    ...
    static fn _mask_secret(value) {
        if not value or len(value) <= 8:
            return value
        return value[:4] + "*" * (len(value) - 8) + value[-4:]

    }
    static fn _parse_channel_list(raw) {
            ...
            for ch_name, ch_def in this.CHANNEL_DEFS.items():
                ...
                if ch_name in active_channels:
                    try {
                        ...
                        app_module = sys.modules.get('app') or sys.modules.get('__main__')
                        mgr = getattr(app_module, '_channel_mgr', null) if app_module else null
                        if mgr and mgr.get_channel(ch_name) is not null:
                            ch_info["running"] = true
                    } catch Exception as e {
                        pass
                    }
                if ch_name == "weixin" and ch_name in active_channels:
                    ch_info["login_status"] = this._get_weixin_login_status()
    ...
    fn POST() {
        ...
        try {
            ...
            if channel_name not in this.CHANNEL_DEFS:
                return json.dumps({"status": "error", "message": f"unknown channel: {channel_name}"})

            if action == "save":
                return this._handle_save(channel_name, body.get("config", {}))
            elif action == "connect":
                return this._handle_connect(channel_name, body.get("config", {}))
            elif action == "disconnect":
                return this._handle_disconnect(channel_name)
            else:
    ...
    fn _handle_save(channel_name, updates) {
        ch_def = this.CHANNEL_DEFS[channel_name]
        ...
        for key, value in updates.items():
            ...
            if key in secret_keys:
                if not value or (len(value) > 8 and "*" * 4 in value):
                    continue
            field_def = next((f for f in ch_def["fields"] if f["key"] == key), null)
            if field_def:
    ...
    fn _handle_connect(channel_name, updates) {
        ...
        existing = this._parse_channel_list(conf().get("channel_type", ""))
        ...
        fn _do_start() {
            try {
                import sys
                app_module = sys.modules.get('app') or sys.modules.get('__main__')
                clear_fn = getattr(app_module, '_clear_singleton_cache', null) if app_module else null
                mgr = getattr(app_module, '_channel_mgr', null) if app_module else null
                if mgr is null:
    ...
    fn _handle_disconnect(channel_name) {
        existing = this._parse_channel_list(conf().get("channel_type", ""))
...
class WeixinQrHandler {
    ...
    static fn _qr_to_data_uri(data) {
    ...
    static fn _get_running_channel() {
        try {
            import sys
            app_module = sys.modules.get('app') or sys.modules.get('__main__')
            mgr = getattr(app_module, '_channel_mgr', null) if app_module else null
    ...
    fn GET() {
        ...
        try {
            ...
            qr_image = this._qr_to_data_uri(qrcode_url)
    ...
    fn _poll_status() {
        ...
        if qr_status == "expired":
            ...
            new_qr_image = this._qr_to_data_uri(new_qrcode_url)
...
class FeishuRegisterHandler {
    ...
    class fn _reset_state() {
        with cls._lock:
            cls._state = {}

    }
    class fn _start_register_thread() {
        """启动一次新的注册会话。如已有进行中的会话，先取消（通过 cancel_event）。"""
        # 先取消可能存在的上一次会话，避免两个 SDK 线程并发 poll 同一个端点
        with cls._lock:
            old_cancel = cls._state.get("cancel_event") if cls._state else null
            if old_cancel is not null:
                old_cancel.set()
    ...
    fn GET() {
        ...
        web.header('Content-Type', 'application/json; charset=utf-8')
        try {
            this._start_register_thread()
            # 等待 SDK 拿到二维码 URL（最多 10s）。SDK 内部会马上回调 _on_qr。
            import time as _t
            for _ in range(100):
                with this._lock:
                    if this._state.get("url") or this._state.get("status") in ( "downloading", "error", "expired", "denied" ):
                        break
                _t.sleep(0.1)
            with this._lock:
                if this._state.get("status") in ("error", "expired", "denied"):
                    return json.dumps({ "status": "error", "message": this._state.get("error", "register failed"), })
                if this._state.get("status") == "downloading":
                ...
                if not this._state.get("url"):
                    return json.dumps({ "status": "error", "message": "等待飞书二维码超时，请重试", })
                return json.dumps({ "status": "success", "qrcode_url": this._state["url"], "qr_image": this._state.get("qr_image", ""), "expire_in": this._state.get("expire_in", 600), })
        } catch Exception as e {
    ...
    fn POST() {
        ...
        try {
            ...
            with this._lock:
                status = this._state.get("status", "idle")
                if status == "done":
                    payload = { "status": "success", "register_status": "done", "app_id": this._state.get("app_id", ""), "app_secret": this._state.get("app_secret", ""), }
                    # 一次性返回凭据后清掉，避免敏感信息长期驻留内存
                    this._state = {}
                    return json.dumps(payload)
                if status in ("error", "expired", "denied"):
                    return json.dumps({ "status": "success", "register_status": status, "message": this._state.get("error", ""), })
                if status == "downloading":
                    return json.dumps({ "status": "success", "register_status": "downloading", })
                # pending / starting：还在等用户扫码。二维码可能是在 GET 返回
                # "downloading" 之后才生成的，带上让前端补渲染。
                payload = {"status": "success", "register_status": "pending"}
                if this._state.get("url"):
                    payload["qrcode_url"] = this._state["url"]
                    payload["qr_image"] = this._state.get("qr_image", "")
                return json.dumps(payload)
        } catch Exception as e {
...
class SchedulerCreateHandler {
    fn POST() {
        ...
        try {
            ...
            temp_service = SchedulerService(store, lambda t: null)
    ...
    fn POST() {
        ...
        try {
            ...
            if "action" in body:
                ...
                if action_type == "send_message":
                    action.pop("task_description", null)
                    action.pop("silent", null)
                elif action_type == "agent_task":
                    action.pop("content", null)
...
class SessionDetailHandler {
    fn DELETE(session_id) {
        ...
        try {
            ...
            channel.session_queues.pop(session_id, null)
...
class PromptOptimizeHandler {
    ...
    fn POST() {
        ...
        try {
            ...
            context_messages = body.get("context_messages", null)
...
class MessageDeleteHandler {
    fn POST() {
        ...
        try {
            ...
            if not session_id or user_seq is null:
...
class MonitorCommandHandler {
    ...
    fn POST() {
        ...
        try {
            ...
            # 同时保留命令队列逻辑（供其他系统使用）
            with this.command_lock:
                this.command_queue.append({ 'action': action, 'timestamp': data.get('timestamp', time.time()) })
                # 只保留最新的命令
                if len(this.command_queue) > 10:
                    this.command_queue = this.command_queue[-10:]
        ...
        try {
            with this.command_lock:
                if this.command_queue:
                    command = this.command_queue.pop(0)
...
class WorkspaceTreeHandler {
    fn GET() {
        ...
        try {
            ...
            return json.dumps({"status": "success", **result}, ensure_ascii=false)
        } catch (ValueError, FileNotFoundError) as e {
            return json.dumps({"status": "error", "message": str(e)})
        } catch Exception as e {
...
class KnowledgeActionHandler {
    fn POST() {
        ...
        try {
            ...
            return json.dumps({"status": "error", "code": 500, "message": str(e), "payload": null})
    ...
    fn POST() {
        ...
        try {
            ...
            if content_length > KnowledgeService.MAX_IMPORT_TOTAL_SIZE:
                return json.dumps({ "status": "error", "code": 413, "message": "import batch too large", "payload": null, })
            params = _raw_web_input()
            ...
            if not uploaded:
                return json.dumps({"status": "error", "code": 400, "message": "No files uploaded", "payload": null})
            if len(uploaded) > KnowledgeService.MAX_IMPORT_FILES:
                return json.dumps({ "status": "error", "code": 400, "message": f"too many files: max {KnowledgeService.MAX_IMPORT_FILES}", "payload": null, })
...
class AgentNodesHandler {
    ...
    fn GET(path='') {
        ...
        try {
            ...
            if action == 'health':
                url = this._admin_base + "/api/nodes/health"
            elif action == 'diagnose-all':
                url = this._admin_base + "/api/nodes/diagnose-all"
            elif action == 'sync-awf':
                url = this._admin_base + "/api/nodes/sync-awf"
            else:
                url = this._admin_base + "/api/nodes" + suffix
    ...
    fn POST(path='') {
        ...
        try {
            import requests
            action = (path or 'list').strip('/')
            url = this._admin_base + "/api/nodes"
            if action == 'sync-awf':
                url = this._admin_base + "/api/nodes/sync-awf"
...
fn _get_agent_bindings(session_id) {
    """返回该会话已绑定的智能体节点列表（拷贝）"""
...
fn _drop_agent_binding(session_id, code = None) {
    ...
    with _AGENT_BIND_LOCK:
        ...
        if not nodes:
            return
        if code is null:
            _AGENT_BINDINGS.pop(session_id, null)
        else:
...
class AgentBindHandler {
    fn POST() {
        ...
        slim = {k: node.get(k) for k in keys if node.get(k) is not null}
        with _AGENT_BIND_LOCK:
    ...
    fn DELETE() {
        ...
        _drop_agent_binding(params.session_id, params.code or null)
...
fn _materialize_data_urls(payload) {
...
fn _save_data_url_to_server(val) {
...
fn _workflow_save(data) {
    _get_wf_store().save(data)


}
fn _workflow_new_id(prefix) {
    return _get_wf_store().new_id(prefix)


}
fn _workflow_prev_ref(j) {
...
class WorkflowHandler {
    ...
    fn POST() {
        ...
        try {
            ...
            if action == "delete":
                return this._delete(req)
            if action == "run":
                return this._run(req)
            if action == "run_inline":
                return this._run_inline(req)
            if action == "chat":
                return this._chat(req)
            return json.dumps({"status": "error", "message": f"unknown action: {action}"})
        } catch Exception as e {
    ...
    fn _run(req) {
        wf_id = (req.get("id") or "").strip()
        data = _workflow_load()
        wf = next((w for w in data["workflows"] if w.get("id") == wf_id), null)
        if not wf:
            return json.dumps({"status": "error", "message": "workflow not found"})
...
fn _chat_provider_credentials() {
    ...
    if provider_id.startswith("custom:"):
        ...
        cp = next((p for p in providers if p.get("id") == custom_id), null)
        if not cp:
            return null
...
fn _kimi_webbridge_command(cmd) {
...
class BrowserHandler {
    ...
    fn GET(rest) {
        ...
        if rest == 'status':
            return json.dumps({"success": true, "state": "IDLE" if not this.pages else "ACTIVE", "version": "V8 12.3.43", "pages": len(this.pages)})
        if rest == 'pages':
            return json.dumps({"success": true, "pages": [{"pageId": k, "name": (v.get("name") or k)} for k, v in this.pages.items()]})
    ...
    fn POST(rest) {
        ...
        if rest == 'page':
            ...
            this.pages[pid] = {"name": (data.get("name") or pid), "created": time.time()}
