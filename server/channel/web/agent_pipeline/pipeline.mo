...
fn _json_safe(obj, depth=0) {
    ...
    if isinstance(obj, (str, int, float, bool)) or obj is null:
        return obj
    return str(obj)


}
fn _first_valid(keys, d) {
    for k in keys:
        v = d.get(k)
        if v is not null:
            return v
...
fn run_agent_pipeline(node_code, payload, invoke_result, workspace=None) {
    """完整管道：加密通道 → 结果加密 → 双桥接 → CodeGenHook 推送"""
    steps = []

    fn add(step, ok, detail) {
...
fn _execute_codegen_hook(lab, ctx) {
