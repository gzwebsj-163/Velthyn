/* =====================================================================
   mocode-cli Console - Main Application Script
   ===================================================================== */
...
const I18N = {
    zh: {
        console: '控制台',
        nav_chat: '对话', nav_manage: '管理', nav_monitor: '监控', nav_expert: '专家', nav_work: '工作', nav_prompt: 'Prompt',
        menu_chat: '对话', menu_config: '配置', menu_models: '模型', menu_skills: '技能',
        menu_memory: '记忆', menu_knowledge: '知识', menu_channels: '通道', menu_tasks: '定时',
        menu_logs: '日志', menu_expert: '系统', menu_protocol: '协议须知', menu_agent: '智能体', menu_workflow: '工作流', menu_wflow_canvas: '视图模式', menu_prompt: 'Prompt 仓库',
        workflow_title: '工作流',
        workflow_desc: '任务编排 · 顺序执行 AWF 节点 · 历史与结果',
        wflow_canvas_title: '视图模式',
        wflow_canvas_desc: '节点分布式规划 · 连线方式编排工作流',
        agent_title: '智能体',
        agent_desc: 'agent',
        agent_offline_title: '节点管理服务不可用',
        agent_offline_desc: 'agetnt admin 服务 (localhost:8088) 当前不可用，请先启动 admin 服务后重试。',
        agent_retry: '刷新',
        models_title: '模型管理',
        ...
        task_receiver: '接收会话',
        task_receiver_hint: '选择接收本次定时任务执行结果的会话',
    ...
    'zh-Hant': {
        ...
        console: '控制台',
        nav_chat: '對話', nav_manage: '管理', nav_monitor: '監控', nav_work: '工作',
        menu_chat: '對話', menu_config: '設定', menu_models: '模型', menu_skills: '技能',
        menu_memory: '記憶', menu_knowledge: '知識', menu_channels: '管道', menu_tasks: '定時',
        menu_logs: '日誌', menu_workflow: '工作流', menu_wflow_canvas: '視圖模式',
        workflow_title: '工作流',
        workflow_desc: '任務編排 · 順序執行 AWF 節點 · 歷史與結果',
        wflow_canvas_title: '視圖模式',
        wflow_canvas_desc: '節點分散式規劃 · 連線方式編排工作流',
        models_title: '模型管理',
        ...
        task_receiver: '接收會話',
        task_receiver_hint: '選擇接收本次定時任務執行結果的會話',
    ...
    en: {
        console: 'Console',
        nav_chat: 'Chat', nav_manage: 'Management', nav_monitor: 'Monitor', nav_expert: 'Expert', nav_work: 'Work', nav_prompt: 'Prompt',
        ...
        menu_logs: 'Logs', menu_expert: 'Expert System', menu_protocol: 'Protocol Guide', menu_agent: 'Agent', menu_workflow: 'Workflow', menu_wflow_canvas: 'Canvas View', menu_prompt: 'Prompt Repo',
        workflow_title: 'Workflow',
        workflow_desc: 'Task orchestration · sequential AWF nodes · history & results',
        wflow_canvas_title: 'Canvas View',
        wflow_canvas_desc: 'Distributed node planning · connect lines to orchestrate',
        agent_title: 'Agent',
        agent_desc: 'Node Management · agetnt admin (localhost:8088)',
        agent_offline_title: 'Node Management Service Offline',
        agent_offline_desc: 'agetnt admin service (localhost:8088) is currently unavailable. Start the admin service and retry.',
        agent_retry: 'Refresh',
        models_title: 'Models',
        ...
        task_receiver: 'Receiver session',
        task_receiver_hint: 'Choose the session that receives this scheduled task result',
...
function applyI18n() {
    ...
    if (docsLink) docsLink.href = currentLang === 'zh' ? 'https://docs.mocode-cli.ai/zh' : 'https://docs.mocode-cli.ai';
...
const VIEW_META = {
    ...
    agent:    { group: 'nav_chat',    page: 'menu_agent' },
    ...
    prompt:   { group: 'nav_prompt',  page: 'menu_prompt' },
    logs:     { group: 'nav_monitor', page: 'menu_logs' },
    'expert': { group: 'nav_expert', page: 'menu_expert' },
    'protocol': { group: 'nav_expert', page: 'menu_protocol' },
    'workflow': { group: 'nav_work', page: 'menu_workflow' },
    'wflow-canvas': { group: 'nav_work', page: 'menu_wflow_canvas' },
};
...
function navigateTo(viewId) {
    ...
    // 离开对话视图时关闭历史会话面板，避免在智能体/管理/监控等
    // 视图里残留展示历史会话
    if (viewId !== 'chat') closeSessionPanel();

    // three.js 3D 背景：进入画布视图时初始化，离开时释放
    if (viewId === 'wflow-canvas') {
        // 自动展开工作流二级菜单
        const wfParent = document.querySelector('.wflow-parent');
        if (wfParent) {
            const wfSub = wfParent.querySelector('.wflow-submenu');
            if (wfSub) wfSub.classList.remove('hidden');
            const wfIcon = wfParent.querySelector('.wflow-toggle i');
            if (wfIcon) wfIcon.style.transform = 'rotate(90deg)';
        }
        cvSyncLoad();  // 每次进入视图模式都刷新任务下拉，同步最新工作流
        cvInitThreeBG();
    } else if (cvThree) cvDisposeThreeBG();
...
// 工作流二级菜单（视图模式）展开/收起
document.querySelectorAll('.wflow-toggle').forEach(btn => {
    btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const sub = btn.closest('.wflow-parent').querySelector('.wflow-submenu');
        if (!sub) return;
        const expanded = sub.classList.toggle('hidden');
...
let appConfig = { use_agent: false, title: 'mocode-cli', subtitle: '', providers: {}, api_bases: {} };
...
fetch('/config').then(r => r.json()).then(data => {
    if (data.status === 'success') {
        appConfig = data;
        const title = data.title || 'mocode-cli';
...
// Pending attachments: [{file_path, file_name, file_type, preview_url, raw_url}]
// raw_url 为本地同源预览地址（优先使用），preview_url 为公网短链（兜底/分享）。
    ...
    attachmentPreview.innerHTML = pendingAttachments.map((att, idx) => {
        ...
        if (att.file_type === 'image') {
            return `<div class="att-thumb" data-idx="${idx}">
                <img src="${att.raw_url || att.preview_url}" alt="${escapeHtml(att.file_name)}">
...
// ===== 加入智能体菜单（输入框左侧机器人图标弹出） =====
const agentAddBtn = document.getElementById('agent-add-btn');
const agentAddMenu = document.getElementById('agent-add-menu');

function isAgentAddMenuVisible() {
    return agentAddMenu && !agentAddMenu.classList.contains('hidden');
}

function hideAgentAddMenu() {
    if (agentAddMenu) agentAddMenu.classList.add('hidden');
}

function toggleAgentAddMenu(event) {
    if (!agentAddMenu) return;
    if (event) {
        event.preventDefault();
        event.stopPropagation();
    }
    const willShow = agentAddMenu.classList.contains('hidden');
    agentAddMenu.classList.toggle('hidden');
    if (willShow) renderAgentAddMenu();
}

// 渲染加入智能体菜单（优先使用节点列表缓存，避免重复请求）
function renderAgentAddMenu() {
    if (!agentAddMenu) return;
    const lastList = window.__agentLastList;
    if (lastList && lastList.ok && lastList.items && lastList.items.length) {
        buildAgentAddMenuItems(lastList.items);
        return;
    }
    agentAddMenu.innerHTML = '<div class="agent-add-menu-empty">' + (currentLang === 'zh' ? '加载中…' : 'Loading…') + '</div>';
    fetch('/api/agent/nodes', { cache: 'no-store' })
        .then(function (r) { return r.json(); })
        .then(function (data) {
            if (!data || !data.ok) {
                agentAddMenu.innerHTML = '<div class="agent-add-menu-empty">' +
                    (currentLang === 'zh' ? '加载失败' : 'Failed to load') + '</div>';
                return;
            }
            window.__agentLastList = data;
            buildAgentAddMenuItems(data.items || []);
        })
        .catch(function () {
            agentAddMenu.innerHTML = '<div class="agent-add-menu-empty">' +
                (currentLang === 'zh' ? '加载失败' : 'Failed to load') + '</div>';
...
function buildAgentAddMenuItems(items) {
    const enabled = items.filter(function (n) { return n.status === 1; });
    const disabled = items.filter(function (n) { return n.status !== 1; });
    const html = [];
    html.push('<div class="agent-add-menu-header">' + (currentLang === 'zh' ? '可用智能体' : 'Available agents') + '</div>');
    if (enabled.length) {
        enabled.forEach(function (n) { html.push(agentAddMenuItemHtml(n)); });
    } else {
        html.push('<div class="agent-add-menu-empty">' + (currentLang === 'zh' ? '暂无可用智能体' : 'No available agents') + '</div>');
    }
    if (disabled.length) {
        html.push('<div class="agent-add-menu-header">' + (currentLang === 'zh' ? '已禁用' : 'Disabled') + '</div>');
        disabled.forEach(function (n) { html.push(agentAddMenuItemHtml(n)); });
    }
    agentAddMenu.innerHTML = html.join('');
}

function agentAddMenuItemHtml(n) {
    const kind = AGENT_KIND_LABELS[n.kind] || n.kind || '';
    const disabled = n.status !== 1;
    const meta = [kind, n.code || ''].filter(Boolean).join(' · ');
    return '<button type="button" class="agent-add-menu-item ' + (disabled ? 'opacity-60' : '') + '"' +
        (disabled ? ' disabled' : '') + ' onclick="pickAgentToChat(' + JSON.stringify(n.id) + ')">' +
        '<span class="aam-icon">' + (n.icon || '🤖') + '</span>' +
        '<span class="min-w-0">' +
        '<span class="aam-name block truncate">' + escapeHtml(n.name || n.code || '-') + '</span>' +
        '<span class="aam-meta block truncate">' + escapeHtml(meta || '-') + '</span>' +
        '</span>' +
        '</button>';
}

// 选择智能体 -> 加入当前对话（逻辑复用 bindNodeToChat）
function pickAgentToChat(id) {
    hideAgentAddMenu();
    const lastList = window.__agentLastList;
    if (!lastList || !lastList.ok) return;
    const node = (lastList.items || []).find(function (n) { return String(n.id) === String(id); });
    if (node) bindNodeToChat(node);
}
        ...
        tasks.push((async () => {
            ...
            try {
                ...
                if (data.status === 'success') {
                    ...
                    placeholder.raw_url = data.raw_url;
...
// 加入智能体菜单：点击外部关闭
document.addEventListener('click', (e) => {
    if (!isAgentAddMenuVisible()) return;
    if (agentAddMenu.contains(e.target) || (agentAddBtn && agentAddBtn.contains(e.target))) return;
    hideAgentAddMenu();
});
...
function startSSE(requestId, loadingEl, timestamp, titleInfo, replayItems) {
    ...
    let accumulatedText = '';
    // Streaming content renderer: coalesce deltas per animation frame and, once
    // the reply outgrows STREAM_MARKDOWN_MAX, switch to append-only plain text.
    // Re-rendering the full markdown on every chunk is O(n^2) and freezes the
    // page on long replies (e.g. big code generation) — same fix as reasoning.
    const STREAM_MARKDOWN_MAX = 20000;  // chars; below this, markdown renders live
    const streamState = { pending: '', rafScheduled: false, plainMode: false, textNode: null };
    function resetContentStream() {
        streamState.pending = '';
        streamState.rafScheduled = false;
        streamState.plainMode = false;
    ...
    function ensureBotEl() {
        ...
        botEl.innerHTML = `
            <img src="assets/logo.png" alt="mocode-cli" class="w-8 h-8 rounded-lg flex-shrink-0">
    ...
    function processSSEItem(item) {
            if (item.type === 'reasoning') {
                ...
                streamState.pending += item.content;
                if (!streamState.rafScheduled) {
                    streamState.rafScheduled = true;
                    requestAnimationFrame(() => {
                        streamState.rafScheduled = false;
                        const pending = streamState.pending;
                        streamState.pending = '';
                        if (!pending) return;
                        if (!streamState.plainMode && accumulatedText.length > STREAM_MARKDOWN_MAX) {
                            // Too large to re-parse every frame; stream as plain
                            // text and let "done" do the one-time markdown pass.
                            streamState.plainMode = true;
                        }
                        if (streamState.plainMode) {
                            if (!streamState.textNode || !streamState.textNode.isConnected) {
                                contentEl.innerHTML = '';
                                const pre = document.createElement('pre');
                                pre.className = 'content-stream-pre';
                                pre.style.whiteSpace = 'pre-wrap';
                                pre.style.wordBreak = 'break-word';
                                pre.style.fontFamily = 'inherit';
                                pre.appendChild(document.createTextNode(''));
                                streamState.textNode = pre.firstChild;
                                contentEl.appendChild(pre);
                            }
                            streamState.textNode.appendData(pending);
                        } else {
    ...
    if (attachments && attachments.length > 0) {
        const items = attachments.map(a => {
            if (a.file_type === 'image') {
                return `<img src="${a.raw_url || a.preview_url}" alt="${escapeHtml(a.file_name)}" class="user-msg-image">`;
            }
...
function newChat(optimistic = true) {
    ...
    ws.innerHTML = `
        <img src="assets/logo.png" alt="mocode-cli" class="w-16 h-16 rounded-2xl mb-6 shadow-lg shadow-primary-500/20">
        <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-3">${appConfig.title || 'mocode-cli'}</h1>
...
// =====================================================================
// 智能体节点调用（一键配置 + 弹窗调用 AWF 5000）
// =====================================================================
let __agentInvokeCode = '';
let __agentInvokeData = {};
// 暂存最近一次运行成功的节点与结果（点击「加入对话」时推送到聊天区）
let __agentLastInvokeResult = null;

// 一键配置：检测/拉起 admin(8088) 与 AWF(5000)
function runAgentSetup() {
    const btn = document.getElementById('agent-setup-btn');
    if (!btn) return;
    const orig = btn.innerHTML;
    btn.disabled = true;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin mr-1"></i>配置中';
    fetch('/api/agent/setup', { method: 'POST', cache: 'no-store' })
        .then(function (r) { return r.json(); })
        .then(function (j) {
            const awf = (j && j.awf) || {};
            const admin = (j && j.admin) || {};
            let parts = [];
            parts.push('AWF(5000):' + (awf.running ? '在线' : '离线' + (awf.detail ? '(' + awf.detail + ')' : '')));
            parts.push('节点管理(8088):' + (admin.running ? '在线' : '离线' + (admin.detail ? '(' + admin.detail + ')' : '')));
            if (j && j.nodes_online) parts.push('可用节点能力:' + j.nodes_online + '个');
            showAgentToast(parts.join(' | '));
            loadAgentNodes();
        })
        .catch(function () { showAgentToast('一键配置失败，请查看服务日志'); })
        .finally(function () {
            btn.disabled = false;
            btn.innerHTML = orig;
        });
...
// 关闭调用弹窗
function closeAgentInvokeModal() {
    const overlay = document.getElementById('agent-invoke-modal-overlay');
    if (overlay) overlay.classList.add('hidden');
    __agentInvokeCode = '';
    __agentInvokeData = {};
}
...
// 打开调用弹窗
function openAgentInvokeModal(code) {
    const overlay = document.getElementById('agent-invoke-modal-overlay');
    const form = document.getElementById('agent-invoke-form');
    const result = document.getElementById('agent-invoke-result');
    const title = document.getElementById('agent-invoke-modal-title');
    const sub = document.getElementById('agent-invoke-modal-sub');
    const icon = document.getElementById('agent-invoke-modal-icon');
    if (!overlay || !form) return;
    const node = findAgentNodeByCode(code);
    const meta = AGENT_INVOKE_META[code];
    __agentInvokeCode = code;
    __agentInvokeData = {};
    // 重置「加入对话」按钮（运行成功后才显示）
    const joinBtn = document.getElementById('agent-invoke-join');
    if (joinBtn) joinBtn.classList.add('hidden');
    if (icon) icon.textContent = (node && node.icon) || '🤖';
    if (title) title.textContent = (node && node.name) || code;
    if (sub) sub.textContent = code + ' · ' + ((node && node.api_url) || '-');
    if (result) { result.classList.add('hidden'); result.innerHTML = ''; }
    if (!meta || !meta.fields.length) {
        form.innerHTML = '<div class="text-xs text-slate-400 py-4 text-center">该节点为输入/入口类节点，无独立生成参数，请直接使用「加入对话」调用。</div>';
    } else {
        form.innerHTML = meta.fields.map(function (f) {
            const req = f.required ? '<span class="text-red-400"> *</span>' : '';
            let inner = '';
            if (f.type === 'textarea') {
                inner = '<textarea data-key="' + f.key + '" rows="3" class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-white/5 text-sm text-slate-800 dark:text-slate-100 focus:outline-none focus:border-indigo-500 transition-colors" placeholder="' + (f.placeholder || '') + '"></textarea>';
            } else if (f.type === 'file') {
                inner = '<div class="flex items-center gap-2">' +
                    '<input type="file" data-key="' + f.key + '" accept="' + (f.accept || '') + '" class="block w-full text-xs text-slate-500 dark:text-slate-400 file:mr-2 file:px-3 file:py-1.5 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-indigo-50 dark:file:bg-indigo-900/30 file:text-indigo-600 dark:file:text-indigo-400 file:cursor-pointer cursor-pointer">' +
                    '<span class="agent-file-status text-[11px] text-emerald-500 flex-shrink-0 hidden"><i class="fas fa-check"></i></span>' +
                    '</div>';
            } else if (f.type === 'select') {
                inner = '<select data-key="' + f.key + '" class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-[#1A1A1A] text-sm text-slate-800 dark:text-slate-100 focus:outline-none focus:border-indigo-500 transition-colors appearance-none cursor-pointer">' +
                    f.options.map(function (o) { return '<option value="' + o[0] + '">' + o[1] + '</option>'; }).join('') + '</select>';
            } else if (f.type === 'number') {
                inner = '<input type="number" data-key="' + f.key + '" placeholder="' + (f.placeholder || '') + '" class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-white/5 text-sm text-slate-800 dark:text-slate-100 focus:outline-none focus:border-indigo-500 transition-colors">';
            } else {
                inner = '<input type="text" data-key="' + f.key + '" placeholder="' + (f.placeholder || '') + '" class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-white/5 text-sm text-slate-800 dark:text-slate-100 focus:outline-none focus:border-indigo-500 transition-colors">';
            }
            return '<div><label class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5">' + f.label + req + '</label>' + inner + '</div>';
        }).join('');
    }
    overlay.classList.remove('hidden');
}

// 文件字段：读为 base64 data URL（AWF _file_param 原生支持 data: 输入，绕开 multipart 解析问题）
function handleAgentInvokeFileUpload(e) {
    const input = e.target.closest('input[type="file"]');
    if (!input || !input.files || !input.files[0]) return;
    const key = input.dataset.key;
    const statusEl = input.parentElement.querySelector('.agent-file-status');
    const file = input.files[0];
    // 大文件（>8MB）提示
    if (file.size > 8 * 1024 * 1024) {
        showAgentToast('文件较大，base64 传输可能较慢');
    }
    if (statusEl) { statusEl.classList.remove('hidden'); statusEl.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 读取中'; }
    const reader = new FileReader();
    reader.onload = function () {
        __agentInvokeData[key] = reader.result || '';
        if (statusEl) { statusEl.classList.remove('hidden'); statusEl.innerHTML = '<i class="fas fa-check"></i> 已就绪'; }
    };
    reader.onerror = function () {
        __agentInvokeData[key] = '';
        if (statusEl) statusEl.classList.add('hidden');
        showAgentToast('文件读取失败');
    };
    reader.readAsDataURL(file);
}

// 渲染调用结果（漂浮窗口内）
function renderAgentInvokeResult(j) {
    const box = document.getElementById('agent-invoke-result');
    if (!box) return;
    box.classList.remove('hidden');
    if (!j || j.ok === false) {
        box.innerHTML = '<div class="rounded-lg border border-red-200 dark:border-red-500/30 bg-red-50 dark:bg-red-900/20 p-3 text-xs text-red-600 dark:text-red-400">' +
            escapeHtml((j && j.error) || '调用失败') + '</div>';
        return;
    }
    box.innerHTML = '<div class="rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-800/40 p-3 space-y-2">' +
        renderAgentInvokeContentHtml(j) + '</div>' +
        renderAgentPipelineResult(j.pipeline);
}

// 渲染节点运行结果的内容 HTML（图片/文字/视频/音频/3D，供弹窗与聊天区共用）
function renderAgentInvokeContentHtml(j) {
    if (!j) return '';
    const items = (j && j.items) || [];
    const dataUrl = (j && (j.data_url || j.image_url)) || '';
    const modelUrl = (j && j.model_url) || '';
    const kind = (j && j.model_kind) || '';
    const text = (j && (j.response || j.text || j.content)) || (j && typeof j.result === 'string' ? j.result : '');
    let html = '';
    // 服务器保存结果 URL（/api/agent/result/... 或 http(s)://）按扩展名识别类型
    const isSrv = dataUrl && (/^\/api\/agent\/result\//.test(dataUrl) || /^https?:\/\//i.test(dataUrl));
    if (isSrv) {
        const low = dataUrl.toLowerCase();
        if (/\.(mp3|wav|ogg|m4a|flac)(\?|$)/.test(low)) {
            html += '<audio controls class="w-full" src="' + dataUrl + '"></audio>';
        } else if (/\.(mp4|webm|mov)(\?|$)/.test(low)) {
            html += '<video controls class="w-full rounded-lg" src="' + dataUrl + '"></video>';
        } else if (/\.(glb|obj|fbx)(\?|$)/.test(low)) {
            html += '<div class="text-xs text-blue-500 break-all">3D 模型：<a href="' + dataUrl + '" target="_blank" class="underline">' + dataUrl + '</a></div>';
        } else {
            html += '<img src="' + dataUrl + '" class="w-full rounded-lg border border-slate-200 dark:border-white/10" alt="结果">';
        }
    } else if (dataUrl) {
        if (dataUrl.startsWith('data:audio') || /\.(mp3|wav|ogg|m4a|flac)(\?|$)/.test(dataUrl)) {
            html += '<audio controls class="w-full" src="' + dataUrl + '"></audio>';
        } else if (dataUrl.startsWith('data:video') || /\.(mp4|webm|mov)(\?|$)/.test(dataUrl)) {
            html += '<video controls class="w-full rounded-lg" src="' + dataUrl + '"></video>';
        } else {
            html += '<img src="' + dataUrl + '" class="w-full rounded-lg border border-slate-200 dark:border-white/10" alt="结果">';
        }
    ...
    if (modelUrl && !dataUrl) {
        html += '<div class="text-xs text-blue-500 break-all">3D 模型：<a href="' + modelUrl + '" target="_blank" class="underline">' + modelUrl + '</a></div>';
        html += '<div class="text-[11px] text-slate-400">类型：' + escapeHtml(kind || 'glb') + '（可用 3D 查看器加载）</div>';
    }
    if (text) {
        html += '<div class="text-xs text-slate-600 dark:text-slate-300 whitespace-pre-wrap leading-relaxed">' + escapeHtml(text) + '</div>';
    }
    if (!dataUrl && !modelUrl && !text && items.length) {
        const first = items[0];
        const fu = (first && first.data_url) || '';
        if (fu) {
            if (/^\/api\/agent\/result\//.test(fu) || /^https?:\/\//i.test(fu)) {
                const low = fu.toLowerCase();
                if (/\.(mp3|wav|ogg|m4a|flac)(\?|$)/.test(low)) {
                    html += '<audio controls class="w-full" src="' + fu + '"></audio>';
                } else if (/\.(mp4|webm|mov)(\?|$)/.test(low)) {
                    html += '<video controls class="w-full rounded-lg" src="' + fu + '"></video>';
                } else {
                    html += '<img src="' + fu + '" class="w-full rounded-lg border border-slate-200 dark:border-white/10" alt="结果">';
                }
            } else {
                html += '<img src="' + fu + '" class="w-full rounded-lg border border-slate-200 dark:border-white/10" alt="结果">';
            }
        }
        if (items.length > 1) html += '<div class="text-[11px] text-slate-400">共生成 ' + items.length + ' 个文件</div>';
    }
    if (!dataUrl && !modelUrl && !text && !items.length) {
        html += '<div class="text-xs text-emerald-600 dark:text-emerald-400">调用成功</div>';
    }
    return html;
}

// 把节点运行结果作为一条消息展示在聊天区中间
function appendAgentInvokeChatMessage(node, j) {
    const messagesDiv = document.getElementById('chat-messages');
    if (!messagesDiv) return;
    // 隐藏欢迎页
    const ws = document.getElementById('welcome-screen');
    if (ws) ws.style.display = 'none';
    const code = (node && node.code) || __agentInvokeCode || '';
    const name = (node && node.name) || code || '智能体';
    const icon = (node && node.icon) || '🤖';
    const failed = !j || j.ok === false;
    const contentHtml = failed
        ? '<div class="text-xs text-red-500 dark:text-red-400">' + escapeHtml((j && j.error) || '运行失败') + '</div>'
        : renderAgentInvokeContentHtml(j);
    const pipelineHtml = failed ? '' : renderAgentPipelineResult(j.pipeline);
    const el = document.createElement('div');
    el.className = 'agent-invoke-msg flex justify-start mb-4';
    el.innerHTML =
        '<div class="max-w-[85%] sm:max-w-[75%]">' +
            '<div class="bg-slate-100 dark:bg-white/10 rounded-2xl px-4 py-3 text-sm leading-relaxed msg-content bot-bubble">' +
                '<div class="flex items-center gap-2 mb-2">' +
                    '<span class="text-base leading-none">' + icon + '</span>' +
                    '<span class="text-xs font-semibold text-slate-700 dark:text-slate-200">' + escapeHtml(name) + '</span>' +
                    (code ? '<span class="text-[10px] font-mono text-slate-400">' + escapeHtml(code) + '</span>' : '') +
                    '<span class="text-[10px] text-slate-400">' + (failed ? '运行失败' : '运行完成') + '</span>' +
                '</div>' +
                '<div class="space-y-2">' + contentHtml + pipelineHtml + '</div>' +
            '</div>' +
            '<div class="text-xs text-slate-400 dark:text-slate-500 mt-1.5">' + formatTime(new Date()) + '</div>' +
        '</div>';
    messagesDiv.appendChild(el);
    scrollChatToBottom(true);
}
...
// =====================================================================
// 聊天区：图片放大（点击）+ 右键菜单（加入输入框 / 删除对话 / 撤回）
// =====================================================================
let __agentCtxMenu = null;        // 右键菜单 DOM
let __agentCtxTarget = null;      // 右键目标：{ img, msg }
let __agentLightbox = null;       // 图片放大遮罩 DOM

// 图片放大：全屏预览
function showAgentImageLightbox(src) {
    if (!__agentLightbox) {
        const lb = document.createElement('div');
        lb.id = 'agent-lightbox';
        lb.className = 'fixed inset-0 z-[300] bg-black/85 hidden items-center justify-center p-6';
        lb.innerHTML =
            '<button class="absolute top-4 right-5 text-slate-300 hover:text-white text-2xl cursor-pointer z-10" title="关闭"><i class="fas fa-xmark"></i></button>' +
            '<img class="max-w-full max-h-full rounded-lg shadow-2xl select-none" alt="放大预览">';
        lb.addEventListener('click', function (e) {
            if (e.target === lb || e.target.closest('button')) hideAgentImageLightbox();
        });
        document.body.appendChild(lb);
        __agentLightbox = lb;
    }
    const img = __agentLightbox.querySelector('img');
    img.src = src;
    __agentLightbox.classList.remove('hidden');
    __agentLightbox.classList.add('flex');
}
function hideAgentImageLightbox() {
    if (!__agentLightbox) return;
    __agentLightbox.classList.add('hidden');
    __agentLightbox.classList.remove('flex');
}

// 确保右键菜单 DOM 存在
function ensureAgentCtxMenu() {
    if (__agentCtxMenu) return __agentCtxMenu;
    const m = document.createElement('div');
    m.id = 'agent-ctx-menu';
    m.className = 'hidden fixed z-[200] min-w-[190px] rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#232323] shadow-xl py-1.5 text-sm';
    m.innerHTML =
        '<button class="agent-ctx-item w-full text-left px-4 py-2 text-xs text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-white/10 cursor-pointer transition-colors flex items-center gap-2" data-act="attach">' +
            '<i class="fas fa-paperclip text-slate-400"></i>加入到输入消息框</button>' +
        '<button class="agent-ctx-item w-full text-left px-4 py-2 text-xs text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 cursor-pointer transition-colors flex items-center gap-2" data-act="delmsg">' +
            '<i class="fas fa-trash text-red-400"></i>删除当前消息</button>' +
        '<div class="border-t border-slate-200 dark:border-white/10 my-1"></div>' +
        '<button class="agent-ctx-item w-full text-left px-4 py-2 text-xs text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-white/10 cursor-pointer transition-colors flex items-center gap-2" data-act="undo">' +
            '<i class="fas fa-rotate-left text-slate-400"></i>撤回操作</button>';
    document.body.appendChild(m);
    // 点击其他区域关闭菜单
    m.addEventListener('click', function (e) {
        const btn = e.target.closest('.agent-ctx-item');
        if (!btn) return;
        const act = btn.dataset.act;
        hideAgentCtxMenu();
        const t = __agentCtxTarget || {};
        if (act === 'attach') {
            ctxMenuAttach(t);
        } else if (act === 'delmsg') {
            showConfirmModal('删除消息', '确定删除当前选中的这条消息吗？', function () {
                deleteCtxMessage(t);
            });
        } else if (act === 'undo') {
            undoLastChatMessage();
    ...
    document.addEventListener('click', function (e) {
        if (__agentCtxMenu && !__agentCtxMenu.contains(e.target)) hideAgentCtxMenu();
    });
...
// 显示右键菜单
function showAgentCtxMenu(e, target) {
    const menu = ensureAgentCtxMenu();
    const img = (target && target.closest) ? target.closest('img') : null;
    const msg = (target && target.closest) ? (target.closest('.agent-invoke-msg') || target.closest('.user-message-group') || target.closest('.bot-message-group')) : null;
    __agentCtxTarget = { img: img, msg: msg };
    menu.style.left = Math.max(8, Math.min(e.clientX, window.innerWidth - 210)) + 'px';
    menu.style.top = Math.max(8, Math.min(e.clientY, window.innerHeight - 150)) + 'px';
    menu.classList.remove('hidden');
}
function hideAgentCtxMenu() {
    if (__agentCtxMenu) __agentCtxMenu.classList.add('hidden');
}

// =====================================================================
// 智能体内容加密：utp + obf + oem + hex + ugg + ob + kv 七步算法链
// 用于「加入到输入消息框」前加密内容，发送时后端按同一算法链解密还原
// =====================================================================
const AGENT_ENC_KEY = 'mocode-agent-2026';
const AGENT_HEX_CHARS = '0123456789abcdef';

// kv 步校验和（同步，前后端一致）
function agentChecksum(s) {
    let h = 0;
    for (let i = 0; i < s.length; i++) h = (h * 31 + s.charCodeAt(i)) & 0x7fffffff;
    return h.toString(16);
}

// 加密：utp(UTF-8 打包) → obf(XOR 混淆) → oem(字符映射) → hex → ugg(位移) → ob(反转) → kv(校验打包)
function agentEncrypt(text) {
    const enc = new TextEncoder();
    const kb = enc.encode(AGENT_ENC_KEY);
    let bytes = enc.encode(String(text));
    // obf: XOR 密钥
    for (let i = 0; i < bytes.length; i++) bytes[i] = bytes[i] ^ kb[i % kb.length];
    // oem: +17 取模映射
    for (let i = 0; i < bytes.length; i++) bytes[i] = (bytes[i] + 17) % 256;
    // hex
    let hex = '';
    for (let i = 0; i < bytes.length; i++) hex += bytes[i].toString(16).padStart(2, '0');
    // ugg: hex 字符右移 7
    let shifted = '';
    for (let i = 0; i < hex.length; i++) shifted += AGENT_HEX_CHARS[(AGENT_HEX_CHARS.indexOf(hex[i]) + 7) % 16];
    // ob: 反转
    const payload = shifted.split('').reverse().join('');
    // kv: 校验和 + base64url 打包
    const final = payload + '.' + agentChecksum(payload);
    const b64 = btoa(final).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
    return 'ENC1:' + b64;
}

// 右键「加入到输入消息框」：图片 -> 图片 URL；文本消息 -> 消息文本（加密后入框）
function ctxMenuAttach(t) {
    if (t.img && t.img.src) {
        chatInput.value = (chatInput.value ? chatInput.value + '\n' : '') + agentEncrypt(t.img.src);
    } else if (t.msg) {
        const textEl = t.msg.querySelector('.msg-content, .agent-content-body');
        const txt = textEl ? (textEl.innerText || '').trim() : '';
        if (txt) chatInput.value = (chatInput.value ? chatInput.value + '\n' : '') + agentEncrypt(txt);
    }
    chatInput.focus();
    chatInput.dispatchEvent(new Event('input', { bubbles: true }));
    showAgentToast('已加密后加入到输入消息框');
}

// 右键「删除当前消息」：删除右键选中的单条消息（用户消息连同回复一起删除）
function deleteCtxMessage(t) {
    if (!t || !t.msg) {
        showAgentToast('请先选中一条消息再删除');
        return;
    }
    const msg = t.msg;
    if (msg.classList.contains('user-message-group')) {
        // 用户消息：连同其 bot 回复一起删除，保持上下文完整
        const seq = msg.dataset.seq;
        const toRemove = [msg];
        let sib = msg.nextElementSibling;
        while (sib && sib.classList && sib.classList.contains('bot-message-group')) {
            toRemove.push(sib);
        ...
        if (seq) {
            fetch('/api/messages/delete', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ session_id: sessionId, user_seq: parseInt(seq) })
            }).catch(function (err) { console.error('delete message failed:', err); });
        }
    } else {
        // 机器人回复 / 智能体运行结果：直接删除该条
        msg.remove();
    }
    showAgentToast('已删除该消息');
}

// 右键「撤回操作」：撤回聊天区最后一条消息（运行结果 / 用户消息）
function undoLastChatMessage() {
    const messagesDiv = document.getElementById('chat-messages');
    if (!messagesDiv) return;
    const children = messagesDiv.children;
    for (let i = children.length - 1; i >= 0; i--) {
        const el = children[i];
        if (!el.classList || !el.id || el.id === 'welcome-screen') break;
        if (el.classList.contains('agent-invoke-msg')) {
            el.remove();
            showAgentToast('已撤回上一条运行结果');
            return;
        }
        if (el.classList.contains('user-message-group')) {
            ...
            if (seq) {
                fetch('/api/messages/delete', {
                    ...
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ session_id: sessionId, user_seq: parseInt(seq) })
                }).catch(function (err) { console.error('undo delete failed:', err); });
            }
            showAgentToast('已撤回上一条消息');
            return;
        }
        if (el.classList.contains('bot-message-group')) {
            el.remove();
            showAgentToast('已撤回上一条回复');
            return;
        }
    }
    showAgentToast('没有可撤回的消息');
}

// 渲染「加密通道 → 双桥接 → CodeGenHook」管道结果
function renderAgentPipelineResult(pl) {
    if (!pl) return '';
    if (pl.ok === false) {
        return '<div class="mt-2 rounded-lg border border-amber-200 dark:border-amber-500/30 bg-amber-50 dark:bg-amber-900/20 p-3 text-xs text-amber-700 dark:text-amber-300">' +
            '管道执行失败：' + escapeHtml((pl.error || '未知错误')) + '</div>';
    }
    const okIcon = '<i class="fas fa-check-circle text-emerald-500"></i>';
    const failIcon = '<i class="fas fa-times-circle text-red-500"></i>';
    let html = '<div class="mt-2 rounded-lg border border-indigo-200 dark:border-indigo-500/30 bg-indigo-50/50 dark:bg-indigo-900/10 p-3 text-xs space-y-2">';
    html += '<div class="flex items-center justify-between text-indigo-700 dark:text-indigo-300 font-medium">' +
        '<span><i class="fas fa-shield-halved mr-1"></i>MPCP 加密管道</span>' +
        '<span class="text-slate-400 font-normal">' + (pl.cost_ms || 0) + 'ms</span></div>';
    // 各步骤
    if (pl.steps && pl.steps.length) {
        html += '<div class="space-y-1">';
        for (const s of pl.steps) {
            html += '<div class="flex items-start gap-2"><span class="mt-0.5">' + (s.ok ? okIcon : failIcon) + '</span>' +
                '<span class="text-slate-700 dark:text-slate-300 font-medium w-32 shrink-0">' + escapeHtml(s.step) + '</span>' +
                '<span class="text-slate-500 dark:text-slate-400 break-all">' + escapeHtml(s.detail || '') + '</span></div>';
        }
        html += '</div>';
    }
    // 通道信息
    if (pl.channel) {
        const c = pl.channel;
        html += '<div class="grid grid-cols-2 gap-x-3 gap-y-1 text-slate-500 dark:text-slate-400 border-t border-indigo-100 dark:border-indigo-500/20 pt-2">' +
            '<div>通道 <b class="text-slate-700 dark:text-slate-300">' + escapeHtml(c.deploy_id || '-') + '</b></div>' +
            '<div>会话 <b class="text-slate-700 dark:text-slate-300">' + escapeHtml(c.session_id || '-') + '</b></div>' +
            '<div>算法链 <b class="text-slate-700 dark:text-slate-300">' + escapeHtml(c.algo_chain || '-') + '</b></div>' +
            '<div>状态 <b class="text-slate-700 dark:text-slate-300">' + escapeHtml((c.state || '').replace('ProtocolState.', '')) + '</b></div>' +
            '</div>';
    }
    // 加密报文摘要（服务器 -> 前端 加密协议通信）
    if (pl.cipher) {
        const ci = pl.cipher;
        html += '<div class="border-t border-indigo-100 dark:border-indigo-500/20 pt-2 space-y-0.5 text-slate-500 dark:text-slate-400">' +
            '<div><i class="fas fa-lock mr-1 text-indigo-400"></i>加密报文 <b class="text-slate-700 dark:text-slate-300">' + escapeHtml(ci.magic || '') + '</b> · 算法 ' +
            '<b class="text-slate-700 dark:text-slate-300">' + escapeHtml(ci.algo || '') + '</b> · 密文 ' + (ci.payload_len || 0) + ' 字符</div>' +
            '<div class="truncate text-[10px] font-mono">摘要 ' + escapeHtml(ci.digest || '') + '</div>' +
            '</div>';
    }
    // 双桥接
    if (pl.bridges) {
        html += '<div class="flex flex-wrap gap-2 border-t border-indigo-100 dark:border-indigo-500/20 pt-2">' +
            '<span class="px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300">OpenAI 桥接器 ✓</span>' +
            '<span class="px-2 py-0.5 rounded-full bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300">Mocode-Lab 桥接器 ' +
            (pl.bridges.mocode_lab && pl.bridges.mocode_lab.connected ? '✓' : '✗') + '</span>' +
            (pl.bridges.mocode_lab && pl.bridges.mocode_lab.hooks ? '<span class="text-slate-400 self-center">钩子：' +
                escapeHtml(pl.bridges.mocode_lab.hooks.join(' · ')) + '</span>' : '') +
            '</div>';
    }
    // 钩子执行明细
    if (pl.hook_exec && pl.hook_exec.length) {
        html += '<div class="border-t border-indigo-100 dark:border-indigo-500/20 pt-2 space-y-0.5">';
        for (const h of pl.hook_exec) {
            html += '<div class="flex items-center gap-2"><span>' + (h.ok ? okIcon : failIcon) + '</span>' +
                '<span class="text-slate-700 dark:text-slate-300">' + escapeHtml(h.name) + '</span>' +
                '<span class="text-slate-400">[' + escapeHtml(h.phase) + ']</span>' +
                '<span class="text-slate-500 dark:text-slate-400 truncate">' + escapeHtml(h.note || '') + '</span></div>';
        }
        html += '</div>';
    }
    // 生成代码
    if (pl.generated_code) {
        const gid = 'codegen-' + Math.random().toString(36).slice(2, 8);
        html += '<div class="border-t border-indigo-100 dark:border-indigo-500/20 pt-2">' +
            '<div class="flex items-center justify-between text-indigo-600 dark:text-indigo-300">' +
            '<span><i class="fas fa-code mr-1"></i>CodeGenHook 生成代码</span>' +
            '<span class="flex items-center gap-2">' +
            (pl.generated_file ? '<span class="text-[10px] text-slate-400">' + escapeHtml(pl.generated_file.split(/[\\/]/).pop()) + '</span>' : '') +
            '<button onclick="copyAgentGeneratedCode(\'' + gid + '\')" class="text-[10px] px-1.5 py-0.5 rounded bg-indigo-100 dark:bg-indigo-900/40 hover:bg-indigo-200 dark:hover:bg-indigo-800/50">复制</button>' +
            '</span></div>' +
            '<pre id="' + gid + '" class="mt-1 max-h-48 overflow-auto rounded bg-slate-900 dark:bg-black/40 text-[10px] text-emerald-300 p-2 whitespace-pre">' +
            escapeHtml(pl.generated_code) + '</pre></div>';
    }
    html += '</div>';
    return html;
}

// 复制 CodeGenHook 生成的代码
function copyAgentGeneratedCode(id) {
    const el = document.getElementById(id);
    if (!el) return;
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(el.textContent).then(function () { showAgentToast('代码已复制'); });
    } else {
        ...
        showAgentToast('代码已复制');
    }
}

// 提交节点调用
function submitAgentInvoke() {
    const meta = AGENT_INVOKE_META[__agentInvokeCode];
    const box = document.getElementById('agent-invoke-result');
    const btn = document.getElementById('agent-invoke-submit');
    if (!meta) {
        showAgentToast('该节点不支持直接调用');
        return;
    }
    const payload = {};
    for (const f of meta.fields) {
        const el = document.querySelector('#agent-invoke-form [data-key="' + f.key + '"]');
        if (!el) continue;
        let val = '';
        if (el.type === 'file') val = __agentInvokeData[f.key] || '';
        else val = (el.value || '').trim();
        if (f.required && !val) {
            showAgentToast('请填写「' + f.label + '」');
    ...
    if (box) {
        box.classList.remove('hidden');
        box.innerHTML = '<div class="text-xs text-slate-400 flex items-center gap-2 py-2"><i class="fas fa-spinner fa-spin"></i>正在调用节点 ' +
            __agentInvokeCode + '，生成类任务可能需要较长时间，请耐心等待…</div>';
    }
    if (btn) btn.disabled = true;
    // 650s 超时（后端节点调用上限 600s，留余量）；超时按节点超时提示而非网络错误
    const ctrl = new AbortController();
    const timer = setTimeout(function () { ctrl.abort(); }, 650000);
    fetch('/api/agent/invoke', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ code: __agentInvokeCode, data: payload }),
        signal: ctrl.signal
    })
        .then(function (r) { return r.json(); })
        .then(function (j) {
            // UI 渲染异常不应被误报为网络错误
            try {
                renderAgentInvokeResult(j);
                // 运行成功后显示「加入对话」按钮
                const joinBtn = document.getElementById('agent-invoke-join');
                if (joinBtn) joinBtn.classList.toggle('hidden', !(j && j.ok));
                // 暂存运行结果（点击「加入对话」时推送到对话框中间）
                if (j && j.ok) {
                    __agentLastInvokeResult = { node: findAgentNodeByCode(__agentInvokeCode), result: j };
                }
                // 运行结果同步展示在聊天区中间（图片/文字/视频/语音）
                const node = findAgentNodeByCode(__agentInvokeCode);
                appendAgentInvokeChatMessage(node, j);
            } catch (uiErr) {
                console.error('[AgentInvoke] render failed:', uiErr);
            }
        })
        .catch(function (e) {
            console.error('[AgentInvoke] invoke error:', e && e.name === 'AbortError' ? 'timeout' : e);
            if (e && e.name === 'AbortError') {
                renderAgentInvokeResult({ ok: false, error: '节点运行超时（超过 10 分钟），请确认 AWF 服务是否正常后重试' });
            } else {
                renderAgentInvokeResult({ ok: false, error: '网络错误，请重试' });
            }
        })
        .finally(function () {
            clearTimeout(timer);
...
function _persistCapability(capId, provider, model, onAfterSuccess, extras) {
    ...
    fetch('/api/models', {
        ...
        if (data.status === 'success') {
            ...
            setTimeout(() => {
                loadModelsView({ preserveScroll: true });
                // Refresh chat model selector when chat capability is updated
                if (capId === 'chat') {
                    refreshChatModelSelector();
                }
...
// Refresh chat model selector in the conversation page
function refreshChatModelSelector() {
    fetch('/api/models')
        .then(r => r.json())
        .then(data => {
            if (data.status !== 'success') return;
            ...
            const scrollPos = modelSelect.scrollTop;

            // Refill model options
            ...
            customOpt.textContent = currentLang === 'zh' ? '自定义模型…' : 'Custom model…';
            ...
            modelSelect.scrollTop = scrollPos;
        })
        .catch(err => console.error('Failed to refresh chat model selector:', err));
}
...
function renderActiveChannels() {
    ...
    activeChannels.forEach(ch => {
        ...
        if (weixinWaiting) {
            ...
            statusDot = 'bg-amber-400 animate-pulse';
            statusText = `<span class="text-xs text-amber-500">启动中...</span>`;
        } else {
...
/**
 * Show an error toast/banner when a channel connection fails.
 * Displays the error message from the backend (e.g. missing config, missing dependency).
 */
function showChannelConnectError(chName, message) {
    // Remove any existing error banner
    const existing = document.getElementById('channel-connect-error');
    if (existing) existing.remove();

    const errorDiv = document.createElement('div');
    errorDiv.id = 'channel-connect-error';
    errorDiv.className = 'fixed bottom-6 right-6 z-50 max-w-md bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-xl shadow-lg p-4 flex items-start gap-3 animate-[fadeInUp_0.3s_ease]';
    errorDiv.innerHTML = `
        <div class="flex-shrink-0 mt-0.5">
            <i class="fas fa-exclamation-circle text-red-500 text-lg"></i>
        </div>
        <div class="flex-1 min-w-0">
            <p class="text-sm font-semibold text-red-700 dark:text-red-300">通道连接失败</p>
            <p class="text-xs text-red-600 dark:text-red-400 mt-1 break-words">${escapeHtml(message)}</p>
        </div>
        <button onclick="this.closest('#channel-connect-error').remove()"
            class="flex-shrink-0 text-red-400 hover:text-red-600 dark:hover:text-red-200 cursor-pointer transition-colors">
    ...
    setTimeout(() => {
        const el = document.getElementById('channel-connect-error');
        if (el) el.remove();
    }, 8000);
}
...
function submitAddChannel() {
    ...
    .then(data => {
        if (data.status === 'success') {
            ...
            const errMsg = data.message || t('channels_connect_error') || '连接失败';
            showChannelConnectError(chName, errMsg);
...
function startWeixinQrLogin() {
    ...
    fetch('/api/weixin/qrlogin')
        ...
        .then(data => {
            ...
            renderWeixinQr(data.qr_image || data.qrcode_url, 'waiting', data.qrcode_url);
            // Always use pollWeixinQrStatus for proper QR status tracking
            pollWeixinQrStatus();
        })
        .catch(() => {
            const panel = document.getElementById('weixin-qr-panel');
...
function renderWeixinQr(qrcodeUrl, status, fallbackUrl) {
    const panel = document.getElementById('weixin-qr-panel');
    if (!panel) return;
    ...
    if (status === 'scanned') {
        ...
        // Show a prominent confirmation reminder
        confirmBanner = `
            <div class="w-full mt-3 p-3 rounded-lg bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-800">
                <div class="flex items-center gap-2">
                    <i class="fas fa-mobile-alt text-primary-500 animate-pulse"></i>
                    <p class="text-xs font-semibold text-primary-700 dark:text-primary-300">请在手机上点击「确认登录」</p>
                </div>
            </div>`;
    ...
    // Determine image source: prefer base64 data URI, fallback to URL if it looks like an image
    const isDataUri = qrcodeUrl && qrcodeUrl.startsWith('data:image/');
    const isImageUrl = qrcodeUrl && !isDataUri && /\.(png|jpg|jpeg|gif|svg|webp)(\?|$)/i.test(qrcodeUrl);
    let imgHtml = '';
    if (isDataUri || isImageUrl) {
        // When scanned, add a green overlay to indicate the code was scanned
        const overlayClass = status === 'scanned'
            ? 'relative' : '';
        const overlayHtml = status === 'scanned'
            ? `<div class="absolute inset-0 bg-primary-500/10 rounded-lg flex items-center justify-center">
                    <div class="bg-primary-500 text-white rounded-full w-10 h-10 flex items-center justify-center shadow-lg">
                        <i class="fas fa-check"></i>
                    </div>
               </div>` : '';
        imgHtml = `<div class="bg-white p-3 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 mb-3 ${overlayClass}">
                <img src="${escapeHtml(qrcodeUrl)}" alt="QR Code" class="w-52 h-52 ${status === 'scanned' ? 'opacity-60' : ''}" style="image-rendering: pixelated;"/>
                ${overlayHtml}
            </div>`;
    } else if (fallbackUrl || qrcodeUrl) {
        // No QR image available; show a link to open in browser
        const link = fallbackUrl || qrcodeUrl;
        imgHtml = `<div class="bg-white p-4 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 mb-3 flex flex-col items-center justify-center w-52 h-52">
                <i class="fas fa-qrcode text-4xl text-slate-300 mb-3"></i>
                <p class="text-xs text-slate-400 text-center">二维码生成失败</p>
                <a href="${escapeHtml(link)}" target="_blank" rel="noopener"
                   class="text-xs text-blue-500 hover:text-blue-600 underline mt-2">在浏览器中打开</a>
            </div>`;
    } else {
        imgHtml = `<div class="bg-white p-4 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 mb-3 flex items-center justify-center w-52 h-52">
...
const WECOM_BOT_SOURCE = 'mocode-cli';
...
// 对话消息框：模型服务商选择条
// =====================================================================
let chatModelsState = null;   // /api/models 返回的完整数据（providers + capabilities）

function initChatModelBar() {
    fetch('/api/models', { cache: 'no-store' })
        ...
        .catch(function () { /* 静默失败：不阻塞聊天 */ });
}

// 渲染模型服务商 / 模型下拉（原生 select，样式对齐输入框）
function renderChatModelBar() {
    const bar = document.getElementById('chat-model-bar');
    if (!bar || !chatModelsState) return;
    const providers = (chatModelsState.providers || []).filter(function (p) { return p.configured; });
    const chat = (chatModelsState.capabilities && chatModelsState.capabilities.chat) || {};
    const curProv = chat.current_provider || '';
    const curModel = chat.current_model || '';

    // 空态：无已配置厂商 -> 提示去模型页配置
    if (!providers.length) {
        bar.innerHTML = '<span class="text-[11px] text-slate-400">' + (currentLang === 'zh' ? '未配置模型服务商' : 'No model provider configured') + '</span>' +
            '<button type="button" onclick="navigateTo(\'models\')" class="px-2 py-0.5 text-[11px] font-medium rounded-lg text-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 hover:bg-indigo-100 dark:hover:bg-indigo-900/30 cursor-pointer transition-colors">' +
            (currentLang === 'zh' ? '去配置' : 'Configure') + '</button>';
        return;
    }

    const selCls = 'text-xs rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-white/5 text-black px-2 py-1 focus:outline-none focus:border-primary-500 cursor-pointer';

    // 厂商下拉：仅已配置厂商；当前厂商未配置时默认第一个
    const provSel = document.createElement('select');
    provSel.id = 'chat-provider-select';
    provSel.className = 'max-w-[9.5rem] ' + selCls;
    provSel.title = currentLang === 'zh' ? '模型服务商' : 'Model provider';
    providers.forEach(function (p) {
        const opt = document.createElement('option');
        opt.value = p.id;
        opt.textContent = localizedLabel(p.label) || p.id;
        provSel.appendChild(opt);
    });
    let selectedProv = providers.some(function (p) { return p.id === curProv; }) ? curProv : providers[0].id;
    provSel.value = selectedProv;

    // 模型下拉：当前厂商的模型列表 + 自定义选项
    const modelSel = document.createElement('select');
    modelSel.id = 'chat-model-select';
    modelSel.className = 'max-w-[12rem] ' + selCls;
    modelSel.title = currentLang === 'zh' ? '模型' : 'Model';

    // 自定义模型输入（选中“自定义模型…”时显示）
    const modelCustom = document.createElement('input');
    modelCustom.id = 'chat-model-custom';
    modelCustom.type = 'text';
    modelCustom.placeholder = currentLang === 'zh' ? '自定义模型名' : 'Custom model';
    modelCustom.className = 'hidden w-36 text-xs rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-white/5 text-black px-2 py-1 focus:outline-none focus:border-primary-500';

    function fillModelOptions(providerId, keepCurrent) {
        modelSel.innerHTML = '';
        const prov = providers.find(function (p) { return p.id === providerId; });
        const models = (prov && prov.models) || [];
        const cur = (keepCurrent !== undefined) ? keepCurrent : curModel;
        let matched = false;
    ...
    fillModelOptions(selectedProv);

    function persistChatModel() {
        ...
        fetch('/api/models', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'set_capability', capability: 'chat', provider_id: provSel.value, model: model })
        })
        .then(function (r) { return r.json(); })
        .then(function (data) {
            if (data.status === 'success') {
                ...
                showAgentToast(currentLang === 'zh' ? '模型保存失败' : 'Failed to save model');
            }
        })
        .catch(function () {
            if (typeof showAgentToast === 'function') {
                showAgentToast(currentLang === 'zh' ? '网络错误' : 'Network error');
            }
        });
    }

    provSel.addEventListener('change', function () {
        selectedProv = provSel.value;
        fillModelOptions(selectedProv, '');
        persistChatModel();
    });
    modelSel.addEventListener('change', function () {
        if (modelSel.value === '__custom__') {
            modelCustom.classList.remove('hidden');
            modelCustom.focus();
            return; // 输入完成后由 blur / change 保存
        }
        modelCustom.classList.add('hidden');
        persistChatModel();
    });
    modelCustom.addEventListener('change', persistChatModel);
    modelCustom.addEventListener('blur', function () {
        if ((modelCustom.value || '').trim()) persistChatModel();
    });

    bar.innerHTML = '';
    bar.appendChild(provSel);
    bar.appendChild(modelSel);
    bar.appendChild(modelCustom);
}
...
function initApp() {
    ...
    fetch('/api/version').then(r => r.json()).then(data => {
        ...
        document.getElementById('sidebar-version').textContent = `mocode-cli ${APP_VERSION}`;
    }).catch(() => {
        document.getElementById('sidebar-version').textContent = 'mocode-cli';
    });
    initChatModelBar();
    chatInput.focus();
}
...
function loadTaskReceiverOptions(receiver, selectedChannelType) {
    // 接收会话下拉：仅 web 通道显示。从 /api/sessions 拉取会话列表填充。
    const wrap = document.getElementById('task-edit-receiver-wrap');
    const select = document.getElementById('task-edit-receiver-select');
    const hidden = document.getElementById('task-edit-receiver');
    if (!wrap || !select || !hidden) return;

    const isWeb = selectedChannelType === 'web';
    wrap.classList.toggle('hidden', !isWeb);

    if (!isWeb) {
        // 非 web 通道没有可见下拉，回填到隐藏字段
    ...
    fetch('/api/sessions?page=1&page_size=50')
        .then(r => r.json())
        .then(data => {
            select.innerHTML = '<option value="">-- 选择接收会话 --</option>';
            const sessions = (data.status === 'success' ? (data.sessions || []) : []);
            let hasMatch = false;
            sessions.forEach(s => {
                const sid = s.session_id || '';
                const title = s.title || (currentLang === 'zh' ? '未命名会话' : 'Untitled');
                const opt = document.createElement('option');
                opt.value = sid;
                opt.textContent = title + ' (' + sid + ')';
                select.appendChild(opt);
                if (receiver && sid === receiver) hasMatch = true;
            });
            // 选中当前任务已有 receiver；否则若有全局当前会话则默认选中
            if (receiver && sessions.some(s => s.session_id === receiver)) {
                select.value = receiver;
                hidden.value = receiver;
            } else if (typeof sessionId !== 'undefined' && sessionId && sessions.some(s => s.session_id === sessionId)) {
        ...
        .catch(() => {
            // 加载失败时回退：保留已有 receiver
            hidden.value = receiver || '';
        });

    // 用户切换下拉时同步到隐藏字段（保存时读取隐藏字段）
...
function openTaskEditModal(task) {
    ...
    // Load receiver sessions dropdown for web channel
    loadTaskReceiverOptions(action.receiver || '', action.channel_type || 'web');
...
function closeTaskEditModal() {
    ...
    isAddMode = false;
    // Re-enable channel selector for next use
    const channelTypeSelect = document.getElementById('task-edit-channel-type');
    if (channelTypeSelect) channelTypeSelect.disabled = false;
}
...
function openAddTaskModal() {
    ...
    cronInput.value = '0 9 * * *';  // Default: every day at 9:00
    intervalInput.value = '';
    onceInput.value = '';
    ...
    // Load channel options and enable selector for new tasks
    loadTaskChannelOptions('web');
    channelTypeSelect.disabled = false;
    // Load receiver sessions dropdown for the default web channel
    loadTaskReceiverOptions('', 'web');
...
function saveNewTask() {
    ...
    if (scheduleType === 'cron') {
        ...
        if (fields.length < 5 || fields.length > 6) {
            statusEl.textContent = currentLang === 'zh' ? 'Cron 表达式格式错误' : 'Invalid cron expression format';
            statusEl.style.opacity = '1';
            setTimeout(() => { statusEl.style.opacity = '0'; }, 3000);
            return;
        }
    ...
    const action = {
        ...
        receiver_name: '',
        is_group: false,
        notify_session_id: receiverInput.value.trim()
    };
    ...
    fetch('/api/scheduler/create', {
        ...
        if (res.status === 'success') {
            ...
            statusEl.textContent = res.message || (currentLang === 'zh' ? '创建失败' : 'Create failed');
            statusEl.style.opacity = '1';
            setTimeout(() => { statusEl.style.opacity = '0'; }, 3000);
        }
    }).catch(() => {
...
function saveTaskEdit() {
    ...
    if (currentEditingTask && currentEditingTask.action) {
        action.receiver = receiverInput.value.trim() || (currentEditingTask.action.receiver || '');
        action.receiver_name = currentEditingTask.action.receiver_name || '';
        action.is_group = currentEditingTask.action.is_group || false;
        action.notify_session_id = receiverInput.value.trim() || (currentEditingTask.action.notify_session_id || '');
...
document.getElementById('task-edit-channel-type').addEventListener('change', function() {
    const ch = this.value;
    const hidden = document.getElementById('task-edit-receiver');
    loadTaskReceiverOptions((hidden && hidden.value) || '', ch);
});
document.getElementById('task-edit-modal-cancel').addEventListener('click', closeTaskEditModal);
document.getElementById('task-edit-modal-save').addEventListener('click', function() {
    if (isAddMode) {
        saveNewTask();
    } else {
        saveTaskEdit();
...
// =====================================================================
// Agent view: node management (proxy via /api/agent/nodes)
// =====================================================================
const AGENT_KIND_LABELS = { text: '文本', image: '图像', audio: '音频', video: '视频', model3d: '3D 模型' };
const AGENT_HEALTH_LABELS = {
    pass: { text: '在线', color: 'text-emerald-500', dot: 'bg-emerald-500' },
    warn: { text: '警告', color: 'text-amber-500', dot: 'bg-amber-500' },
    fail: { text: '离线', color: 'text-red-500', dot: 'bg-red-500' },
    info: { text: '跳过', color: 'text-slate-400', dot: 'bg-slate-400' }
};

// 业务规则体系（与 Mo 规则引擎 agent_rule_engine.mo 一一对应）
// 每个节点：desc=功能分析, allow=适用场景, block=禁用场景, compliance=合规红线, depends=输入依赖
const AGENT_RULES = {
    chat: { desc: '用户输入文本，作为工作流起点。常用于 prompt 注入与对话入口。', allow: ['对话入口', '工作流起点', 'prompt 注入'], block: ['图片处理', '音频合成', '视频生成', '3D 建模'], compliance: ['不得注入越权指令', '不得携带违法提示词'], depends: ['文本'] },
    image_input: { desc: '用户上传一张图片，作为工作流起点。常用于上游输入与素材接入。', allow: ['图片上传', '工作流起点', '素材输入', '图像任务入口'], block: ['纯文本对话', '语音合成', '视频生成'], compliance: ['不得上传侵权/涉密图片', '涉及人像需有授权'], depends: ['图片文件'] },
    tts: { desc: 'Microsoft Edge TTS（200+ 语音），文本 → 语音。用于配音、播报与朗读。', allow: ['语音播报', '配音', '无障碍朗读', '语音朗读', '有声内容'], block: ['语音克隆', '声纹认证', '乐器音乐生成'], compliance: ['不得克隆未经授权的声音', '不得伪造名人语音实施诈骗'], depends: ['文本', '可选音色'] },
    asr: { desc: 'OpenAI Whisper，语音/音频 → 文本。用于转写、字幕与语音命令。', allow: ['会议转写', '字幕生成', '语音命令', '音频归档', '采访整理'], block: ['窃听', '监听未授权通话', '声纹识别'], compliance: ['仅处理已授权音频', '不得窃听未经授权的录音'], depends: ['音频文件', '任务类型'] },
    music: { desc: 'Meta MusicGen，提示词 → 音乐。用于配乐、背景音乐与创作灵感。', allow: ['背景音乐', '配乐', '音乐创作灵感', '音效设计'], block: ['冒充歌手作品', '商业翻唱发行', '版权旋律克隆'], compliance: ['不得生成含他人版权旋律并冒用', '商用需确认授权'], depends: ['音乐描述提示词'] },
    deepresearch: { desc: 'DuckDuckGo + 抓取 + 综合分析，生成研究报告。用于调研与资料整理。', allow: ['行业调研', '竞品分析', '学术资料整理', '市场报告', '背景研究'], block: ['编造数据报告', '投资决策依据', '医疗诊断建议'], compliance: ['引用需可溯源', '不得编造实验数据', '不得输出误导性投资建议'], depends: ['研究问题', '可选步数'] },
    wdtagger: { desc: 'WD Tagger，图片 → 标签文本。用于分类、归档、检索与审核辅助。', allow: ['图片分类', '图库归档', '内容检索', '内容审核辅助', '训练集标注'], block: ['人脸身份识别', '人脸追踪监控', '隐私画像'], compliance: ['人脸标签不得关联个人身份', '不得用于无授权的人员跟踪'], depends: ['图片'] },
    flux: { desc: 'FLUX / Kontext / Imagen 4 等多模型，文本 → 图片。用于创意设计与插画。', allow: ['创意设计', '插画', '海报', '概念图', '电商素材', '头像壁纸'], block: ['违法内容生成', '伪造证件', '虚假新闻配图'], compliance: ['不得生成毒品/武器/暴力/色情/仇恨内容', '真实人物肖像需授权'], depends: ['提示词', '可选模型'] },
    bgremove: { desc: 'RemBG 17 种模型，图片 → 去背景图片。用于抠图与素材处理。', allow: ['电商白底图', '证件照处理', '设计抠图', '产品图处理', '素材准备'], block: ['去除版权水印后商用', '篡改证据图片'], compliance: ['不得规避版权保护标识', '处理他人作品需授权'], depends: ['图片', '可选模型'] },
    rmbg20: { desc: 'BRIA-RMBG-2.0，图片 → 透明背景 PNG。用于设计素材与贴图。', allow: ['透明背景素材', '设计贴图', '图标制作', '电商素材'], block: ['去除版权水印后商用', '篡改证据图片'], compliance: ['不得规避版权保护标识', '处理他人作品需授权'], depends: ['图片'] },
    upscale: { desc: '图片 2x / 4x 放大。用于老照片修复、印刷放大与素材提升。', allow: ['老照片修复', '印刷放大', '素材高清化', '截图增强'], block: ['伪造证据', '篡改凭证图像'], compliance: ['不得用于伪造证据/凭证', '放大后标注处理痕迹更稳妥'], depends: ['图片', '可选模型'] },
    enhance: { desc: 'Finegrain 图片增强 / 超分。用于画质提升与摄影后期。', allow: ['画质提升', '摄影后期', '素材优化', '细节还原'], block: ['篡改凭证/证据图像', '美化用于误导'], compliance: ['不得篡改关键凭证', '处理真实场景需保留原始底片'], depends: ['图片', '可选提示词', '放大倍数'] },
    imgedit: { desc: 'Omni-Edit：文生图 / 单图编辑 / 超分。综合图像处理能力。', allow: ['文生图', '图像编辑', '风格转换', '细节修改', '超分'], block: ['违法内容生成', '未授权人像修改', '伪造身份图片'], compliance: ['内容安全红线', '肖像修改需本人授权'], depends: ['模式', '可选图片', '可选提示词'] },
    tryon: { desc: 'AI 虚拟试穿：人物 + 服装 → 试穿效果。用于电商服饰与搭配推荐。', allow: ['电商服饰', '搭配推荐', '试穿预览', '服装展示'], block: ['裸体试穿', '色情内容', '未授权他人形象试穿'], compliance: ['人物需本人授权', '不得生成不当暴露内容'], depends: ['人物图', '服装图'] },
    anypose: { desc: 'Qwen AnyPose：参考人物 + 姿态图 → 新姿态图。用于动画与姿态设计。', allow: ['动画姿态参考', '运动分析', '3D 姿态设计', '动作设计'], block: ['色情姿态生成', '暴力姿态', '未授权人物姿态'], compliance: ['不得生成不当内容', '人物需本人授权'], depends: ['参考人物图', '姿态图', '可选提示词'] },
    faceswap: { desc: '源人脸 + 目标图 → 换脸图。用于授权影视后期与个人娱乐创作。', allow: ['影视后期(已授权)', '本人素材娱乐创作', '历史影像修复(授权)'], block: ['深度伪造', '冒用他人身份', '诈骗', '诽谤', '色情换脸', '政治人物恶搞'], compliance: ['高风险节点：必须本人授权', '合成内容需显著标注', '不得用于欺骗/诈骗/诽谤'], depends: ['源人脸图', '目标图'] },
    i2v: { desc: '图片 + 提示词 → 视频。用于动态素材、短视频与产品展示。', allow: ['动态素材', '短视频创作', '产品展示动画', '广告动效'], block: ['深度伪造视频', '虚假新闻', '违法视频生成'], compliance: ['内容安全红线', '涉及真实人物需授权', '合成视频建议标注'], depends: ['起始图', '可选运动描述', '时长'] },
    threed: { desc: '单图 → 3D 网格（OBJ/GLB）。用于建模、游戏资产与产品预览。', allow: ['3D 建模', '游戏资产', '产品 3D 预览', '3D 打印模型', '数字孪生'], block: ['武器等受限模型', '涉及商业秘密产品逆向'], compliance: ['遵守当地武器/受限物品法规', '他人产品 3D 化需授权'], depends: ['图片', '可选去背景', '种子'] },
    mocap: { desc: '视频 → 3D 运动数据 (FBX)。用于动画、游戏动捕与运动分析。', allow: ['动画制作', '游戏动捕', '体育动作分析', '康复评估', '表演存档'], block: ['非授权动作数据采集', '侵犯隐私拍摄'], compliance: ['视频来源需授权', '涉及人员需知情同意'], depends: ['视频', '来源类型', '示例ID'] }
};

// 节点调用参数元数据（与 AWF 5000 节点端点一致，field.type: text|textarea|number|select|file）
const AGENT_INVOKE_META = {
    chat: { fields: [{ key: 'message', label: '消息内容', type: 'textarea', required: true, placeholder: '请输入要发送的消息' }] },
    tts: { fields: [
        { key: 'prompt', label: '要合成的文本', type: 'textarea', required: true },
        { key: 'voice', label: '音色（可选）', type: 'text', placeholder: '如 zh-CN-XiaoxiaoNeural - zh-CN (Female)' }
    ] },
    asr: { fields: [{ key: 'audio', label: '音频文件', type: 'file', required: true, accept: 'audio/*,.wav,.mp3,.m4a,.flac,.ogg' }] },
    music: { fields: [{ key: 'prompt', label: '音乐描述', type: 'textarea', required: true, placeholder: '如：轻快的钢琴背景音乐' }] },
    deepresearch: { fields: [
        { key: 'query', label: '研究问题', type: 'textarea', required: true },
        { key: 'max_steps', label: '最大步数（可选）', type: 'number', placeholder: '6' }
    ] },
    wdtagger: { fields: [{ key: 'image', label: '图片', type: 'file', required: true, accept: 'image/*' }] },
    flux: { fields: [
        { key: 'prompt', label: '画面描述', type: 'textarea', required: true, placeholder: '如：一只猫在月球上，赛博朋克风格' },
        { key: 'model', label: '模型（可选）', type: 'text', placeholder: 'flux1 / flux1-kontext / turbo / imagen4' }
    ] },
    bgremove: { fields: [
        { key: 'image', label: '图片', type: 'file', required: true, accept: 'image/*' },
        { key: 'model', label: '模型（可选）', type: 'text', placeholder: 'u2net / isnet-general-use 等' }
    ] },
    rmbg20: { fields: [{ key: 'image', label: '图片', type: 'file', required: true, accept: 'image/*' }] },
    upscale: { fields: [
        { key: 'image', label: '图片', type: 'file', required: true, accept: 'image/*' },
        { key: 'model', label: '模型（可选）', type: 'text', placeholder: 'modelx4 / real4x-anime 等' }
    ] },
    enhance: { fields: [
        { key: 'image', label: '图片', type: 'file', required: true, accept: 'image/*' },
        { key: 'prompt', label: '增强描述（可选）', type: 'text' },
        { key: 'scale', label: '放大倍数（可选）', type: 'number', placeholder: '2' }
    ] },
    imgedit: { fields: [
        { key: 'mode', label: '模式', type: 'select', options: [['text2img', '文生图'], ['img2img', '单图编辑'], ['upscale', '图片超分']] },
        { key: 'prompt', label: '提示词', type: 'textarea', required: true },
        { key: 'image', label: '图片（编辑/超分时需要）', type: 'file', accept: 'image/*' }
    ] },
    tryon: { fields: [
        { key: 'human_image', label: '人物图', type: 'file', required: true, accept: 'image/*' },
        { key: 'garment_image', label: '服装图', type: 'file', required: true, accept: 'image/*' }
    ] },
    anypose: { fields: [
        { key: 'reference_image', label: '参考人物图', type: 'file', required: true, accept: 'image/*' },
        { key: 'pose_image', label: '姿态图', type: 'file', required: true, accept: 'image/*' },
        { key: 'prompt', label: '提示词（可选）', type: 'text' }
    ] },
    faceswap: { fields: [
        { key: 'src_img', label: '源人脸图', type: 'file', required: true, accept: 'image/*' },
        { key: 'dest_img', label: '目标图', type: 'file', required: true, accept: 'image/*' }
    ] },
    i2v: { fields: [
        { key: 'image', label: '起始图', type: 'file', required: true, accept: 'image/*' },
        { key: 'prompt', label: '运动描述（可选）', type: 'textarea', placeholder: '如：画面动起来，电影级运镜' },
        { key: 'duration_seconds', label: '时长（秒，可选）', type: 'number', placeholder: '3.5' }
    ] },
    threed: { fields: [
        { key: 'image', label: '图片', type: 'file', required: true, accept: 'image/*' },
    ...
    mocap: { fields: [
        { key: 'source_type', label: '数据来源', type: 'select', options: [['sample', '官方示例'], ['upload', '上传视频']] },
        { key: 'sample_id', label: '示例ID（示例模式）', type: 'text', placeholder: 'case0' },
        { key: 'video', label: '视频文件（上传模式）', type: 'file', accept: 'video/*,.mp4,.mov,.webm' }
    ] }
};

let agentHealthMap = {};          // node_id -> health entry
let agentHealthTimer = null;

function renderAgentNodes(data) {
    const grid = document.getElementById('agent-node-grid');
    const countEl = document.getElementById('agent-list-count');
    if (!data || !data.ok || !data.items) {
        grid.innerHTML = '<div class="col-span-full text-center text-red-400 text-sm py-10">' +
            (data && data.error ? data.error : '加载失败') + '</div>';
        countEl.textContent = '';
        return;
    }
    const items = data.items || [];
    countEl.textContent = '共 ' + data.total + ' 个节点';
    if (!items.length) {
        grid.innerHTML = '<div class="col-span-full text-center text-slate-400 text-sm py-10">暂无节点</div>';
        return;
    }
    // 统计
    const enabled = items.filter(n => n.status === 1).length;
    const kinds = {};
    items.forEach(n => { kinds[n.kind] = (kinds[n.kind] || 0) + 1; });
    document.getElementById('stat-total').textContent = items.length;
    document.getElementById('stat-enabled').textContent = enabled;
    document.getElementById('stat-disabled').textContent = items.length - enabled;
    document.getElementById('stat-kinds').textContent = Object.keys(kinds).map(k => (AGENT_KIND_LABELS[k] || k) + ' ' + kinds[k]).join(' · ') || '-';

    grid.innerHTML = items.map(function (n) {
        const kind = AGENT_KIND_LABELS[n.kind] || n.kind || '-';
        const health = agentHealthMap[n.id];
        const hMeta = AGENT_HEALTH_LABELS[(health && health.health_status) || (n.status === 1 ? 'pass' : 'info')] || AGENT_HEALTH_LABELS.info;
        const latencyText = health && health.latency ? ' · ' + health.latency + 'ms' : '';
        const disabled = n.status !== 1;
        const rule = AGENT_RULES[n.code] || null;
        // 标签组（限制展示数量，超出折叠）
        function ruleChips(arr, max, cls) {
            if (!arr || !arr.length) return '';
            const shown = arr.slice(0, max);
            const more = arr.length > max ? '<span class="text-[10px] text-slate-400">+' + (arr.length - max) + '</span>' : '';
            return '<span class="px-1.5 py-0.5 text-[10px] rounded bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 ' + cls + '">' + shown.join('</span><span class="px-1.5 py-0.5 text-[10px] rounded bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 ' + cls + '">') + '</span>' + more;
        }
        return '<div class="bg-white dark:bg-[#1A1A1A] rounded-xl border border-slate-200 dark:border-white/10 p-4 flex items-start gap-3 transition-all duration-200 hover:shadow-md hover:border-emerald-300/60 dark:hover:border-emerald-500/40 ' +
            (disabled ? 'opacity-60' : '') + '">' +
            // 左侧图标块（对齐技能页卡片）
            '<div class="w-9 h-9 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 flex items-center justify-center flex-shrink-0 text-lg">' + (n.icon || '🔧') + '</div>' +
            // 右侧内容
            '<div class="flex-1 min-w-0">' +
            // 头部：名称 + 健康度
            '<div class="flex items-center gap-2 mb-0.5">' +
            '<span class="font-medium text-sm text-slate-700 dark:text-slate-200 truncate flex-1">' + (n.name || '-') + '</span>' +
            '<span class="inline-flex items-center gap-1.5 text-xs ' + hMeta.color + ' flex-shrink-0"><span class="w-1.5 h-1.5 rounded-full ' + hMeta.dot + '"></span>' + hMeta.text + latencyText + '</span>' +
            '</div>' +
            // 编码
            '<div class="text-[11px] font-mono text-slate-400 mb-1 truncate">' + (n.code || '-') + '</div>' +
            // 分类 + 状态
            '<div class="flex items-center gap-1.5 flex-wrap mb-1.5">' +
            '<span class="px-2 py-0.5 text-[11px] rounded-full bg-slate-100 dark:bg-white/10 text-slate-500 dark:text-slate-400">' + kind + '</span>' +
            (disabled
                ? '<span class="px-2 py-0.5 text-[11px] rounded-full bg-slate-100 dark:bg-white/10 text-slate-400">已禁用</span>'
                : '<span class="px-2 py-0.5 text-[11px] rounded-full bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400">启用</span>') +
            '</div>' +
            // 功能描述（对齐技能页描述样式）
            '<p class="text-xs text-slate-400 dark:text-slate-500 line-clamp-2 mb-1.5">' + (rule ? rule.desc : (n.remark || '')) + '</p>' +
            // API 地址
            '<div class="text-[11px] font-mono text-blue-500 truncate mb-1">' + (n.api_url || '<span class="text-slate-300 dark:text-slate-600">未配置 API</span>') + '</div>' +
            // 适用场景
            (rule ? '<div class="flex items-start gap-1 flex-wrap mb-1"><span class="text-[10px] text-emerald-500 font-semibold leading-4 mt-0.5 shrink-0">适用</span>' + ruleChips(rule.allow, 3, 'text-emerald-600 dark:text-emerald-400 border-emerald-200/70 dark:border-emerald-500/25 bg-emerald-50 dark:bg-emerald-900/25') + '</div>' : '') +
            // 禁用场景
            (rule && rule.block.length ? '<div class="flex items-start gap-1 flex-wrap mb-1"><span class="text-[10px] text-red-400 font-semibold leading-4 mt-0.5 shrink-0">禁用</span>' + ruleChips(rule.block, 3, 'text-red-500 dark:text-red-400 border-red-200/70 dark:border-red-500/25 bg-red-50 dark:bg-red-900/25') + '</div>' : '') +
            // 合规红线（提示）
            (rule && rule.compliance.length ? '<div class="text-[10px] text-amber-500/90 leading-snug line-clamp-1 mb-1" title="' + rule.compliance.join('；') + '">⚠ ' + rule.compliance[0] + (rule.compliance.length > 1 ? ' 等' + rule.compliance.length + '条红线' : '') + '</div>' : '') +
            // 操作按钮（幽灵样式，对齐技能页按钮）
            '<div class="flex gap-2 mt-1.5">' +
            '<button class="agent-invoke-btn flex-1 px-3 py-1.5 text-xs font-medium rounded-lg transition-colors duration-150 cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed ' +
            (disabled ? 'bg-slate-100 dark:bg-slate-800 text-slate-400' : 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-500 hover:bg-indigo-100 dark:hover:bg-indigo-900/30') + '"' +
            (disabled ? ' disabled' : '') + ' data-code="' + n.code + '">' +
            '<i class="fas fa-bolt mr-1"></i>调用</button>' +
            '<button class="agent-bind-btn flex-1 px-3 py-1.5 text-xs font-medium rounded-lg transition-colors duration-150 cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed ' +
            (disabled ? 'bg-slate-100 dark:bg-slate-800 text-slate-400' : 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-500 hover:bg-emerald-100 dark:hover:bg-emerald-900/30') + '"' +
            (disabled ? ' disabled' : '') + ' data-node-id="' + n.id + '">' +
            '<i class="fas fa-comment-dots mr-1"></i>加入对话</button>' +
            '</div>' +
            '</div>' +
            '</div>';
    }).join('');
...
function loadAgentHealth() {
    fetch('/api/agent/nodes/health', { cache: 'no-store' })
        .then(function (r) { return r.json(); })
        .then(function (data) {
            if (data && data.ok) {
                const map = {};
                (data.nodes || []).forEach(function (h) { map[h.id] = h; });
                agentHealthMap = map;
                const summary = data.summary || {};
                const statusEl = document.getElementById('agent-status');
                if (statusEl) {
                    statusEl.innerHTML = '<span class="inline-block w-2 h-2 rounded-full bg-emerald-500 mr-1.5"></span>' +
                        (currentLang === 'zh' ? '健康' : 'Healthy') + ' ' + (summary.ok || 0) + '/' + (summary.total || 0) +
                        (summary.avg_latency ? ' · ' + summary.avg_latency + 'ms' : '');
                }
                // 重新渲染列表（复用最新列表数据）
                const lastList = window.__agentLastList;
                if (lastList) renderAgentNodes(lastList);
            }
        })
        .catch(function () {
            const statusEl = document.getElementById('agent-status');
            if (statusEl) statusEl.innerHTML = '<span class="inline-block w-2 h-2 rounded-full bg-red-500 mr-1.5"></span>' + (currentLang === 'zh' ? '健康检测失败' : 'Health check failed');
        });
}

function loadAgentNodes() {
    const statusEl = document.getElementById('agent-status');
    if (!statusEl) return;
    statusEl.innerHTML = '<span class="inline-block w-2 h-2 rounded-full bg-amber-400 animate-pulse mr-1.5"></span>' + (currentLang === 'zh' ? '检测中…' : 'Checking…');
    fetch('/api/agent/nodes?size=200', { cache: 'no-store' })
        .then(function (r) { return r.json(); })
        .then(function (data) {
            window.__agentLastList = data;
            if (data && data.ok) {
                renderAgentNodes(data);
            } else {
                statusEl.innerHTML = '<span class="inline-block w-2 h-2 rounded-full bg-red-500 mr-1.5"></span>' + (currentLang === 'zh' ? '离线' : 'Offline');
                renderAgentNodes(data);
                return;
            }
            loadAgentHealth();
        ...
        .catch(function () {
            statusEl.innerHTML = '<span class="inline-block w-2 h-2 rounded-full bg-red-500 mr-1.5"></span>' + (currentLang === 'zh' ? '离线' : 'Offline');
            renderAgentNodes({ ok: false, error: currentLang === 'zh' ? '节点管理服务不可用' : 'Node service unavailable' });
        });
}

// ---- 加入对话：绑定节点 -> 新建会话 -> 切换到对话 ----
function showAgentToast(msg) {
    let el = document.getElementById('agent-toast');
    if (!el) {
        el = document.createElement('div');
        el.id = 'agent-toast';
        el.className = 'fixed top-4 left-1/2 -translate-x-1/2 z-[300] px-4 py-2 rounded-lg text-sm text-white bg-slate-800/95 shadow-lg border border-white/10 transition-opacity duration-300 pointer-events-none';
...
function bindNodeToChat(node, done) {
    if (!node || !node.code) return;
    // 智能逻辑：当前会话已绑定智能体 -> 直接追加到当前会话；
    // 否则新建一个对话并绑定（注意 newChat 会重新生成 sessionId，
    // 必须先 newChat 再取 sessionId 作为绑定目标）
    fetch('/api/agent/bind?session_id=' + encodeURIComponent(sessionId), { cache: 'no-store' })
        .then(function (r) { return r.json(); })
        .then(function (cur) {
            const alreadyBound = cur && cur.status === 'success' && cur.bound;
            if (!alreadyBound) {
                newChat(false);
            }
            const targetSid = sessionId;
            return fetch('/api/agent/bind', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ session_id: targetSid, node: node })
            }).then(function (r) { return r.json(); }).then(function (res) {
                if (res.status !== 'success') throw new Error(res.message || '绑定失败');
                navigateTo('chat');
                refreshAgentPanel();
                showAgentToast(alreadyBound
                    ? (currentLang === 'zh' ? '已将智能体「' + (node.name || node.code) + '」加入当前对话' : 'Agent "' + (node.name || node.code) + '" added to current chat')
                    : (currentLang === 'zh' ? '已新建对话并绑定智能体「' + (node.name || node.code) + '」' : 'New chat created with agent "' + (node.name || node.code) + '"'));
                if (done) done();
            });
        })
        .catch(function (e) {
            showAgentToast(e && e.message ? e.message : '网络错误');
        });
}

// 渲染聊天区智能体侧栏 + 顶部横幅（仅绑定智能体的会话显示）
function renderAgentSidePanel(data) {
    const panel = document.getElementById('agent-side-panel');
    const listEl = document.getElementById('agent-side-list');
    const banner = document.getElementById('agent-bound-banner');
    const label = document.getElementById('agent-bound-label');
    const nodes = (data && data.status === 'success' && data.nodes) ? data.nodes : [];

    // 顶部横幅（多节点时显示 N 个智能体）
    if (banner && label) {
        if (nodes.length) {
            label.textContent = nodes.length === 1
                ? (nodes[0].icon || '🤖') + ' 当前智能体：' + nodes[0].name + '（' + nodes[0].code + '）'
                : '已加入 ' + nodes.length + ' 个智能体：' + nodes.map(n => (n.name || n.code)).join('、');
    ...
    // 侧栏：仅当前会话绑定智能体时展示；普通对话隐藏
    if (panel) panel.classList.toggle('hidden', nodes.length === 0);
    if (!listEl) return;
    if (!nodes.length) {
        listEl.innerHTML = '<div class="text-xs text-slate-400 text-center py-8">' +
            (currentLang === 'zh' ? '尚未加入智能体' : 'No agent added yet') +
            '</div>';
        return;
    }
    listEl.innerHTML = nodes.map(function (n) {
        return '<div class="agent-side-item flex items-start gap-2 rounded-lg border border-slate-200 dark:border-white/10 p-2 bg-slate-50 dark:bg-white/5 cursor-pointer hover:border-emerald-300/60 dark:hover:border-emerald-500/40 transition-colors" data-code="' + n.code + '" draggable="true" title="双击查看该智能体的全部功能；拖拽到聊天区运行">' +
            '<span class="text-base leading-none mt-0.5">' + (n.icon || '🤖') + '</span>' +
            '<div class="flex-1 min-w-0">' +
            '<div class="text-xs font-semibold text-slate-800 dark:text-slate-100 truncate">' + (n.name || n.code || '') + '</div>' +
            '<div class="text-[11px] text-slate-400 mt-0.5 truncate">' + (AGENT_KIND_LABELS[n.kind] || n.kind || '') + ' · ' + (n.code || '') + '</div>' +
            '<div class="text-[11px] text-blue-400 font-mono truncate">' + (n.api_url || '-') + '</div>' +
            '</div>' +
            '<button class="agent-side-remove flex-shrink-0 text-slate-300 hover:text-red-500 cursor-pointer transition-colors" data-code="' + n.code + '" title="移除">' +
            '<i class="fas fa-xmark text-xs"></i></button>' +
            '</div>';
    }).join('');
}
...
function refreshAgentPanel() {
    fetch('/api/agent/bind?session_id=' + encodeURIComponent(sessionId), { cache: 'no-store' })
        .then(function (r) { return r.json(); })
        .then(function (data) { renderAgentSidePanel(data); })
        .catch(function () { renderAgentSidePanel({ status: 'error' }); });
}
...
// 按 code 在节点全量列表中查找节点
function findAgentNodeByCode(code) {
    const lastList = window.__agentLastList;
    if (!lastList || !lastList.ok) return null;
    return (lastList.items || []).find(function (n) { return n.code === code; }) || null;
}

// 关闭漂浮窗口
function closeAgentFeatureModal() {
    const overlay = document.getElementById('agent-feature-modal-overlay');
    if (overlay) overlay.classList.add('hidden');
}

// 展示智能体的全部功能
function showAgentFeatureModal(code) {
    const overlay = document.getElementById('agent-feature-modal-overlay');
    const body = document.getElementById('agent-feature-modal-body');
    if (!overlay || !body) return;
    const node = findAgentNodeByCode(code);
    const rule = AGENT_RULES[code] || null;
    if (!node && !rule) {
        body.innerHTML = '<div class="text-center text-slate-400 text-sm py-10">未找到该智能体的信息</div>';
    } else {
        const name = (node && node.name) || code || '未知智能体';
        const icon = (node && node.icon) || '🤖';
        const kind = node ? (AGENT_KIND_LABELS[node.kind] || node.kind || '-') : '-';
        const apiUrl = (node && node.api_url) || '-';
        const desc = rule ? rule.desc : (node && node.remark ? node.remark : '暂无功能说明');
        // 健康度
        const health = node ? agentHealthMap[node.id] : null;
        const hMeta = AGENT_HEALTH_LABELS[(health && health.health_status) || (node && node.status === 1 ? 'pass' : 'info')] || AGENT_HEALTH_LABELS.info;
        const healthText = '<span class="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400"><span class="w-1.5 h-1.5 rounded-full ' + hMeta.dot + ' inline-block mr-1"></span>' + hMeta.text + (health && health.latency ? ' ' + health.latency + 'ms' : '') + '</span>';
        // 区块辅助
        function section(title, inner) {
            return '<div class="mb-4"><div class="text-[11px] font-semibold text-slate-500 dark:text-slate-400 mb-1.5">' + title + '</div>' + inner + '</div>';
        }
        function chips(items, cls) {
            return '<div class="flex flex-wrap gap-1.5">' + items.map(function (s) {
                return '<span class="px-2 py-0.5 text-[11px] rounded-md ' + cls + '">' + s + '</span>';
            }).join('') + '</div>';
        }
        body.innerHTML =
            // 头部：图标 + 名称 + 编码 + 分类 + 健康度 + 关闭
            '<div class="flex items-start gap-3 mb-5">' +
                '<div class="w-12 h-12 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-white/10 flex items-center justify-center text-2xl flex-shrink-0">' + icon + '</div>' +
                '<div class="min-w-0 flex-1">' +
                    '<div class="text-base font-bold text-slate-800 dark:text-slate-100 truncate">' + name + '</div>' +
                    '<div class="text-[11px] mt-1.5 flex items-center gap-1.5 flex-wrap">' +
                        '<span class="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 font-mono">' + code + '</span>' +
                        '<span class="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400">' + kind + '</span>' +
                        healthText +
                    '</div>' +
                '</div>' +
                '<button class="agent-feature-close flex-shrink-0 text-slate-300 hover:text-slate-600 dark:hover:text-slate-200 cursor-pointer transition-colors text-lg leading-none" title="关闭" onclick="closeAgentFeatureModal()"><i class="fas fa-xmark"></i></button>' +
            '</div>' +
            // 功能分析
            section('功能分析', '<div class="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">' + desc + '</div>') +
            // 适用场景
            (rule ? section('适用场景', chips(rule.allow, 'text-emerald-600 dark:text-emerald-400 border border-emerald-200/70 dark:border-emerald-500/25 bg-emerald-50 dark:bg-emerald-900/25')) : '') +
            // 禁用场景
            (rule ? section('禁用场景', chips(rule.block, 'text-red-500 dark:text-red-400 border border-red-200/70 dark:border-red-500/25 bg-red-50 dark:bg-red-900/25')) : '') +
            // 合规红线
            (rule && rule.compliance.length ? section('合规红线', '<div class="space-y-1.5">' + rule.compliance.map(function (c) {
                return '<div class="text-[11px] text-amber-600 dark:text-amber-400 flex items-start gap-1.5"><i class="fas fa-triangle-exclamation text-[10px] mt-0.5 flex-shrink-0"></i><span>' + c + '</span></div>';
            }).join('') + '</div>') : '') +
            // 输入依赖
            (rule ? section('输入依赖', chips(rule.depends, 'text-blue-600 dark:text-blue-400 border border-blue-200/70 dark:border-blue-500/25 bg-blue-50 dark:bg-blue-900/25')) : '') +
            // 调用地址
            section('调用地址', '<div class="text-[11px] font-mono text-blue-500 break-all bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2">' + apiUrl + '</div>');
    }
    overlay.classList.remove('hidden');
}

function unbindAgentNode(code) {
    let url = '/api/agent/bind?session_id=' + encodeURIComponent(sessionId);
    if (code) url += '&code=' + encodeURIComponent(code);
    fetch(url, { method: 'DELETE', cache: 'no-store' })
        .then(function (r) { return r.json(); })
        .then(function () { refreshAgentPanel(); });
}

// 侧栏折叠/展开
function toggleAgentSidePanel() {
    const panel = document.getElementById('agent-side-panel');
    const btn = document.getElementById('agent-side-toggle');
    if (!panel) return;
    const list = document.getElementById('agent-side-list');
    const add = document.getElementById('agent-side-add');
    const title = panel.querySelector('.agent-side-title');
    const collapsed = panel.dataset.collapsed === '1';
    if (collapsed) {
        panel.style.width = '15rem';
        panel.dataset.collapsed = '0';
        if (btn) btn.innerHTML = '<i class="fas fa-angles-right"></i>';
        ...
        if (btn) btn.innerHTML = '<i class="fas fa-angles-left"></i>';
        // 折叠时隐藏列表内容，只留图标
...
// =====================================================================
// Workflow 工作流任务管理
// 任务为中心：创建/保存配置 -> 运行（顺序执行 AWF 节点）-> 历史与结果
// =====================================================================
let wfNodes = [];          // 可用节点列表（来自 /api/agent/nodes，code/name/icon）
let wfStepDraft = [];      // 编辑中的步骤 [{code, params:{}}]
let wfEditingId = null;    // 正在编辑的工作流 id（null = 新建）

// 确保拿到节点列表（优先使用智能体页已加载的缓存）
function ensureWfNodes(cb) {
    const lastList = window.__agentLastList;
    if (lastList && lastList.ok && lastList.items && lastList.items.length) {
        wfNodes = lastList.items || [];
        if (cb) cb();
        return;
    }
    fetch('/api/agent/nodes?size=200', { cache: 'no-store' })
...
function wfNodeName(code) {
    const hit = wfNodes.find(function (n) { return n.code === code; });
    if (hit && hit.name) return hit.name;
    return code || '-';
}

function wfNodeIcon(code) {
    const hit = wfNodes.find(function (n) { return n.code === code; });
    return (hit && hit.icon) || '🔧';
}
    ...
    fetch('/api/workflow', { cache: 'no-store' })
        ...
        .then(function (data) {
            if (!data || data.status !== 'success') {
                ...
                if (list) list.innerHTML = '<div class="col-span-full text-center text-red-400 text-sm py-10">加载失败</div>';
                return;
            }
        ...
        .catch(function () {
            const list = document.getElementById('wf-list');
            if (list) list.innerHTML = '<div class="col-span-full text-center text-red-400 text-sm py-10">网络错误</div>';
        });
}
...
function renderWorkflows(data) {
    ...
    window.__workflowsCache = workflows;
    // 统计
    const ok = runs.filter(function (r) { return r.status === 'success'; }).length;
    document.getElementById('wf-stat-total').textContent = workflows.length;
    document.getElementById('wf-stat-runs').textContent = runs.length;
    document.getElementById('wf-stat-ok').textContent = ok;
    document.getElementById('wf-stat-fail').textContent = runs.length - ok;
    const statusEl = document.getElementById('wf-status');
    if (statusEl) statusEl.innerHTML = '<span class="inline-block w-2 h-2 rounded-full bg-emerald-500 mr-1.5"></span>' +
        (currentLang === 'zh' ? '就绪' : 'Ready') + ' · ' + workflows.length + ' 个任务';

    const countEl = document.getElementById('wf-list-count');
    countEl.textContent = '共 ' + workflows.length + ' 个';
    countEl.classList.toggle('hidden', !workflows.length);

    const list = document.getElementById('wf-list');
    if (!workflows.length) {
        list.innerHTML = '<div class="col-span-full text-center text-slate-400 text-sm py-10">' +
            (currentLang === 'zh' ? '暂无工作流，点击右上角「新建工作流」创建' : 'No workflows yet — create one first') + '</div>';
    } else {
        list.innerHTML = workflows.map(function (wf) {
            const steps = wf.steps || [];
            const chips = steps.slice(0, 4).map(function (s) {
                return '<span class="px-1.5 py-0.5 text-[10px] rounded bg-slate-100 dark:bg-white/10 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-white/10">' +
                    escapeHtml(wfNodeName(s.code)) + '</span>';
            }).join('');
            const more = steps.length > 4 ? '<span class="text-[10px] text-slate-400">+' + (steps.length - 4) + '</span>' : '';
            return '<div class="bg-white dark:bg-[#1A1A1A] rounded-xl border border-slate-200 dark:border-white/10 p-4 flex items-start gap-3 transition-all duration-200 hover:shadow-md hover:border-emerald-300/60 dark:hover:border-emerald-500/40">' +
                '<div class="w-9 h-9 rounded-lg bg-indigo-50 dark:bg-indigo-900/20 flex items-center justify-center flex-shrink-0 text-indigo-500"><i class="fas fa-diagram-project text-sm"></i></div>' +
                '<div class="flex-1 min-w-0">' +
                '<div class="flex items-center gap-2 mb-0.5">' +
                '<span class="font-medium text-sm text-slate-700 dark:text-slate-200 truncate flex-1">' + escapeHtml(wf.name || '未命名') + '</span>' +
                '<span class="text-[10px] text-slate-400 flex-shrink-0">' + steps.length + ' 步</span>' +
                '</div>' +
                '<div class="text-[11px] text-slate-400 line-clamp-1 mb-1.5">' + escapeHtml(wf.desc || '--') + '</div>' +
                '<div class="flex items-center gap-1 flex-wrap mb-2">' + chips + more + '</div>' +
                '<div class="flex gap-2">' +
                '<button class="wf-run-btn flex-1 px-3 py-1.5 text-xs font-medium rounded-lg transition-colors duration-150 cursor-pointer bg-emerald-50 dark:bg-emerald-900/20 text-emerald-500 hover:bg-emerald-100 dark:hover:bg-emerald-900/30" data-id="' + wf.id + '"><i class="fas fa-play mr-1"></i>运行</button>' +
                '<button class="wf-edit-btn px-3 py-1.5 text-xs font-medium rounded-lg transition-colors duration-150 cursor-pointer bg-indigo-50 dark:bg-indigo-900/20 text-indigo-500 hover:bg-indigo-100 dark:hover:bg-indigo-900/30" data-id="' + wf.id + '"><i class="fas fa-pen mr-1"></i>编辑</button>' +
                '<button class="wf-del-btn px-3 py-1.5 text-xs font-medium rounded-lg transition-colors duration-150 cursor-pointer bg-red-50 dark:bg-red-900/20 text-red-400 hover:bg-red-100 dark:hover:bg-red-900/30" data-id="' + wf.id + '"><i class="fas fa-trash"></i></button>' +
                '</div>' +
                '</div>' +
                '</div>';
        }).join('');
...
function renderWfRuns(runs) {
    const wrap = document.getElementById('wf-runs');
    if (!wrap) return;
    cvDisposeGlbViewers();
    if (!runs.length) {
        wrap.innerHTML = '<div class="text-center text-slate-400 text-sm py-10">' +
            (currentLang === 'zh' ? '暂无运行记录' : 'No run history yet') + '</div>';
        return;
    }
    wrap.innerHTML = runs.map(function (run) {
        const badge = run.status === 'success'
            ? '<span class="px-2 py-0.5 text-[10px] rounded-full bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400">成功</span>'
            : '<span class="px-2 py-0.5 text-[10px] rounded-full bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400">失败</span>';
        const stepsHtml = (run.steps || []).map(function (s, i) {
            const ok = s.status === 'success';
            const resHtml = ok ? wfResultHtml(s.result || {}) : '';
            return '<div class="flex gap-3">' +
                '<div class="flex flex-col items-center flex-shrink-0">' +
                '<span class="w-6 h-6 rounded-full flex items-center justify-center text-[10px] ' + (ok ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-500' : 'bg-red-50 dark:bg-red-900/20 text-red-500') + '">' + (i + 1) + '</span>' +
                (i < (run.steps || []).length - 1 ? '<span class="w-px flex-1 bg-slate-200 dark:bg-white/10 my-1"></span>' : '') +
                '</div>' +
                '<div class="flex-1 min-w-0 pb-3">' +
                '<div class="flex items-center gap-2 mb-1">' +
                '<span class="text-xs font-semibold text-slate-700 dark:text-slate-200">' + wfNodeIcon(s.code) + ' ' + escapeHtml(wfNodeName(s.code)) + '</span>' +
                (ok ? '<span class="text-[10px] text-emerald-500">✓</span>' : '<span class="text-[10px] text-red-500">✗</span>') +
                '</div>' +
                (ok ? (resHtml ? '<div class="text-xs space-y-1.5 max-w-md">' + resHtml + '</div>' : '') :
                    '<div class="text-[11px] text-red-500">' + escapeHtml(s.error || '失败') + '</div>') +
                '</div>' +
                '</div>';
        }).join('');
        return '<div class="bg-white dark:bg-[#1A1A1A] rounded-xl border border-slate-200 dark:border-white/10 overflow-hidden">' +
            '<div class="flex items-center gap-2 px-4 py-2.5 bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-white/10">' +
            '<i class="fas fa-diagram-project text-[11px] text-slate-400"></i>' +
            '<span class="text-xs font-semibold text-slate-700 dark:text-slate-200 truncate flex-1">' + escapeHtml(run.workflow_name || '工作流') + '</span>' +
            badge +
            '<span class="text-[10px] text-slate-400 flex-shrink-0 font-mono">' + escapeHtml(run.finished_at || '') + '</span>' +
            '</div>' +
            '<div class="p-4">' + stepsHtml + '</div>' +
            (run.error ? '<div class="px-4 pb-3 text-[11px] text-red-500">' + escapeHtml(run.error) + '</div>' : '') +
            '</div>';
    }).join('');
    cvInitGlbViewers();
}
...
function openWorkflowModal(wf) {
    wfEditingId = wf ? wf.id : null;
    document.getElementById('wf-modal-title').textContent = wf ? '编辑工作流' : '新建工作流';
    document.getElementById('wf-modal-name').value = wf ? (wf.name || '') : '';
    document.getElementById('wf-modal-desc').value = wf ? (wf.desc || '') : '';
    wfStepDraft = (wf && wf.steps ? wf.steps : []).map(function (s) {
        return { code: s.code || '', params: Object.assign({}, s.params || {}) };
    });
    renderWfSteps();
    ensureWfNodes(function () { renderWfSteps(); });
    document.getElementById('wf-modal-overlay').classList.remove('hidden');
}

function closeWorkflowModal() {
    document.getElementById('wf-modal-overlay').classList.add('hidden');
}

function saveWorkflowModal() {
    const name = document.getElementById('wf-modal-name').value.trim();
    if (!name) { showAgentToast(currentLang === 'zh' ? '请输入工作流名称' : 'Workflow name required'); return; }
    if (!wfStepDraft.length) { showAgentToast(currentLang === 'zh' ? '请至少添加一个步骤节点' : 'Add at least one step'); return; }
    // 收集步骤参数
    const steps = wfStepDraft.map(function (step, idx) {
        const params = {};
        document.querySelectorAll('.wf-step-param[data-idx="' + idx + '"]').forEach(function (el) {
            const key = el.dataset.key;
            const val = el.value.trim();
            if (val !== '') params[key] = val;
        });
        return { code: step.code, name: wfNodeName(step.code), params: params };
    });
    const payload = { action: wfEditingId ? 'update' : 'create', name: name, desc: document.getElementById('wf-modal-desc').value.trim(), steps: steps };
    ...
    .then(function (data) {
        if (data.status === 'success') {
            closeWorkflowModal();
            loadWorkflows();
            showAgentToast(currentLang === 'zh' ? '工作流已保存' : 'Workflow saved');
        } else {
            showAgentToast(data.message || '保存失败');
        }
    })
    .catch(function () { showAgentToast(currentLang === 'zh' ? '网络错误' : 'Network error'); });
}
...
function renderWfSteps() {
    ...
    if (!wrap) return;
    if (!wfStepDraft.length) {
        wrap.innerHTML = '<div class="text-xs text-slate-400 text-center py-6 border border-dashed border-slate-200 dark:border-white/10 rounded-xl">' +
            (currentLang === 'zh' ? '尚未添加步骤，点击上方「添加节点」' : 'No steps yet — click "Add node"') + '</div>';
        return;
    }
    const nodeOpts = wfNodes.length
        ? wfNodes.map(function (n) {
            return '<option value="' + n.code + '">' + escapeHtml(n.name || n.code) + '</option>';
        }).join('')
        : Object.keys(AGENT_INVOKE_META).map(function (c) {
            return '<option value="' + c + '">' + c + '</option>';
        }).join('');
    wrap.innerHTML = wfStepDraft.map(function (step, idx) {
        const meta = AGENT_INVOKE_META[step.code] || { fields: [] };
        return '<div class="wf-step-item rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-800/60 p-3 space-y-2">' +
            '<div class="flex items-center gap-2">' +
            '<span class="text-xs text-slate-400 w-5 flex-shrink-0">' + (idx + 1) + '</span>' +
            '<select class="wf-step-code flex-1 min-w-0 text-xs rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-white/5 text-black dark:text-white px-2 py-1.5 focus:outline-none focus:border-primary-500 cursor-pointer" data-idx="' + idx + '" onchange="wfStepCodeChanged(this, ' + idx + ')">' +
            '<option value="">' + (currentLang === 'zh' ? '选择节点…' : 'Select node…') + '</option>' + nodeOpts +
            '</select>' +
            '<button class="flex-shrink-0 w-7 h-7 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-white/10 cursor-pointer transition-colors' + (idx === 0 ? ' opacity-30 pointer-events-none' : '') + '" onclick="wfStepMove(' + idx + ', -1)" title="上移"><i class="fas fa-chevron-up text-[10px]"></i></button>' +
            '<button class="flex-shrink-0 w-7 h-7 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-white/10 cursor-pointer transition-colors' + (idx === wfStepDraft.length - 1 ? ' opacity-30 pointer-events-none' : '') + '" onclick="wfStepMove(' + idx + ', 1)" title="下移"><i class="fas fa-chevron-down text-[10px]"></i></button>' +
            '<button class="flex-shrink-0 w-7 h-7 flex items-center justify-center rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 cursor-pointer transition-colors" onclick="wfStepRemove(' + idx + ')" title="删除"><i class="fas fa-xmark text-xs"></i></button>' +
            '</div>' +
            '<div class="wf-step-params grid grid-cols-1 md:grid-cols-2 gap-2" data-idx="' + idx + '">' +
            (meta.fields.length ? meta.fields.map(function (f) {
                const v = (step.params && step.params[f.key] !== undefined) ? step.params[f.key] : '';
                if (f.type === 'select') {
                    const opts = (f.options || []).map(function (o) {
                        const ov = Array.isArray(o) ? o[0] : o;
                        const ol = Array.isArray(o) ? o[1] : o;
                        return '<option value="' + ov + '"' + (String(v) === String(ov) ? ' selected' : '') + '>' + ol + '</option>';
                    }).join('');
                    return '<label class="block"><span class="text-[10px] text-slate-500 dark:text-slate-400">' + f.label + (f.required ? ' *' : '') + '</span>' +
                        '<select class="wf-step-param w-full mt-0.5 text-xs rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-white/5 text-black dark:text-white px-2 py-1.5 focus:outline-none focus:border-primary-500" data-idx="' + idx + '" data-key="' + f.key + '">' + opts + '</select></label>';
                }
                const isFile = f.type === 'file';
                return '<label class="block"><span class="text-[10px] text-slate-500 dark:text-slate-400">' + f.label + (f.required ? ' *' : '') + (isFile ? '（' + (currentLang === 'zh' ? '填 {{prev}} 或文件 URL' : '{{prev}} or file URL') + '）' : '') + '</span>' +
                    '<input type="' + (f.type === 'number' ? 'number' : 'text') + '" class="wf-step-param w-full mt-0.5 text-xs rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-white/5 text-black dark:text-white px-2 py-1.5 focus:outline-none focus:border-primary-500" data-idx="' + idx + '" data-key="' + f.key + '" value="' + escapeHtml(String(v)) + '" placeholder="' + escapeHtml(f.placeholder || '') + '"' + (f.required ? ' required' : '') + '></label>';
            }).join('') : '<div class="text-[10px] text-slate-400 col-span-full">' + (currentLang === 'zh' ? '该节点无需参数' : 'No params needed') + '</div>') +
            '</div>' +
            '</div>';
    }).join('');
    // 回填已选节点
    wfStepDraft.forEach(function (step, idx) {
        const sel = wrap.querySelector('.wf-step-code[data-idx="' + idx + '"]');
        if (sel && step.code) sel.value = step.code;
    });
}

function wfStepCodeChanged(select, idx) {
    wfStepDraft[idx].code = select.value;
    wfStepDraft[idx].params = {};
    renderWfSteps();
}

function wfStepAdd() {
    wfStepDraft.push({ code: '', params: {} });
    renderWfSteps();
}

function wfStepRemove(idx) {
    wfStepDraft.splice(idx, 1);
    renderWfSteps();
}

function wfStepMove(idx, dir) {
    const target = idx + dir;
    if (target < 0 || target >= wfStepDraft.length) return;
    const t = wfStepDraft[idx];
    wfStepDraft[idx] = wfStepDraft[target];
    wfStepDraft[target] = t;
    renderWfSteps();
}

// ---------- 运行 / 删除 ----------
function runWorkflow(id, btn) {
    if (!id) return;
    if (btn) {
        btn.disabled = true;
        btn.innerHTML = '<i class="fas fa-spinner fa-spin mr-1"></i>' + (currentLang === 'zh' ? '运行中' : 'Running');
    }
    fetch('/api/workflow', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    .then(function (data) {
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-play mr-1"></i>' + (currentLang === 'zh' ? '运行' : 'Run');
        }
        if (data.status === 'success') {
            loadWorkflows();
            showAgentToast(currentLang === 'zh' ? '运行完成' : 'Run finished');
        } else {
            showAgentToast(data.message || '运行失败');
            loadWorkflows();
        }
    })
    .catch(function () {
...
function deleteWorkflow(id) {
    showConfirmDialog({
        title: currentLang === 'zh' ? '删除工作流' : 'Delete workflow',
        message: currentLang === 'zh' ? '确认删除该工作流？此操作无法撤销。' : 'Delete this workflow? This cannot be undone.',
        ...
        onConfirm: function () {
            ...
            .then(function (r) { return r.json(); })
            .then(function (data) {
                if (data.status === 'success') loadWorkflows();
                else showAgentToast(data.message || '删除失败');
            })
            .catch(function () { showAgentToast(currentLang === 'zh' ? '网络错误' : 'Network error'); });
        }
    });
...
// =====================================================================
// Workflow Canvas（视图模式：节点分布式布局 + 连线编排）
// =====================================================================
let cvNodes = [];          // [{id, code, x, y, d, params}]
let cvSeq = 0;
let cvEditingId = null;    // 正在编辑参数的节点 id
let cvDrag = null;         // {id, dx, dy}
let cvResize = null;       // {id, cx, cy, startDist, startD}
const CV_NODE_D = 84;      // 圆形触点默认直径
const CV_NODE_MIN = 48;
const CV_NODE_MAX = 140;
const CV_BASE_SCALE = 0.82;  // 神经网络虚幻触感：默认收缩比例
const CV_HOVER_SCALE = 1.18; // 鼠标触碰放大比例
let cvHoverId = null;        // 当前被触碰的节点 id
const CV_COLS = 5;
const CV_GAP_X = 212;
const CV_GAP_Y = 112;

// 初始化节点下拉（复用 ensureWfNodes 的节点数据）
function initCanvasView() {
    const picker = document.getElementById('cv-node-picker');
    if (!picker) return;
    ensureWfNodes(function () {
        const codes = wfNodes.length
            ? wfNodes.filter(function (n) { return AGENT_INVOKE_META[n.code]; }).map(function (n) { return n.code; })
            : Object.keys(AGENT_INVOKE_META);
        if (!codes.length) return;
        picker.innerHTML = '<option value="">选择节点…</option>' + codes.map(function (c) {
            return '<option value="' + c + '">' + escapeHtml(wfNodeName(c)) + '</option>';
        }).join('');
    });
    cvSyncLoad();
}
...
// ---------- 同步普通工作流任务到画布 ----------
let __cvSyncWorkflows = [];   // 缓存普通工作流任务列表
function cvSyncLoad() {
    const picker = document.getElementById('cv-sync-picker');
    if (!picker) return;
    picker.innerHTML = '<option value="">同步任务…</option>';
    fetch('/api/workflow', { cache: 'no-store' })
        .then(function (r) { return r.json(); })
        .then(function (data) {
            if (!data || data.status !== 'success') return;
            const workflows = (data.workflows || []).filter(function (w) { return (w.steps || []).length; });
            __cvSyncWorkflows = workflows;
            if (!workflows.length) {
                picker.innerHTML = '<option value="">暂无任务可同步</option>';
                return;
            }
            picker.innerHTML = '<option value="">同步任务…</option>' + workflows.map(function (w, i) {
                return '<option value="' + i + '">' + escapeHtml(w.name || ('任务 ' + (i + 1))) + '（' + (w.steps || []).length + '步）</option>';
            }).join('');
        })
        .catch(function () {});
}

// 将选中任务的步骤同步为画布节点（顺序布局 + 复制参数）
function cvSyncFrom() {
    const picker = document.getElementById('cv-sync-picker');
    const idx = picker ? parseInt(picker.value, 10) : NaN;
    const wf = __cvSyncWorkflows[idx];
    if (!wf || !wf.steps || !wf.steps.length) {
        showAgentToast(currentLang === 'zh' ? '请先选择要同步的任务' : 'Pick a task to sync');
        return;
    }
    const steps = wf.steps;
    if (cvNodes.length) {
        showConfirmDialog({
            title: currentLang === 'zh' ? '同步任务' : 'Sync workflow',
            message: currentLang === 'zh' ? '同步将替换画布上现有的 ' + cvNodes.length + ' 个节点，继续？' : 'This will replace the current ' + cvNodes.length + ' nodes. Continue?',
            okText: currentLang === 'zh' ? '同步' : 'Sync',
            cancelText: currentLang === 'zh' ? '取消' : 'Cancel',
            onConfirm: function () { cvSyncApply(wf); }
        });
        return;
    }
    cvSyncApply(wf);
...
function cvSyncApply(wf) {
    cvNodes = (wf.steps || []).map(function (s, i) {
        const col = i % CV_COLS;
        const row = Math.floor(i / CV_COLS);
        return {
            id: 'cvn_' + (++cvSeq),
            code: s.code,
            x: 36 + col * CV_GAP_X,
    ...
    cvRender();
    showAgentToast(currentLang === 'zh' ? '已同步「' + wf.name + '」' + cvNodes.length + ' 个节点' : 'Synced "' + wf.name + '" with ' + cvNodes.length + ' nodes');
}

// =====================================================================
// 对话模式（自然语言编排工作流：厂商模型经加密协议 -> 执行 -> OptimizeHook）
// =====================================================================
let __cvChatLastSteps = [];

function openCvChat() {
    const ov = document.getElementById('cv-chat-modal-overlay');
    if (!ov) return;
...
function closeCvChat() {
    const ov = document.getElementById('cv-chat-modal-overlay');
    if (ov) ov.classList.add('hidden');
}

function cvChatAppend(role, html) {
    const box = document.getElementById('cv-chat-msgs');
    if (!box) return;
    const wrap = document.createElement('div');
    wrap.className = 'flex gap-2.5' + (role === 'user' ? ' justify-end' : '');
    wrap.innerHTML = role === 'user'
        ? '<div class="max-w-[80%] rounded-xl rounded-tr-sm bg-violet-500 text-white px-3.5 py-2.5 text-[13px] leading-relaxed">' + html + '</div>'
        : '<div class="w-7 h-7 rounded-full bg-violet-100 dark:bg-violet-900/30 flex items-center justify-center text-violet-500 flex-shrink-0 text-xs"><i class="fas fa-bolt"></i></div>' +
          '<div class="max-w-[85%] min-w-0 rounded-xl rounded-tl-sm bg-slate-100 dark:bg-white/5 px-3.5 py-2.5 text-[13px] text-slate-600 dark:text-slate-300 leading-relaxed">' + html + '</div>';
    box.appendChild(wrap);
    box.scrollTop = box.scrollHeight;
}

function cvChatResultHtml(data) {
    const steps = data.steps || [];
    const reply = data.reply || data.summary || '';
    const chips = steps.map(function (s, i) {
        return '<span class="inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] rounded bg-violet-50 dark:bg-violet-900/20 text-violet-500 border border-violet-200 dark:border-violet-800/50">' +
            '<span class="w-3.5 h-3.5 rounded-full bg-violet-500 text-white flex items-center justify-center text-[8px] font-semibold">' + (i + 1) + '</span>' +
            escapeHtml(wfNodeName(s.code)) + '</span>';
    }).join(' ');
    const pl = data.pipeline || {};
    const optHook = (pl.hook_exec || []).find(function (h) { return h.name === 'OptimizeHook'; });
    const hookHtml = optHook
        ? '<div class="flex items-center gap-1.5 text-[12px] ' + (optHook.ok ? 'text-emerald-500' : 'text-red-500') + '">' +
          '<i class="fas ' + (optHook.ok ? 'fa-circle-check' : 'fa-circle-xmark') + ' text-[10px]"></i>' +
          'OptimizeHook · ' + escapeHtml(optHook.note || '') + '</div>'
        : (pl.ok === false ? '<div class="text-[11px] text-red-500">OptimizeHook 推送失败</div>' : '');
    const cipher = data.cipher || {};
    return '<div class="space-y-2.5">' +
        '<div class="whitespace-pre-wrap text-[13px] text-slate-700 dark:text-slate-200 leading-relaxed">' + escapeHtml(reply) + '</div>' +
        (steps.length ? '<div class="flex items-center gap-1.5 flex-wrap pt-1"><span class="text-[10px] text-slate-400 mr-0.5">可选节点计划：</span>' + chips +
            '<button onclick="cvChatApply()" class="px-2 py-1 text-[10px] font-medium rounded-md text-white bg-emerald-500 hover:bg-emerald-600 cursor-pointer transition-colors"><i class="fas fa-arrow-down-to-line text-[9px] mr-1"></i>应用到画布</button></div>' : '') +
        (cipher.magic ? '<div class="text-[11px] font-mono text-slate-400">加密协议 · magic=' + escapeHtml(cipher.magic) + ' · req=' + escapeHtml(String(cipher.req_id || '').slice(0, 16)) + '</div>' : '') +
        (hookHtml ? '<div class="pt-1">' + hookHtml + '</div>' : '') +
        '</div>';
}

function cvChatApply() {
    if (!__cvChatLastSteps.length) return;
    cvNodes = __cvChatLastSteps.map(function (s, i) {
        const col = i % CV_COLS;
        const row = Math.floor(i / CV_COLS);
        return {
            id: 'cvn_' + (++cvSeq),
    ...
    cvRender();
    showAgentToast(currentLang === 'zh' ? '已应用 ' + cvNodes.length + ' 个节点到画布' : 'Applied ' + cvNodes.length + ' nodes to canvas');
}

function cvChatSend() {
    const input = document.getElementById('cv-chat-input');
    const btn = document.getElementById('cv-chat-send');
    if (!input) return;
    const msg = input.value.trim();
    if (!msg) return;
    input.value = '';
    cvChatAppend('user', escapeHtml(msg));
    const lid = 'cvchat_loading_' + Date.now();
    cvChatAppend('assistant', '<span id="' + lid + '" class="inline-flex items-center gap-2 text-slate-400"><i class="fas fa-spinner fa-spin text-xs"></i>正在调用厂商模型编排工作流…</span>');
    if (btn) btn.disabled = true;
    fetch('/api/workflow', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    .then(function (data) {
        ...
        if (l) { const row = l.closest('.flex'); if (row) row.remove(); }
        if (data.status !== 'success') {
            cvChatAppend('assistant', '<span class="text-red-500"><i class="fas fa-triangle-exclamation mr-1"></i>' + escapeHtml(data.message || '调用失败') + '</span>');
            return;
        }
        __cvChatLastSteps = data.steps || [];
        cvChatAppend('assistant', cvChatResultHtml(data));
    })
    .catch(function () {
        const l = document.getElementById(lid);
        if (l) { const row = l.closest('.flex'); if (row) row.remove(); }
        cvChatAppend('assistant', '<span class="text-red-500"><i class="fas fa-triangle-exclamation mr-1"></i>网络错误</span>');
    })
...
function cvAddNode() {
    const picker = document.getElementById('cv-node-picker');
    const code = picker && picker.value;
    if (!code) { showAgentToast(currentLang === 'zh' ? '请先选择节点' : 'Pick a node first'); return; }
    const idx = cvNodes.length;
    const col = idx % CV_COLS;
    const row = Math.floor(idx / CV_COLS);
    cvNodes.push({
        id: 'cvn_' + (++cvSeq),
        code: code,
        x: 36 + col * CV_GAP_X,
...
function cvRemoveNode(id) {
    cvNodes = cvNodes.filter(function (n) { return n.id !== id; });
    cvRender();
}
...
function cvRender() {
    ...
    if (!wrap) return;
    empty.classList.toggle('hidden', cvNodes.length > 0);
    wrap.innerHTML = cvNodes.map(function (n, i) {
        const d = n.d || CV_NODE_D;
        const showName = d >= 80;
        return '<div class="cv-node absolute rounded-full border-2 bg-white dark:bg-[#1E1E1E] shadow-sm select-none cursor-move flex items-center justify-center" ' +
            'style="left:' + n.x + 'px; top:' + n.y + 'px; width:' + d + 'px; height:' + d + 'px; border-color:rgba(16,185,129,0.4);" ' +
            'data-id="' + n.id + '" title="拖动调整位置 · 拖动右下角圆点缩放 · 双击编辑参数 · 右键删除">' +
            '<span class="absolute -top-1 -left-1 w-5 h-5 rounded-full bg-emerald-500 text-white flex items-center justify-center text-[10px] font-semibold shadow">' + (i + 1) + '</span>' +
            '<span class="cv-node-icon leading-none" style="font-size:' + Math.min(26, d * 0.32) + 'px;">' + wfNodeIcon(n.code) + '</span>' +
            (showName ? '<span class="cv-node-name absolute left-1/2 -translate-x-1/2 text-[9px] font-medium text-slate-600 dark:text-slate-300 whitespace-nowrap" style="top:calc(100% + 6px)">' + escapeHtml(wfNodeName(n.code)) + '</span>' : '') +
            '<button class="cv-node-del absolute -top-1 -right-1 w-4 h-4 flex items-center justify-center rounded-full bg-slate-200 dark:bg-slate-700 text-slate-500 hover:text-red-500 cursor-pointer" onclick="event.stopPropagation(); cvRemoveNode(\'' + n.id + '\')" title="删除"><i class="fas fa-xmark text-[8px]"></i></button>' +
            '<span class="cv-node-resize absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-emerald-500 border-2 border-white dark:border-[#1E1E1E] cursor-nwse-resize shadow" title="拖动调整大小"></span>' +
            '</div>';
    }).join('');
    cvDrawLines();
    cvBindDrag();
...
// SVG 贝塞尔连线（节点顺序即连线顺序，锚点在圆形触点边缘）
function cvDrawLines() {
    const svg = document.getElementById('cv-lines');
    if (!svg) return;
    let path = '';
    for (let i = 0; i < cvNodes.length - 1; i++) {
        const a = cvNodes[i], b = cvNodes[i + 1];
        // 视觉直径：触碰放大的节点按放大比例计算，其余按收缩比例（与 CSS transform 一致）
        const sa = cvHoverId === a.id ? CV_HOVER_SCALE : CV_BASE_SCALE;
        const sb = cvHoverId === b.id ? CV_HOVER_SCALE : CV_BASE_SCALE;
        const da = (a.d || CV_NODE_D) * sa, db = (b.d || CV_NODE_D) * sb;
        // 圆心 = 布局中心（CSS transform 围绕该中心缩放，位置不变），半径 = 视觉直径/2
        const ax = a.x + (a.d || CV_NODE_D) / 2, ay = a.y + (a.d || CV_NODE_D) / 2;
        const bx = b.x + (b.d || CV_NODE_D) / 2, by = b.y + (b.d || CV_NODE_D) / 2;
        ...
        const y2 = by - uy * (db / 2);
        const mx = (x1 + x2) / 2;
        path += '<path d="M ' + x1 + ' ' + y1 + ' C ' + mx + ' ' + y1 + ',' + mx + ' ' + y2 + ',' + x2 + ' ' + y2 + '" fill="none" stroke="rgba(16,185,129,0.55)" stroke-width="2" marker-end="url(#cv-arrow)"></path>';
    }
    svg.innerHTML = '<defs><marker id="cv-arrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto">' +
        '<path d="M0,0 L8,4 L0,8 z" fill="rgba(16,185,129,0.7)"></path></marker></defs>' + path;
}

function cvBindDrag() {
    document.querySelectorAll('#cv-nodes .cv-node').forEach(function (el) {
        // 神经网络触感：触碰放大时同步连线锚点
        el.addEventListener('mouseenter', function () {
            const node = cvNodes.find(function (n) { return n.id === el.dataset.id; });
            if (!node) return;
            cvHoverId = node.id;
            cvDrawLines();
        });
        ...
        el.addEventListener('mousedown', function (e) {
            if (e.target.closest('.cv-node-del') || e.target.closest('.cv-node-resize')) return;
            e.preventDefault();
            const node = cvNodes.find(function (n) { return n.id === el.dataset.id; });
            if (!node) return;
            cvDrag = { id: node.id, dx: e.clientX - node.x, dy: e.clientY - node.y };
            el.style.borderColor = 'rgba(16,185,129,0.95)';
            el.style.boxShadow = '0 8px 24px -6px rgba(16,185,129,0.35)';
        });
        el.addEventListener('dblclick', function () {
            const node = cvNodes.find(function (n) { return n.id === el.dataset.id; });
            if (node) openCanvasParamModal(node);
        });
        el.addEventListener('contextmenu', function (e) {
            e.preventDefault();
            cvRemoveNode(el.dataset.id);
        });
    });
}

// 圆形触点缩放：拖拽右下角手柄，按鼠标到圆心的距离比例调整直径
function cvBindResize() {
    document.querySelectorAll('#cv-nodes .cv-node-resize').forEach(function (el) {
        el.addEventListener('mousedown', function (e) {
            e.preventDefault();
            e.stopPropagation();
            const nodeEl = el.closest('.cv-node');
            const node = cvNodes.find(function (n) { return n.id === nodeEl.dataset.id; });
            if (!node) return;
...
document.addEventListener('mousemove', function (e) {
    // 圆形触点缩放
    if (cvResize) {
        const node = cvNodes.find(function (n) { return n.id === cvResize.id; });
        if (node) {
            const dist = Math.hypot(e.clientX - cvResize.cx, e.clientY - cvResize.cy) || 1;
            node.d = Math.round(Math.max(CV_NODE_MIN, Math.min(CV_NODE_MAX, cvResize.startD * (dist / cvResize.startDist))));
            const el = document.querySelector('#cv-nodes .cv-node[data-id="' + cvResize.id + '"]');
            if (el) {
                el.style.width = node.d + 'px';
                el.style.height = node.d + 'px';
                const icon = el.querySelector('.cv-node-icon');
                if (icon) icon.style.fontSize = Math.min(26, node.d * 0.32) + 'px';
                const name = el.querySelector('.cv-node-name');
                if (name) name.style.display = node.d >= 80 ? '' : 'none';
            }
            cvDrawLines();
        }
        return;
    ...
    if (!cvDrag) return;
    const node = cvNodes.find(function (n) { return n.id === cvDrag.id; });
    if (!node) return;
    node.x = Math.max(0, Math.min(1200 - (node.d || CV_NODE_D), e.clientX - cvDrag.dx));
    node.y = Math.max(0, Math.min(620 - (node.d || CV_NODE_D), e.clientY - cvDrag.dy));
    const el = document.querySelector('#cv-nodes .cv-node[data-id="' + cvDrag.id + '"]');
    if (el) { el.style.left = node.x + 'px'; el.style.top = node.y + 'px'; }
    cvDrawLines();
});
document.addEventListener('mouseup', function () {
    if (cvResize) { cvResize = null; return; }
    if (!cvDrag) return;
    const el = document.querySelector('#cv-nodes .cv-node[data-id="' + cvDrag.id + '"]');
    if (el) { el.style.borderColor = 'rgba(16,185,129,0.4)'; el.style.boxShadow = ''; }
    cvDrag = null;
});

// ---------- 节点参数编辑 ----------
function openCanvasParamModal(node) {
    cvEditingId = node.id;
    document.getElementById('cv-param-modal-title').textContent = wfNodeName(node.code) + ' · ' + (currentLang === 'zh' ? '参数' : 'Params');
    const meta = AGENT_INVOKE_META[node.code] || { fields: [] };
    const form = document.getElementById('cv-param-form');
    form.innerHTML = meta.fields.length ? meta.fields.map(function (f) {
        const v = (node.params && node.params[f.key] !== undefined) ? node.params[f.key] : '';
        const inputCls = 'cv-param-input w-full mt-1 text-xs rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-white/5 text-black dark:text-white px-2 py-1.5 focus:outline-none focus:border-primary-500';
        if (f.type === 'select') {
            const opts = (f.options || []).map(function (o) {
                const ov = Array.isArray(o) ? o[0] : o;
                const ol = Array.isArray(o) ? o[1] : o;
            ...
            return '<label class="block"><span class="text-[11px] text-slate-500 dark:text-slate-400">' + f.label + (f.required ? ' *' : '') + '</span><select class="' + inputCls + '" data-key="' + f.key + '">' + opts + '</select></label>';
        }
        const isFile = f.type === 'file';
        return '<label class="block"><span class="text-[11px] text-slate-500 dark:text-slate-400">' + f.label + (f.required ? ' *' : '') + (isFile ? '（' + (currentLang === 'zh' ? '填 {{prev}} 或文件 URL' : '{{prev}} or file URL') + '）' : '') + '</span>' +
            '<input type="' + (f.type === 'number' ? 'number' : 'text') + '" class="' + inputCls + '" data-key="' + f.key + '" value="' + escapeHtml(String(v)) + '" placeholder="' + escapeHtml(f.placeholder || '') + '"></label>';
    }).join('') : '<div class="text-xs text-slate-400">' + (currentLang === 'zh' ? '该节点无需参数' : 'No params needed') + '</div>';
    document.getElementById('cv-param-modal-overlay').classList.remove('hidden');
}

function closeCanvasParamModal() {
    document.getElementById('cv-param-modal-overlay').classList.add('hidden');
}

function saveCanvasParamModal() {
    const node = cvNodes.find(function (n) { return n.id === cvEditingId; });
    if (!node) return;
    const params = {};
    document.querySelectorAll('.cv-param-input').forEach(function (el) {
        const val = el.value.trim();
        if (val !== '') params[el.dataset.key] = val;
    });
    node.params = params;
    closeCanvasParamModal();
    showAgentToast(currentLang === 'zh' ? '参数已保存' : 'Params saved');
}

// ---------- 运行 / 保存 ----------
function cvCollectSteps() {
    return cvNodes.map(function (n) {
        return { code: n.code, name: wfNodeName(n.code), params: Object.assign({}, n.params || {}) };
    });
}

function cvRenderRun(run) {
    const statusEl = document.getElementById('cv-run-status');
    const resultEl = document.getElementById('cv-run-result');
    cvDisposeGlbViewers();
    statusEl.classList.remove('hidden');
    if (run.status === 'success') {
        statusEl.innerHTML = '<span class="inline-block w-2 h-2 rounded-full bg-emerald-500 mr-1"></span>' + (currentLang === 'zh' ? '运行成功' : 'Success');
        statusEl.className = 'text-[11px] text-emerald-500';
    } else {
        statusEl.innerHTML = '<span class="inline-block w-2 h-2 rounded-full bg-red-500 mr-1"></span>' + (currentLang === 'zh' ? '运行失败' : 'Failed');
    ...
    resultEl.innerHTML = stepsHtml || (run.error ? '<div class="text-[11px] text-red-500">' + escapeHtml(run.error) + '</div>' : '<div class="text-xs text-slate-400">' + (currentLang === 'zh' ? '无输出' : 'No output') + '</div>');
    cvInitGlbViewers();
}

function cvRun() {
    if (!cvNodes.length) { showAgentToast(currentLang === 'zh' ? '画布为空，请先添加节点' : 'Canvas is empty'); return; }
    const statusEl = document.getElementById('cv-run-status');
    statusEl.classList.remove('hidden');
    statusEl.className = 'text-[11px] text-amber-500';
    statusEl.innerHTML = '<span class="inline-block w-2 h-2 rounded-full bg-amber-400 animate-pulse mr-1"></span>' + (currentLang === 'zh' ? '运行中…' : 'Running…');
    document.getElementById('cv-run-result').innerHTML = '<div class="text-xs text-slate-400">' + (currentLang === 'zh' ? '正在按连线顺序执行…' : 'Executing in order…') + '</div>';
    fetch('/api/workflow', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'run_inline', name: '视图模式画布', steps: cvCollectSteps() })
    })
    .then(function (r) { return r.json(); })
    .then(function (data) {
        if (data.status === 'success') cvRenderRun(data.run);
        else {
            statusEl.className = 'text-[11px] text-red-500';
            statusEl.innerHTML = '<span class="inline-block w-2 h-2 rounded-full bg-red-500 mr-1"></span>' + (data.message || '运行失败');
        }
    })
    .catch(function () {
        statusEl.className = 'text-[11px] text-red-500';
        statusEl.innerHTML = '<span class="inline-block w-2 h-2 rounded-full bg-red-500 mr-1"></span>' + (currentLang === 'zh' ? '网络错误' : 'Network error');
    });
}

function cvSave() {
    if (!cvNodes.length) { showAgentToast(currentLang === 'zh' ? '画布为空，请先添加节点' : 'Canvas is empty'); return; }
    const name = window.prompt(currentLang === 'zh' ? '输入工作流名称：' : 'Workflow name:');
    if (!name || !name.trim()) return;
    fetch('/api/workflow', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'create', name: name.trim(), desc: currentLang === 'zh' ? '来自视图模式画布' : 'From canvas view', steps: cvCollectSteps() })
    })
    .then(function (r) { return r.json(); })
    .then(function (data) {
        if (data.status === 'success') {
            showAgentToast(currentLang === 'zh' ? '已保存到工作流列表' : 'Saved to workflow list');
            cvSyncLoad();
        } else showAgentToast(data.message || '保存失败');
    })
    .catch(function () { showAgentToast(currentLang === 'zh' ? '网络错误' : 'Network error'); });
}

// =====================================================================
// Workflow Canvas 3D（three.js：3D 背景 + 画布缩放 + GLB 模型查看器）
// =====================================================================

// ---------- 画布缩放（滚轮 + 按钮） ----------
let cvZoom = 1;
const CV_ZOOM_MIN = 0.25;
const CV_ZOOM_MAX = 3;

function applyCvZoom() {
    const t = document.getElementById('cv-transform');
    if (t) t.style.transform = 'scale(' + cvZoom + ')';
...
function cvZoomBy(factor) {
    cvZoom = Math.max(CV_ZOOM_MIN, Math.min(CV_ZOOM_MAX, cvZoom * factor));
    applyCvZoom();
}
...
// ---------- three.js 3D 背景（粒子星云 + 网格地面） ----------
let cvThree = null;

function cvInitThreeBG() {
    if (cvThree) return;
    if (typeof THREE === 'undefined') return;
    const canvas = document.getElementById('cv-three-bg');
    if (!canvas) return;
    const W = 1200, H = 620;
    try {
        ...
        // 网格地面（3D 空间感）
        const grid = new THREE.GridHelper(1500, 22, 0x2dd4bf, 0x1e293b);
        grid.position.y = -200;
        ...
        for (let i = 0; i < count; i++) {
            pos[i * 3] = (Math.random() - 0.5) * 1350;
            pos[i * 3 + 1] = (Math.random() - 0.5) * 680;
            pos[i * 3 + 2] = (Math.random() - 0.5) * 380;
            const t = Math.random();
            col[i * 3] = 0.13 + t * 0.28;
            col[i * 3 + 1] = 0.55 + t * 0.45;
            col[i * 3 + 2] = 0.45 + t * 0.35;
        }
        geo.setAttribute('position', new THREE.BufferAttribute(pos, 3));
        geo.setAttribute('color', new THREE.BufferAttribute(col, 3));
        const mat = new THREE.PointsMaterial({ size: 2.4, vertexColors: true, transparent: true, opacity: 0.8, depthWrite: false });
        ...
        function animate() {
            ...
            points.rotation.x = Math.sin(Date.now() * 0.0002) * 0.05;
            points.position.y = Math.sin(Date.now() * 0.0004) * 10;
            renderer.render(scene, camera);
        }
        animate();
        cvThree = { scene: scene, camera: camera, renderer: renderer, points: points, grid: grid, raf: raf };
    } catch (e) {
        console.error('[Canvas3D] init failed:', e);
        cvThree = null;
    }
}

function cvDisposeThreeBG() {
    if (!cvThree) return;
    cancelAnimationFrame(cvThree.raf);
    cvThree.renderer.dispose();
    cvThree = null;
}

// ---------- 运行结果中的 GLB 3D 模型内嵌查看器 ----------
function wfResultHtml(j) {
    if (!j) return '';
    const url = (j && (j.model_url || j.data_url)) || '';
    if (url && /\.(glb|gltf)(\?|$)/i.test(url)) {
        return '<div class="cv-glb-viewer w-full h-52 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-slate-900 overflow-hidden" data-url="' + url + '"></div>' +
            '<div class="text-[10px] font-mono text-blue-500 truncate mt-1">' + url + '</div>';
    }
    if (url && /\.(obj|fbx)(\?|$)/i.test(url)) {
        return '<div class="text-xs text-blue-500 break-all">3D 模型：<a href="' + url + '" target="_blank" class="underline">' + url + '</a>（OBJ/FBX 暂以链接打开）</div>';
    }
    return renderAgentInvokeContentHtml(j);
}

function cvInitGlbViewers() {
    if (typeof THREE === 'undefined' || typeof THREE.OrbitControls === 'undefined') return;
    document.querySelectorAll('.cv-glb-viewer:not([data-init])').forEach(function (el) {
        el.setAttribute('data-init', '1');
        const url = el.dataset.url;
        if (!url) return;
        try {
            const W = el.clientWidth || 420;
            const H = el.clientHeight || 208;
            const scene = new THREE.Scene();
            ...
            loader.load(url, function (gltf) {
                ...
                el.innerHTML = '<div class="text-xs text-red-400 p-3">3D 加载失败：' + (err && err.message ? escapeHtml(err.message) : '网络错误') + '</div>';
            });
            let raf = 0;
            function tick() {
            ...
            el.innerHTML = '<div class="text-xs text-red-400 p-3">3D 初始化失败</div>';
        }
    });
}

function cvDisposeGlbViewers() {
    document.querySelectorAll('.cv-glb-viewer').forEach(function (el) {
        if (el._raf) cancelAnimationFrame(el._raf);
        if (el._renderer) {
            el._renderer.dispose();
            if (el._renderer.domElement && el._renderer.domElement.parentNode) {
                el._renderer.domElement.parentNode.removeChild(el._renderer.domElement);
            }
        }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Workflow 视图初始化
    loadWorkflows();
    const wfRefresh = document.getElementById('wf-refresh');
    if (wfRefresh) wfRefresh.addEventListener('click', loadWorkflows);
    const wfCreate = document.getElementById('wf-create-btn');
    if (wfCreate) wfCreate.addEventListener('click', function () { openWorkflowModal(null); });
    const wfListEl = document.getElementById('wf-list');
    if (wfListEl) wfListEl.addEventListener('click', function (e) {
        const runBtn = e.target.closest('.wf-run-btn');
        if (runBtn) { runWorkflow(runBtn.dataset.id, runBtn); return; }
        const editBtn = e.target.closest('.wf-edit-btn');
        if (editBtn) {
            const last = window.__workflowsCache || [];
            const wf = last.find(function (w) { return String(w.id) === String(editBtn.dataset.id); });
            if (wf) openWorkflowModal(wf);
            return;
        }
        const delBtn = e.target.closest('.wf-del-btn');
        if (delBtn) deleteWorkflow(delBtn.dataset.id);
    });
    const wfOverlay = document.getElementById('wf-modal-overlay');
    if (wfOverlay) wfOverlay.addEventListener('click', function (e) { if (e.target === wfOverlay) closeWorkflowModal(); });
    // 视图模式（画布）初始化
    initCanvasView();
    const cvAdd = document.getElementById('cv-add-node');
    if (cvAdd) cvAdd.addEventListener('click', cvAddNode);
    const cvSyncBtn = document.getElementById('cv-sync');
    if (cvSyncBtn) cvSyncBtn.addEventListener('click', cvSyncFrom);
    const cvClear = document.getElementById('cv-clear');
    if (cvClear) cvClear.addEventListener('click', function () {
        if (!cvNodes.length) return;
        showConfirmDialog({
            title: currentLang === 'zh' ? '清空画布' : 'Clear canvas',
            message: currentLang === 'zh' ? '确认清空画布上的所有节点？' : 'Clear all nodes on the canvas?',
            okText: currentLang === 'zh' ? '清空' : 'Clear',
            cancelText: currentLang === 'zh' ? '取消' : 'Cancel',
            onConfirm: function () { cvNodes = []; cvRender(); }
        });
    });
    const cvRunBtn = document.getElementById('cv-run');
    if (cvRunBtn) cvRunBtn.addEventListener('click', cvRun);
    const cvSaveBtn = document.getElementById('cv-save');
    if (cvSaveBtn) cvSaveBtn.addEventListener('click', cvSave);
    const cvParamOverlay = document.getElementById('cv-param-modal-overlay');
    if (cvParamOverlay) cvParamOverlay.addEventListener('click', function (e) { if (e.target === cvParamOverlay) closeCanvasParamModal(); });
    // 对话模式：打开 / 关闭 / 回车发送
    const cvChatBtn = document.getElementById('cv-chat-mode');
    if (cvChatBtn) cvChatBtn.addEventListener('click', openCvChat);
    const cvChatOverlay = document.getElementById('cv-chat-modal-overlay');
    if (cvChatOverlay) cvChatOverlay.addEventListener('click', function (e) { if (e.target === cvChatOverlay) closeCvChat(); });
    const cvChatInput = document.getElementById('cv-chat-input');
    if (cvChatInput) cvChatInput.addEventListener('keydown', function (e) {
        if (e.key === 'Enter') cvChatSend();
    });
    // 画布缩放控件
    const cvZoomInBtn = document.getElementById('cv-zoom-in');
    if (cvZoomInBtn) cvZoomInBtn.addEventListener('click', function () { cvZoomBy(1.2); });
    const cvZoomOutBtn = document.getElementById('cv-zoom-out');
    if (cvZoomOutBtn) cvZoomOutBtn.addEventListener('click', function () { cvZoomBy(1 / 1.2); });
    const cvZoomResetBtn = document.getElementById('cv-zoom-reset');
    if (cvZoomResetBtn) cvZoomResetBtn.addEventListener('click', function () { cvZoomReset(); });
    const cvCanvasEl = document.getElementById('cv-canvas');
    if (cvCanvasEl) cvCanvasEl.addEventListener('wheel', function (e) {
        e.preventDefault();
        cvZoomBy(e.deltaY < 0 ? 1.1 : 1 / 1.1);
    }, { passive: false });
    if (refreshBtn) refreshBtn.addEventListener('click', loadAgentNodes);
    // 监听：每 30 秒自动刷新健康度，实时反馈节点是否可用
    agentHealthTimer = setInterval(loadAgentHealth, 30000);
    // 节点卡片事件委托（调用 / 加入对话）
    document.getElementById('agent-node-grid').addEventListener('click', function (e) {
        const invBtn = e.target.closest('.agent-invoke-btn');
        if (invBtn && !invBtn.disabled) {
            openAgentInvokeModal(invBtn.dataset.code);
            return;
        }
        const btn = e.target.closest('.agent-bind-btn');
        if (!btn) return;
        const lastList = window.__agentLastList;
        if (!lastList || !lastList.ok) return;
        const node = (lastList.items || []).find(function (n) { return String(n.id) === String(btn.dataset.nodeId); });
        if (node) bindNodeToChat(node);
    });
    // 一键配置
    const setupBtn = document.getElementById('agent-setup-btn');
    if (setupBtn) setupBtn.addEventListener('click', runAgentSetup);
    // 调用弹窗：关闭 / 提交 / 文件上传
    const invOverlay = document.getElementById('agent-invoke-modal-overlay');
    if (invOverlay) {
        invOverlay.addEventListener('click', function (e) {
            if (e.target === invOverlay) closeAgentInvokeModal();
        });
        const invClose = document.getElementById('agent-invoke-modal-close');
        if (invClose) invClose.addEventListener('click', closeAgentInvokeModal);
        const invCancel = document.getElementById('agent-invoke-cancel');
        if (invCancel) invCancel.addEventListener('click', closeAgentInvokeModal);
        const invSubmit = document.getElementById('agent-invoke-submit');
        if (invSubmit) invSubmit.addEventListener('click', submitAgentInvoke);
        // 「运行」成功后出现的「加入对话」按钮
        const invJoin = document.getElementById('agent-invoke-join');
        if (invJoin) invJoin.addEventListener('click', function () {
            const node = findAgentNodeByCode(__agentInvokeCode);
            if (!node) return;
            const pending = __agentLastInvokeResult;
            closeAgentInvokeModal();
            // 绑定节点到会话（未绑定时会新建对话）；完成后把本次运行结果推送到对话框中间展示
            bindNodeToChat(node, function () {
                if (pending && pending.result) {
                    appendAgentInvokeChatMessage(pending.node || node, pending.result);
                }
            });
        });
        const invForm = document.getElementById('agent-invoke-form');
        if (invForm) invForm.addEventListener('change', handleAgentInvokeFileUpload);
    }
    // 全部解除
    const unbindBtn = document.getElementById('agent-unbind-btn');
    if (unbindBtn) unbindBtn.addEventListener('click', function () { unbindAgentNode(); });
    // 侧栏：移除单个智能体
    const sideList = document.getElementById('agent-side-list');
    if (sideList) sideList.addEventListener('click', function (e) {
        const btn = e.target.closest('.agent-side-remove');
        if (!btn) return;
        unbindAgentNode(btn.dataset.code);
    });
    // 侧栏：双击节点 -> 漂浮窗口查看该智能体的全部功能
    if (sideList) sideList.addEventListener('dblclick', function (e) {
        if (e.target.closest('.agent-side-remove')) return;
        const item = e.target.closest('.agent-side-item');
        if (!item || !item.dataset.code) return;
        showAgentFeatureModal(item.dataset.code);
    });
    // 侧栏：拖拽智能体到聊天区 -> 运行该智能体
    if (sideList) sideList.addEventListener('dragstart', function (e) {
        const item = e.target.closest('.agent-side-item');
        if (!item || !item.dataset.code) return;
        e.dataTransfer.setData('text/agent-code', item.dataset.code);
    ...
    if (chatMessagesEl) {
        let dragDepth = 0;
        chatMessagesEl.addEventListener('dragenter', function (e) {
            if (!e.dataTransfer.types || e.dataTransfer.types.indexOf('text/agent-code') === -1) return;
            dragDepth++;
            e.preventDefault();
            chatMessagesEl.classList.add('agent-drop-target');
        });
        ...
        chatMessagesEl.addEventListener('dragleave', function (e) {
            dragDepth--;
            if (dragDepth <= 0) { dragDepth = 0; chatMessagesEl.classList.remove('agent-drop-target'); }
        });
        chatMessagesEl.addEventListener('drop', function (e) {
            e.preventDefault();
            dragDepth = 0;
            chatMessagesEl.classList.remove('agent-drop-target');
            const code = e.dataTransfer.getData('text/agent-code');
            if (!code) return;
            openAgentInvokeModal(code);
        });
        // 图片放大：点击聊天区任意图片 -> 全屏预览
        chatMessagesEl.addEventListener('click', function (e) {
            if (e.target && e.target.tagName === 'IMG') {
                showAgentImageLightbox(e.target.src);
            }
        });
        // 右键菜单：聊天区任意位置右键 -> 弹出菜单
        chatMessagesEl.addEventListener('contextmenu', function (e) {
            e.preventDefault();
            showAgentCtxMenu(e, e.target);
        });
    }
    // 漂浮窗口：遮罩点击 / 关闭按钮
    const featureOverlay = document.getElementById('agent-feature-modal-overlay');
    if (featureOverlay) {
        featureOverlay.addEventListener('click', function (e) {
            if (e.target === featureOverlay) closeAgentFeatureModal();
        });
        const featureClose = featureOverlay.querySelector('.agent-feature-close');
        if (featureClose) featureClose.addEventListener('click', closeAgentFeatureModal);
    }
    // 侧栏：折叠/展开
    const toggleBtn = document.getElementById('agent-side-toggle');
    if (toggleBtn) toggleBtn.addEventListener('click', toggleAgentSidePanel);
    // 侧栏：添加智能体 -> 跳转智能体页
    const addBtn = document.getElementById('agent-side-add');
    if (addBtn) addBtn.addEventListener('click', function () { navigateTo('agent'); });
    // 初始化侧栏 + 横幅
    refreshAgentPanel();
});
