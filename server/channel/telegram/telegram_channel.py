...
class TelegramChannel(ChatChannel):
    ...
    async def _on_message(self, update, _context):
        ...
        try:
            ...
            tg_msg = TelegramMessage( update, is_group=is_group, bot_username=self.bot_username, ctype=ctype, content=content, )
            ...
            context = self._compose_context( tg_msg.ctype, tg_msg.content, isgroup=is_group, msg=tg_msg, )
            if context:
    ...
    async def _download_file(self, file_id, suffix = "", original_name = ""):
    ...
    def _should_reply_in_group(self, update):
    ...
    @staticmethod
    def _is_mentioned(message, bot_username):
    ...
    def _strip_at_mention(self, content):
    ...
    @staticmethod
    def _compute_session_id(update):
        chat = update.effective_chat
    ...
    async def _send_with_retry(self, send_fn, *, label):
    ...
    async def _async_send(self, reply, chat_id, reply_to_msg_id):
        try:
            rtype = reply.type
            ...
            if rtype == ReplyType.TEXT or rtype == ReplyType.INFO or rtype == ReplyType.ERROR:
                ...
                for chunk in _split_text(text, 4000):
                    await self._send_with_retry( lambda c=chunk: self._bot.send_message( chat_id=chat_id, text=c, reply_to_message_id=reply_to_msg_id,  allow_sending_without_reply=True, ), label="send_message", )

            elif rtype == ReplyType.IMAGE:
                # Already a local BytesIO; send it directly
                content.seek(0)
                await self._send_with_retry( lambda: self._bot.send_photo( chat_id=chat_id, photo=content, reply_to_message_id=reply_to_msg_id, allow_sending_without_reply=True, ), label="send_photo", )
                ...
                if url.startswith("file://"):
                    ...
                    async def _send_local_photo():
                        with open(local, "rb") as f:
                            return await self._bot.send_photo( chat_id=chat_id, photo=f, reply_to_message_id=reply_to_msg_id, allow_sending_without_reply=True, )
                    await self._send_with_retry(_send_local_photo, label="send_photo(file)")
                else:
                    await self._send_with_retry( lambda: self._bot.send_photo( chat_id=chat_id, photo=url, reply_to_message_id=reply_to_msg_id, allow_sending_without_reply=True, ), label="send_photo(url)", )

            elif rtype == ReplyType.VOICE:
                local = content[7:] if isinstance(content, str) and content.startswith("file://") else content
                async def _send_voice():
                    with open(local, "rb") as f:
                        return await self._bot.send_voice( chat_id=chat_id, voice=f, reply_to_message_id=reply_to_msg_id, allow_sending_without_reply=True, )
                ...
                async def _send_file():
                    with open(local, "rb") as f:
                        if is_video:
                            return await self._bot.send_video( chat_id=chat_id, video=f, caption=caption, reply_to_message_id=reply_to_msg_id, allow_sending_without_reply=True, )
                        return await self._bot.send_document( chat_id=chat_id, document=f, caption=caption, reply_to_message_id=reply_to_msg_id, allow_sending_without_reply=True, )
                await self._send_with_retry(_send_file, label="send_video" if is_video else "send_document")

            else:
                # Fallback: send as plain text
                await self._send_with_retry( lambda: self._bot.send_message( chat_id=chat_id, text=str(content), reply_to_message_id=reply_to_msg_id, allow_sending_without_reply=True, ), label="send_message(fallback)", )
