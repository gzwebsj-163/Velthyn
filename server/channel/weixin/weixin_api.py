...
class WeixinApi:
    ...
    def __init__(self, base_url = DEFAULT_BASE_URL, token = "", cdn_base_url = CDN_BASE_URL):
        self.base_url = base_url
        self.token = token
        self.cdn_base_url = cdn_base_url

    def _post(self, endpoint, body, timeout = DEFAULT_API_TIMEOUT):
    ...
    def get_updates(self, get_updates_buf = "", timeout = DEFAULT_LONG_POLL_TIMEOUT):
        return self._post("ilink/bot/getupdates", { "get_updates_buf": get_updates_buf, }, timeout=timeout + 5)

    # ── sendMessage ────────────────────────────────────────────────────

    def send_text(self, to, text, context_token):
        return self._post("ilink/bot/sendmessage", { "msg": { "from_user_id": "", "to_user_id": to, "client_id": uuid.uuid4().hex[:16], "message_type": 2,   "message_state": 2,   "item_list": [{"type": 1, "text_item": {"text": text}}], "context_token": context_token, } })

    def send_image_item(self, to, context_token, encrypt_query_param, aes_key_b64, ciphertext_size, text = ""):
        items = []
        if text:
    ...
    def send_file_item(self, to, context_token, encrypt_query_param, aes_key_b64, file_name, file_size, text = ""):
        items = []
        if text:
            items.append({"type": 1, "text_item": {"text": text}})
    ...
    def send_video_item(self, to, context_token, encrypt_query_param, aes_key_b64, ciphertext_size, text = ""):
        items = []
        if text:
            items.append({"type": 1, "text_item": {"text": text}})
    ...
    def _send_items(self, to, context_token, items):
        return self._post("ilink/bot/sendmessage", { "msg": { "from_user_id": "", "to_user_id": to, "client_id": uuid.uuid4().hex[:16], "message_type": 2, "message_state": 2, "item_list": items, "context_token": context_token, } })

    # ── getUploadUrl ───────────────────────────────────────────────────

    def get_upload_url(self, filekey, media_type, to_user_id, rawsize, rawfilemd5, filesize, aeskey):
        return self._post("ilink/bot/getuploadurl", { "filekey": filekey, "media_type": media_type, "to_user_id": to_user_id, "rawsize": rawsize, "rawfilemd5": rawfilemd5, "filesize": filesize, "aeskey": aeskey, "no_need_thumb": True, })

    # ── getConfig / sendTyping ─────────────────────────────────────────

    def get_config(self, user_id, context_token = ""):
        return self._post("ilink/bot/getconfig", { "ilink_user_id": user_id, "context_token": context_token, }, timeout=10)

    def send_typing(self, user_id, typing_ticket, status = 1):
        return self._post("ilink/bot/sendtyping", { "ilink_user_id": user_id, "typing_ticket": typing_ticket, "status": status, }, timeout=10)
    ...
    def poll_qr_status(self, qrcode, timeout = QR_POLL_TIMEOUT):
        url = (_ensure_trailing_slash(self.base_url) + f"ilink/bot/get_qrcode_status?qrcode={requests.utils.quote(qrcode)}")
...
def upload_media_to_cdn(api, file_path, to_user_id, media_type):
    """
...
def download_media_from_cdn(cdn_base_url, encrypt_query_param, aes_key, save_path):
    """
