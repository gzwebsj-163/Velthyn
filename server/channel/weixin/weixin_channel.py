...
def _save_credentials(cred_path, data):
...
class WeixinChannel(ChatChannel):
    ...
    def _login_with_retry(self, base_url):
    ...
    def _relogin(self):
        ...
        if not result:
            self.login_status = self.LOGIN_STATUS_IDLE
            return False
        self.api = WeixinApi( base_url=result.get("base_url", base_url), token=result["token"], cdn_base_url=self.api.cdn_base_url if self.api else CDN_BASE_URL, )
        self.login_status = self.LOGIN_STATUS_OK
        return True
    ...
    def _restore_context_tokens_from_creds(self, creds):
    ...
    def _persist_context_tokens_locked(self):
    ...
    def _update_context_token(self, user_id, token):
    ...
    def _invalidate_context_token(self, user_id):
    ...
    def _process_message(self, raw_msg):
        ...
        context = self._compose_context( wx_msg.ctype, wx_msg.content, isgroup=False, msg=wx_msg, no_need_at=True, )
        if context:
    ...
    def _get_context_token(self, receiver, msg=None):
    ...
    def _check_send_response(self, resp, receiver):
    ...
    def _send_text(self, text, receiver, context_token):
    ...
    def _send_image(self, img_path_or_url, receiver, context_token):
        ...
        try:
            ...
            resp = self.api.send_image_item( to=receiver, context_token=context_token, encrypt_query_param=result["encrypt_query_param"], aes_key_b64=result["aes_key_b64"], ciphertext_size=result["ciphertext_size"], )
            self._check_send_response(resp, receiver)
    ...
    def _send_file(self, file_path_or_url, receiver, context_token):
        ...
        try:
            ...
            resp = self.api.send_file_item( to=receiver, context_token=context_token, encrypt_query_param=result["encrypt_query_param"], aes_key_b64=result["aes_key_b64"], file_name=os.path.basename(local_path), file_size=result["raw_size"], )
            self._check_send_response(resp, receiver)
    ...
    def _send_video(self, video_path_or_url, receiver, context_token):
        ...
        try:
            ...
            resp = self.api.send_video_item( to=receiver, context_token=context_token, encrypt_query_param=result["encrypt_query_param"], aes_key_b64=result["aes_key_b64"], ciphertext_size=result["ciphertext_size"], )
            self._check_send_response(resp, receiver)
    ...
    @staticmethod
    def _resolve_media_path(path_or_url):
