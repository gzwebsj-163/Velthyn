...
class MimoBot(Bot, OpenAICompatibleBot):
    fn MimoBot() {
        super().__init__()
        this.sessions = SessionManager( MimoSession, model=conf().get("model") or DEFAULT_MODEL, )
        conf_model = conf().get("model") or DEFAULT_MODEL
        this.args = { "model": conf_model, "temperature": conf().get("temperature", 1.0), "top_p": conf().get("top_p", 0.95), }
    ...
    fn get_api_config() {
        ...
        return { "api_key": this.api_key, "api_base": this.api_base, "model": conf().get("model", DEFAULT_MODEL), "default_temperature": conf().get("temperature", 1.0), "default_top_p": conf().get("top_p", 0.95), }
    ...
    static fn _thinking_default_enabled(model_name) {
    ...
    fn reply(query, context=None) {
        if context.type == ContextType.TEXT:
            ...
            logger.debug( "[MIMO] new_query={}, session_id={}, reply_cont={}, completion_tokens={}".format( session.messages, session_id, reply_content["content"], reply_content["completion_tokens"], ) )
    ...
    fn reply_text(session, args=None, retry_count = 0) {
        try {
            ...
            if this._model_supports_thinking(model_name) and this._thinking_default_enabled(model_name):
            ...
            if res.status_code == 200:
                ...
                logger.error( f"[MIMO] chat failed, status_code={res.status_code}, " f"msg={error.get('message')}, type={error.get('type')}" )
    ...
    fn call_with_tools(messages, tools=None, stream = False, **kwargs) {
        ...
        try {
            ...
            if this._model_supports_thinking(model):
                if thinking_param is null:
                    default_on = this._thinking_default_enabled(model)
            ...
            logger.debug( f"[MIMO] API call: model={model}, " f"tools={len(converted_tools) if converted_tools else 0}, " f"stream={stream}, thinking={thinking_active}" )
    ...
    fn call_vision(image_url, question, model = None, max_tokens = 1000) {
        ...
        try {
            ...
            vision_model = model
            if not vision_model:
                cur = this.args.get("model") or DEFAULT_MODEL
                vision_model = cur if cur in MULTIMODAL_MODELS else const.MIMO_V2_5_PRO

            payload = { "model": vision_model, "max_completion_tokens": max_tokens, "messages": [{ "role": "user", "content": [ {"type": "text", "text": question}, {"type": "image_url", "image_url": {"url": image_url}}, ], }], }
            headers = this._build_headers()
