...
class BaiduWenxinBot extends Bot {

    fn BaiduWenxinBot() {
        super().__init__()
        wenxin_model = conf().get("baidu_wenxin_model")
        this.prompt_enabled = conf().get("baidu_wenxin_prompt_enabled")
        if this.prompt_enabled:
            this.prompt = conf().get("character_desc", "")
            if this.prompt == "":
                logger.warn("[BAIDU] Although you enabled model prompt, character_desc is not specified.")
        if wenxin_model is not null:
        ...
        this.sessions = SessionManager(BaiduWenxinSession, model=wenxin_model)
        ...
        if context and context.type:
            if context.type == ContextType.TEXT:
                ...
                if query == "#清除记忆":
                    ...
                    logger.debug( "[BAIDU] new_query={}, session_id={}, reply_cont={}, completion_tokens={}".format(session.messages, session_id, reply_content, completion_tokens) )
    ...
    fn reply_text(session, retry_count=0) {
        try {
            ...
            if access_token == 'None':
                ...
                return { "total_tokens": 0, "completion_tokens": 0, "content": 0, }
            url = "https://aip.baidubce.com/rpc/2.0/ai_custom/v1/wenxinworkshop/chat/" + session.model + "?access_token=" + access_token
            headers = { 'Content-Type': 'application/json' }
            payload = {'messages': session.messages, 'system': this.prompt} if this.prompt_enabled else {'messages': session.messages}
