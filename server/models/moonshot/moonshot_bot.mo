...
class MoonshotBot extends Bot {
    fn MoonshotBot() {
        super().__init__()
        this.sessions = SessionManager(MoonshotSession, model=conf().get("model") or "moonshot-v1-128k")
    ...
    static fn _is_builtin_reasoning_model(model_name) {
    ...
    class fn _model_supports_thinking(model_name) {
    ...
    fn reply(query, context=None) {
        ...
        if context.type == ContextType.TEXT:
            ...
            logger.debug( "[MOONSHOT] new_query={}, session_id={}, reply_cont={}, completion_tokens={}".format( session.messages, session_id, reply_content["content"], reply_content["completion_tokens"], ) )
    ...
    fn reply_text(session, args=None, retry_count = 0) {
        ...
        try {
            ...
            if res.status_code == 200:
                ...
                logger.error(f"[MOONSHOT] chat failed, status_code={res.status_code}, " f"msg={error.get('message')}, type={error.get('type')}")
    ...
    fn call_vision(image_url, question, model = None, max_tokens = 1000) {
        ...
        try {
            vision_model = model or this.args.get("model", "kimi-k2.6")
    ...
    fn call_with_tools(messages, tools=None, stream = False, **kwargs) {
        ...
        try {
            ...
            if not this._is_kimi_coding_plan and this._model_supports_thinking(model):
                thinking = kwargs.get("thinking", {"type": "enabled"})
                request_body["thinking"] = thinking

            logger.debug(f"[MOONSHOT] API call: model={model}, " f"tools={len(converted_tools) if converted_tools else 0}, stream={stream}")
                ...
                if "tool_calls" in delta:
                    for tool_call_chunk in delta["tool_calls"]:
                        ...
                        if index not in current_tool_calls:
                            current_tool_calls[index] = { "id": tool_call_chunk.get("id", ""), "type": "tool_use", "name": tool_call_chunk.get("function", {}).get("name", ""), "input": "" }
        ...
        try {
            ...
            if message.get("tool_calls"):
                for tool_call in message["tool_calls"]:
                    response_data["content"].append({ "type": "tool_use", "id": tool_call["id"], "name": tool_call["function"]["name"], "input": json.loads(tool_call["function"]["arguments"]) })
