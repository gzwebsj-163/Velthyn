...
class ChatGPTBot(Bot, OpenAIImage, OpenAICompatibleBot):
    def __init__(self):
        ...
        self._http_client = OpenAIHTTPClient( api_key=self._api_key, api_base=self._api_base, proxy=self._proxy, )
        ...
        self.args = { "model": conf_model,   "temperature": conf().get("temperature", 0.9),    "top_p": conf().get("top_p", 1), "frequency_penalty": conf().get("frequency_penalty", 0.0),   "presence_penalty": conf().get("presence_penalty", 0.0),   "request_timeout": conf().get("request_timeout", None),   "timeout": conf().get("request_timeout", None),   }
        # 部分模型暂不支持一些参数，特殊处理
    ...
    def reply_image(self, context):
        ...
        try:
            ...
            response = self._http_client.chat_completions( api_key=api_key or None, api_base=api_base or None, model=model, messages=messages, max_tokens=1000, )
    ...
    def reply_text(self, session, api_key=None, args=None, retry_count=0):
        """
        ...
        try:
            ...
            response = self._http_client.chat_completions( api_key=api_key or None, timeout=timeout, messages=session.messages, **call_args, )
            ...
            return self._handle_reply_error( wrap_http_error(http_err), session, api_key, args, retry_count )
        except Exception as e:
...
class AzureChatGPTBot(ChatGPTBot):
    ...
    def __init__(self):
        ...
        self._http_client = _AzureChatHTTPClient( api_key=self._api_key, api_base=azure_base, api_version=self._azure_api_version, proxy=self._proxy, )
