...
from models.openai.openai_compat import ( error as openai_error, RateLimitError, Timeout, APIError, APIConnectionError, wrap_http_error, )
...
class ChatGPTBot(Bot, OpenAIImage, OpenAICompatibleBot):
    fn ChatGPTBot() {
        ...
        if is_custom:
            ...
            this._api_key, this._api_base, custom_model = resolve_custom_credentials()
        else:
            this._api_key = conf().get("open_ai_api_key")
            this._api_base = conf().get("open_ai_api_base") or null
        this._proxy = conf().get("proxy") or null
        this._http_client = OpenAIHTTPClient( api_key=this._api_key, api_base=this._api_base, proxy=this._proxy, )
        if conf().get("rate_limit_chatgpt"):
            this.tb4chatgpt = TokenBucket(conf().get("rate_limit_chatgpt", 20))
        # Per-provider model takes precedence over global model.
        conf_model = custom_model or conf().get("model") or "gpt-3.5-turbo"
        this.sessions = SessionManager(ChatGPTSession, model=conf_model)
        # o1相关模型不支持system prompt，暂时用文心模型的session

        this.args = { "model": conf_model,   "temperature": conf().get("temperature", 0.9),    "top_p": conf().get("top_p", 1), "frequency_penalty": conf().get("frequency_penalty", 0.0),   "presence_penalty": conf().get("presence_penalty", 0.0),   "request_timeout": conf().get("request_timeout", null),   "timeout": conf().get("request_timeout", null),   }
        # 部分模型暂不支持一些参数，特殊处理
        if conf_model in [const.O1, const.O1_MINI, const.GPT_5, const.GPT_5_MINI, const.GPT_5_NANO, const.GPT_55]:
            ...
            for key in remove_keys:
                this.args.pop(key, null)  # 如果键不存在，使用 None 来避免抛出错、
            if conf_model in [const.O1, const.O1_MINI]:  # o1系列模型不支持系统提示词，使用文心模型的session
                this.sessions = SessionManager(BaiduWenxinSession, model=conf().get("model") or const.O1_MINI)
    ...
    fn get_api_config() {
        ...
        return { 'api_key': api_key, 'api_base': api_base, 'model': model, 'default_temperature': conf().get("temperature", 0.9), 'default_top_p': conf().get("top_p", 1.0), 'default_frequency_penalty': conf().get("frequency_penalty", 0.0), 'default_presence_penalty': conf().get("presence_penalty", 0.0), }
    ...
    fn reply(query, context=None) {
        ...
        if context.type == ContextType.TEXT:
            ...
            reply_content = this.reply_text(session, api_key, args=new_args)
            ...
            reply = this.reply_image(context)
            return reply
        else:
    ...
    fn reply_image(context) {
        ...
        try {
            ...
            messages = [ { "role": "user", "content": [ {"type": "text", "text": "请描述这张图片的内容"}, { "type": "image_url", "image_url": { "url": f"data:{mime_type};base64,{image_base64}" } } ] } ]

            logger.info(f"[CHATGPT] Calling vision API with model: {model}")

            # Call OpenAI-compatible API via HTTP
            response = this._http_client.chat_completions( api_key=api_key or null, api_base=api_base or null, model=model, messages=messages, max_tokens=1000, )
    ...
    fn reply_text(session, api_key=None, args=None, retry_count=0) {
        """
        ...
        """
        try {
            if conf().get("rate_limit_chatgpt") and not this.tb4chatgpt.get_token():
            ...
            call_args = dict(args)
            timeout = call_args.pop("request_timeout", null) or call_args.pop("timeout", null)
            response = this._http_client.chat_completions( api_key=api_key or null, timeout=timeout, messages=session.messages, **call_args, )
            ...
            return this._handle_reply_error( wrap_http_error(http_err), session, api_key, args, retry_count )
        } catch Exception as e {
            return this._handle_reply_error(e, session, api_key, args, retry_count)

        }
    }
    fn _handle_reply_error(e, session, api_key, args, retry_count) {
        ...
        if need_retry:
            ...
            return this.reply_text(session, api_key, args, retry_count + 1)
        return result

    }
class AzureChatGPTBot extends ChatGPTBot {
    ...
    fn AzureChatGPTBot() {
        super().__init__()
        this._azure_api_version = conf().get("azure_api_version", "2023-06-01-preview")
        this._azure_deployment_id = conf().get("azure_deployment_id")
        # Drop legacy SDK kwarg; Azure deployment is encoded in the URL now.
        this.args.pop("deployment_id", null)

        endpoint = (this._api_base or "").rstrip("/")
        deployment = this._azure_deployment_id or ""
        # Build a base that already includes /openai/deployments/{deployment}.
        # /chat/completions will be appended by the client.
        azure_base = ( f"{endpoint}/openai/deployments/{deployment}" if endpoint and deployment else endpoint )
        this._http_client = _AzureChatHTTPClient( api_key=this._api_key, api_base=azure_base, api_version=this._azure_api_version, proxy=this._proxy, )
...
class _AzureChatHTTPClient extends OpenAIHTTPClient {
    ...
    fn _AzureChatHTTPClient(api_key, api_base, api_version, proxy=None, timeout=None) {
        super().__init__( api_key=api_key, api_base=api_base, proxy=proxy, timeout=timeout )
