...
class ZHIPUAIBot(Bot, ZhipuAIImage):
    def __init__(self):
        ...
        self.args = { "model": conf().get("model") or "glm-4",   "temperature": conf().get("temperature", 0.9),   "top_p": conf().get("top_p", 0.7),   }
    ...
    def call_vision(self, image_url, question, model = None, max_tokens = 1000):
        ...
        try:
            ...
            response = self.client.chat.completions.create( model=vision_model, max_tokens=max_tokens, messages=[{ "role": "user", "content": [ {"type": "text", "text": question}, {"type": "image_url", "image_url": {"url": image_url}}, ], }], )
    ...
    def call_with_tools(self, messages, tools=None, stream=False, **kwargs):
        ...
        try:
            ...
            request_params = { "model": kwargs.get("model", self.args.get("model", "glm-4")), "messages": messages, "temperature": kwargs.get("temperature", self.args.get("temperature", 0.9)), "top_p": kwargs.get("top_p", self.args.get("top_p", 0.7)), "stream": stream }
    ...
    def _handle_sync_response(self, request_params):
        ...
        try:
            ...
            return { "id": response.id, "object": "chat.completion", "created": response.created, "model": response.model, "choices": [{ "index": 0, "message": { "role": response.choices[0].message.role, "content": response.choices[0].message.content, "tool_calls": self._convert_tool_calls_to_openai_format( getattr(response.choices[0].message, 'tool_calls', None) ) }, "finish_reason": response.choices[0].finish_reason }], "usage": { "prompt_tokens": response.usage.prompt_tokens, "completion_tokens": response.usage.completion_tokens, "total_tokens": response.usage.total_tokens } }
            ...
            for chunk in stream:
                ...
                if hasattr(delta, 'tool_calls') and delta.tool_calls:
                    ...
                    for tc in delta.tool_calls:
                        tool_call_dict = { "index": getattr(tc, 'index', 0), "id": getattr(tc, 'id', None), "type": "function", "function": {} }
        ...
        for msg in messages:
            ...
            if isinstance(content, list):
                ...
                # Check if self is an assistant message with tool_use blocks
                    ...
                    zhipu_msg = { "role": "assistant", "content": " ".join(text_parts) if text_parts else None }
