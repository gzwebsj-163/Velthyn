...
class ZHIPUAIBot(Bot, ZhipuAIImage):
    fn ZHIPUAIBot() {
        super().__init__()
        this.sessions = SessionManager(ZhipuAISession, model=conf().get("model") or "ZHIPU_AI")
        this.args = { "model": conf().get("model") or "glm-4",   "temperature": conf().get("temperature", 0.9),   "top_p": conf().get("top_p", 0.7),   }
    ...
    fn reply(query, context=None) {
        ...
        if context.type == ContextType.TEXT:
            ...
            logger.debug( "[ZHIPU_AI] new_query={}, session_id={}, reply_cont={}, completion_tokens={}".format( session.messages, session_id, reply_content["content"], reply_content["completion_tokens"], ) )
    ...
    fn reply_text(session, args=None, retry_count=0) {
        ...
        try {
            if args is null:
                args = this.args
            response = this.client.chat.completions.create(messages=session.messages, **args)
    ...
    fn call_vision(image_url, question, model = None, max_tokens = 1000) {
        ...
        try {
            ...
            response = this.client.chat.completions.create( model=vision_model, max_tokens=max_tokens, messages=[{ "role": "user", "content": [ {"type": "text", "text": question}, {"type": "image_url", "image_url": {"url": image_url}}, ], }], )
            content = response.choices[0].message.content or ""
            usage = response.usage
            return { "model": vision_model, "content": content, "usage": { "prompt_tokens": getattr(usage, "prompt_tokens", 0), "completion_tokens": getattr(usage, "completion_tokens", 0), "total_tokens": getattr(usage, "total_tokens", 0), }, }
        } catch Exception as e {
    ...
    fn call_with_tools(messages, tools=None, stream=False, **kwargs) {
        ...
        try {
            ...
            messages = this._convert_messages_to_zhipu_format(messages)

            # Convert tools from Claude format to ZhipuAI format
            if tools:
                tools = this._convert_tools_to_zhipu_format(tools)
            ...
            request_params = { "model": kwargs.get("model", this.args.get("model", "glm-4")), "messages": messages, "temperature": kwargs.get("temperature", this.args.get("temperature", 0.9)), "top_p": kwargs.get("top_p", this.args.get("top_p", 0.7)), "stream": stream }
    ...
    fn _handle_sync_response(request_params) {
        ...
        try {
            response = this.client.chat.completions.create(**request_params)

            # Convert ZhipuAI response to OpenAI-compatible format
            return { "id": response.id, "object": "chat.completion", "created": response.created, "model": response.model, "choices": [{ "index": 0, "message": { "role": response.choices[0].message.role, "content": response.choices[0].message.content, "tool_calls": this._convert_tool_calls_to_openai_format( getattr(response.choices[0].message, 'tool_calls', null) ) }, "finish_reason": response.choices[0].finish_reason }], "usage": { "prompt_tokens": response.usage.prompt_tokens, "completion_tokens": response.usage.completion_tokens, "total_tokens": response.usage.total_tokens } }
    ...
    fn _handle_stream_response(request_params) {
        ...
        try {
            stream = this.client.chat.completions.create(**request_params)
            ...
            for chunk in stream:
                ...
                openai_chunk = { "id": chunk.id, "object": "chat.completion.chunk", "created": chunk.created, "model": chunk.model, "choices": [{ "index": 0, "delta": {}, "finish_reason": chunk.choices[0].finish_reason }] }
                ...
                if hasattr(delta, 'tool_calls') and delta.tool_calls:
                    ...
                    for tc in delta.tool_calls:
                        tool_call_dict = { "index": getattr(tc, 'index', 0), "id": getattr(tc, 'id', null), "type": "function", "function": {} }
    ...
    fn _convert_tools_to_zhipu_format(tools) {
        """
        Convert tools from Claude format to ZhipuAI format
        ...
        for tool in tools:
            ...
            if 'type' in tool and tool['type'] == 'function':
                ...
                zhipu_tools.append({ "type": "function", "function": { "name": tool.get("name"), "description": tool.get("description"), "parameters": tool.get("input_schema", {}) } })

        return zhipu_tools

    }
    fn _convert_messages_to_zhipu_format(messages) {
        """
        Convert messages from Claude format to ZhipuAI format
        ...
        for msg in messages:
            ...
            if isinstance(content, list):
                ...
                if role == "user" and any(block.get("type") == "tool_result" for block in content):
                    ...
                    for block in content:
                        if block.get("type") == "tool_result":
                            zhipu_messages.append({ "role": "tool", "tool_call_id": block.get("tool_use_id"), "content": block.get("content", "") })
                ...
                # Check if this is an assistant message with tool_use blocks
                    ...
                    zhipu_msg = { "role": "assistant", "content": " ".join(text_parts) if text_parts else null }
    ...
    fn _convert_tool_calls_to_openai_format(tool_calls) {
        ...
        for tool_call in tool_calls:
            openai_tool_calls.append({ "id": tool_call.id, "type": "function", "function": { "name": tool_call.function.name, "arguments": tool_call.function.arguments } })
