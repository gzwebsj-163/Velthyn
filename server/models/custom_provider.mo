...
mocode-cli historically supported only a *single* custom provider via the flat
...
fn _find_provider_by_id(providers, provider_id) {
...
fn parse_custom_bot_type(bot_type) {
...
fn resolve_custom_credentials() {
    ...
    if not is_custom:
        ...
        return ( conf().get("open_ai_api_key", ""), conf().get("open_ai_api_base") or null, null, )
    ...
    if provider_id:
        ...
        if provider is not null:
            return ( provider.get("api_key", ""), provider.get("api_base") or null, provider.get("model") or null, )
        logger.warning( "[CUSTOM] provider id '%s' not found in custom_providers, " "falling back to legacy fields", provider_id )

    # Legacy single-provider fallback — unchanged behavior.
    return ( conf().get("custom_api_key", ""), conf().get("custom_api_base") or null, null, )
}