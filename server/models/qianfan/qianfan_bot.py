...
class QianfanBot(Bot, OpenAICompatibleBot):
    ...
    def __init__(self):
        ...
        self.args = { "model": model, "temperature": conf().get("temperature", 0.7), "top_p": conf().get("top_p", 1.0), "frequency_penalty": conf().get("frequency_penalty", 0.0), "presence_penalty": conf().get("presence_penalty", 0.0), }
    ...
    def get_api_config(self):
        return { "api_key": self.api_key, "api_base": self.api_base, "model": self._resolve_model(), "default_temperature": conf().get("temperature", 0.7), "default_top_p": conf().get("top_p", 1.0), "default_frequency_penalty": conf().get("frequency_penalty", 0.0), "default_presence_penalty": conf().get("presence_penalty", 0.0), }

    def _build_headers(self):
        return { "Content-Type": "application/json", "Authorization": "Bearer {}".format(self.api_key), }
    ...
    def reply_text(self, session, args=None, retry_count=0):
        try:
            ...
            response = requests.post( "{}/chat/completions".format(self.api_base), headers=self._build_headers(), json=body, timeout=conf().get("request_timeout", 180), )
    ...
    def call_vision(self, image_url, question, model = None, max_tokens = 1000):
        ...
        try:
            response = requests.post( "{}/chat/completions".format(self.api_base), headers=self._build_headers(), json=payload, timeout=conf().get("request_timeout", 180), )
            if response.status_code != 200:
