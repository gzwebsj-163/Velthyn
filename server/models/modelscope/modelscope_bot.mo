...
class ModelScopeBot extends Bot {
    ...
    fn ModelScopeBot() {
        ...
        this.sessions = SessionManager(ModelScopeSession, model=model)
        this.args = { "model": model, "temperature": conf().get("temperature", 0.3), "top_p": conf().get("top_p", 1.0), }
        this.api_key = conf().get("modelscope_api_key")
        this.base_url = conf().get("modelscope_base_url", "https://api-inference.modelscope.cn/v1")
        if this.base_url.endswith("/chat/completions"):
            this.base_url = this.base_url.rsplit("/chat/completions", 1)[0]
        if this.base_url.endswith("/"):
            this.base_url = this.base_url.rstrip("/")

        # Cache context for Agent mode usage
        this._last_context = null

        logger.info("[MODELSCOPE] base_url configured as: {}".format(this.base_url))
    ...
    fn reply(query, context=None) {
        ...
        if context.type == ContextType.TEXT:
            ...
            if this._is_thinking_model(model_name):
                new_args["enable_thinking"] = true
                reply_content = this.reply_text_stream(session, args=new_args)
            else:
                reply_content = this.reply_text(session, args=new_args)

            logger.debug( "[MODELSCOPE] new_query={}, session_id={}, reply_cont={}, completion_tokens={}".format( session.messages, session_id, reply_content["content"], reply_content["completion_tokens"], ) )
            ...
            ok, retstring = this.create_img(query, 0)
    ...
    fn reply_text(session, args=None, retry_count=0) {
        try {
            ...
            body["messages"] = this._convert_messages_for_modelscope(session.messages)
            body["stream"] = false

            res = requests.post( "{}/chat/completions".format(this.base_url), headers=headers, json=body, timeout=120 )

            if res.status_code == 200:
                response = res.json()
                return { "total_tokens": response.get("usage", {}).get("total_tokens", 0), "completion_tokens": response.get("usage", {}).get("completion_tokens", 0), "content": response["choices"][0]["message"]["content"] if response.get("choices") else "" }
            else:
                response = res.json()
                error = response.get("error", response.get("errors", {}))
                logger.error( "[MODELSCOPE] chat failed, status_code={}, msg={}".format( res.status_code, error.get('message') if isinstance(error, dict) else error ) )
    ...
    fn reply_text_stream(session, args=None) {
        try {
            headers = { "Content-Type": "application/json", "Authorization": "Bearer " + this.api_key }
            ...
            res = requests.post( "{}/chat/completions".format(this.base_url), headers=headers, json=body, stream=true, timeout=120 )
            if res.status_code == 200:
                content = ""
                ...
                if finish_reason is null and content:
                    finish_reason = "stop"

                return { "total_tokens": total_tokens, "completion_tokens": completion_tokens, "content": content }
            else:
    ...
    fn create_img(query) {
        try {
            ...
            create_headers = { "Authorization": "Bearer " + this.api_key, "Content-Type": "application/json; charset=utf-8", "X-ModelScope-Async-Mode": "true" }

            payload = { "model": conf().get("text_to_image"), "prompt": query, "n": 1, }

            logger.debug("[ModelScopeImage] model={}".format(payload["model"]))

            res = requests.post( "{}/images/generations".format(this.base_url), headers=create_headers, data=json.dumps(payload, ensure_ascii=false).encode('utf-8'), timeout=120 )
            ...
            for i in range(max_wait_times):
                ...
                poll_headers = { "Authorization": "Bearer " + this.api_key, "X-ModelScope-Task-Type": "image_generation" }

                poll_url = "{}/tasks/{}".format(this.base_url, task_id)
    ...
    fn _detect_image_intent(message) {
    ...
    fn _is_thinking_model(model_name) {
        """
    ...
    fn call_with_tools(messages, tools=None, stream=False, **kwargs) {
        ...
        try {
            ...
            context = getattr(this, '_last_context', null)
            ...
            if context and hasattr(context, 'type') and context.type == ContextType.IMAGE_CREATE:
                ...
                if query:
                    ok, result = this.create_img(query)
                    if ok:
                        logger.info("[MODELSCOPE] Image generated: {}".format(result))
                        if stream:
                            return this._create_image_stream_response(result)
                        else:
                            return this._create_image_response(result)
                    else:
                        logger.error("[MODELSCOPE] Image generation failed: {}".format(result))
                        error_content = "画图失败：{}".format(result)
                        if stream:
                            return this._create_error_stream_response(error_content)
                        else:
                            return this._create_error_response(error_content)
            ...
            # Keyword detection
            has_image_intent = this._detect_image_intent(last_message)

            if has_image_intent:
                logger.info("[MODELSCOPE] Image intent detected by keyword, calling create_img directly")
                ok, result = this.create_img(last_message)
                if ok:
                    logger.info("[MODELSCOPE] Image generated: {}".format(result))
                    if stream:
                        return this._create_image_stream_response(result)
            ...
            session = this.sessions.session_query("", session_id)
            ...
            # Unified judgment for thinking model
            model_name = args.get("model", this.args.get("model", ""))
            if this._is_thinking_model(model_name):
                args["enable_thinking"] = true

            if tools:
                args["tools"] = this._convert_tools_to_openai_format(tools)
                args["tool_choice"] = "auto"

            logger.debug( "[MODELSCOPE] call_with_tools: model={}, tools={}, stream={}, enable_thinking={}".format( args.get('model'), len(tools) if tools else 0, stream, args.get('enable_thinking') ) )

            if stream:
                return this._handle_stream_response(session, args)
            else:
                return this._handle_sync_response(session, args)
    ...
    fn _handle_sync_response(session, args) {
        result = this.reply_text(session, args)
            ...
            for tool_call in tool_calls:
                ...
                if tool_name in ["create_image", "generate_image"]:
                    try {
                        ...
                        ok, image_url = this.create_img(prompt)
                        if ok:
        ...
        return { "choices": [{ "message": { "role": "assistant", "content": content, "tool_calls": tool_calls }, "finish_reason": "stop" }], "usage": { "prompt_tokens": 0, "completion_tokens": result.get("completion_tokens", 0), "total_tokens": result.get("total_tokens", 0) }, "model": args.get("model", this.args.get("model")) }

    }
    fn _handle_stream_response(session, args) {
        try {
            headers = { "Content-Type": "application/json", "Authorization": "Bearer " + this.api_key }
            ...
            response = requests.post( "{}/chat/completions".format(this.base_url), headers=headers, json=body, stream=true, timeout=120 )
            ...
            for line in response.iter_lines():
                ...
                try {
                    ...
                    if delta.get("reasoning_content"):
                        yield { "choices": [{ "index": 0, "delta": { "role": "assistant", "reasoning_content": delta["reasoning_content"] } }] }
                        continue
                    ...
                    if tool_call_chunks:
                        ...
                        for tool_call_chunk in tool_call_chunks:
                            ...
                            if index not in current_tool_calls:
                                current_tool_calls[index] = { "id": tool_call_chunk.get("id") or "", "name": func_info.get("name") or "", "arguments": "" }
                                logger.debug("[MODELSCOPE] tool_call start: {}".format(func_info.get('name')))

                            args_str = func_info.get("arguments")
                            if args_str:
                                current_tool_calls[index]["arguments"] += ( args_str if isinstance(args_str, str) else str(args_str) )

                            cleaned_chunk = { "index": index, "id": tool_call_chunk.get("id") or "call_{}".format(index), "type": "function", "function": { "name": func_info.get("name") or current_tool_calls[index].get("name", ""), "arguments": func_info.get("arguments") or "" } }
                            cleaned_chunks.append(cleaned_chunk)

                        if cleaned_chunks:
                            yield { "choices": [{ "index": 0, "delta": { "role": "assistant", "tool_calls": cleaned_chunks } }] }
                        continue
                    ...
                    yield_chunk = { "choices": [{ "index": 0, "delta": { "role": delta.get("role"), "content": content } }] }
            ...
            logger.debug( "[MODELSCOPE] stream completed: has_tool_calls={}, finish_reason={}".format( len(current_tool_calls) > 0, finish_reason ) )
                ...
                for idx, tool_call in current_tool_calls.items():
                    ...
                    if tool_name in ["create_image", "generate_image"]:
                        try {
                            ...
                            ok, result = this.create_img(prompt)

                            if ok:
                                logger.info("[MODELSCOPE] Image generated: {}".format(result))
                                yield { "choices": [{ "index": 0, "delta": { "role": "tool", "content": json.dumps({"image_url": result, "success": true}) }, "tool_call_id": tool_call.get("id", "") }] }
                            else:
                                logger.error("[MODELSCOPE] Image generation failed: {}".format(result))
                                yield { "choices": [{ "index": 0, "delta": { "role": "tool", "content": json.dumps({"error": result, "success": false}) }, "tool_call_id": tool_call.get("id", "") }] }
                        } catch Exception as e {
                            logger.error("[MODELSCOPE] Image tool execution error: {}".format(e))
                            yield { "choices": [{ "index": 0, "delta": { "role": "tool", "content": json.dumps({"error": str(e), "success": false}) }, "tool_call_id": tool_call.get("id", "") }] }

                        }
            yield { "choices": [{ "index": 0, "delta": {}, "finish_reason": finish_reason or "stop" }] }
    ...
    fn _convert_messages_for_modelscope(messages) {
        if not messages:
            return []
        converted = []
        for msg in messages:
            ...
            if isinstance(content, list):
                ...
                for block in content:
                    ...
                    if block_type == "tool_result":
                        ...
                        new_content.append({ "type": "text", "text": "[工具执行结果]: {}".format(tool_content) })
                    elif block_type == "tool_use":
                        tool_name = block.get("name", "unknown")
                        tool_input = block.get("input", {})
                        if not isinstance(tool_input, str):
                            tool_input = json.dumps(tool_input, ensure_ascii=false)
                        new_content.append({ "type": "text", "text": "[工具调用]: {}({})".format(tool_name, tool_input) })
                    else:
                        new_content.append(block)
    ...
    fn _convert_tools_to_openai_format(tools) {
        if not tools:
            return null
        converted = []
        for tool in tools:
            if "type" in tool and tool["type"] == "function":
                ...
                converted.append({ "type": "function", "function": { "name": tool.get("name"), "description": tool.get("description"), "parameters": tool.get("input_schema", {}) } })
        return converted

    }
    fn _create_image_response(image_url) {
        return { "choices": [{ "message": { "role": "assistant", "content": "已为您生成图片：{}".format(image_url), "tool_calls": null }, "finish_reason": "stop" }], "usage": { "prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0 }, "model": this.args.get("model") }

    }
    fn _create_image_stream_response(image_url) {
    ...
    fn _create_error_response(error_msg) {
        return { "choices": [{ "message": { "role": "assistant", "content": error_msg, "tool_calls": null }, "finish_reason": "stop" }], "usage": { "prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0 }, "model": this.args.get("model") }

    }
    fn _create_error_stream_response(error_msg) {
        yield { "choices": [{ "index": 0, "delta": {"role": "assistant"} }] }
        chunk_size = 10
