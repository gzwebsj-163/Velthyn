...
class ModelScopeBot(Bot):
    ...
    def __init__(self):
        ...
        self.args = { "model": model, "temperature": conf().get("temperature", 0.3), "top_p": conf().get("top_p", 1.0), }
    ...
    def reply_text(self, session, args=None, retry_count=0):
        try:
            ...
            res = requests.post( "{}/chat/completions".format(self.base_url), headers=headers, json=body, timeout=120 )
    ...
    def reply_text_stream(self, session, args=None):
        try:
            ...
            res = requests.post( "{}/chat/completions".format(self.base_url), headers=headers, json=body, stream=True, timeout=120 )
            if res.status_code == 200:
                content = ""
    ...
    def create_img(self, query):
        try:
            ...
            create_headers = { "Authorization": "Bearer " + self.api_key, "Content-Type": "application/json; charset=utf-8", "X-ModelScope-Async-Mode": "true" }

            payload = { "model": conf().get("text_to_image"), "prompt": query, "n": 1, }

            logger.debug("[ModelScopeImage] model={}".format(payload["model"]))

            res = requests.post( "{}/images/generations".format(self.base_url), headers=create_headers, data=json.dumps(payload, ensure_ascii=False).encode('utf-8'), timeout=120 )
            ...
            for i in range(max_wait_times):
                ...
                poll_headers = { "Authorization": "Bearer " + self.api_key, "X-ModelScope-Task-Type": "image_generation" }
    ...
    def _handle_sync_response(self, session, args):
        ...
        return { "choices": [{ "message": { "role": "assistant", "content": content, "tool_calls": tool_calls }, "finish_reason": "stop" }], "usage": { "prompt_tokens": 0, "completion_tokens": result.get("completion_tokens", 0), "total_tokens": result.get("total_tokens", 0) }, "model": args.get("model", self.args.get("model")) }
    ...
    def _handle_stream_response(self, session, args):
        try:
            ...
            response = requests.post( "{}/chat/completions".format(self.base_url), headers=headers, json=body, stream=True, timeout=120 )
    ...
    def _create_image_response(self, image_url):
        return { "choices": [{ "message": { "role": "assistant", "content": "已为您生成图片：{}".format(image_url), "tool_calls": None }, "finish_reason": "stop" }], "usage": { "prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0 }, "model": self.args.get("model") }
    ...
    def _create_error_response(self, error_msg):
        return { "choices": [{ "message": { "role": "assistant", "content": error_msg, "tool_calls": None }, "finish_reason": "stop" }], "usage": { "prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0 }, "model": self.args.get("model") }
