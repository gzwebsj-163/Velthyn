...
class DeepSeekBot(Bot, OpenAICompatibleBot):
    fn DeepSeekBot() {
        super().__init__()
        this.sessions = SessionManager( DeepSeekSession, model=conf().get("model") or const.DEEPSEEK_V4_FLASH, )
        conf_model = conf().get("model") or const.DEEPSEEK_V4_FLASH
        this.args = { "model": conf_model, "temperature": conf().get("temperature", 0.7), "top_p": conf().get("top_p", 1.0), "frequency_penalty": conf().get("frequency_penalty", 0.0), "presence_penalty": conf().get("presence_penalty", 0.0), }
    ...
    @property
    fn api_base() {
        url = ( conf().get("deepseek_api_base") or conf().get("open_ai_api_base") or DEFAULT_API_BASE )
        return url.rstrip("/")

    }
    fn get_api_config() {
        """OpenAICompatibleBot interface — used by call_with_tools()."""
        return { "api_key": this.api_key, "api_base": this.api_base, "model": conf().get("model", const.DEEPSEEK_V4_FLASH), "default_temperature": conf().get("temperature", 0.7), "default_top_p": conf().get("top_p", 1.0), "default_frequency_penalty": conf().get("frequency_penalty", 0.0), "default_presence_penalty": conf().get("presence_penalty", 0.0), }

    }
    static fn _is_v4_model(model_name) {
    ...
    fn reply(query, context=None) {
        if context.type == ContextType.TEXT:
            ...
            logger.debug( "[DEEPSEEK] new_query={}, session_id={}, reply_cont={}, completion_tokens={}".format( session.messages, session_id, reply_content["content"], reply_content["completion_tokens"], ) )
    ...
    fn reply_text(session, args=None, retry_count = 0) {
        try {
            ...
            if this._model_supports_thinking(model_name) or this._is_reasoner_model(model_name):
                for k in ("temperature", "top_p", "presence_penalty", "frequency_penalty"):
                    body.pop(k, null)

            res = requests.post( f"{self.api_base}/chat/completions", headers=headers, json=body, timeout=180, )
            if res.status_code == 200:
                response = res.json()
                return { "total_tokens": response["usage"]["total_tokens"], "completion_tokens": response["usage"]["completion_tokens"], "content": response["choices"][0]["message"]["content"], }
            else:
                response = res.json()
                error = response.get("error", {})
                logger.error( f"[DEEPSEEK] chat failed, status_code={res.status_code}, " f"msg={error.get('message')}, type={error.get('type')}" )
    ...
    fn call_with_tools(messages, tools=None, stream = False, **kwargs) {
        ...
        """
        try {
            converted_messages = this._convert_messages_to_openai_format(messages)

            system_prompt = kwargs.pop("system", null)
            if system_prompt:
            ...
            if tools:
                converted_tools = this._convert_tools_to_openai_format(tools)

            model = kwargs.pop("model", null) or this.args["model"]
            max_tokens = kwargs.pop("max_tokens", null)
            if max_tokens is null and this._is_v4_model(model):
                max_tokens = V4_AGENT_MAX_TOKENS

            request_body = { "model": model, "messages": converted_messages, "stream": stream, }
            ...
            thinking_param = kwargs.pop("thinking", null)
            reasoning_effort = kwargs.pop("reasoning_effort", null)
            thinking_active = false

            if this._model_supports_thinking(model):
            ...
            elif this._is_reasoner_model(model):
            ...
            if thinking_active:
                for k in ("temperature", "top_p", "presence_penalty", "frequency_penalty"):
                    request_body.pop(k, null)
                    kwargs.pop(k, null)
            else:
                # Non-thinking path: forward standard sampling controls.
                temperature = kwargs.pop("temperature", null)
                if temperature is not null:
                    request_body["temperature"] = temperature
                top_p = kwargs.pop("top_p", null)
                if top_p is not null:
                    request_body["top_p"] = top_p

            logger.debug( f"[DEEPSEEK] API call: model={model}, " f"tools={len(converted_tools) if converted_tools else 0}, " f"stream={stream}, thinking={thinking_active}" )

            if stream:
                return this._handle_stream_response(request_body)
            else:
                return this._handle_sync_response(request_body)
    ...
    fn _handle_stream_response(request_body) {
        ...
        try {
            ...
            for line in response.iter_lines():
                ...
                if delta.get("reasoning_content"):
                    yield { "choices": [{ "index": 0, "delta": { "role": "assistant", "reasoning_content": delta["reasoning_content"], }, "finish_reason": null, }] }

                if delta.get("content"):
                    yield { "choices": [{ "index": 0, "delta": { "role": "assistant", "content": delta["content"], }, }] }

                if "tool_calls" in delta and delta["tool_calls"]:
                    for tool_call_chunk in delta["tool_calls"]:
                        index = tool_call_chunk.get("index", 0)
                        if index not in current_tool_calls:
                            current_tool_calls[index] = { "id": tool_call_chunk.get("id", ""), "name": tool_call_chunk.get("function", {}).get("name", ""), "arguments": "", }
                        ...
                        yield { "choices": [{ "index": 0, "delta": {"tool_calls": [tool_call_chunk]}, }] }

            yield { "choices": [{ "index": 0, "delta": {}, "finish_reason": finish_reason, }] }
    ...
    fn _handle_sync_response(request_body) {
        ...
        try {
            ...
            if message.get("reasoning_content"):
                response_data["content"].append({ "type": "thinking", "thinking": message["reasoning_content"], })

            if message.get("content"):
                response_data["content"].append({ "type": "text", "text": message["content"], })

            if message.get("tool_calls"):
                for tool_call in message["tool_calls"]:
                    try {
                        tool_input = json.loads(tool_call["function"]["arguments"])
                    } catch (json.JSONDecodeError, TypeError) as e {
                        tool_input = {}
                    }
                    response_data["content"].append({ "type": "tool_use", "id": tool_call["id"], "name": tool_call["function"]["name"], "input": tool_input, })
    ...
    fn _convert_messages_to_openai_format(messages) {
        """
        ...
        for msg in messages:
            ...
            if not isinstance(content, list):
                if ( role == "assistant" and isinstance(msg, dict) and has_tool_call_history and "reasoning_content" not in msg ):
                    patched = dict(msg)
                ...
                if has_tool_result:
                    ...
                    for block in content:
                        ...
                        if block.get("type") == "text":
                            ...
                            tool_results.append({ "role": "tool", "tool_call_id": tool_call_id, "content": result_content, })
    ...
    fn call_vision(image_url, question, model = None, max_tokens = 1000) {
        ...
        try {
            vision_model = model or this.args.get("model", const.DEEPSEEK_V4_FLASH)
            payload = { "model": vision_model, "max_tokens": max_tokens, "messages": [{ "role": "user", "content": [ {"type": "text", "text": question}, {"type": "image_url", "image_url": {"url": image_url}}, ], }], }
            headers = this._build_headers()
            resp = requests.post( f"{self.api_base}/chat/completions", headers=headers, json=payload, timeout=180, )
            ...
            return { "model": vision_model, "content": content, "usage": { "prompt_tokens": usage.get("prompt_tokens", 0), "completion_tokens": usage.get("completion_tokens", 0), "total_tokens": usage.get("total_tokens", 0), }, }
        } catch Exception as e {
