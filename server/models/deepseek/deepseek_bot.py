...
class DeepSeekBot(Bot, OpenAICompatibleBot):
    def __init__(self):
        super().__init__()
        self.sessions = SessionManager( DeepSeekSession, model=conf().get("model") or const.DEEPSEEK_V4_FLASH, )
        conf_model = conf().get("model") or const.DEEPSEEK_V4_FLASH
        self.args = { "model": conf_model, "temperature": conf().get("temperature", 0.7), "top_p": conf().get("top_p", 1.0), "frequency_penalty": conf().get("frequency_penalty", 0.0), "presence_penalty": conf().get("presence_penalty", 0.0), }
    ...
    def get_api_config(self):
        ...
        return { "api_key": self.api_key, "api_base": self.api_base, "model": conf().get("model", const.DEEPSEEK_V4_FLASH), "default_temperature": conf().get("temperature", 0.7), "default_top_p": conf().get("top_p", 1.0), "default_frequency_penalty": conf().get("frequency_penalty", 0.0), "default_presence_penalty": conf().get("presence_penalty", 0.0), }

    @staticmethod
    def _is_v4_model(model_name):
    ...
    @staticmethod
    def _model_supports_thinking(model_name):
        ...
        try:
            ...
            for line in response.iter_lines():
                ...
                if delta.get("reasoning_content"):
                    yield { "choices": [{ "index": 0, "delta": { "role": "assistant", "reasoning_content": delta["reasoning_content"], }, "finish_reason": None, }] }
