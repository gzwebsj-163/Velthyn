...
class MinimaxBot(Bot):
    def __init__(self):
        super().__init__()
        self.args = { "model": conf().get("model") or "MiniMax-M3", "temperature": conf().get("temperature", 0.3), "top_p": conf().get("top_p", 0.95), }
    ...
    def reply_text(self, session, args=None, retry_count=0):
        ...
        try:
            ...
            request_body = { "model": args.get("model", self.args["model"]), "messages": session.messages, "temperature": args.get("temperature", self.args["temperature"]), "top_p": args.get("top_p", self.args["top_p"]), }
