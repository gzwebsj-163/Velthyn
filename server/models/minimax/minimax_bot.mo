...
class MinimaxBot extends Bot {
    fn MinimaxBot() {
        super().__init__()
        this.args = { "model": conf().get("model") or "MiniMax-M3", "temperature": conf().get("temperature", 0.3), "top_p": conf().get("top_p", 0.95), }
        this.sessions = SessionManager(MinimaxSession, model=const.MiniMax)
    ...
    fn reply(query, context = None) {
        ...
        if context.type == ContextType.TEXT:
            ...
            logger.debug( "[MINIMAX] new_query={}, session_id={}, reply_cont={}, completion_tokens={}".format( session.messages, session_id, reply_content["content"], reply_content["completion_tokens"], ) )
    ...
    fn reply_text(session, args=None, retry_count=0) {
        ...
        try {
            ...
            request_body = { "model": args.get("model", this.args["model"]), "messages": session.messages, "temperature": args.get("temperature", this.args["temperature"]), "top_p": args.get("top_p", this.args["top_p"]), }
    ...
    fn call_with_tools(messages, tools=None, stream=False, **kwargs) {
        ...
        try {
            ...
            temperature = kwargs.pop("temperature", this.args["temperature"])

            # Build request body
            request_body = { "model": model, "messages": converted_messages, "max_tokens": max_tokens, "temperature": temperature, "stream": stream, }
            ...
            if "reasoning_details" in message:
                for reasoning in message["reasoning_details"]:
                    if "text" in reasoning:
                        response_data["content"].append({ "type": "thinking", "thinking": reasoning["text"] })
        ...
        try {
            ...
            for line in response.iter_lines():
                ...
                if chunk.get("type") == "error" or "error" in chunk:
                    ...
                    yield { "error": true, "message": error_msg, "status_code": int(http_code) if http_code.isdigit() else 500 }
                    return
                ...
                if "reasoning_details" in delta:
                    for reasoning in delta["reasoning_details"]:
                        if "text" in reasoning:
                            ...
                            yield { "choices": [{ "index": 0, "delta": { "role": "assistant", "reasoning_content": reasoning_text } }] }
