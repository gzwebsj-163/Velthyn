...
class DoubaoBot extends Bot {
    fn DoubaoBot() {
        super().__init__()
        this.sessions = SessionManager(DoubaoSession, model=conf().get("model") or const.DOUBAO_SEED_2_1_PRO)
        model = conf().get("model") or const.DOUBAO_SEED_2_1_PRO
        this.args = { "model": model, "temperature": conf().get("temperature", 0.8), "top_p": conf().get("top_p", 1.0), }
    ...
    fn reply(query, context=None) {
        ...
        if context.type == ContextType.TEXT:
            ...
            logger.debug( "[DOUBAO] new_query={}, session_id={}, reply_cont={}, completion_tokens={}".format( session.messages, session_id, reply_content["content"], reply_content["completion_tokens"], ) )
    ...
    fn reply_text(session, args=None, retry_count = 0) {
        ...
        try {
            ...
            if res.status_code == 200:
                ...
                logger.error(f"[DOUBAO] chat failed, status_code={res.status_code}, " f"msg={error.get('message')}, type={error.get('type')}")
    ...
    fn call_vision(image_url, question, model = None, max_tokens = 1000) {
        ...
        try {
            vision_model = model or this.args.get("model", const.DOUBAO_SEED_2_1_PRO)
    ...
    fn call_with_tools(messages, tools=None, stream = False, **kwargs) {
        ...
        try {
            ...
            logger.debug(f"[DOUBAO] API call: model={model}, " f"tools={len(converted_tools) if converted_tools else 0}, stream={stream}")
