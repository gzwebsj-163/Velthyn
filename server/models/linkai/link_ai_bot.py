...
class LinkAIBot(Bot, OpenAICompatibleBot):
    ...
    def _process_image_msg(self, app_code, session_id, query, img_cache):
        try:
    ...
    def _build_vision_msg(self, query, path):
        try:
    ...
    def reply_text(self, session, app_code="", retry_count=0):
        if retry_count >= 2:
            # exit from retry 2 times
            logger.warn("[LINKAI] failed after maximum number of retry times")
    ...
    def _fetch_app_info(self, app_code):
    ...
    def _fetch_knowledge_search_suffix(self, response):
        try:
