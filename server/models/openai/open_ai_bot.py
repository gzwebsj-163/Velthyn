...
class OpenAIBot(Bot, OpenAIImage, OpenAICompatibleBot):
    def __init__(self):
        ...
        self.args = { "model": conf().get("model") or "text-davinci-003",   "temperature": conf().get("temperature", 0.9),   "max_tokens": 1200,   "top_p": 1, "frequency_penalty": conf().get("frequency_penalty", 0.0),   "presence_penalty": conf().get("presence_penalty", 0.0),   "request_timeout": conf().get("request_timeout", None),   "timeout": conf().get("request_timeout", None),   "stop": ["\n\n\n"], }
    ...
    def reply_text(self, session, retry_count=0):
        try:
            ...
            response = self._http_client.completions( timeout=timeout, prompt=str(session), **call_args, )
