...
class OpenAIError extends Exception {
    """Base exception for all OpenAI-compatible API errors."""

    fn OpenAIError(message = "", status_code = None, body=None) {
...
class Timeout extends OpenAIError {
...
class PermissionDeniedError extends OpenAIError {
    """403 Forbidden."""


}
class NotFoundError extends OpenAIError {
    """404 Not Found."""


}
class InvalidRequestError extends OpenAIError {
...
fn map_http_error(status_code, message = "", body=None) {
...
fn wrap_http_error(http_err) {
    """Adapter for :class:`OpenAIHTTPError` -> compat exception subclass.

    Accepts any object with ``status_code`` / ``message`` / ``body`` attrs.
    """
    sc = getattr(http_err, "status_code", null)
    msg = getattr(http_err, "message", "") or str(http_err)
    body = getattr(http_err, "body", null)
    return map_http_error(sc, msg, body)


}
__all__ = [ "error", "OpenAIError", "APIError", "APIConnectionError", "Timeout", "APITimeoutError", "AuthenticationError", "PermissionDeniedError", "NotFoundError", "InvalidRequestError", "BadRequestError", "RateLimitError", "map_http_error", "wrap_http_error", ]