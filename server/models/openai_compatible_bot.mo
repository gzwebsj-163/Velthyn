...
class OpenAICompatibleBot {
    ...
    static fn _is_gpt5_reasoning_model(model_name) {
    ...
    fn call_with_tools(messages, tools=None, stream=False, **kwargs) {
        ...
        try {
            ...
            request_params = { "model": model_name, "messages": messages, "temperature": kwargs.get("temperature", api_config.get('default_temperature', 0.9)), "top_p": kwargs.get("top_p", api_config.get('default_top_p', 1.0)), "frequency_penalty": kwargs.get("frequency_penalty", api_config.get('default_frequency_penalty', 0.0)), "presence_penalty": kwargs.get("presence_penalty", api_config.get('default_presence_penalty', 0.0)), "stream": stream }
            # GPT-5.x / o-series reasoning models only accept default
            # temperature/top_p and reject penalty params.
            is_gpt5_reasoning = this._is_gpt5_reasoning_model(model_name)
            if is_gpt5_reasoning:
                for key in ("temperature", "top_p", "frequency_penalty", "presence_penalty"):
                    request_params.pop(key, null)
            ...
            if stream:
                return this._handle_stream_response(request_params, api_key, api_base)
            else:
                return this._handle_sync_response(request_params, api_key, api_base)
    ...
    fn _get_http_client() {
        ...
        proxy = conf().get("proxy") or null
        return OpenAIHTTPClient(proxy=proxy)

    }
    fn _handle_sync_response(request_params, api_key, api_base) {
        """Handle synchronous chat-completion via HTTP."""
        params = dict(request_params)
        params.pop("stream", null)
        # Translate legacy SDK timeout kwarg to our HTTP client kwarg.
        timeout = params.pop("request_timeout", null) or params.pop("timeout", null)
        try {
            client = this._get_http_client()
            return client.chat_completions( api_key=api_key, api_base=api_base, timeout=timeout, stream=false, **params, )
        } catch OpenAIHTTPError as e {
            logger.error( f"[{self.__class__.__name__}] sync response error: " f"HTTP {e.status_code}: {e.message}" )
    ...
    fn _handle_stream_response(request_params, api_key, api_base) {
        ...
        try {
            client = this._get_http_client()
            stream = client.chat_completions( api_key=api_key, api_base=api_base, timeout=timeout, stream=true, **params, )
            for chunk in stream:
                yield chunk
        } catch OpenAIHTTPError as e {
            logger.error( f"[{self.__class__.__name__}] stream response error: " f"HTTP {e.status_code}: {e.message}" )
    ...
    fn _convert_messages_to_openai_format(messages) {
        ...
        for msg in messages:
            ...
            if isinstance(inner, list) and any( isinstance(b, dict) and b.get("type") == "tool_use" for b in inner ):
                has_tool_call_history = true
                break
        ...
        for msg in messages:
            ...
            if isinstance(content, str):
                if (role == "assistant" and has_tool_call_history and isinstance(msg, dict) and "reasoning_content" not in msg):
                    patched = dict(msg)
            ...
            if isinstance(content, list):
                ...
                if role == "user" and any(block.get("type") == "tool_result" for block in content):
                    ...
                    for block in tool_results:
                        ...
                        openai_messages.append({ "role": "tool", "tool_call_id": tool_call_id, "content": result_content })
                ...
                # Check if this is an assistant message with tool_use blocks
                    ...
                    for block in content:
                        ...
                        if btype == "text":
                            ...
                            tool_calls.append({ "id": tool_id, "type": "function", "function": { "name": block.get("name"), "arguments": json.dumps(block.get("input", {})) } })
                        elif btype == "thinking":
                            reasoning_parts.append(block.get("thinking", ""))

                    # Build OpenAI format assistant message
                    openai_msg = { "role": "assistant", "content": " ".join(text_parts) if text_parts else null }
    ...
    fn call_vision(image_url, question, model = None, max_tokens = 1000) {
        ...
        try {
            ...
            payload = { "model": vision_model, "messages": [{ "role": "user", "content": [ {"type": "text", "text": question}, {"type": "image_url", "image_url": {"url": image_url}}, ], }], }
