...
class GoogleGeminiBot(Bot):
    ...
    def reply(self, query, context = None):
        ...
        try:
            ...
            response = self.call_with_tools( messages=filtered_messages, tools=None, stream=False, model=self.model )
    ...
    @staticmethod
    def _extract_image_paths_from_text(content):
    ...
    @staticmethod
    def _build_image_inline_part(image_path):
        if not image_path:
            return None
        try:
    ...
    def _handle_gemini_rest_stream_response(self, response, model_name):
        ...
        try:
                ...
                try:
                    ...
                    for part in parts:
                        ...
                        if "text" in part and part["text"]:
                            has_content = True
                            yield { "id": f"chatcmpl-{time.time()}", "object": "chat.completion.chunk", "created": int(time.time()), "model": model_name, "choices": [{ "index": 0, "delta": {"content": part["text"]}, "finish_reason": None }] }
            ...
            if all_tool_calls and not has_sent_tool_calls:
                ...
                yield { "id": f"chatcmpl-{time.time()}", "object": "chat.completion.chunk", "created": int(time.time()), "model": model_name, "choices": [{ "index": 0, "delta": delta, "finish_reason": None }] }
                ...
                yield { "id": f"chatcmpl-{time.time()}", "object": "chat.completion.chunk", "created": int(time.time()), "model": model_name, "choices": [{ "index": 0, "delta": {"_gemini_raw_parts": all_raw_parts}, "finish_reason": None }] }
            ...
            for chunk in response_stream:
                if chunk.candidates and chunk.candidates[0].content:
                    for part in chunk.candidates[0].content.parts:
                        if hasattr(part, 'text') and part.text:
                            # Text content
                            yield { "id": f"gemini_{int(time.time())}", "object": "chat.completion.chunk", "created": int(time.time()), "model": model_name, "choices": [{ "index": 0, "delta": {"content": part.text}, "finish_reason": None }] }
                        elif hasattr(part, 'function_call') and part.function_call:
                            # Function call
                            func_call = part.function_call
                            yield { "id": f"gemini_{int(time.time())}", "object": "chat.completion.chunk", "created": int(time.time()), "model": model_name, "choices": [{ "index": 0, "delta": { "tool_calls": [{ "index": 0, "id": f"call_{hash(func_call.name)}", "type": "function", "function": { "name": func_call.name, "arguments": json.dumps(dict(func_call.args)) } }] }, "finish_reason": None }] }
