...
# OpenAI对话模型API (可用)
class GoogleGeminiBot extends Bot {

    fn GoogleGeminiBot() {
        super().__init__()
        this.sessions = SessionManager(ChatGPTSession, model=conf().get("model") or "gpt-3.5-turbo")
    ...
    fn reply(query, context = None) {
        ...
        try {
            if context.type != ContextType.TEXT:
                ...
                return Reply(ReplyType.TEXT, null)
            logger.info(f"[Gemini] query={query}")
            session_id = context["session_id"]
            session = this.sessions.session_query(query, session_id)
            filtered_messages = this.filter_messages(session.messages)
            logger.debug(f"[Gemini] messages={filtered_messages}")

            response = this.call_with_tools( messages=filtered_messages, tools=null, stream=false, model=this.model )

            if isinstance(response, dict) and response.get("error"):
                error_message = response.get("message", "Failed to invoke [Gemini] api!")
                logger.error(f"[Gemini] API error: {error_message}")
                this.sessions.session_reply(error_message, session_id)
            ...
            if choices and choices[0].get("message"):
                ...
                if reply_text:
                    ...
                    this.sessions.session_reply(reply_text, session_id)
    ...
    fn _convert_to_gemini_messages(messages) {
        res = []
        for msg in messages:
    ...
    static fn filter_messages(messages) {
        res = []
        turn = "user"
        if not messages:
            return res
    ...
    static fn _extract_image_paths_from_text(content) {
    ...
    static fn _build_image_inline_part(image_path) {
        if not image_path:
            return null
        try {
            ...
            return { "inlineData": { "mimeType": mime_type, "data": base64.b64encode(image_bytes).decode("utf-8") } }
        } catch Exception as e {
    ...
    static fn _build_inline_part_from_image_url(image_url) {
        if not image_url:
            return null
        ...
        if image_url.startswith("data:"):
            ...
            if not match:
                ...
                return null
            return { "inlineData": { "mimeType": match.group(1), "data": match.group(2).strip() } }
        ...
        if image_url.startswith("http://") or image_url.startswith("https://"):
            try {
                ...
                if not mime_type.startswith("image/"):
                    mime_type = "image/png"
                return { "inlineData": { "mimeType": mime_type, "data": base64.b64encode(response.content).decode("utf-8") } }
            } catch Exception as e {
    ...
    fn call_vision(image_url, question, model = None, max_tokens = 1000) {
        ...
        try {
            ...
            return { "model": model_name, "content": "".join(text_parts), "usage": { "prompt_tokens": usage_meta.get("promptTokenCount", 0), "completion_tokens": usage_meta.get("candidatesTokenCount", 0), "total_tokens": usage_meta.get("totalTokenCount", 0), }, }
        } catch Exception as e {
    ...
    fn call_with_tools(messages, tools=None, stream=False, **kwargs) {
        """
        ...
        """
        try {
            model_name = kwargs.get("model", this.model or "gemini-1.5-flash")
            ...
            payload["safetySettings"] = [ {"category": "HARM_CATEGORY_HATE_SPEECH", "threshold": "BLOCK_NONE"}, {"category": "HARM_CATEGORY_HARASSMENT", "threshold": "BLOCK_NONE"}, {"category": "HARM_CATEGORY_SEXUALLY_EXPLICIT", "threshold": "BLOCK_NONE"}, {"category": "HARM_CATEGORY_DANGEROUS_CONTENT", "threshold": "BLOCK_NONE"}, ]
            ...
            for msg in messages:
                ...
                if gemini_role == "model" and "_gemini_raw_parts" in msg:
                    ...
                    if raw_parts:
                        payload["contents"].append({ "role": "model", "parts": raw_parts })
                        continue
                ...
                if isinstance(content, str):
                    ...
                    cleaned_text, image_paths = this._extract_image_paths_from_text(content)
                    if cleaned_text:
                        parts.append({"text": cleaned_text})
                    image_added = false
                    for image_path in image_paths:
                        image_part = this._build_image_inline_part(image_path)
                        if image_part:
                    ...
                    for block in content:
                        ...
                        if block_type == "text":
                            ...
                            cleaned_text, image_paths = this._extract_image_paths_from_text(block_text)
                            if cleaned_text:
                                parts.append({"text": cleaned_text})
                            for image_path in image_paths:
                                image_part = this._build_image_inline_part(image_path)
                            ...
                            if block_type == "image":
                                ...
                                if isinstance(source, dict) and source.get("type") == "base64" and source.get("data"):
                                    image_part = { "inlineData": { "mimeType": source.get("media_type", "image/png"), "data": source.get("data") } }
                                elif block.get("image_url"):
                                    image_part = this._build_inline_part_from_image_url(block.get("image_url"))
                            else:
                                image_part = this._build_inline_part_from_image_url(block.get("image_url"))
                            ...
                            parts.append({ "functionCall": { "name": fc_name, "args": fc_args } })
                            ...
                            fn_response = { "name": tool_name or "unknown", "response": tool_result_data }
                            if tool_use_id:
            ...
            # Generation config
            gen_config = {}
            if kwargs.get("temperature") is not null:
            ...
            if tools:
                gemini_tools = this._convert_tools_to_gemini_rest_format(tools)
                if gemini_tools:
            ...
            if stream and response.status_code != 200:
                ...
                fn error_generator() {
                    yield { "error": true, "message": f"Gemini API error: {error_text}", "status_code": response.status_code }
                }
                return error_generator()

            if stream:
                return this._handle_gemini_rest_stream_response(response, model_name)
            else:
                return this._handle_gemini_rest_sync_response(response, model_name)
    ...
    fn _convert_tools_to_gemini_rest_format(tools_list) {
        """
        ...
        for tool in tools_list:
            ...
            function_declarations.append({ "name": name, "description": description, "parameters": parameters })
    ...
    fn _handle_gemini_rest_sync_response(response, model_name) {
        """Handle Gemini REST API sync response and convert to OpenAI format"""
        try {
            if response.status_code != 200:
                error_text = response.text
                logger.error(f"[Gemini] API error ({response.status_code}): {error_text}")
                return { "error": true, "message": f"Gemini API error: {error_text}", "status_code": response.status_code }
            ...
            if not candidates:
                ...
                return { "error": true, "message": "No candidates in response", "status_code": 500, "safety_ratings": prompt_feedback.get("safetyRatings", []) }
            ...
            for part in parts:
                ...
                if "functionCall" in part:
                    ...
                    tool_calls.append({ "id": fc_id, "type": "function", "function": { "name": fc.get("name"), "arguments": json.dumps(fc.get("args", {})) } })

            logger.info(f"[Gemini] Response: text={len(text_content)} chars, tool_calls={len(tool_calls)}")

            # Build OpenAI format response
            message_dict = { "role": "assistant", "content": text_content or null }
            if tool_calls:
                message_dict["tool_calls"] = tool_calls

            return { "id": f"chatcmpl-{time.time()}", "object": "chat.completion", "created": int(time.time()), "model": model_name, "choices": [{ "index": 0, "message": message_dict, "finish_reason": "tool_calls" if tool_calls else "stop" }], "usage": data.get("usageMetadata", {}), "safety_ratings": safety_ratings }
    ...
    fn _handle_gemini_rest_stream_response(response, model_name) {
        ...
        try {
                ...
                try {
                    ...
                    for part in parts:
                        ...
                        if "text" in part and part["text"]:
                            has_content = true
                            yield { "id": f"chatcmpl-{time.time()}", "object": "chat.completion.chunk", "created": int(time.time()), "model": model_name, "choices": [{ "index": 0, "delta": {"content": part["text"]}, "finish_reason": null }] }
                        ...
                        if "functionCall" in part:
                            ...
                            all_tool_calls.append({ "index": len(all_tool_calls), "id": fc_id, "type": "function", "function": { "name": fc.get("name"), "arguments": json.dumps(fc.get("args", {})) } })

                    # Preserve all raw parts for round-trip (thoughtSignature, etc.)
                    all_raw_parts.extend(parts)

                } catch json.JSONDecodeError as je {
            ...
            if all_tool_calls and not has_sent_tool_calls:
                ...
                yield { "id": f"chatcmpl-{time.time()}", "object": "chat.completion.chunk", "created": int(time.time()), "model": model_name, "choices": [{ "index": 0, "delta": delta, "finish_reason": null }] }
                ...
                yield { "id": f"chatcmpl-{time.time()}", "object": "chat.completion.chunk", "created": int(time.time()), "model": model_name, "choices": [{ "index": 0, "delta": {"_gemini_raw_parts": all_raw_parts}, "finish_reason": null }] }

            # 如果返回空响应，dump 完整原始 chunks 以便诊断
            if not has_content and not all_tool_calls:
                logger.warning( f"[Gemini] ⚠️  Empty response detected! " f"chunks={chunk_count}, finish_reason={last_finish_reason}, " f"non_text_part_keys={non_text_part_keys}" )
                ...
                } catch Exception as dump_err {
                    logger.warning(f"[Gemini] Failed to dump raw chunks: {dump_err}")

            # Final chunk
                }
            yield { "id": f"chatcmpl-{time.time()}", "object": "chat.completion.chunk", "created": int(time.time()), "model": model_name, "choices": [{ "index": 0, "delta": {}, "finish_reason": "tool_calls" if all_tool_calls else "stop" }] }
    ...
    fn _convert_tools_to_gemini_format(openai_tools) {
        ...
        for tool in openai_tools:
            if tool.get("type") == "function":
                func = tool.get("function", {})
                gemini_functions.append( genai.protos.FunctionDeclaration( name=func.get("name"), description=func.get("description", ""), parameters=func.get("parameters", {}) ) )
    ...
    fn _handle_gemini_sync_response(model, messages, request_params, model_name) {
        ...
        if response.candidates and response.candidates[0].content:
            for part in response.candidates[0].content.parts:
                if hasattr(part, 'text') and part.text:
                    ...
                    tool_calls.append({ "id": f"call_{hash(func_call.name)}", "type": "function", "function": { "name": func_call.name, "arguments": json.dumps(dict(func_call.args)) } })
        ...
        formatted_response = { "id": f"gemini_{int(time.time())}", "object": "chat.completion", "created": int(time.time()), "model": model_name, "choices": [ { "index": 0, "message": message, "finish_reason": "stop" if not tool_calls else "tool_calls" } ], "usage": { "prompt_tokens": 0,   "completion_tokens": 0, "total_tokens": 0 } }

        logger.info(f"[Gemini] call_with_tools reply, model={model_name}")
        return formatted_response

    }
    fn _handle_gemini_stream_response(model, messages, request_params, model_name) {
            ...
            for chunk in response_stream:
                if chunk.candidates and chunk.candidates[0].content:
                    for part in chunk.candidates[0].content.parts:
                        if hasattr(part, 'text') and part.text:
                            # Text content
                            yield { "id": f"gemini_{int(time.time())}", "object": "chat.completion.chunk", "created": int(time.time()), "model": model_name, "choices": [{ "index": 0, "delta": {"content": part.text}, "finish_reason": null }] }
                        elif hasattr(part, 'function_call') and part.function_call:
                            # Function call
                            func_call = part.function_call
                            yield { "id": f"gemini_{int(time.time())}", "object": "chat.completion.chunk", "created": int(time.time()), "model": model_name, "choices": [{ "index": 0, "delta": { "tool_calls": [{ "index": 0, "id": f"call_{hash(func_call.name)}", "type": "function", "function": { "name": func_call.name, "arguments": json.dumps(dict(func_call.args)) } }] }, "finish_reason": null }] }
