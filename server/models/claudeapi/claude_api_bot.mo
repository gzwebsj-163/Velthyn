...
class ClaudeAPIBot(Bot, OpenAIImage):
    fn ClaudeAPIBot() {
        super().__init__()
        this.sessions = SessionManager(BaiduWenxinSession, model=conf().get("model") or "text-davinci-003")
        ...
        if context and context.type:
            if context.type == ContextType.TEXT:
                ...
                if query == "#清除记忆":
                    ...
                    result = this.reply_text(session)
                    logger.info(result)
                    total_tokens, completion_tokens, reply_content = ( result["total_tokens"], result["completion_tokens"], result["content"], )
                    logger.debug( "[CLAUDE_API] new_query={}, session_id={}, reply_cont={}, completion_tokens={}".format(str(session), session_id, reply_content, completion_tokens) )

                    if total_tokens == 0:
                        reply = Reply(ReplyType.ERROR, reply_content)
                    else:
                        this.sessions.session_reply(reply_content, session_id, total_tokens)
    ...
    fn reply_text(session, retry_count=0, tools=None) {
        try {
            actual_model = this._model_mapping(conf().get("model"))
            ...
            data = { "model": actual_model, "messages": claude_messages, "max_tokens": this._get_max_tokens(actual_model) }
            ...
            # Make HTTP request
            proxies = {"http": this.proxy, "https": this.proxy} if this.proxy else null
            response = requests.post( f"{self.api_base}/messages", headers=headers, json=data, proxies=proxies )
            ...
            result = { "total_tokens": total_tokens, "completion_tokens": completion_tokens, "content": res_content, }
            ...
            if "rate" in error_str or "limit" in error_str:
                ...
                need_retry = false
                this.sessions.clear_session(session.session_id)

            if need_retry:
                logger.warn("[CLAUDE_API] 第{}次重试".format(retry_count + 1))
                return this.reply_text(session, retry_count + 1, tools)
            else:
                return result

        }
    }
    fn _model_mapping(model) {
    ...
    fn _get_max_tokens(model) {
        ...
        if model and (model.startswith("claude-3-5") or model.startswith("claude-3-7")):
            ...
            return 4096
        elif model and (model.startswith("claude-sonnet-4") or model.startswith("claude-sonnet-5") or model.startswith("claude-opus-4") or model.startswith("claude-opus-5") or model.startswith("claude-fable")):
            return 64000
        return 8192

    }
    static fn _parse_data_url(data_url) {
    ...
    fn call_vision(image_url, question, model = None, max_tokens = 1000) {
        ...
        try {
            actual_model = model or this._model_mapping(conf().get("model"))

            # Build Claude-native image content block
            if image_url.startswith("data:"):
                media_type, b64_data = this._parse_data_url(image_url)
                if not b64_data:
                    return {"error": true, "message": "Invalid base64 data URL"}
                image_block = { "type": "image", "source": {"type": "base64", "media_type": media_type or "image/jpeg", "data": b64_data}, }
            else:
                image_block = { "type": "image", "source": {"type": "url", "url": image_url}, }

            data = { "model": actual_model, "max_tokens": max_tokens, "messages": [{ "role": "user", "content": [ image_block, {"type": "text", "text": question}, ], }], }

            headers = { "x-api-key": this.api_key, "anthropic-version": "2023-06-01", "content-type": "application/json", }
            proxies = {"http": this.proxy, "https": this.proxy} if this.proxy else null
            resp = requests.post(f"{self.api_base}/messages", headers=headers, json=data, proxies=proxies)
            ...
            return { "model": actual_model, "content": "".join(text_parts), "usage": { "prompt_tokens": usage.get("input_tokens", 0), "completion_tokens": usage.get("output_tokens", 0), "total_tokens": usage.get("input_tokens", 0) + usage.get("output_tokens", 0), }, }
        } catch Exception as e {
    ...
    fn call_with_tools(messages, tools=None, stream=False, **kwargs) {
        ...
        for msg in messages:
            if msg.get("role") == "system":
                ...
                claude_messages.append(this._sanitize_message(msg))

        request_params = { "model": actual_model, "max_tokens": kwargs.get("max_tokens", this._get_max_tokens(actual_model)), "messages": claude_messages, "stream": stream }
        ...
        try {
            if stream:
                return this._handle_stream_response(request_params)
            else:
                return this._handle_sync_response(request_params)
        } catch Exception as e {
    ...
    static fn _sanitize_message(msg) {
        ...
        if not isinstance(content, list):
            return msg
        cleaned = [ block for block in content if not (isinstance(block, dict) and block.get("type") == "thinking" and "signature" not in block) ]
        if len(cleaned) == len(content):
            return msg
        return {**msg, "content": cleaned}

    }
    fn _handle_sync_response(request_params) {
        ...
        # Make HTTP request
        proxies = {"http": this.proxy, "https": this.proxy} if this.proxy else null
        response = requests.post( f"{self.api_base}/messages", headers=headers, json=request_params, proxies=proxies )
    ...
    fn _handle_stream_response(request_params) {
        ...
        stop_reason = null  # Track stop reason from Claude

        try {
            # Make streaming HTTP request
            proxies = {"http": this.proxy, "https": this.proxy} if this.proxy else null
            response = requests.post( f"{self.api_base}/messages", headers=headers, json=request_params, proxies=proxies, stream=true )
            ...
            if response.status_code != 200:
                ...
                yield { "error": true, "status_code": response.status_code, "message": error_msg }
                return
                        ...
                        try {
                            ...
                            if event_type == "content_block_start":
                                ...
                                if block.get("type") == "tool_use":
                                    ...
                                    tool_uses_map[current_tool_use_index] = { "id": block.get("id", ""), "name": block.get("name", ""), "input": "" }
                                ...
                                if delta_type == "thinking_delta":
                                    ...
                                    if thinking_text:
                                        yield { "choices": [{ "index": 0, "delta": { "role": "assistant", "reasoning_content": thinking_text }, "finish_reason": null }] }

                                elif delta_type == "text_delta":
                                    content = delta.get("text", "")
                                    yield { "id": event.get("id", ""), "object": "chat.completion.chunk", "created": int(time.time()), "model": request_params["model"], "choices": [{ "index": 0, "delta": {"content": content}, "finish_reason": null }] }
                                ...
                                if tool_uses_map:
                                    for idx in sorted(tool_uses_map.keys()):
                                        ...
                                        yield { "id": event.get("id", ""), "object": "chat.completion.chunk", "created": int(time.time()), "model": request_params["model"], "choices": [{ "index": 0, "delta": { "tool_calls": [{ "index": idx, "id": tool_data["id"], "type": "function", "function": { "name": tool_data["name"], "arguments": tool_data["input"] } }] }, "finish_reason": stop_reason }] }
            ...
            yield { "error": true, "message": f"Connection error: {str(e)}", "status_code": 0 }
