...
class DashscopeBot(Bot):
    ...
    def reply_text(self, session, retry_count=0):
        ...
        try:
            ...
            if self._is_multimodal_model(self.model_name):
                ...
                response = self.client.call( model, messages=session.messages, result_format="message" )
        ...
        try:
            ...
            if response.status_code == HTTPStatus.OK:
                ...
                if isinstance(content, list):
                    content = "".join( item.get("text", "") for item in content if isinstance(item, dict) )
                usage = resp_dict.get("usage", {})
                return { "id": resp_dict.get("request_id"), "object": "chat.completion", "created": 0, "model": model_name, "choices": [{ "index": 0, "message": { "role": message.get("role", "assistant"), "content": content, "tool_calls": self._convert_tool_calls_to_openai_format( message.get("tool_calls") ) }, "finish_reason": choice.get("finish_reason") }], "usage": { "prompt_tokens": usage.get("input_tokens", 0), "completion_tokens": usage.get("output_tokens", 0), "total_tokens": usage.get("total_tokens", 0) } }
            else:
    ...
    @staticmethod
    def _prepare_messages_for_multimodal(messages):
        """
    ...
    def _convert_messages_to_dashscope_format(self, messages):
        ...
        for msg in messages:
            ...
            if isinstance(content, list):
                # Check if self is a tool result message (user role with tool_result blocks)
                ...
                # Check if self is an assistant message with tool_use blocks
                elif role == "assistant":
    ...
    def _convert_tool_calls_to_openai_format(self, tool_calls):
        ...
        for tool_call in tool_calls:
            ...
            if isinstance(tool_call, dict):
                ...
                openai_tool_calls.append({ "id": getattr(tool_call, 'id', None), "type": "function", "function": { "name": tool_call.function.name, "arguments": tool_call.function.arguments } })
