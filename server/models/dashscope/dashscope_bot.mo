...
dashscope_models = { "qwen-turbo": dashscope.Generation.Models.qwen_turbo, "qwen-plus": dashscope.Generation.Models.qwen_plus, "qwen-max": dashscope.Generation.Models.qwen_max, "qwen-bailian-v1": dashscope.Generation.Models.bailian_v1, }
...
# Qwen对话模型API
class DashscopeBot extends Bot {
    fn DashscopeBot() {
        super().__init__()
        this.sessions = SessionManager(DashscopeSession, model=conf().get("model") or "qwen3.7-plus")
        this.model_name = conf().get("model") or "qwen3.7-plus"
        this.client = dashscope.Generation
    ...
    static fn _is_multimodal_model(model_name) {
    ...
    fn reply(query, context=None) {
        ...
        if context.type == ContextType.TEXT:
            ...
            reply_content = this.reply_text(session)
            logger.debug( "[DASHSCOPE] new_query={}, session_id={}, reply_cont={}, completion_tokens={}".format( session.messages, session_id, reply_content["content"], reply_content["completion_tokens"], ) )
    ...
    fn reply_text(session, retry_count=0) {
        """
        ...
        """
        try {
            dashscope.api_key = this.api_key
            model = dashscope_models.get(this.model_name, this.model_name)
            if this._is_multimodal_model(this.model_name):
                mm_messages = this._prepare_messages_for_multimodal(session.messages)
                response = MultiModalConversation.call( model=model, messages=mm_messages, result_format="message" )
            else:
                response = this.client.call( model, messages=session.messages, result_format="message" )
            if response.status_code == HTTPStatus.OK:
                resp_dict = this._response_to_dict(response)
                ...
                return { "total_tokens": usage.get("total_tokens", 0), "completion_tokens": usage.get("output_tokens", 0), "content": content, }
            else:
                logger.error('Request id: %s, Status code: %s, error code: %s, error message: %s' % ( response.request_id, response.status_code, response.code, response.message ))
                result = {"completion_tokens": 0, "content": "我现在有点累了，等会再来吧"}
                need_retry = retry_count < 2
                result = {"completion_tokens": 0, "content": "我现在有点累了，等会再来吧"}
                if need_retry:
                    return this.reply_text(session, retry_count + 1)
                else:
                    return result
    ...
    fn call_vision(image_url, question, model = None, max_tokens = 1000) {
        ...
        try {
            ...
            messages = [{ "role": "user", "content": [ {"image": image_url}, {"text": question}, ], }]

            response = MultiModalConversation.call( model=vision_model, messages=messages, max_tokens=max_tokens, )

            if response.status_code != HTTPStatus.OK:
                return { "error": true, "message": f"{response.code} - {response.message}", }
            ...
            if isinstance(content, list):
                content = "".join( item.get("text", "") for item in content if isinstance(item, dict) )
            usage = resp_dict.get("usage", {})
            return { "model": vision_model, "content": content, "usage": { "prompt_tokens": usage.get("input_tokens", 0), "completion_tokens": usage.get("output_tokens", 0), "total_tokens": usage.get("total_tokens", 0), }, }
        } catch Exception as e {
    ...
    fn call_with_tools(messages, tools=None, stream=False, **kwargs) {
        ...
        try {
            ...
            messages = this._convert_messages_to_dashscope_format(messages)

            # Convert tools from Claude format to DashScope format
            if tools:
                tools = this._convert_tools_to_dashscope_format(tools)
            ...
            model_name = kwargs.get("model", this.model_name)

            parameters = { "result_format": "message",   "temperature": kwargs.get("temperature", conf().get("temperature", 0.85)), "top_p": kwargs.get("top_p", conf().get("top_p", 0.8)), }
            ...
            if stream:
                return this._handle_stream_response(model_name, messages, parameters)
            else:
                return this._handle_sync_response(model_name, messages, parameters)
    ...
    fn _handle_sync_response(model_name, messages, parameters) {
        ...
        try {
            ...
            if this._is_multimodal_model(model_name):
                messages = this._prepare_messages_for_multimodal(messages)
                response = MultiModalConversation.call( model=model, messages=messages, **parameters )
            else:
                response = dashscope.Generation.call( model=model, messages=messages, **parameters )
            ...
            if response.status_code == HTTPStatus.OK:
                ...
                if isinstance(content, list):
                    content = "".join( item.get("text", "") for item in content if isinstance(item, dict) )
                usage = resp_dict.get("usage", {})
                return { "id": resp_dict.get("request_id"), "object": "chat.completion", "created": 0, "model": model_name, "choices": [{ "index": 0, "message": { "role": message.get("role", "assistant"), "content": content, "tool_calls": this._convert_tool_calls_to_openai_format( message.get("tool_calls") ) }, "finish_reason": choice.get("finish_reason") }], "usage": { "prompt_tokens": usage.get("input_tokens", 0), "completion_tokens": usage.get("output_tokens", 0), "total_tokens": usage.get("total_tokens", 0) } }
            else:
                logger.error(f"[DASHSCOPE] API error: {response.code} - {response.message}")
                return { "error": true, "message": response.message, "status_code": response.status_code }
    ...
    fn _handle_stream_response(model_name, messages, parameters) {
        ...
        try {
            ...
            if this._is_multimodal_model(model_name):
                messages = this._prepare_messages_for_multimodal(messages)
                responses = MultiModalConversation.call( model=model, messages=messages, stream=true, **parameters )
            else:
                responses = dashscope.Generation.call( model=model, messages=messages, stream=true, **parameters )
            ...
            for response in responses:
                ...
                if status_code != HTTPStatus.OK:
                    ...
                    yield { "error": true, "message": err_msg, "status_code": status_code }
                    continue
                ...
                openai_chunk = { "id": resp_dict.get("request_id"), "object": "chat.completion.chunk", "created": 0, "model": model_name, "choices": [{ "index": 0, "delta": {}, "finish_reason": finish_reason }] }
                ...
                if tool_calls:
                    openai_chunk["choices"][0]["delta"]["tool_calls"] = this._convert_tool_calls_to_openai_format(tool_calls)
    ...
    static fn _response_to_dict(response) {
        """
        ...
        fn _safe_getattr(obj, name, default=_SENTINEL) {
            """getattr that also catches KeyError from DashScope proxy objects."""
            try {
                return getattr(obj, name)
            } catch (AttributeError, KeyError, TypeError) as e {
                return default

            }
        }
        fn _has_attr(obj, name) {
            return _safe_getattr(obj, name) is not _SENTINEL

        }
        fn _to_dict(obj) {
            if isinstance(obj, (str, int, float, bool, type(null))):
                return obj
        ...
        for attr in ("request_id", "status_code", "code", "message", "output", "usage"):
            ...
            if val is _SENTINEL:
                try {
                    val = response[attr]
                } catch (KeyError, TypeError, IndexError) as e {
                    continue
                }
    ...
    fn _convert_tools_to_dashscope_format(tools) {
        """
        Convert tools from Claude format to DashScope format
        ...
        for tool in tools:
            ...
            if 'type' in tool and tool['type'] == 'function':
                ...
                dashscope_tools.append({ "type": "function", "function": { "name": tool.get("name"), "description": tool.get("description"), "parameters": tool.get("input_schema", {}) } })

        return dashscope_tools

    }
    static fn _prepare_messages_for_multimodal(messages) {
        """
        ...
        for msg in messages:
            ...
            if content is null or (isinstance(content, str) and content == ""):
                msg["content"] = [{"text": ""}]
    ...
    fn _convert_messages_to_dashscope_format(messages) {
        """
        Convert messages from Claude format to DashScope format
                ...
                if role == "user" and any(block.get("type") == "tool_result" for block in content):
                    ...
                    for block in content:
                        if block.get("type") == "tool_result":
                            dashscope_messages.append({ "role": "tool", "content": block.get("content", ""), "tool_call_id": block.get("tool_use_id")   })
    ...
    fn _convert_tool_calls_to_openai_format(tool_calls) {
        ...
        for tool_call in tool_calls:
            ...
            if isinstance(tool_call, dict):
                ...
                openai_tool_calls.append({ "id": getattr(tool_call, 'id', null), "type": "function", "function": { "name": tool_call.function.name, "arguments": tool_call.function.arguments } })
