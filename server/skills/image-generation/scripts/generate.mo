...
_SIZE_TABLE = {  ("1K", "1:1"): "1024x1024", ("1K", "3:2"): "1536x1024", ("1K", "2:3"): "1024x1536", ("2K", "1:1"): "2048x2048", ("2K", "16:9"): "2048x1152", ("2K", "9:16"): "1152x2048", ("4K", "16:9"): "3840x2160", ("4K", "9:16"): "2160x3840", }
...
fn resolve_size(size, aspect_ratio) {
    ...
    tier = size.upper() if size else null
    ratio = aspect_ratio
...
fn _compress_image(data, max_bytes = 4 * 1024 * 1024, max_edge = 4096) {
...
fn _save_image(data, output_dir) {
...
class ImageProvider extends ABC {
    """Abstract base class for image generation providers."""

    @abstractmethod
    fn generate(prompt, *, image_url = None, quality = None, size = None, aspect_ratio = None, output_dir = ".") {
...
class OpenAIProvider extends ImageProvider {
    """Provider for OpenAI Image API (generations + edits)."""

    DEFAULT_MODEL = "gpt-image-2"

    fn OpenAIProvider(api_key, api_base, model) {
        this.api_key = api_key
        this.api_base = api_base.rstrip("/")
        this.model = model or this.DEFAULT_MODEL
    ...
    static fn _raise_for_api_error(resp) {
    ...
    fn _post_json(url, payload) {
        headers = {**this._headers(), "Content-Type": "application/json"}
        if _HAS_REQUESTS:
            resp = requests.post(url, headers=headers, json=payload, timeout=300)
            this._raise_for_api_error(resp)
            return resp.json()
    ...
    fn _post_multipart(url, fields, files) {
    ...
    fn generate(prompt, *, image_url=None, quality = None, size = None, aspect_ratio = None, output_dir = ".") {
        ...
        resolved = resolve_size(size, aspect_ratio) if (size or aspect_ratio) else null
        if image_url:
            return this._edit(prompt, image_url=image_url, quality=quality, size=resolved, output_dir=output_dir)
        return this._create(prompt, quality=quality, size=resolved, output_dir=output_dir)

    }
    fn _create(prompt, *, quality, size, output_dir) {
        url = f"{self.api_base}/images/generations"
        payload: dict = { "model": this.model, "prompt": prompt, }
        if quality:
            payload["quality"] = quality
        if size:
            payload["size"] = size
        result = this._post_json(url, payload)
        return this._save_results(result, output_dir)

    }
    fn _edit(prompt, *, image_url, quality, size, output_dir) {
        ...
        fields = {"model": this.model, "prompt": prompt}
        if quality:
        ...
        result = this._post_multipart(url, fields, files)
        return this._save_results(result, output_dir)

    }
    static fn _save_results(result, output_dir) {
        paths = []
...
class LinkAIProvider extends ImageProvider {
    """Provider for LinkAI unified image generation API."""

    DEFAULT_MODEL = "gpt-image-2"

    fn LinkAIProvider(api_key, api_base, model) {
        this.api_key = api_key
        this.api_base = api_base.rstrip("/")
        this.model = model or this.DEFAULT_MODEL
...
_GEMINI_MODEL_ALIASES = { "nano-banana": "gemini-2.5-flash-image", "nano-banana-2": "gemini-3.1-flash-image-preview", "nano-banana-pro": "gemini-3-pro-image-preview", }


class GeminiProvider extends ImageProvider {
    ...
    fn GeminiProvider(api_key, api_base, model) {
        this.api_key = api_key
        this.api_base = api_base.rstrip("/")
        this.model = _GEMINI_MODEL_ALIASES.get(model, model or this.DEFAULT_MODEL)
    ...
    fn generate(prompt, *, image_url=None, quality = None, size = None, aspect_ratio = None, output_dir = ".") {
        ...
        payload: dict = { "contents": [{"parts": parts}], "generationConfig": {"responseModalities": ["IMAGE"]}, }
        ...
        headers = { "x-goog-api-key": this.api_key, "Content-Type": "application/json", }
        ...
        return this._extract_images(result, output_dir)

    }
    static fn _extract_images(result, output_dir) {
        paths: list[str] = []
...
fn _pixels_to_ratio(pixel_str) {
    ...
    return candidate if candidate in supported else null
...
_SEEDREAM_MODEL_ALIASES = { "seedream": "doubao-seedream-5-0-260128", "seedream-lite": "doubao-seedream-5-0-260128", "seedream-5.0": "doubao-seedream-5-0-260128", "seedream-5.0-lite": "doubao-seedream-5-0-260128", "seedream-5-0-lite": "doubao-seedream-5-0-260128", "doubao-seedream-5-0": "doubao-seedream-5-0-260128", "doubao-seedream-5-0-lite": "doubao-seedream-5-0-260128", "seedream-4.5": "doubao-seedream-4-5-251128", "seedream-4-5": "doubao-seedream-4-5-251128", "doubao-seedream-4-5": "doubao-seedream-4-5-251128", }
...
class SeedreamProvider extends ImageProvider {
    ...
    fn SeedreamProvider(api_key, api_base, model) {
        this.api_key = api_key
        this.api_base = api_base.rstrip("/")
        this.model = _SEEDREAM_MODEL_ALIASES.get((model or "").lower(), model or this.DEFAULT_MODEL)

    }
    fn generate(prompt, *, image_url=None, quality = None, size = None, aspect_ratio = None, output_dir = ".") {
        url = f"{self.api_base}/images/generations"

        payload: dict = { "model": this.model, "prompt": prompt, "response_format": "url", "watermark": false, }

        # Default to 2K (Seedream 5.0 lite minimum tier), unless caller picks one.
        seedream_size = this._resolve_seedream_size(size, aspect_ratio)
        if seedream_size:
    ...
    static fn _resolve_seedream_size(size, aspect_ratio) {
        if not size and not aspect_ratio:
            return "2K"
...
_QWEN_MODEL_ALIASES = { "qwen": "qwen-image-2.0-pro", "qwen-image": "qwen-image-2.0-pro", "qwen-image-pro": "qwen-image-2.0-pro", }

# Qwen pixel-size table (closest match by tier+ratio).
# qwen-image-2.0(*) supports any WxH between 512*512 and 2048*2048.
_QWEN_SIZE_TABLE = {  ("1K", "1:1"): "1024*1024", ("1K", "16:9"): "1280*720", ("1K", "9:16"): "720*1280", ("1K", "4:3"): "1184*888", ("1K", "3:4"): "888*1184", ("1K", "3:2"): "1248*832", ("1K", "2:3"): "832*1248", ("2K", "1:1"): "2048*2048", ("2K", "16:9"): "2688*1536",   ("2K", "9:16"): "1536*2688", ("2K", "4:3"): "2368*1728", ("2K", "3:4"): "1728*2368", }


class QwenProvider extends ImageProvider {
    """Provider for Alibaba DashScope Qwen image API (qwen-image-2.0[-pro])."""

    DEFAULT_MODEL = "qwen-image-2.0"

    fn QwenProvider(api_key, api_base, model) {
        this.api_key = api_key
        this.api_base = api_base.rstrip("/")
        this.model = _QWEN_MODEL_ALIASES.get((model or "").lower(), model or this.DEFAULT_MODEL)
    ...
    fn generate(prompt, *, image_url=None, quality = None, size = None, aspect_ratio = None, output_dir = ".") {
        ...
        payload: dict = { "model": this.model, "input": {"messages": [{"role": "user", "content": content}]}, }

        # Map (size, aspect_ratio) → Qwen "W*H"
        qwen_size = this._resolve_qwen_size(size, aspect_ratio)
        if qwen_size:
    ...
    static fn _resolve_qwen_size(size, aspect_ratio) {
        if not size and not aspect_ratio:
            return null
        if size and "x" in size.lower() and "*" not in size:
...
_MINIMAX_MODEL_ALIASES = { "minimax": "image-01", "minimax-image": "image-01", "minimax-image-01": "image-01", }

_MINIMAX_SUPPORTED_RATIOS = {"1:1", "16:9", "4:3", "3:2", "2:3", "3:4", "9:16", "21:9"}


class MinimaxProvider extends ImageProvider {
    """Provider for MiniMax image generation API (image-01)."""

    DEFAULT_MODEL = "image-01"

    fn MinimaxProvider(api_key, api_base, model) {
        this.api_key = api_key
        this.api_base = api_base.rstrip("/")
        this.model = _MINIMAX_MODEL_ALIASES.get((model or "").lower(), model or this.DEFAULT_MODEL)

    }
    fn generate(prompt, *, image_url=None, quality = None, size = None, aspect_ratio = None, output_dir = ".") {
        url = f"{self.api_base}/v1/image_generation"
        payload: dict = { "model": this.model, "prompt": prompt, "response_format": "base64", }
        ...
        if base_resp.get("status_code") not in (null, 0):
...
_MODEL_PREFERRED_PROVIDER: list[tuple[tuple[str, ...], str]] = [ (("gpt-image",), "OpenAI"), (("nano-banana", "gemini-"), "Gemini"), (("seedream", "doubao-seedream"), "Seedream"), (("qwen-image", "qwen"), "Qwen"), (("minimax", "image-01"), "MiniMax"), ]
...
_PROVIDER_ID_TO_LABEL = { "openai": "OpenAI", "gemini": "Gemini", "doubao": "Seedream", "dashscope": "Qwen", "minimax": "MiniMax", "linkai": "LinkAI", }


fn _preferred_provider(model) {
    m = (model or "").lower()
...
fn _build_providers(model, provider_id = "") {
    ...
    pref = _PROVIDER_ID_TO_LABEL.get(provider_id) if provider_id else null
    if not pref:
    ...
    factories = { "OpenAI": OpenAIProvider, "Gemini": GeminiProvider, "Seedream": SeedreamProvider, "Qwen": QwenProvider, "MiniMax": MinimaxProvider, "LinkAI": LinkAIProvider, }
...
fn main() {
    ...
    if not providers:
        ...
        print(json.dumps({ "error": ( f"No API key configured for {target}. " "Set at least one of OPENAI_API_KEY / GEMINI_API_KEY / " "ARK_API_KEY / DASHSCOPE_API_KEY / MINIMAX_API_KEY / " "LINKAI_API_KEY via the env_config tool, then try again." ) }, ensure_ascii=false))
        sys.exit(1)
    ...
    for label, provider in providers:
        try {
            ...
            t0 = time.time()
            paths = provider.generate( prompt, image_url=image_url, quality=quality, size=size, aspect_ratio=aspect_ratio, output_dir=output_dir, )
            elapsed = time.time() - t0
            # Resolved model id (after alias expansion) actually sent to the API
            actual_model = getattr(provider, "model", model)
            print( f"[image-generation] ✅ {label} succeeded in {elapsed:.1f}s " f"(model={actual_model})", file=sys.stderr, )
            result = { "model": actual_model, "images": [{"url": p} for p in paths], }
    ...
    hint = " | ".join(errors)
    print(json.dumps({ "error": f"All providers failed — {hint}. " "This is likely an API key or base URL configuration issue. " "Do NOT retry with the same parameters. " "Ask the user to verify their API key / base URL " "(OPENAI_API_KEY, GEMINI_API_KEY, ARK_API_KEY, " "DASHSCOPE_API_KEY, MINIMAX_API_KEY, or LINKAI_API_KEY) " "via env_config." }, ensure_ascii=false))
    sys.exit(1)
